...
//==============================================================================
// Constructor
STMDAQSupervisor::STMDAQSupervisor(xdaq::ApplicationStub* s) : CoreSupervisorBase(s)
{
...
void STMDAQSupervisor::transitionConfiguring(toolbox::Event::Reference e)
{
	...
	if(currentChannel == 0)
	{
		std::string commandResponse = StringMacros::exec(
		    "nohup python $STM_DQM > $STM_DQM_LOG/dqmout_HPGe.log 2>&1 &");
	}
	//LaBr DQM start
	else
	{
		std::string commandResponse = StringMacros::exec(
		    "nohup python $STM_DQM > $STM_DQM_LOG/dqmout_LaBr.log 2>&1 &");
	}
...
std::vector<SupervisorInfo::SubappInfo> STMDAQSupervisor::getSubappInfo(void)
{
	...
	if(stop::should_critical_stop())
	{
		...
		if(!failureReported_)
		{
			...
			__SUP_SS__ << "STMDAQ critical stop detected. Please go to halted -> "
		...
		info.status = theStateMachine_.getCurrentStateName();
