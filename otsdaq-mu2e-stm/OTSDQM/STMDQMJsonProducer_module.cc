...
{
	hist.resize(NBINS, 0);
	peaks = {{"Annihilation", 511.0, 460.0, 0.0},
...
void STMDQMJsonProducer::computeCalibration(SpectrumState& s, double& a, double& b)
{
...
void STMDQMJsonProducer::computeChi2(SpectrumState& s, const std::vector<double>& model)
{
	double chi2 = 0;
...
{
	return cfg_.logBasePath() + "/logs/stm_alarm_log_" + detectorName(ch) + "_run_" +
...
void STMDQMJsonProducer::analyze(art::Event const& event)
{
	...
	for(const auto& frag : *handle)
	{
		...
		for(size_t i = 0; i < cont.block_count(); ++i)
		{
			...
			s.alarm_clock_error = (s.acc_dtc_jumps > 15000 || s.acc_adc_jumps > 5000);
			s.alarm_correlation_error = (slope < -7000.0 || slope > 7000.0);

			bool current_alarm = s.alarm_ewt_error || s.alarm_transition_error ||
			                     s.alarm_clock_error || s.alarm_correlation_error;
			...
			if(current_alarm && !s.prev_alarm_state)
			{
				...
				alarm << "\"ewt_error\":" << (s.alarm_ewt_error ? "true" : "false")
				      << ",";
				alarm << "\"transition_error\":"
				      << (s.alarm_transition_error ? "true" : "false") << ",";
				alarm << "\"clock_error\":" << (s.alarm_clock_error ? "true" : "false")
				      << ",";
				alarm << "\"correlation_error\":"
				      << (s.alarm_correlation_error ? "true" : "false");
				alarm << "}}";
			...
			auto idleSec =
			    std::chrono::duration_cast<std::chrono::seconds>(now - s.lastDataTime)
			        .count();
			...
			// Write to json
			if(!s.stale &&
			   std::chrono::duration_cast<std::chrono::milliseconds>(now - s.lastWrite)
			           .count() > 500)
			{
				double a, b;
				...
				for(size_t i = 0; i < spec.peaks.size(); i++)
				{
					...
					int   bin =
					    std::max(0,
					             std::min((int)(p.mean / SpectrumState::ADC_PER_BIN),
					                      (int)SpectrumState::NBINS - 1));
				...
				entries.insert(entries.end(), s.alarmBuffer.begin(), s.alarmBuffer.end());
