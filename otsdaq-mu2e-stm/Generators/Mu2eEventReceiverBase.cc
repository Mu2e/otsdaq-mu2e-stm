...
mu2e::Mu2eEventReceiverBase::Mu2eEventReceiverBase(fhicl::ParameterSet const& ps)
    ...
    , mode_(DTCLib::DTC_SimModeConverter::ConvertToSimMode(
          ps.get<std::string>("sim_mode", "Disabled")))
    ...
    , frag_sent_(0)
{
	...
	theInterface_ = std::make_unique<DTCLib::DTC>(
	    mode_,
	    ps.get<int>("dtc_id", -1),
	...
	if(request_rate_ <= 0)
		request_rate_ = std::numeric_limits<double>::max();
	...
	if(mode_ != 0)
	{
		...
		theCFO_ = std::make_unique<DTCLib::DTCSoftwareCFO>(
		    theInterface_.get(),
		    cfoConfig.get<bool>("use_dtc_cfo_emulator", true),
		    cfoConfig.get<size_t>("debug_packet_count", 0),
		    DTCLib::DTC_DebugTypeConverter::ConvertToDebugType(
		        cfoConfig.get<std::string>("debug_type", "2")),
	...
	if(ps.get<bool>("load_sim_file", false))
	{
		...
		if(sim_file.size() > 0)
		{
			simFileRead_ = false;
			std::thread reader(
			    &mu2e::Mu2eEventReceiverBase::readSimFile_, this, sim_file);
			reader.detach();
		}
	}
	else
	{
		theInterface_
		    ->ClearDetectorEmulatorInUse();  // Needed if we're doing ROC Emulator...make sure Detector Emulation
		                                     // is disabled
		simFileRead_ = true;
	}
}
...
bool mu2e::Mu2eEventReceiverBase::getNextDTCFragment(
    artdaq::FragmentPtrs&           frags,
    DTCLib::DTC_EventWindowTag      ts_in,
    artdaq::Fragment::sequence_id_t seq_in)
{
	...
	while(data.size() == 0 && retryCount >= 0)
	{
		try
		{
			TLOG(TLVL_TRACE + 25) << "Calling theInterface->GetData("
			                      << ts_in.GetEventWindowTag(true) << ")";
			data = theInterface_->GetData(ts_in);
			TLOG(TLVL_TRACE + 25)
			    << "Done calling theInterface->GetData(" << ts_in.GetEventWindowTag(true)
			    << ") data.size()=" << data.size() << ", retryCount=" << retryCount;
		}
	...
	artdaq::Fragment::sequence_id_t seq_out =
	    seq_in == 0 ? getCurrentSequenceID() : seq_in;
	if(ts_out.GetEventWindowTag(true) != ts_in.GetEventWindowTag(true))
	{
		TLOG(TLVL_TRACE) << "Requested timestamp " << ts_in.GetEventWindowTag(true)
		                 << ", received data with timestamp "
		                 << ts_out.GetEventWindowTag(true);
	}
	...
	if(data.size() == 1)
	{
		...
		frags.emplace_back(new artdaq::Fragment(
		    seq_out, fragment_ids_[0], FragmentType::DTCEVT, fragment_timestamp));
		...
		memcpy(frags.back()->dataBegin(),
		       data[0]->GetRawBufferPointer(),
		       data[0]->GetEventByteCount());
	}
	else
	{
		TLOG(TLVL_TRACE + 20) << "Creating ContainerFragment, sz=" << data.size();
		frags.emplace_back(new artdaq::Fragment(
		    seq_out, fragment_ids_[0], artdaq::Fragment::ContainerFragmentType));
		...
		{
			TLOG(TLVL_TRACE + 20)
			    << "Creating Fragment, sz=" << data[0]->GetEventByteCount();
			...
			memcpy(frags.back()->dataBegin(),
			       evt->GetRawBufferPointer(),
			       evt->GetEventByteCount());
	...
	metricMan->sendMetric("DTC Read Time",
	                      artdaq::TimeUtils::GetElapsedTime(after_read, after_copy),
	                      "s",
	                      3,
	                      artdaq::MetricMode::Average);
	metricMan->sendMetric("Fragment Prep Time",
	                      artdaq::TimeUtils::GetElapsedTime(before_read, after_read),
	                      "s",
	                      3,
	                      artdaq::MetricMode::Average);
	metricMan->sendMetric("HW Timestamp Rate",
	                      hw_timestamp_rate,
	                      "timestamps/s",
	                      1,
	                      artdaq::MetricMode::Average);
	metricMan->sendMetric(
	    "PCIe Transfer Rate", hw_data_rate, "B/s", 1, artdaq::MetricMode::Average);
