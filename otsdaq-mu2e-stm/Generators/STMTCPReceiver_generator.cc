...
STMTCPReceiver::STMTCPReceiver(fhicl::ParameterSet const& ps)
    ...
    , rcvbuf_bytes_(ps.get<int>("rcvbuf_bytes",
                                314572800))  // Buffer bytes (Default: 300*1024*1024)
    , recv_buffer_size_(ps.get<int>(
          "recv_buffer_size",
          10485760))  // Size of the temporary TCP receive buffer (Default: 10*1024*1024)
    , event_anchor_word_(ps.get<uint16_t>("event_anchor_word",
                                          51966))  // Anchor word in Event header (0xCAFE)
    ...
    , container_stream_id_(
          ps.get<uint64_t>("container_stream_id", 3))  // Container fragment id
    ...
    , events_per_container_(
          ps.get<size_t>("events_per_container",
                         500))  // Default number of events per container in getNext()
    , offspill_events_per_container_(
          ps.get<size_t>("offspill_events_per_container",
                         100))  // Number of off-spill events per container in getNext()
    , use_spill_condition_(ps.get<bool>("use_spill_condition",
                                        false))  // Condition for batching on spill flags
    , eos_stream_id_(
          ps.get<uint64_t>("eos_stream_id", 4))  // End-of-subrun marker fragment id
    , rollover_subrun_interval_(ps.get<uint64_t>("rollover_subrun_interval",
                                                 0))  // EWTs per subrun (0 = disabled)
{
	// Log configuration
	TLOG_DEBUG(1) << "[STM_BR] STMTCPReceiver: Channel = " << chan_
...
void STMTCPReceiver::start()
{
	...
	TLOG(TLVL_INFO)
	    << "[STM_BR][START] Starting STMTCPReceiver, opening listening socket... host="
	    << host_ << " port=" << port_;
...
void mu2e::STMTCPReceiver::receiverThread_()
{
	// Pin thread to core
	if(pin_threads_)
		cpu_->get_next_core("[STM_BR][RECEIVER]");
	...
	while(!stop_requested_.load(std::memory_order_acquire))
	{
		...
		TLOG(TLVL_INFO) << "[STM_BR][RECEIVER] Client connected (fd=" << client_fd_
		                << ")";
		...
		if(rmem_max > 0)
		{
			...
			if(setsockopt(client_fd_, SOL_SOCKET, SO_RCVBUF, &bufsize, sizeof(bufsize)) <
			   0)
			{
			...
			getsockopt(client_fd_, SOL_SOCKET, SO_RCVBUF, &bufsize, &len);

			TLOG(TLVL_INFO) << "[STM_BR][RECEIVER] SO_RCVBUF set to " << bufsize
			                << " bytes";
		}
		...
		while(connection_alive && !stop_requested_.load(std::memory_order_acquire))
		{
			...
			if(client_pfd.revents & POLLIN)
			{
				...
				{
					ssize_t n =
					    recv(client_fd_, byte_buffer.data(), byte_buffer.size(), 0);
					...
					if(n > 0)
					{
						...
						while(remaining > 0)
						{
							...
							size_t contiguous = ring_->contiguousWritable();

							size_t to_copy = std::min(remaining, contiguous);

							uint8_t* dst = ring_->writePtr();

							std::memcpy(dst, byte_buffer.data() + copied, to_copy);
					...
					if(n == 0)
					{
						TLOG(TLVL_WARNING) << "[STM_BR][RECEIVER] Sender disconnected";
						connection_alive = false;
						break;
					}
			...
			if(client_pfd.revents & (POLLHUP | POLLERR | POLLNVAL))
			{
				TLOG(TLVL_WARNING) << "[STM_BR][RECEIVER] Socket hangup/error";
		...
		if(!stop_requested_.load(std::memory_order_acquire))
		{
			TLOG(TLVL_INFO) << "[STM_BR][RECEIVER] Waiting for new sender connection";
		}
	}

	receiver_done_.store(true, std::memory_order_release);
	TLOG(TLVL_INFO) << "[STM_BR][RECEIVER] Receiver thread exiting";
}
...
void mu2e::STMTCPReceiver::parserThread_()
{
	// Pin thread to core
	if(pin_threads_)
		cpu_->get_next_core("[STM_BR][PARSER]");
	...
	while(true)
	{
		...
		if(receiver_done_.load(std::memory_order_acquire) && available == 0)
			break;
		...
		if(expected_event_bytes == 0)
		{
			if(available < EVENT_HEADER_BYTES)
			{
				...
				{
					TLOG(TLVL_WARNING)
					    << "[STM_BR][PARSER] Incomplete event at shutdown (" << available
			...
			else
			{
				for(size_t i = 0; i < EVENT_HEADER_WORDS; ++i)
				{
					...
					header_words[i] = uint16_t(p[0]) | (uint16_t(p[1]) << 8);
				}
			...
			expected_event_bytes =
			    EVENT_HEADER_BYTES + size_t(raw_len + zs_len + ph_len) * sizeof(uint16_t);
		...
		for(size_t i = 0; i < EVENT_HEADER_WORDS; ++i)
		{
			const uint8_t* p = resolveEventPointer(evt, i * 2);
		...
		evt.event_num = int64_t(hdr[EWT_0]) | (int64_t(hdr[EWT_1]) << 16) |
		...
		// Subrun transition flag
		evt.subrun_flag = (hdr[EM_2_DRTDC] >> 1) & 0x1;

		uint16_t em4      = header_words[EM_2_DRTDC];
		uint8_t  low_byte = em4 & 0xFF;  // Event Mode Byte 4 (per HB packet diagram)

		/*TLOG(TLVL_INFO) << "Event number " << evt.event_num
			<< " full_EM_2_DRTDC=0x" << hdr[EM_2_DRTDC]
			<< " EM_2_DRTDC low byte = 0x" << std::hex
                        << static_cast<int>(low_byte) << std::dec
                        << "  onspill=" << (low_byte & 0x1)
                        << "  subrun_handling=" << ((low_byte >> 1) & 0x7)
                        << "  stm=" << ((low_byte >> 4) & 0x3)
                        << "  tem=" << ((low_byte >> 6) & 0x3);*/
		...
		{
			TLOG(TLVL_ERROR) << "[STM_BR][PARSER] computeDatasetView failed";
		...
		const size_t target_batch_size = (batch_spill_flag == 0)
		                                     ? offspill_events_per_container_
		                                     : events_per_container_;
		...
		metricMan->sendMetric("Expected event bytes",
		                      expected_event_bytes,
		                      "status",
		                      3,
	...
	TLOG(TLVL_INFO) << "[STM_BR][PARSER] Parser+Batcher exiting";
}
...
void mu2e::STMTCPReceiver::builderThread_()
{
	// Pin thread to core
	if(pin_threads_)
		cpu_->get_next_core("[STM_BR][BUILDER]");
	...
	while(true)
	{
		...
		for(const auto& e : batch.events)
		{
			...
			const bool sw_subrun_trigger =
			    (rollover_subrun_interval_ > 0) &&
			    (event_count_.load() % rollover_subrun_interval_ == 0);
			if((hw_subrun_trigger || sw_subrun_trigger) && e.event_num > 0)
			{
				const auto next_subrun = ++subrun_number_;
				if(debug_level_ > 0)
				{
					TLOG(TLVL_INFO)
					    << "[BUILDER] Subrun transition "
					       "(hw="
					    << hw_subrun_trigger << "(sw=" << sw_subrun_trigger
					    << ") at Event number =" << event_count_.load()
					    << ") at EWT=" << e.event_num << " -> subrun " << next_subrun;
				}
				...
				metricMan->sendMetric(
				    "SubrunNumber",
				    static_cast<uint64_t>(next_subrun),
				    "subrun",
				    1,
				    artdaq::MetricMode::LastPoint | artdaq::MetricMode::Persist);

				// Only send from channel 0 BR
				if(chan_ == 0)
				{
					// Make and push EOS fragment
					std::unique_ptr<artdaq::Fragment> eos_frag =
					    artdaq::MetadataFragment::CreateEndOfSubrunFragment(
					        my_rank, e.event_num - 1, next_subrun, eos_frag_id);
			...
			if((debug_level_ > 0) && ((debug_evt_counter % debug_print_every) == 0))
			{
				TLOG(TLVL_INFO)
				    ...
				    << " events= " << batch.events.size() << " raw=" << e.raw.size
				    << " zs=" << e.zs.size << " ph=" << e.ph.size;
			}
		...
		if(builder_to_getNext_queue_->pop(raw_ptr))
		{
			...
			if(debug_level_ > 0)
			{
				TLOG(TLVL_INFO)
				    ...
				    << " SeqID=" << raw_ptr->sequenceID() << " Type=" << raw_ptr->type()
				...
				if(raw_ptr->type() == artdaq::Fragment::ContainerFragmentType)
				{
					...
					TLOG(TLVL_INFO) << "[GETNEXT] --> Container with "
					                << cont.block_count() << " inner fragments";
				}  // end container check
			}      // end Debug
...
void mu2e::STMTCPReceiver::stop()
{
	...
	// Print summary
	TLOG(TLVL_INFO) << "[STM_BR][STOP] Summary: events=" << event_count_.load()
	...
	double runtime_sec = std::chrono::duration<double>(total_runtime).count();

	double receiver_sec = std::chrono::duration<double>(receiver_active_).count();

	double parser_sec = std::chrono::duration<double>(parser_active_).count();

	double builder_sec = std::chrono::duration<double>(builder_active_).count();

	TLOG(TLVL_INFO) << "\n========= [STM_BR][STOP] PERFORMANCE REPORT =========\n"
	                << "Total runtime: " << runtime_sec << " s\n"
	                << "Events: " << event_count_.load() << "\n"
	                << "Throughput: " << (event_count_.load() / runtime_sec)
	                << " events/sec\n\n"
	                << "Receiver active: " << receiver_sec << " s ("
	                << (receiver_sec / runtime_sec * 100.0) << "%)\n"
	                << "Parser active:   " << parser_sec << " s ("
	                << (parser_sec / runtime_sec * 100.0) << "%)\n"
	                << "Builder active:  " << builder_sec << " s ("
	                << (builder_sec / runtime_sec * 100.0) << "%)\n"
	                << "Avg parse time per event: "
	                << (parser_sec / event_count_.load()) * 1e6 << " us\n"
	                << "Avg build time per event: "
	                << (builder_sec / event_count_.load()) * 1e6 << " us\n"
	                << "====================================================";
}
