...
namespace mu2e {
  ...
  private:
    ...
    bool computeDatasetView(const std::vector<ChunkRef>& chunks,
			    EventView& evt)
    {

      const uint16_t* hdr_words = reinterpret_cast<const uint16_t*>(evt.buffer_chunks.front().chunk->data() + evt.buffer_chunks.front().offset);
      if (hdr_words[anchor_start] != event_anchor_word_ || hdr_words[anchor_end] != event_anchor_word_) {
	TLOG(TLVL_ERROR)
	...
	return false;
      ...
      // Payload starts immediately after event header
      size_t offset = 0;

      evt.raw = { offset, raw_len };
      offset += raw_len;
