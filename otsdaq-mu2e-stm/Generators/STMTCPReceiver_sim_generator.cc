...
namespace mu2e {
  ...
  STMTCPReceiver::STMTCPReceiver(fhicl::ParameterSet const& ps)
    ...
    , anchor_word_(ps.get<uint16_t>("anchor_word", 51966)) // Anchor word in header (0xCAFE)
  ...
  // =====================================================================================
  void STMTCPReceiver::parserThread_()
  {
    ...
    while (!done_.load() || !recv_to_parser_queue_->empty()) {
      ...
      while (!chunks.empty()) {
        ...
        if (!getEventHeaderPtr(header_ptr, header_tmp, chunks, offset)) break;

        const uint16_t* h = reinterpret_cast<const uint16_t*>(header_ptr);

        // Anchor check
        if (h[0] != anchor_word_ || h[3] != anchor_word_ || h[7] != anchor_word_) {
          TLOG(TLVL_ERROR) << "[PARSER] Invalid event anchor";
          done_.store(true);
          return;
        }

        uint32_t payload_bytes = build_len_from_2_words(h[1], h[2]);

        uint16_t raw_len = h[4];
        uint16_t zs_len  = h[5];
        uint16_t mwd_len = h[6];

        size_t expected_payload =
          (raw_len + zs_len + mwd_len) * sizeof(int16_t);

        if (payload_bytes != expected_payload) {
          TLOG(TLVL_ERROR)
            << "[PARSER] Payload size mismatch: hdr=" << payload_bytes
            << " calc=" << expected_payload;
          done_.store(true);
          return;
        }

        size_t evt_total_bytes = EVENT_HEADER_BYTES + payload_bytes;

        if (total_bytes < evt_total_bytes) break;

        // Build ChunkRefs
        std::vector<ChunkRef> refs;
        size_t need = evt_total_bytes;
        size_t off = offset;
        ...
        for (auto& c : chunks) {
          ...
          size_t take = std::min(c->size() - off, need);
          refs.push_back({c, off, take});
          need -= take;
          off = 0;
        }

        EventView evt;
        evt.buffer_chunks = std::move(refs);

        // Assign event number from global counter
        evt.event_num = event_counter_.fetch_add(1, std::memory_order_relaxed);
        ...
        // Advance chunks
        offset += evt_total_bytes;
        auto it = chunks.begin();
        while (it != chunks.end() && offset >= (*it)->size()) {
          offset -= (*it)->size();
          it = chunks.erase(it);
        }
      }
