...
void test_funcs::client(UDPsocket& udp,
                        int        socket,
                        int16_t*   data,
                        uint64_t   packet_num,
                        uint32_t   number_hb,
                        uint32_t   send_num)
{
	...
	std::cout << "Sending " << (send_num - 1) * (packet_num - 1) + packet_num
	          << " packets" << std::endl;
	...
	for(uint i = 0; i < to_send; i++)
	{
		...
		last_event = update_headers(
		    snd_buffer[msgNum], packet_num, i, offset, number_hb, send_num);
	...
	// Notify user
	std::cout << "Sent " << sendCount << " packets [" << data_size << " Gbytes] at "
	          << sendSpeed << " Gbit/s" << std::endl;
}

// Update event headers to send more than generated in memory
bool test_funcs::update_headers(int16_t*& data,
                                uint64_t  packet_num,
                                uint      i,
                                int       offset,
                                uint32_t  number_hb,
                                uint32_t  send_num)
{
	...
	uint32_t pnum = (uint16_t)data[1] << 16 | (uint16_t)data[0];

	// Check for error
	if(i % packet_num != pnum)
	{
		std::cout << "You fucked up the packet count in test_funcs::update_headers"
		          << std::endl;
		exit(0);
	}
	...
	while(leftInPacket > fw_tHdr_Len)
	{
		...
		for(size_t i = 0; i < fw_tHdr_Len / 2; i++)
		{
			...
			if((uint16_t)(data[hdr_start_loc + i * 2] & 0xFFFF) == BEEF and
			   (uint16_t)(data[hdr_start_loc + i * 2 + 1] & 0xFFFF) == DEAD)
			{
				// is 0xDEADBEEF
				deadbeef = true;
			}
		...
		{
			std::cout << "You fucked up the event count in test_funcs::update_headers"
			          << std::endl;
			std::cout << event_count << " " << number_hb << " " << event_count % number_hb
			          << " " << evNum << std::endl;
			exit(0);
		}
		...
		// Adjust the packet number
		uint64_t new_evNum =
		    std::floor(event_count / number_hb) * number_hb + event_count % number_hb;
		...
		if(send_num > 1 && current_event % number_hb == number_hb - 1 &&
		   new_evNum != send_num * number_hb - 1)
		{
