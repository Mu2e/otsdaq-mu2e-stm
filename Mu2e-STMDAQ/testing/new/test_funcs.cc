                        ...
                        const uint32_t number_hb)
{
	...
	while(1)
	{
		...
		{snd_buffer[packet_num % UDPsocket::SENDMMSG_NUM], EWT} =
		    gen_packet(packet_num, data);
	...
	// Notify user
	std::cout << "Sent " << send_count << " packets [" << data_size << " Gbytes] at "
