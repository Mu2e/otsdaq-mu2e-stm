...
// Constructor
GenData::GenData(size_t event_num_, size_t event_len_, const std::string& filepath)
    : event_num(static_cast<double>(event_num_)), event_len(event_len_)
{
	...
	// Notify user
	std::cout << "Event len = " << event_len << " ns = " << event_len_adc
	...
	else
	{
		...
		std::cout << "Would you like to change the number of events to this value to "
		             "send the file contents only once?"
		          << std::endl;
		while(true)
		{
			char input;
...
void GenData::get_packets()
{
	...
	while(data_count < data_len)
	{
		...
		while(left_in_packet > 0)
		{
			...
			std::vector<int16_t> header =
			    form_event_header(EWT, length, event_start, event_in_packet);
			...
			if(copy_end < copy_start)
			{
				...
				std::copy(
				    data.begin(), data.begin() + copy_end, packet.begin() + copy_index);
	...
	std::vector<int16_t> GenData::form_event_header(
	    EWT_, length, event_start, event_in_packet)
	{
		// %%%%%%%%%%%%%%%%%%%%%%%%% //
		...
		// Add header start
		memcpy(
		    &header[tHdr.HdrStart_start], &(tHdr.HdrStart_data[0]), tHdr.HdrStart_size);
