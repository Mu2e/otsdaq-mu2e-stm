...
int main()
{
	...
	std::cout << "Generated " << packetNum * MAX_PACKET_SIZE * 1e-9 << " Gbytes as "
	          << packetNum << " packets of size " << MAX_PACKET_SIZE * 1e-3 << " kbytes"
	          << std::endl;

	std::cout << "Sending this " << send_num
	          << " times = " << packetNum * send_num * MAX_PACKET_SIZE * 1e-9
	          << " Gbytes as " << packetNum * send_num << " packets" << std::endl;
