...
int UDPsocket::createSocket()
{
	...
	std::cout << "CREATED SERVER: IP = " + ip + ", PORT = " + std::to_string(port) +
...
int UDPsocket::setupClient()
{
	...
	std::cout << "CREATED CLIENT: IP = " + ip + ", PORT = " + std::to_string(port) +
	                 ", socket = " + std::to_string(sock)
	          << std::endl;
...
int UDPsocket::set_SO_SNDBUF(int socket, uint64_t size)
{
	...
	std::cout << "Setting SO_SNDBUF size to " + std::to_string(size) +
	                 ": old SO_SNDBUF = " + std::to_string(oldsize) +
	                 ", new SO_SNDBUF = " + std::to_string(newsize)
	          << std::endl;

	// Log the calcuated number of UDP packets to send
	std::cout << "Calculated SENDMMSG_NUM =  " + std::to_string(SENDMMSG_NUM) +
	                 " * 8198 byte UDP packets = " +
