...
struct DataStruct
{
	std::vector<int16_t> data;  // First data vector
	// std::vector<int16_t> data2; // Second data vector
	// std::vector<int16_t> data3; // Third data vector
	size_t size;  // Actual size of data in data1
	// size_t size2;              // Actual size of data in data2
	// size_t size3;              // Actual size of data in data3
	char metadata[128];  // Fixed-size metadata array
...
void consumerThread(RingBuffer<DataStruct>& inputQueue,
                    BufferPool&             pool,
                    size_t                  core_id,
                    size_t                  threadIndex)
{
...
int main()
{
	...
	std::vector<int16_t> dummy_data;
	dummy_data.resize(buffer_size);
	std::fill(dummy_data.begin(), dummy_data.end(), rand() % 100);
	...
	std::thread finalConsumerThread(
	    consumerThread, std::ref(mwdBufferQueue), std::ref(pool), 6, 6);
