...
void workerThread(RingBuffer<DataStruct>& inputQueue, RingBuffer<DataStruct>& outputQueue, BufferPool& pool, const char* stage, std::atomic<uint64_t>& totalBytesProcessed, size_t core_id, std::function<void(std::shared_ptr<DataStruct>&)> operation) {
...
void consumerThread(RingBuffer<DataStruct>& inputQueue, BufferPool& pool, size_t core_id) {
    ...
    while (!kernelDone || !inputQueue.is_empty()) {
        ...
        // Simulate consumption of the final data
        pool.release(buffer);
    }
...
int main() {
    ...
    std::thread checkDataThread(workerThread, std::ref(kernelBufferQueue), std::ref(checkDataQueue), std::ref(pool), " | Checked", std::ref(totalBytesCheck), 1, operationExample);
    std::thread formEventsThread(workerThread, std::ref(checkDataQueue), std::ref(formEventsQueue), std::ref(pool), " | Events Formed", std::ref(totalBytesForm), 2, operationExample);
    std::thread rawProcessThread(workerThread, std::ref(formEventsQueue), std::ref(rawBufferQueue), std::ref(pool), " | Algorithm Processed", std::ref(totalBytesRaw), 3, operationExample);
    std::thread zsProcessThread(workerThread, std::ref(rawBufferQueue), std::ref(zsBufferQueue), std::ref(pool), " | ZS Processed", std::ref(totalBytesZS), 4, operationExample);
    std::thread mwdProcessThread(workerThread, std::ref(zsBufferQueue), std::ref(mwdBufferQueue), std::ref(pool), " | MWD Processed", std::ref(totalBytesMWD), 5, operationExample);
