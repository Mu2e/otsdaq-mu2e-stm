...
void kernelBufferTask(RingBuffer<DataStruct>& bufferQueue, BufferPool& pool)
{
	...
	buffer->size =
	    (rand() % (60 * 1024 * 1024 / sizeof(int16_t))) + 1000;  // Simulate variable size
	std::fill(buffer->data.begin(), buffer->data.begin() + buffer->size, rand() % 100);
	strncpy(
	    buffer->metadata, "Generated by kernelBufferThread", sizeof(buffer->metadata));
	bufferQueue.push(buffer);
}

// General processing task
void processingTask(RingBuffer<DataStruct>& inputQueue,
...
int main()
{
	...
	for(int i = 0; i < 100; ++i)
	{
		...
		threadPool.enqueue([&]() {
			processingTask(kernelBufferQueue, checkDataQueue, pool, " | Checked");
		});
		threadPool.enqueue([&]() {
			processingTask(checkDataQueue, formEventsQueue, pool, " | Events Formed");
		});
		threadPool.enqueue([&]() {
			processingTask(
			    formEventsQueue, rawBufferQueue, pool, " | Algorithm Processed");
		});
		threadPool.enqueue([&]() {
			processingTask(rawBufferQueue, zsBufferQueue, pool, " | ZS Processed");
		});
		threadPool.enqueue([&]() {
			processingTask(zsBufferQueue, mwdBufferQueue, pool, " | MWD Processed");
		});
	}
