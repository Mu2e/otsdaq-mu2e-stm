...
auto kernelBufferThread = [](RingBuffer<DataStruct>& bufferQueue, BufferPool& pool) {
	while(true)
	{
		...
		std::fill(
		    buffer->data.begin(), buffer->data.begin() + buffer->size, rand() % 100);
...
auto checkDataThread = [](RingBuffer<DataStruct>& inputQueue,
	...
	while(true)
	{
		...
		if(!buffer)
			break;
		strncpy(buffer->metadata + strlen(buffer->metadata),
		        " | Checked",
...
auto formEventsThread = [](RingBuffer<DataStruct>& inputQueue,
                           RingBuffer<DataStruct>& outputQueue,
                           BufferPool&             pool) {
	while(true)
	{
...
auto processAlgorithmThread = [](RingBuffer<DataStruct>& inputQueue,
                                 RingBuffer<DataStruct>& outputQueue,
                                 BufferPool&             pool) {
	while(true)
	{
...
int main()
{
	...
	std::thread kernelThread(
	    kernelBufferThread, std::ref(kernelBufferQueue), std::ref(pool));
	std::thread checkData(checkDataThread,
	                      std::ref(kernelBufferQueue),
	                      std::ref(checkDataQueue),
	                      std::ref(pool));
	std::thread formEvents(formEventsThread,
	                       std::ref(checkDataQueue),
	                       std::ref(formEventsQueue),
	                       std::ref(pool));
	std::thread rawProcess(processAlgorithmThread,
	                       std::ref(formEventsQueue),
	                       std::ref(rawBufferQueue),
	                       std::ref(pool));
	std::thread zsProcess(processAlgorithmThread,
	                      std::ref(rawBufferQueue),
	                      std::ref(zsBufferQueue),
	                      std::ref(pool));
	std::thread mwdProcess(processAlgorithmThread,
	                       std::ref(zsBufferQueue),
