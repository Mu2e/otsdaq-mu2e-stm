...
int main()
{
	...
	// Using a lambda to bind the member function to the instance
	auto zs_ = [zs](std::shared_ptr<DataStruct>& buffer) { zs->operation(buffer); };

	// Set up MWD operation
	std::shared_ptr<MWD> mwd = std::make_shared<MWD>();
	// Using a lambda to bind the member function to the instance
	auto mwd_ = [mwd](std::shared_ptr<DataStruct>& buffer) { mwd->operation(buffer); };

	// Launch worker threads for each stage
	std::thread starter(
	    &ThreadFuncs::starterThread, tf, std::ref(startBufferQueue), std::ref(pool), 0);
	std::thread checkData(&ThreadFuncs::workerThread,
	                      tf,
	                      std::ref(startBufferQueue),
	                      ...
	                      operationExample,
	                      1);
	std::thread formEvents(&ThreadFuncs::workerThread,
	                       tf,
	                       std::ref(checkDataQueue),
	                       ...
	                       operationExample,
	                       2);
	std::thread rawProcess(&ThreadFuncs::workerThread,
	                       tf,
	                       std::ref(formEventsQueue),
	                       ...
	                       operationExample,
	                       3);
	std::thread zsProcess(&ThreadFuncs::workerThread,
	                      tf,
	                      std::ref(rawBufferQueue),
	                      ...
	                      zs_,
	                      4);
	std::thread mwdProcess(&ThreadFuncs::workerThread,
	                       tf,
	                       std::ref(zsBufferQueue),
	...
	std::thread finalConsumer(
	    &ThreadFuncs::endingThread, tf, std::ref(mwdBufferQueue), std::ref(pool), 6, 6);
