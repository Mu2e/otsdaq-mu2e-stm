...
void ThreadFuncs::workerThread(
    RingBuffer<DataStruct>&                           inputQueue,
    ...
    size_t                                            threadIndex)
{
	...
	while(!threadDone[0].load() || !inputQueue.is_empty() ||
	      !threadDone[threadIndex - 1].load())
	{
	...
	std::cout << "[" << stage << "] Processed " << std::fixed << std::setprecision(2)
	          << (gbytes) << " Gbytes at\n"
	          << std::fixed << std::setprecision(2) << (gbits / elapsed.count())
	          << " Gbit/s\n";
}
                               ...
                               size_t                  threadIndex)
{
	      ...
	      !threadDone[threadIndex - 1].load())
	{
		...
		for(size_t i = 0; i < buffer->size; ++i)
		{
			...
			{
				std::cout << "BUFFER 1 ERROR: " << buffer->data[i] << " != " << test
				          << std::endl;
				exit(0);
			}
			...
			{
				std::cout << "BUFFER 2 ERROR: " << buffer->data2[i] << " != " << test2
				          << std::endl;
				exit(0);
			}
			...
			{
				std::cout << "BUFFER 3 ERROR: " << buffer->data3[i] << " != " << test3
				          << std::endl;
				exit(0);
			}
		}
