...
struct DataStruct
{
	...
	{  // Constructor to initialize data buffer
		std::fill(std::begin(metadata),
		          std::end(metadata),
		          '\0');  // Initialize metadata to null characters
	}
};
...
class RingBuffer
{
  ...
  public:
	...
	~RingBuffer() { std::cout << "RingBuffer destructor called.\n"; }
	...
	std::shared_ptr<T> pop()
	{
		std::unique_lock<std::mutex> lock(mutex);  // Lock for thread safety
		cv_pop.wait(lock,
		            [this]() { return stop || size > 0; });  // Wait if buffer is empty
		if(size == 0)
			return nullptr;                 // Return null if buffer is empty
  ...
  public:
	...
	BufferPool(size_t pool_size, size_t buffer_size)
	{
		...
		{  // Create specified number of buffers
			pool.emplace_back(
			    std::make_shared<DataStruct>(buffer_size));  // Add buffers to pool
		}
	}

	// Destructor for logging
	~BufferPool() { std::cout << "BufferPool destructor called.\n"; }
	...
	std::shared_ptr<DataStruct> acquire()
	{
		...
		if(pool.empty())
		{  // If pool is empty
			return std::make_shared<DataStruct>(60 * 1024 * 1024 /
			                                    sizeof(int16_t));  // Create a new buffer
		}
		else
		{
...
void pin_thread_to_core(size_t core_id)
{
	...
	pthread_setaffinity_np(
	    pthread_self(), sizeof(cpu_set_t), &cpuset);  // Set thread affinity
}
                  ...
                  size_t                  core_id)
{
	...
	while(!kernelDone || !inputQueue.is_empty())
	{  // Continue until kernel is done and input queue is empty
		auto buffer = inputQueue.pop();  // Retrieve a buffer from the input queue
		if(buffer)
		{  // If buffer is valid
			strncat(buffer->metadata,
			        stage,
			        sizeof(buffer->metadata) - strlen(buffer->metadata) -
			            1);  // Append stage info to metadata
			uint64_t bytesProcessed =
			    buffer->size * sizeof(int16_t);     // Calculate bytes processed
	...
	std::chrono::duration<double> elapsed =
	    end_time - start_time;                              // Calculate elapsed time
	double gbits = totalBytesProcessed.load() * 8.0 / 1e9;  // Convert bytes to Gbits
	std::cout << "[" << stage << "] Processing speed: " << std::fixed
	          << std::setprecision(2) << (gbits / elapsed.count())
	          << " Gbit/s\n";  // Print processing speed
}

// Kernel buffer simulation function
void kernelBufferWorker(RingBuffer<DataStruct>& outputQueue,
                        BufferPool&             pool,
                        size_t                  core_id)
{
	pin_thread_to_core(core_id);  // Bind thread to specific core
	...
	{  // Continue until total bytes processed reaches target
		...
		buffer->size = (rand() % (60 * 1024 * 1024 / sizeof(int16_t))) +
		               1000;  // Simulate variable buffer size
		memset(buffer->data.data(),
		       rand() % 100,
		       buffer->size * sizeof(int16_t));  // Fill buffer with simulated data
		strncpy(buffer->metadata,
		        "Generated by kernelBufferWorker",
		        sizeof(buffer->metadata));  // Add metadata
		uint64_t bytesGenerated =
		    buffer->size * sizeof(int16_t);  // Calculate bytes generated
...
int main()
{
	...
	const size_t buffer_size =
	    60 * 1024 * 1024 / sizeof(int16_t);  // Set size of each buffer in elements

	BufferPool             pool(buffer_pool_size, buffer_size);  // Create buffer pool
	RingBuffer<DataStruct> kernelBufferQueue(
	    max_queue_size);                                     // Create kernel buffer queue
	...
	std::atomic<uint64_t> totalBytesKernel{
	    0};  // Initialize total bytes processed for kernel
	std::atomic<uint64_t> totalBytesCheck{
	    0};  // Initialize total bytes processed for check data
	std::atomic<uint64_t> totalBytesForm{
	    0};  // Initialize total bytes processed for form events
	std::atomic<uint64_t> totalBytesRaw{
	    0};  // Initialize total bytes processed for raw buffer
	std::atomic<uint64_t> totalBytesZS{
	    0};  // Initialize total bytes processed for ZS buffer
	std::atomic<uint64_t> totalBytesMWD{
	    0};  // Initialize total bytes processed for MWD buffer

	std::thread kernelThread(kernelBufferWorker,
	                         std::ref(kernelBufferQueue),
	                         std::ref(pool),
	                         0);  // Launch kernel buffer worker thread
	std::thread checkDataThread(workerThread,
	                            std::ref(kernelBufferQueue),
	                            std::ref(checkDataQueue),
	                            std::ref(pool),
	                            " | Checked",
	                            std::ref(totalBytesCheck),
	                            1);  // Launch check data worker thread
	std::thread formEventsThread(workerThread,
	                             std::ref(checkDataQueue),
	                             std::ref(formEventsQueue),
	                             std::ref(pool),
	                             " | Events Formed",
	                             std::ref(totalBytesForm),
	                             2);  // Launch form events worker thread
	std::thread rawProcessThread(workerThread,
	                             std::ref(formEventsQueue),
	                             std::ref(rawBufferQueue),
	                             std::ref(pool),
	                             " | Algorithm Processed",
	                             std::ref(totalBytesRaw),
	                             3);  // Launch raw processing worker thread
	std::thread zsProcessThread(workerThread,
	                            std::ref(rawBufferQueue),
	                            std::ref(zsBufferQueue),
	                            std::ref(pool),
	                            " | ZS Processed",
	                            std::ref(totalBytesZS),
	                            4);  // Launch ZS processing worker thread
	std::thread mwdProcessThread(workerThread,
	                             std::ref(zsBufferQueue),
	                             std::ref(mwdBufferQueue),
	                             std::ref(pool),
	                             " | MWD Processed",
	                             std::ref(totalBytesMWD),
	                             5);  // Launch MWD processing worker thread
	std::thread finalConsumerThread(consumerThread,
	                                std::ref(mwdBufferQueue),
	                                std::ref(pool),
	                                6);  // Launch consumer thread
