...
std::unordered_map<size_t, std::vector<int16_t>> preGeneratedData; // Store pre-generated data
...
bool validateFinalBuffer() {
    ...
    for (const auto& buffer : finalBuffer) {
        ...
        if (it == preGeneratedData.end() || !std::equal(buffer->data.begin(), buffer->data.end(), it->second.begin())) {
            return false;
        }
    }
    return true;
...
int main() {
    ...
    std::thread checkDataThread(workerThread, std::ref(kernelBufferQueue), std::ref(checkDataQueue), std::ref(pool), " | Checked", std::ref(totalBytesCheck), 1);
    std::thread formEventsThread(workerThread, std::ref(checkDataQueue), std::ref(formEventsQueue), std::ref(pool), " | Events Formed", std::ref(totalBytesForm), 2);
    std::thread rawProcessThread(workerThread, std::ref(formEventsQueue), std::ref(rawBufferQueue), std::ref(pool), " | Algorithm Processed", std::ref(totalBytesRaw), 3);
    std::thread zsProcessThread(workerThread, std::ref(rawBufferQueue), std::ref(zsBufferQueue), std::ref(pool), " | ZS Processed", std::ref(totalBytesZS), 4);
    std::thread mwdProcessThread(workerThread, std::ref(zsBufferQueue), std::ref(mwdBufferQueue), std::ref(pool), " | MWD Processed", std::ref(totalBytesMWD), 5);
    std::thread finalConsumerThread(consumerThread, std::ref(mwdBufferQueue), std::ref(pool), 6);
