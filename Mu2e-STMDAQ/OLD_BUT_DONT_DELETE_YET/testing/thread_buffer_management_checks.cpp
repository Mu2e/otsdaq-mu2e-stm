...
std::unordered_map<size_t, std::vector<int16_t>>
           preGeneratedData;  // Store pre-generated data
std::mutex dataMapMutex;      // Mutex for accessing preGeneratedData
std::vector<std::shared_ptr<DataStruct>> finalBuffer;  // Final buffer for validation
std::mutex finalBufferMutex;                           // Mutex for accessing finalBuffer

// Generate unique data for validation
void generateData(std::shared_ptr<DataStruct>& buffer)
{
	std::lock_guard<std::mutex> lock(dataMapMutex);
	for(size_t i = 0; i < buffer->size; ++i)
	{
		buffer->data[i] = static_cast<int16_t>(i % 32768);  // Generate predictable data
	}
	preGeneratedData[reinterpret_cast<size_t>(buffer.get())] = buffer->data;
}

// Validate data after processing
bool validateFinalBuffer()
{
	std::lock_guard<std::mutex> lock(dataMapMutex);
	for(const auto& buffer : finalBuffer)
	{
		auto it = preGeneratedData.find(reinterpret_cast<size_t>(buffer.get()));
		if(it == preGeneratedData.end() ||
		   !std::equal(buffer->data.begin(), buffer->data.end(), it->second.begin()))
		{
			return false;
		}
	}
                  ...
                  size_t                  core_id)
{
	...
	while(!kernelDone || !inputQueue.is_empty())
	{
		...
		if(!buffer)
			continue;  // Wait for remaining data

		// Process the data and append metadata
		strncat(buffer->metadata,
		        stage,
		        sizeof(buffer->metadata) - strlen(buffer->metadata) - 1);
	...
	std::cout << "[" << stage << "] Processing speed: " << std::fixed
	          << std::setprecision(2) << (gbits / elapsed.count()) << " Gbit/s\n";
}
                        ...
                        size_t                  core_id)
{
	...
	while(globalTotalBytes < targetTotalBytes)
	{
		...
		buffer->size = (rand() % (60 * 1024 * 1024 / sizeof(int16_t))) +
		               1000;   // Simulate variable size
		generateData(buffer);  // Generate predictable data
...
void consumerThread(RingBuffer<DataStruct>& inputQueue, BufferPool& pool, size_t core_id)
{
	...
	while(!kernelDone || !inputQueue.is_empty())
	{
		...
		{
			// std::lock_guard<std::mutex> lock(finalBufferMutex);
			// finalBuffer.push_back(buffer); // Collect data for final validation
		}

		pool.release(buffer);  // Return buffer to pool
	}
	std::cout << "[Consumer] Final buffer collected.\n";
}
...
int main()
{
	...
	const size_t buffer_size =
	    60 * 1024 * 1024 / sizeof(int16_t);  // Buffer size in elements
	...
	std::thread kernelThread(
	    kernelBufferWorker, std::ref(kernelBufferQueue), std::ref(pool), 0);
	...
	std::thread mwdProcessThread(workerThread,
	                             ...
	                             5);
	std::thread finalConsumerThread(
	    consumerThread, std::ref(mwdBufferQueue), std::ref(pool), 6);
	...
	bool success = validateFinalBuffer();
	if(success)
	{
		std::cout << "[Validation] All data validated successfully.\n";
	}
	else
	{
		std::cerr << "[Validation] Data validation failed.\n";
	}
