...
{
	// Notify user
	Logger::Instance()->write(1, "In DQM thread: socket = " + std::to_string(socket));
...
void stmDQM::build_dqm_array()
{
	...
	for(int i = 0; i < info_num; i++)
	{
		...
		{
			std::cout << "ERROR: could not find DQM map ID: " << info_id[i]
			          << ". Exiting..." << std::endl;
			exit(0);
		}
	}
...
void stmDQM::get_run_number(int map)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], datVars.split_uint64_t(run_number), sizeof(uint64_t));
}
...
void stmDQM::get_subrun_number(int map)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], datVars.split_uint64_t(subrun_number), sizeof(uint64_t));
}
...
void stmDQM::get_EWT(int map)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], datVars.split_uint64_t(event), sizeof(uint64_t));
}
...
void stmDQM::get_pulse_rates(int map, int chan)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], convert_double(rate), sizeof(uint32_t));
}
...
void stmDQM::get_zs_factor(int map, int chan)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], convert_double(factor), sizeof(uint32_t));
}
...
void stmDQM::get_dropped_packets(int map, int chan)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], datVars.split_uint64_t(dropped), sizeof(uint64_t));
}
...
void stmDQM::get_event_count(int map, int chan)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], datVars.split_uint64_t(events), sizeof(uint64_t));
}
...
void stmDQM::get_on_count(int map, int chan)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], datVars.split_uint64_t(on_events), sizeof(uint64_t));
}
...
void stmDQM::get_off_count(int map, int chan)
{
	...
	// Memcpy to dqm data array
	memcpy(&dqm_data[map], datVars.split_uint64_t(off_events), sizeof(uint64_t));
}
