...
void client(int chan, int socket)
{
	...
	for(uint i = 0; i < numberhb; i++)
	{
		...
		memcpy(snd_buffer[msgNum],
		       &event_data[chan][(i % genNum) * event_len],
	...
	cout << "Channel = " << chan << " [" << udp[chan].get_channel_name(chan) << "] sent "
	     << sendCount << " *  " << deltahb_ns << " ns events [" << data_size
...
int main()
{
	...
	cout << "Sending " << numberhb * (fw_tHdr_Size + 2 * deltahb_len) * 1e-9
	     << " Gbytes as " << numberhb << " * " << deltahb_ns << " ns events of size "
	     << fw_tHdr_Size + 2 * deltahb_len << " bytes" << endl;
