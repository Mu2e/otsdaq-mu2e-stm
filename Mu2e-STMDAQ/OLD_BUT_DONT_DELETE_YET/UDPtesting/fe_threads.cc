...
void FEthreads::server(int chan, int socket, UDPsocket& udp, queue_buffer*& pushq)
{
	// Notify user
	Logger::Instance()->write(1,
	                          "In receive thread: Channel = " + std::to_string(chan) +
	...
	Logger::Instance()->write(2,
	                          "UDP timeout: Channel = " + std::to_string(chan) + " [" +
	                              udp.get_channel_name(chan) +
	                              "], socket = " + std::to_string(socket));

	// Reconstruct the final packet number
	uint64_t lastPacketStart = (final_retval - 1) * MAX_PACKET_LEN;
	uint32_t lastPacketNum   = (uint16_t)pushq->process_buffer[chan][lastPacketStart + 1]
	                             << 16 |
	                         (uint16_t)pushq->process_buffer[chan][lastPacketStart];
	// Print for user
	std::cout << "Channel: " << chan << " last packet = " << lastPacketNum << std::endl;
	...
	Logger::Instance()->write(1,
	                          "Channel = " + std::to_string(chan) + " [" +
	                              udp.get_channel_name(chan) + "] received " +
	                              std::to_string(recvCount) + " packets [" +
	                              std::to_string(data_size) + " Gbytes] at " +
	                              std::to_string(recvSpeed) + " Gbit/s");
}

// Function to check data packets in queue
void FEthreads::check_data(int           chan,
                           UDPsocket&    udp,
                           queue_buffer* pullq,
                           queue_buffer* pushq)
{
	// Notify user
	Logger::Instance()->write(1,
	                          "In check data thread: Channel = " + std::to_string(chan) +
	                              " [" + udp.get_channel_name(chan) + "]");
	...
	Logger::Instance()->write(1,
	                          "Channel = " + std::to_string(chan) + " [" +
	                              udp.get_channel_name(chan) + "] checked " +
	                              std::to_string(checkCount) + " packets [" +
	                              std::to_string(data_size) + " Gbytes] at " +
	                              std::to_string(checkSpeed) + " Gbit/s");
}

// Function to return data as single events
void FEthreads::form_events(
    int chan, formEvents& ev, UDPsocket& udp, queue_buffer* pullq, queue_buffer* pushq)
{
	// Notify user
	Logger::Instance()->write(1,
	                          "In form events thread: Channel = " + std::to_string(chan) +
	                              " [" + udp.get_channel_name(chan) + "]");
	...
	while(!udp.timeout)
	{
		...
		if(pull_len > 0)
		{
			...
			eventCount +=
			    ev.get_events(chan, pull_len, pushq->process_buffer[chan], pushq);
	...
	Logger::Instance()->write(
	    1,
	    "Channel = " + std::to_string(chan) + " [" + udp.get_channel_name(chan) +
	        "] formed " + std::to_string(packetCount) + " packets [" +
	        std::to_string(data_size) + " Gbytes] into " + std::to_string(eventCount) +
	        " events at " + std::to_string(eventSpeed) + " Gbit/s");
}

// Function to zero suppress event data
void FEthreads::zs_data(
    int chan, daqZS& zs, UDPsocket& udp, queue_buffer* pullq, queue_buffer* pushq)
{
	// Notify user
	Logger::Instance()->write(
	    1,
	    "In zero suppress thread: Channel = " + std::to_string(chan) + " [" +
	        udp.get_channel_name(chan) + "]");
...
void FEthreads::prescale_data(int           chan,
                              UDPsocket&    udp,
                              queue_buffer* pullq,
                              queue_buffer* pushq)
{
	...
	Logger::Instance()->write(
	    1,
	    "In prescale data thread: Channel = " + std::to_string(chan) + " [" +
	        udp.get_channel_name(chan) + "]");
	...
	Logger::Instance()->write(1,
	                          "Channel = " + std::to_string(chan) + " [" +
	                              udp.get_channel_name(chan) + "] prescaled " +
	                              std::to_string(data_size) + " Gbytes of data into " +
	                              std::to_string(eventCount) + " events at " +
	                              std::to_string(prescaleSpeed) + " Gbit/s");
}

//  Function for final data pull from queue for distribution
void FEthreads::distribute_raw(int           chan,
                               int           socket,
                               UDPsocket&    udp,
                               UDPsocket&    sw_udp,
                               queue_buffer* pullq,
                               uint64_t      buffer_len)
{
	...
	Logger::Instance()->write(
	    1,
	    "In distribute data thread: Channel = " + std::to_string(chan) + " [" +
	        udp.get_channel_name(chan) + "], socket = " + std::to_string(socket));
	...
	while(!udp.timeout)
	{
		...
		while(count < retval)
		{
			...
			if(sendToArtDaq)
				sw_udp.sendOne(event_buffer, size_to_copy, socket);
			// Increment counters
	...
	Logger::Instance()->write(1,
	                          "Channel = " + std::to_string(chan) + " [" +
	                              udp.get_channel_name(chan) + "] distributed " +
	                              std::to_string(distCount) + " events");
...
void FEthreads::write_data(int           chan,
                           uint64_t      buffer_len,
                           UDPsocket&    udp,
                           queue_buffer* pullq,
                           queue_write*  pushq)
{
	// Notify user
	Logger::Instance()->write(1,
	                          "In write data thread: Channel = " + std::to_string(chan) +
	                              " [" + udp.get_channel_name(chan) + "]");
	...
	Logger::Instance()->write(1,
	                          "Channel = " + std::to_string(chan) + " [" +
	                              udp.get_channel_name(chan) + "] wrote " +
	                              std::to_string(writeCount) + " events");
}
