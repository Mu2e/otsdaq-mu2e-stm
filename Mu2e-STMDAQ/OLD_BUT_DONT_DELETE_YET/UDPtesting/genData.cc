...
genData::genData(std::string file)
{
	...
	std::cout << numberhb << " " << deltahb * 8 * 1e-3 << " us events in file"
	          << std::endl;
}
...
void genData::readBinaryFile()
{
	...
	// Print file info
	std::cout << "Input data file size = " << binary_size * 1e-9
	          << " Gbytes = " << binary_len << " elements " << std::endl;
}

// Form an event/trigger headder
int16_t* genData::form_tHdr(uint16_t chan,
                            uint64_t ADCclock,
                            uint64_t eventNum,
                            uint64_t EWT,
                            uint64_t EM,
...
std::pair<uint64_t, int16_t*> genData::genEvents(int c, int eventNum)
{
	...
	for(uint i = 0; i < eventNum; i++)
	{
		...
		std::cout << "start_event_num = " << start_event_num << " | i = " << i
		          << "  |  eventNum: "
		          << "\n";
		...
		int16_t* tH_event = form_tHdr(c,
		                              ...
		                              runNum,
		                              DTCclock);
		for(int x = 0; x < fw_tHdr_Len; x++)
		{
			std::cout << "Header val at " << x << " is : " << tH_event[x] << "\n";
		}
		// Check event header
		chData.check_fw_tHdr(c, tH_event, 0);
		...
		for(int j = 0; j < eventLength; j++)
		{
			...
			if(dataFromFile)
			{
				events[event_start + fw_tHdr_Len + j] =
				    binary_data[adc_count % binary_len];
				adc_count++;
			}
	...
	if(c == 0)
	{  // only once
		std::cout << "Number of events = " << HEX(numberhb, 4) << " = " << numberhb
		          << std::endl;
		std::cout << "Event length = " << HEX(deltahb, 4) << " = " << deltahb << " = "
		          << deltahb_ns << " ns = " << deltahb_s << " s" << std::endl;
		if(dataFromFile)
		{
			std::cout << "Input binary file repeated "
			          << double(adc_count * sizeof(int16_t)) / (double)binary_size
			          << " times" << std::endl;
		}
		std::cout << "Total data size = " << event_array_len * sizeof(int16_t) * 1e-9
		          << " Gbytes" << std::endl;
	}
...
std::pair<uint64_t, int16_t*> genData::genPackets(int c, int16_t* events, int eventNum)
{
	...
	vector<int16_t>         current_packet(MAX_PACKET_LEN);
	...
	for(uint i = 0; i < eventNum; i++)
	{
		...
		while(eventToAdd != 0)
		{
			...
			if(fw_tHdr_Len >= leftInPacket)
			{
				...
				for(int j = start; j < start + leftInPacket; j++)
				{
					// If odd entry
					if(j % 2 != 0)
						current_packet[j] = BEEF;
					// If event entry
					if(j % 2 == 0)
						current_packet[j] = DEAD;
				}  // End j-loop
			...
			tH[fw_tHdr::EvLen] = eventLength;
			// Copy event header to start of packet
			memcpy(&current_packet[MAX_PACKET_LEN - leftInPacket], tH, fw_tHdr_Size);
		...
		{
			std::cout << "ERROR in genPackets: event_counter != deltahb_len!!!"
			          << std::endl;
			exit(0);
		}
	...
	if(c == 0)
	{  // only once
		std::cout << "Number of packets = " << packetNum << std::endl;
	}
...
// Update event header
void genData::update_tHdr(int16_t* data,
                          uint64_t hdr_start,
                          uint16_t chan,
                          uint64_t event_num)
{
