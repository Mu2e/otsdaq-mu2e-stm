...
int main()
{
	...
	std::cout << "Available system memory  = " << free_mem * 1e-9 << " Gbytes"
	          << std::endl;
	std::cout << "Allocating 50% of available memory (" << buffer_mem * 1e-9
	...
	std::cout << "Allocating " << buffer_size * 1e-9 << " Gbytes each to " << buffer_num
	          << " data buffers..." << std::endl;
	...
	{
		std::cout << "ERROR: More threads to be initialised than available cores!"
		          << std::endl;
		...
		if(processID[i] != SEND)
		{
			...
			for(int j = 0; j < chNum; j++)
			{
				...
				if(processID[i] == CHECK)
				{
					...
					process_thread[i][j] =
					    new std::thread(&FEthreads::check_data,  // Check data function
					                    ref(fe),                 // Frontend threads class
					                    j,                       // Channel number
					                    std::ref(udp[j]),        // UDP socket class
				...
				{
					process_thread[i][j] =
					    new std::thread(&FEthreads::form_events,  // Check data function
					                    ref(fe),               // Frontend threads class
					                    j,                     // Channel number
				...
				if(processID[i] == WRITE)
				{
					...
					write_monitor_thread[j] = new std::thread(
					    &queue_write::write_monitor_func, writeq, j, ref(udp[j]));

					// Start write data thread
					write_thread[j] = new std::thread(
					    &queue_write::write_data_func, writeq, j, ref(udp[j]));

					// Start write push thread
					process_thread[i][j] = new std::thread(
					    &FEthreads::write_data,  // Push data to write function
					    ref(fe),                 // Frontend threads class
					    j,                       // Channel number
				...
				{
					process_thread[i][j] = new std::thread(
					    &FEthreads::distribute_raw,  // Check data function
					    ref(fe),                     // Frontend threads class
					    j,                           // Channel number
					    dist_sock_raw[j],
					    std::ref(udp[j]),  // UDP socket class
