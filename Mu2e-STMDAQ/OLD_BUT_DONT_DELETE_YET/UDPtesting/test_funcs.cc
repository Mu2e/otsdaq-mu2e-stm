...
void test_funcs::client(
    int chan, UDPsocket& udp, int socket, int16_t* data, int packet_num)
{
	// Notify user
	...
	for(uint i = 0; i < packet_num; i++)
	{
		...
		if(msgNum == UDPsocket::SENDMMSG_NUM)
		{
			...
			// Send buffer
			int retval =
			    udp.send(snd_buffer, size_buffer, UDPsocket::SENDMMSG_NUM, socket);
	...
	// Notify user
	cout << "Channel = " << chan << " [" << udp.get_channel_name(chan) << "] sent "
	     << sendCount << " packets [" << data_size << " Gbytes] at " << sendSpeed
	     << " Gbit/s" << endl;
}

// Function for final data pull to compare input data for test
void test_funcs::pull_data(
    int chan, UDPsocket& udp, queue_buffer* pullq, uint64_t buffer_size, int16_t* data)
{
	// Notify user
...
void test_funcs::compare_data(
    int chNum, uint32_t numberhb, uint64_t* data_len, int16_t** input, int16_t** output)
{
	...
	for(uint c = 0; c < chNum; c++)
	{
		...
		while(count < data_len[c])
		{
			...
			{
				std::cout << "Error in compare output data!! Event numbers do not match "
				             "in  channel "
				          << c << std::endl;
			...
			{
				std::cout << "Error in compare output data!! Event lengths do not match "
				             "for event "
		...
		if(!success[c])
			std::cout << "CHANNEL " << c << " ERROR: Input/Output data not identical! "
