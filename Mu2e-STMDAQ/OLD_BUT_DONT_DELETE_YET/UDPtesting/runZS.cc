...
std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, int64_t, int64_t> sup;
...
void pull(int chan, int16_t* data)
{
	...
	while(pull_count < n)
	{
		...
		retval = qb->pull(timeout, chan, buffer);
		// Get time before suppression
		if(pull_count == 0)
			t1 = std::chrono::high_resolution_clock::now();
		// Call zero-supressin algorithmls
		sup = zs->do_ZS(0, retval, buffer, qb);
		// Memcpy sup data
		memcpy(
		    &data[tot_sup], zs->suppressed_data[0], std::get<0>(sup) * sizeof(int16_t));
		// Increase the total suppressed data length
		tot_sup += std::get<0>(sup);
		// Calculate rate
		peak_count += std::get<1>(sup);
		rate += std::get<2>(sup);
		rate_count++;
		// Calculate baseline
		baseline_count += std::get<3>(sup);
	...
	if(baseline_count != 0)
	{
		...
		baseline_rms =
		    sqrt(baseline_rms / baseline_count - baseline_mean * baseline_mean);
	}

	// Get data processing speed in Gbit/s
	double speed = pull_count * sizeof(int16_t) * 8 * 1e-9 / (time_ms * 1e-3);

	std::cout << "Zero Suppression Algorithm computing time = " << time_ms
	          << " milliseconds" << std::endl;
	std::cout << "Zero Suppression Algorithm computing speed = " << speed << " Gbit/s"
	          << std::endl;
	std::cout << "Number of elements in suppressed file: " << sup_data_len << std::endl;
	std::cout << "Suppressed data size = " << sup_data_len * sizeof(int16_t) * 1e-9
	          << " Gbytes" << std::endl;
	std::cout << "Number of triggers/peaks found: " << peak_count << std::endl;
	std::cout << "Rate = " << rate * 1e-3 << " kHz" << std::endl;
	std::cout << "Baseline mean = " << baseline_mean << " ± " << baseline_rms
	          << std::endl;
}
...
int main(int argc, char* argv[])
{
	...
	// Initialise zero classes with data length
	zs = new daqZS(n);
	qb = new queue_buffer(n);
	...
	// Call zero-supressin algorithmls
	std::tuple<int16_t*, uint64_t, uint64_t> output = zs->ZS_safe(0, n, ADC);
	...
	zs->first_peak[0]      = true;
	zs->peak_found[0]      = false;
	zs->left_to_store[0]   = 0;
	zs->peak_separation[0] = 0;
	zs->prev_num[0]        = 0;
	...
	if(success)
		std::cout << "SUCCESS: YOU ARE NOT A TIT!" << std::endl;
	if(!success)
