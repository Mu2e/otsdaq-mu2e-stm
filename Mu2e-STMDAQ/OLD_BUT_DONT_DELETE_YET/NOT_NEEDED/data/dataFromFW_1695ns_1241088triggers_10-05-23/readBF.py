...
# Open input file
with open('genData662keV_30kHz.bin', 'rb') as f:
...
# Open data file
with open('stm_hzdr_raw_HPGe_run0000001_subrun00000.bin', 'rb') as f:
    # Get data from file
...
plt.tight_layout()
plt.savefig('Plots/FW_output_compare.pdf')
...
for i in range(len(E_HDR_NAMES)):
    ...
    plt.tight_layout()
    plt.savefig('Plots/FW_output_%s.pdf' % E_HDR_NAMES[i])
...
plt.tight_layout()
plt.savefig('Plots/FW_output_clocks.pdf')

# raw_max = input_compare[max(input_compare)]
# print (raw_max)
# raw_min = input_compare[min(input_compare)]
# print (raw_min)
# for i in range(len(output_data)):
#     if (output_data[i] < raw_min or output_data[i] > raw_max):
