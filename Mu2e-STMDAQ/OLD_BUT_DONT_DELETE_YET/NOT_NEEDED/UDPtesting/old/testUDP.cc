...
void init_queue()
{
	...
	cout << "            = " << double(BUFFER_SIZE) / double(packetSize) << " packets"
	     << endl;
...
void client(int loop, packet data, int socket)
{
	...
	while(getCount != packetNum)
		cout << "Put: " << putCount << ". Get: " << getCount << "\r" << flush;
	cout << endl;
...
void server(int loop, int socket)
{
	...
	while(!udp.timeout)
	{
		...
		if(putCount == packetNum)
		{
			...
			cout << "put: tail = " << cbq.q.tail << ", head = " << cbq.q.head
			     << ", tail - head = " << cbq.q.tail - cbq.q.head << endl;
	...
	if(fw)
	{
		while(!udp.timeout)
		{
			...
			if(getCount == packetNum)
			{
				...
				cout << "get: putCount = " << putCount << ", getCount " << getCount
				     << endl;
				break;
			}
		}
...
int main()
{
	...
	cout << "Sending " << loopNum << " * " << numGbits / double(bytes2Gbits)
	     << " bytes (as " << packetNum << " packets of size " << packetSize * 1e-3
	     << " kbytes)" << endl;
	...
	for(int p = 1; p <= powerMax; p++)
	{
		...
		for(int i = 0; i < loopNum; i++)
		{
			...
			if(!fw)
				send_thread = new std::thread(client, i, sendPacket, sendSock);
		...
		if(fw)
		{
			cout << "Efficiency = " << eff[0] << " %. "
			     ...
			     << " Get speed = " << getSpeed[0] << " Gbit/s." << endl;
		}
		else
		{
			for(int i = 0; i < loopNum; i++)
	...
	if(!fw)
	{
		...
		cout << "-- Average -- " << endl;
		for(int p = 1; p <= powerMax; p++)
			cout << p << "," << effAvg[p - 1] << "," << sendSpeedAvg[p - 1] << ","
			     << recvSpeedAvg[p - 1] << endl;
		...
		if(success)
			std::cout << "SUCCESS: Input/Output data identical." << endl;
		if(!success)
			std::cout << "ERROR: Input/Output data not identical!" << endl;
	}
