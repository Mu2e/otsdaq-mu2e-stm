...
void queue_get(int chan)
{
	...
	{
		// End condition
		if((recvFinished[chan] && (getCount[chan] == recvCount[chan])) or
		   (getCount[chan] == packetNum))
		{
			// Exit infinite loop
			break;
		}
	...
	// Notify user
	cout << "Channel " << chan << " got " << getCount[chan] << " packets from queue"
	     << endl;
...
int main()
{
	...
	for(uint i = 0; i < chNum; i++)
	{
		...
		if(useQueue)
			get_thread[i] = new std::thread(&queue_get, i);
	...
	for(uint i = 0; i < chNum; i++)
	{
		...
		{  // If using queue...
			...
			cout << "Channel " << i << ": efficiency = " << eff[i] << " %. "
			     ...
			     << " Gbit/s. Get speed = " << getSpeed[i] << " Gbit/s." << endl;
		}
		else
		{  // If not using queue...
