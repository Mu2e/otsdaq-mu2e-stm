...
int main()
{
	...
	cout << "Sending " << numGbits / bytes2Gbits * 1e-9 << " Gbytes as " << packetNum
	     << " packets of size " << packetSize * 1e-3 << " kbytes" << endl;

	// Get time now
	auto start = chrono::steady_clock::now();
	// Loop through and touch all packets
	for(uint i = 0; i < packetNum; i++)
		sendPacket[0][0].data = sendPacket[0][0].data;
	// Get time after loop
	auto end = chrono::steady_clock::now();
	...
	cout << "Speed to loop through 1 * " << packetNum
	     << " packets = " << maxGbit / (timeNano * 1e-9) << " Gbit/s" << endl;
	...
	for(uint i = 0; i < chNum; i++)
	{
		...
		cout << "Channel " << i << ": efficiency = " << eff[i] << " %. "
		     ...
		     << " Gbit/s. Receive + speed = " << recvSpeed[i] << " Gbit/s." << endl;
	}
