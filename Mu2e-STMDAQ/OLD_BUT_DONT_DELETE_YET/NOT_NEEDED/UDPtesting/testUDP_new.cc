...
// Process names
static const string process_name[process_max] = {"SEND",
						 "RECEIVE",
						 "CHECK",
						 "ZS",
						 "PRESCALE",
...
int main(){
  ...
  std::cout << "Allocating 95% of available memory (" << buffer_mem*1e-9 << " Gbytes) to DAQ operations and buffers" << std::endl;
  ...
  if (buffer_num != 0){
    ...
    std::cout << "Allocating " << buffer_size*1e-9 << " Gbytes to each queue buffer" << std::endl;
  }  
  ...
  for (int i = 0; i < process_num; i++){          
	...
	// If intialising zero suppression process
	  ...
	  if (processID[i] == COMPARE){
	    ...
	    process_thread[i][j] = new std::thread (&pull_data, // Pull data function
						    j, // Channel number
  ...
  if (process[SEND]){      
    ...
    for (int j = 0; j < chNum; j++){      
      process_thread[SEND][j] = new std::thread (&client,j,send_packets[j],send_sock[j]);
    }	
  }	
