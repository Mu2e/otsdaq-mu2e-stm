...
// Process names
static const string process_name[process_max] = {
    "SEND", "RECEIVE", "CHECK", "ZS", "PRESCALE", "WRITE", "DISTRIBUTE", "COMPARE"};
...
// Plus Two buffers for each operation
// All multipled by the number of channels
static const uint buffer_max = chNum * 3 * process_max;

// Doing those processes
static const bool process[process_max] = {true,    // Send
                                          true,    // Receive
                                          false,   // Check
                                          false,   // Form Events
                                          false,   // ZS
...
void client(int chan, int16_t* data, int socket)
{
	...
	// Notify user
	cout << "Channel = " << chan << " [" << udp[chan].get_channel_name(chan) << "] sent "
	     << sendCount << " packets [" << data_size << " Gbytes] at " << sendSpeed
	     << " Gbit/s" << endl;
}

// Function for final data pull to compare input data for test
void pull_data(int chan, UDPsocket& udp, queue_buffer* pullq, uint64_t buffer_size)
{
	// Notify user
	...
	Logger::Instance()->write(1,
	                          "Channel = " + std::to_string(chan) + " [" +
	                              udp.get_channel_name(chan) + "] pulled " +
	                              std::to_string(data_size) + " Gbytes at " +
	                              std::to_string(speed) + " Gbit/s");
}
...
int main()
{
	...
	// Write event data
	//  queinitHW_ue_write<dataVars::off_event> writeq(stm_file);
	...
	if(process[SEND])
	{
		gen = new genData(numberhb, deltahb);
		...
		for(uint c = 0; c < chNum; c++)
		{
			...
			std::pair<uint64_t, int16_t*> packets =
			    gen->genPackets(c, event_data[c], numberhb);
			...
			if(compareOutput)
				outData[c] = new int16_t[event_array_len[c]];
		}

		// Print message about data to be sent
		cout << "Sending " << packetNum * MAX_PACKET_SIZE * 1e-9 << " Gbytes as "
		     << packetNum << " packets of size " << MAX_PACKET_SIZE * 1e-3 << " kbytes"
		     << endl;
		...
		if(packetNum <= 1e7)
		{
			...
			for(uint i = 0; i < packetNum; i++)
				send_packets[0][0] = send_packets[0][0];
			// Get time after loop
			auto end = chrono::steady_clock::now();
			...
			cout << "Time taken to loop through 1 * " << packetNum
			     << " packets = " << timeNano * 1e-6 << " ms" << endl;
			// Print speed to loop through pakcets
			cout << "Speed to loop through 1 * " << packetNum << " packets = "
			     << packetNum * MAX_PACKET_SIZE * bytes2Gbits / (timeNano * 1e-9)
			     << " Gbit/s" << endl;
		}
	}
	...
	std::cout << "Available system memory  = " << free_mem * 1e-9 << " Gbytes"
	          << std::endl;
	std::cout << "Allocating 95% of available memory (" << buffer_mem * 1e-9
	          << " Gbytes) to DAQ operations and buffers" << std::endl;
	...
	std::cout << "Allocating " << sock_mem * 1e-9
	          << " Gbytes to all UDP socket kernel buffers" << std::endl;
	buffer_mem -= sock_mem;
	...
	int buffer_num = buffer_max / process_max * process_num;
	if(process[SEND])
		buffer_num -= 1;
	...
	if(buffer_num != 0)
	{
		...
		buffer_len                        = buffer_size / sizeof(int16_t);
		std::cout << "Allocating " << buffer_size * 1e-9 << " Gbytes to each queue buffer"
		          << std::endl;
	}

	// Now create and allocate queues
	queue_buffer* process_queue[buffer_num];
	for(int i = 0; i < buffer_num; i++)
	{
		std::cout << "Process " << i << "/" << process_num - 1 << std::endl;
	...
	{
		std::cout << "ERROR: More threads to be initialised that available cores!"
		          << std::endl;
	...
	// std::thread *dqm_thread = new std::thread (&FEthreads::dqm_client, // dqm
	// 					     ref(fe), // Frontend threads class
	// 					     dqm_sock, // UDP socket number
	// 					     std::ref(udp[0]), // UDP socket
	// 					     std::ref(udp_dqm),
	// 					     std::ref(hw)); // Hardware connection
		...
		if(processID[i] != SEND)
		{
			...
			for(int j = 0; j < chNum; j++)
			{
				...
				if(processID[i] == RECEIVE)
				{
					...
					process_thread[i][j] =
					    new std::thread(&FEthreads::server,  // Server funtion
					                    ref(fe),             // Frontend threads class
					                    j,                   // Channel number
				...
				if(processID[i] == CHECK)
				{
					...
					process_thread[i][j] = new std::thread(
					    &FEthreads::check_data,  // Check data function
					    ref(fe),                 // Frontend threads class
					    j,                       // Channel number
					    std::ref(udp[j]),        // UDP socket class
					    process_queue[RECEIVE],  // Check data buffer queue
					    process_queue[CHECK]);   // Form events buffer queue
				}
				// If intialising form events process
				if(processID[i] == EVENTS)
				{
					process_thread[i][j] = new std::thread(
					    &FEthreads::form_events,  // Check data function
					    ref(fe),                  // Frontend threads class
					    j,                        // Channel number
					    std::ref(ev),             // Form events class
					    std::ref(udp[j]),         // UDP socket class
					    process_queue[CHECK],     // Form events buffer queue
					    process_queue[EVENTS]);   // Zero suppress buffer queue
				}
				// If intialising zero suppression process
				if(processID[i] == ZS)
				{
					process_thread[i][j] = new std::thread(
					    &FEthreads::zs_data,    // Check data function
					    ref(fe),                // Frontend threads class
					    j,                      // Channel number
					    std::ref(udp[j]),       // UDP socket class
					    process_queue[EVENTS],  // Zero suppress buffer queue
					    process_queue[ZS]);     // Prescale buffer queue
				}
				// If intialising prescale process
				if(processID[i] == PRESCALE) {}

				// If intialising comapre process - TESTING ONLY
				if(processID[i] == COMPARE)
				{
					// Start pull data thread
					process_thread[i][j] =
					    new std::thread(&pull_data,   // Pull data function
					                    j,            // Channel number
	...
	if(process[SEND])
	{
		...
		for(int j = 0; j < chNum; j++)
		{
			process_thread[SEND][j] =
			    new std::thread(&client, j, send_packets[j], send_sock[j]);
		}
	}
	...
	if(process[COMPARE])
	{
		...
		for(uint c = 0; c < chNum; c++)
		{
			...
			while(count < event_array_len[c])
			{
				...
				{
					std::cout << "Error in compare output data!! Event numbers do not "
					             "match in  channel "
					          << c << std::endl;
				...
				{
					std::cout << "Error in compare output data!! Event lengths do not "
					             "match for event "
					          << out_event_num << " in  channel " << c << std::endl;
					std::cout << "Input data event length = " << event_len << std::endl;
					std::cout << "Output data event length = " << out_event_len
					          << std::endl;
					std::cout << "Exiting..." << std::endl;
				}
			...
			if(success[c])
				std::cout << "CHANNEL " << c << " SUCCESS: Identical input/output for "
				          << event_count << " events (" << numberhb
				          << " headers). Prescale ~ "
				          << round((double)numberhb / (double)event_count) << std::endl;
			if(!success[c])
				std::cout << "CHANNEL " << c
				          << " ERROR: Input/Output data not identical! " << false_count
				          << "/" << event_array_len[c] << " elements = "
				          << (double)false_count / (double)event_array_len[c] * 100
				          << "% incorrect..." << endl;
		}
	}
