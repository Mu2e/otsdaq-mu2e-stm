...
template<typename T>
int queue_buffer<T>::try_push(int chan, T* data, int n, int index)
{
	int retval = 0;
	...
	if(write_ahead)
	{
		...
		else
		{
			...
			memcpy(
			    &buffer[chan][current_tail[chan]], &data[index], end * sizeof(data[0]));
			...
			if(left <= avail)
			{
				...
				memcpy(&buffer[chan][0], &data[index + end], left * sizeof(data[0]));
				// Update retval
				retval = end + left;
			}
			...
			{
				// memcpy to start of queue buffer
				memcpy(&buffer[chan][0], &data[index + end], avail * sizeof(data[0]));
				// Update retval
				retval = end + avail;
			}
		}
	...
	else
	{
		...
		// memcpy to queue buffer
		memcpy(&buffer[chan][current_tail[chan]], &data[index], retval * sizeof(data[0]));
	}
...
// Push data to queue
template<typename T>
void queue_buffer<T>::push(int chan, T* data, int n)
{
...
template<typename T>
int queue_buffer<T>::try_pull(int chan, T*& data)
{
	int retval = 0;
...
template<typename T>
int queue_buffer<T>::pull(bool* timeout, int chan, T*& data)
{
	int retval = 0;
