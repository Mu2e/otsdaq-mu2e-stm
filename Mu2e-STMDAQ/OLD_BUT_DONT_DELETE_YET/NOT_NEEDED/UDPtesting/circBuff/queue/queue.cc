...
void queue::init(size_t s)
{
	...
	if(s % getpagesize() != 0)
	{
		queue_error("Requested size (%lu) is not a multiple of the page size (%d)",
		            s,
		            getpagesize());
	}
	...
	if((q.buffer = (uint8_t*)mmap(
	        NULL, 2 * s, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)) == MAP_FAILED)
	{
		queue_error_errno("Could not allocate virtual memory");
	}

	// Mmap first region
	if(mmap(q.buffer, s, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, q.fd, 0) ==
	   MAP_FAILED)
	{
	...
	if(mmap(q.buffer + s, s, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, q.fd, 0) ==
	   MAP_FAILED)
	{
		queue_error_errno("Could not map buffer into virtual memory");
	}
...
void queue::put(int16_t* data, int16_t data_size)
{
	// Lock the queue mutex
	pthread_mutex_lock(&q.lock);

	// Wait for space to become available
	while(q.size - (q.tail - q.head) < data_size + sizeof(data_size))
	{
	...
	memcpy(q.buffer + q.tail, &data_size, sizeof(data_size));
	//  memcpy(&q.buffer[q.tail + sizeof(data_size)], data, data_size );
	memcpy(q.buffer + q.tail + sizeof(data_size), data, data_size);

	// Increment write index in bytes
	q.tail += data_size + sizeof(data_size);

	// Signal the queue is readable
...
size_t queue::get(int16_t* data, int16_t max_size)
{
	// Lock the queue mutex
	pthread_mutex_lock(&q.lock);
	...
	memcpy(&data_size, q.buffer + q.head, sizeof(data_size));
	...
	// Read message body
	//  memcpy(data, &q.buffer[q.head + sizeof(message_t)], m.len);
	memcpy(data, q.buffer + q.head + sizeof(data_size), data_size);

	// Consume the message by incrementing the read pointer
	q.head += data_size + sizeof(data_size);
	//  q.head_seq++;
