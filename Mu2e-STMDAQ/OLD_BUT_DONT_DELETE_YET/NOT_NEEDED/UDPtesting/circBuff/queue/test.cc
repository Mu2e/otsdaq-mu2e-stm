...
int main(int argc, char* argv[])
{
	...
	for(int i = 0; i < MESSAGES_TOT; i++)
	{
		for(int j = 0; j < arraySize; j++)
		{
			if(dataToPublish[i][j] != consumedData[i][j])
				success = false;
		}
	}
	if(success)
		std::cout << "SUCCESS: Input/Output data identical." << std::endl;
	if(!success)
		std::cout << "ERROR: Input/Output data not identical!" << std::endl;
