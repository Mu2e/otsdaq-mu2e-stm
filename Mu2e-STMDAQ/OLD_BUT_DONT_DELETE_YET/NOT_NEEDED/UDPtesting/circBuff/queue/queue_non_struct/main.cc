...
void push(int chan, int16_t (*data)[packet_len])
{
	...
	size_buffer[queue_buffer::SIZE_INDEX] = new uint64_t[push_max]();  // size
	...
	{
		size_buffer[queue_buffer::SIZE_INDEX][i] = packet_len * sizeof(int16_t);  // size
	...
	while(retval < (packet_num))
	{
		...
		if((retval + push_num) > packet_num)
			push_num = packet_num - retval;
...
int main()
{
	...
	if(success)
		std::cout << "SIZE SUCCESS: you are not a tit!" << std::endl;
	if(!success)
		std::cout << "SIZE FAILURE: you are a tit!" << std::endl;
	...
	for(int i = 0; i < packet_num; i++)
	{
		for(int j = 0; j < packet_len; j++)
		{
			if(push_packets[i][j] != pull_data[count])
				success = false;
			count++;
		}
	}
	if(success)
		std::cout << "DATA SUCCESS: you are not a tit!" << std::endl;
	if(!success)
		std::cout << "DATA FAILURE: you are a tit!" << std::endl;
