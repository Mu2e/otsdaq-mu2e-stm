...
struct tester
{
	...
	tester(int x = 0, int y = 0) : x(x), y(y) {}
};
...
queue_buffer::queue_buffer()
{
	...
	{
		std::cout << "CRITICAL ERROR. Queue buffer length cannot be more than "
		          << INT32_T_MAX << std::endl;
		exit(0);
	}
...
int queue_buffer::try_push(int chan, uint64_t** sizes, int16_t* data, int n, int index)
{
	...
	current_tail_size[chan] =
	    make_int32_t(current_tail[chan].size_0, current_tail[chan].size_1);
	// Current data buffer pointer
	current_tail_data[chan] =
	    make_int32_t(current_tail[chan].data_0, current_tail[chan].data_1);
	...
	start_index = sizes[ACC_SIZE_INDEX][index] - sizes[SIZE_INDEX][index];
	push_size   = sizes[ACC_SIZE_INDEX][index + push_num - 1] -
	            sizes[ACC_SIZE_INDEX][index] + sizes[SIZE_INDEX][index];
	push_len = push_size / 2;

	// Copy to size buffer
	// If the increase wraps the size buffer around
	if(increment(current_tail_size[chan], push_num, RING_BUFFER_NUM) <
	   current_tail_size[chan])
	{
	...
	else
	{
		...
		memcpy(&size_buffer[chan][current_tail_size[chan]],
		       &sizes[SIZE_INDEX][index],
		       push_num * sizeof(uint64_t));
	}

	// Copy to data buffer
	// If the increase wraps the data buffer around
	if(increment(current_tail_data[chan], push_len, RING_BUFFER_LEN) <
	   current_tail_data[chan])
	{
		// Get the distance to the end of the buffer
	...
	else
	{
		// Memcpy all datagrams
		memcpy(
		    &data_buffer[chan][current_tail_data[chan]], &data[start_index], push_size);
	}
	...
	current_tail_size[chan] =
	    increment(current_tail_size[chan], push_num, RING_BUFFER_NUM);
	...
	current_tail_data[chan] =
	    increment(current_tail_data[chan], push_len, RING_BUFFER_LEN);
...
int queue_buffer::try_pull(int chan, uint64_t*& sizes, int16_t*& data)
{
	...
	// Current size buffer pointer
	current_head_size[chan] =
	    make_int32_t(current_head[chan].size_0, current_head[chan].size_1);
	// Current data buffer pointer
	current_head_data[chan] =
	    make_int32_t(current_head[chan].data_0, current_head[chan].data_1);
	...
	if(increment(current_head_size[chan], pull_num, RING_BUFFER_NUM) <
	   current_head_size[chan])
	{
		// Get the distance to the end of the buffer
		...
		memcpy(
		    sizes, &size_buffer[chan][current_head_size[chan]], end * sizeof(uint64_t));
	...
	else
	{
		// Memcpy all datagrams
		memcpy(sizes,
		       &size_buffer[chan][current_head_size[chan]],
		       pull_num * sizeof(uint64_t));
	}

	// Copy from data buffer
	// If the increase wraps the buffer around
	if(increment(current_head_data[chan], pull_len, RING_BUFFER_LEN) <
	   current_head_data[chan])
	{
		// Get the distance to the end of the buffer
	...
	// Get new current size buffer pointer values
	current_head_size[chan] =
	    increment(current_head_size[chan], pull_num, RING_BUFFER_NUM);
	...
	// Get new current data buffer pointer
	current_head_data[chan] =
	    increment(current_head_data[chan], pull_len, RING_BUFFER_LEN);
