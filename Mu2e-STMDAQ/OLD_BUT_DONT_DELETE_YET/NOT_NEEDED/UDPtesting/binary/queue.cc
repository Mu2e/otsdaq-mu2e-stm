...
// Put data into queue
void put(int chan){
  std::cout << "Pushing packets to write queue for channel " << chan << "..." << std::endl;
  for(int i = 0; i < packetNum; i++){
    //    cbq.push(0,packets[i]);
    bfq.push(chan,packets[chan][i]);
  }
  std::cout << "All packets for channel " << chan << " pushed to write queue" << std::endl;
}
...
int main (int argc, char** argv){
  ...
  for (int c = 0; c < channels; c++){
    ...
    for (int i = 0; i < file_num; i++){
      // Get file name
      std::string filename = "data/channel" + std::to_string(c)
  ...
  for (int c = 0; c < channels; c++){
    for(int i = 0; i < packetNum; i++){
  	...
  	if (packets[c][i][j] != packets2[c][i][j]){
  	  std::cout << i << "   " << j << "   " << packets[c][i][j] << "   " <<  packets2[c][i][j] << std::endl;	
