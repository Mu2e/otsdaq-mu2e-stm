...
// Number of packets
static const int packetNum = 5e5;
// Packet array
int16_t** packets[channels];
int16_t** packets2[channels];

// Signal exitFE to stop the data file write
bool exitFE = false;

// Monitor files to open/close
void monitor(int chan)
{
	while(!bfq.check_write_finished(chan))
		bfq.monitor_files(chan);
}

// Put data into queue
void put(int chan)
{
	std::cout << "Pushing packets to write queue for channel " << chan << "..."
	          << std::endl;
	for(int i = 0; i < packetNum; i++)
	{
		//    cbq.push(0,packets[i]);
		bfq.push(chan, packets[chan][i]);
	}
	std::cout << "All packets for channel " << chan << " pushed to write queue"
	          << std::endl;
}
...
int main(int argc, char** argv)
{
	...
	{
		packets[c]  = new int16_t*[packetNum];
		packets2[c] = new int16_t*[packetNum];
		for(int i = 0; i < packetNum; i++)
		{
			packets[c][i]  = new int16_t[packetLen]();
			packets2[c][i] = new int16_t[packetLen]();
	...
	// Define threads
	std::thread* monitor_thread[channels];
	std::thread* put_thread[channels];
	//  std::thread *get_thread[channels];
	std::thread* write_thread[channels];

	// Start threads
	for(int i = 0; i < channels; i++)
	{
		monitor_thread[i] = new std::thread(&monitor, i);
		put_thread[i]     = new std::thread(&put, i);
		//    get_thread[i] = new std::thread (&get,i);
	...
	for(int i = 0; i < channels; i++)
	{
		...
		monitor_thread[i]->join();
	}

	// Get the number of packets per binary file
	int packets_per_file = bfq.get_packets_per_file();
	...
	for(int c = 0; c < channels; c++)
	{
		...
		{
			// Get file name
			std::string filename = "data/channel" + std::to_string(c) + "_subrun" +
			                       std::to_string(i) + ".bin";
			// Open binary file
			...
			for(int j = 0; j < packets_per_file; j++)
			{
				...
				bf.read((char*)packets2[c][p], packetSize);
	...
	for(int c = 0; c < channels; c++)
	{
		for(int i = 0; i < packetNum; i++)
		{
			for(int j = 0; j < packetLen; j++)
			{
				if(packets[c][i][j] != packets2[c][i][j])
				{
					std::cout << i << "   " << j << "   " << packets[c][i][j] << "   "
					          << packets2[c][i][j] << std::endl;
					std::cout << "CHNANEL " << c << " ERROR" << std::endl;
					exit(0);
				}
			}
		}
		std::cout << "CHANNEL " << c << " SUCCESS" << std::endl;
	}
