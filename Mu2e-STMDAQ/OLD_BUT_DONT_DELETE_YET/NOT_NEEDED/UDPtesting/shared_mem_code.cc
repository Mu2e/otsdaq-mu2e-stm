...
genData gen(
    "/data1/claudia_simulation_binary_files/662keV_0.32mV/genData662keV_20kHz.bin");
...
int main()
{
	...
	std::cout << "Available system memory  = " << free_mem * 1e-9 << " Gbytes"
	          << std::endl;
	std::cout << "Allocating 80% of available memory (" << buffer_mem * 1e-9
	...
	std::cout << "Allocating " << queue_buffer_mem * 1e-9
	          << " Gbytes to each queue buffer" << std::endl;
	...
	for(uint i = 0; i < chNum; i++)
	{
		...
		// Start form events thread
		zero_sup_thread[i] =
		    new std::thread(&FEthreads::zs_data,  // Check data function
		                    ref(fe),              // Frontend threads class
		                    i,                    // Channel number
		                    std::ref(udp[i]),     // UDP socket class
		...
		prescale_thread[i] =
		    new std::thread(&FEthreads::prescale_data,  // Check data function
		                    ref(fe),                    // Frontend threads class
		                    i,                          // Channel number
		                    std::ref(udp[i]),           // UDP socket class
