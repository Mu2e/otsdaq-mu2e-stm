...
int main(int argc, char* argv[])
{
	...
	std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, int64_t, int64_t> sup =
	    zsup.do_ZS(0, event_array_len, event_data, zq);

	// Get time after suppression
	t2 = std::chrono::high_resolution_clock::now();

	// Get output data
	sup_data_len           = std::get<0>(sup);
	peak_count             = std::get<1>(sup);
	double  rate           = std::get<2>(sup);
	int64_t baseline_count = std::get<3>(sup);
	...
	while(event_count < gen.numberhb)
	{
		...
		t1 = std::chrono::high_resolution_clock::now();
		// Call zero-supressin algorithmls
		sup = zsup.do_ZS(0, array_len, array, zq);
		// Get time after suppression
		t2 = std::chrono::high_resolution_clock::now();
		// Get output data
