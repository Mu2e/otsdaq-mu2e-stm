...
int main(int nargv, char** args)
{
	...
	std::cout << "filename " << filename << " run = " << run << " subrun = " << subrun
	          << " # triggers = " << num_triggers << std::endl;

	for(int i = 0; i < num_triggers; ++i)
	{
		peaks* peak_data = bf_in->read_MWD_data();
		std::cout << "Trigger number = " << peak_data->trigger_number
		          << " Trigger Type =  " << peak_data->trigger_type
		          << " # peaks = " << peak_data->npeaks
		          << " # ADC values = " << peak_data->nadc_values << std::endl;
		if(peak_data->npeaks > 0)
		{
			std::cout << " First peak : energy = " << peak_data->peak_heights.at(0)
			          << std::endl;
			std::cout << " First peak : times = " << peak_data->peak_times.at(0)
			          << std::endl;
		}
		delete peak_data;
	}
