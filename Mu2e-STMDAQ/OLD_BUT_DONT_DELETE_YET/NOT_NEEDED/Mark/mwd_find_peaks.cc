...
int main(int nargv, char** args)
{
	...
	int adc_offset_time_clock =
	    xml_file->int_value("stm.adc_offset_time_clock", 125);  // in MHz
	...
	int raw_filesize_header_type =
	    xml_file->int_value("stm.raw_filesize_header_type", 1);  // 1: new header, 0: old
	...
	std::cout << "filename " << filename << " run = " << run << " subrun = " << subrun
	          << std::endl;
	...
	if(results.size() == 2)
	{
		std::cout << "Mean for this run, subrun = " << results["mean"]
		          << " and rms = " << results["rms"] << std::endl;
	}
	...
	std::vector<uint32_t> positions = bf_in->get_trigger_locations(
	    true, true);  // return array of file positions of external & internal triggers
	if(positions.size() == 0)
	{
	...
	else
	{
		...
		std::cout << " Number of triggers being analysed = " << positions.size()
		          << std::endl;
		for(unsigned int i = 0; i < positions.size(); i++)
		{
			...
			double time_offset =
			    ((double)bf_in->getTriggerTime(header)) / ((double)trigger_time_clock) +
			    ((double)bf_in->getADC0Time(header)) /
			        ((double)adc_offset_time_clock);  // in us.
