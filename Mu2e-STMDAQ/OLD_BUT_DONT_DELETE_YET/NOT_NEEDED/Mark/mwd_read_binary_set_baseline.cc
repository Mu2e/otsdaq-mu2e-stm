...
int main(int nargv, char** args)
{
	...
	std::vector<uint32_t> positions = bf->get_trigger_locations(
	    false, true);  // return array of file positions of internal triggers
	...
	else
	{
		...
		db->write_hpge_baseline(
		    run, subrun, mean_baseline, mean_rms_baseline);  // run,subrun,mean,rms
	}
	db->close();
	delete xml_file;
	delete mwd;
