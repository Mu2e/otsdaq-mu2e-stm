...
boost::variate_generator<boost::hellekalek1995, boost::uniform_real<float> > frand(
    boost::hellekalek1995(), boost::uniform_real<float>(0.0f, 1.0f));
...
void random_signal_test_pattern(int16_t* data, int trigger_rate, unsigned int N)
{
	...
	std::cout << "Time to generate data for this trigger = " << nsec.count() << " ns "
	          << std::endl;
}
...
void begin_run(int irun)
{
	...
	int max_binary_file_size =
	    xml_file->int_value("stm.max_binary_file_size", 100);  // in Mb
...
void end_run(int irun) { bf->close_output_file(); }

void generate_header(int16_t* header_data,
                     ...
                     uint16_t header_size)
{
	...
	uint32_t payload_size =
	    uint32_t(data_size) + (uint32_t)(2 * header_size);  // in bytes not int16
...
void event_trigger(int itrig)
{
	...
	if(itrig == 0)
	{
		std::cout << "Samples per trigger = " << nsamples_per_trigger << "; "
		          << float(nsamples_per_trigger * 2) / 1048576.0 << " Mb " << std::endl;
