...
void FMC144::clocktree_init(IPBusManager* hw, int clockmode)
{
	...
	spi_144.spi_write(hw,
	                  "LMK04828_CTRL",
	                  0x100,
	                  0x08);  //DIV_CLKOUT1_0 DIV_BY_7 IDL/ODL ==>In/Out Drive level
	...
	spi_144.spi_write(
	    hw, "LMK04828_CTRL", 0x107, 0x11);  // FMT_CLK1_0 LVDS,LVDS !DCLK_INV , !DCLK_INV
	...
	spi_144.spi_write(hw,
	                  "LMK04828_CTRL",
	                  0x10F,
	                  0x55);  //FMT_CLK3_2 LVPECL,LVPECL, !DCLK_INV, !SCLK_INV
	...
	spi_144.spi_write(
	    hw, "LMK04828_CTRL", 0x117, 0x15);  //DIV_CLKOUT7_6 // 16 to enable sysref
	...
	spi_144.spi_write(
	    ...
	    "LMK04828_CTRL",
	    0x138,
	    0x00);  //VCO_OSC_OUT VCO0 (Low speed), PLL1_FB=OSCin, OSSTD::COUT=input
	...
	spi_144.spi_write(
	    hw, "LMK04828_CTRL", 0x13F, 0x00);  //FB_CTRL PLL2_FB=prescaler, PLL1_FB=OSCIN
	...
	if(clockmode == getClockMode("EXTERNAL") or
	   clockmode == getClockMode("EXTERNAL_REF") or clockmode == getClockMode("DDS"))
	{
	...
	{  //internal clock
		spi_144.spi_write(
		    ...
		    "LMK04828_CTRL",
		    0x147,
		    0x2A);  //CLKIN_MUX CLKIN: !INVERT,Auto, CLKIN1=PLL1, CLKIN0=PLL1
	}
	...
	{
		int check =
		    spi_144.spi_check(hw, "LMK04828_CTRL", 0x182, 0x02, 0x02, "PLL1 not locked");
		if(check == 1)
		{
	...
	{
		int check =
		    spi_144.spi_check(hw, "LMK04828_CTRL", 0x183, 0x02, 0x02, "PLL2 not locked");
		if(check == 1)
		{
...
int FMC144::adc_init(IPBusManager* hw, int adc_mode)
{
	...
	std::cout << HEX(dword, 8) << std::endl;
	int check = spi_144.spi_check(hw,
	                              "ADC16DX370_CTRL0",
	                              0x04,
	                              NULL,
	                              getADCpartID(),
	                              "Error - wrong ADC0 part no.");
	if(check == 1)
	{
...
int FMC144::dac_init(IPBusManager* hw, int odelay, int number_of_lanes)
{
	...
	if(number_of_lanes == getADCmode("2CH_4LANE") or
	   number_of_lanes == getADCmode("4CH_4LANE"))
	{
		reg_3B    = 0x0000;
		reg_31    = 0x6C08;
		reg_32    = 0x01C0;
	...
	dword     = dword & 0xFF;
	int check = spi_144.spi_check(
	    hw, "DAC38J84_CTRL0", 0x7F, 0xFF, getDACpartID(), "Error - wrong DAC part ID");
	if(check == 1)
	{
...
int FMC144::init(IPBusManager* hw, int clockmode, int adc_mode)
{
	...
	else if(clockmode == getClockMode("EXTERNAL") or
	        clockmode == getClockMode("EXTERNAL_REF"))
	{
		hw->write(node, 0x08);
	}
	else
