...
int HWcomms::setRegisters(IPBusManager* hw)
{
	...
	// Set prescale value
	uint prescale = xml_file->int_from_hex_value("stm.fmc144_prescale");
	hw->write("Buffers.Test_beam.prescale", prescale);
	//  cout << "prescale = " << HEX(prescale,2) << endl;

	// Set external mode ADC offset
	uint ext_ADCoffset = xml_file->int_from_hex_value("stm.fmc144_ext_ADC_offset");
	...
	uint interpacket_pause =
	    xml_file->int_from_hex_value("stm.fmc144_teng_interpacket_pause");
	...
	uint ext_sliceLength = xml_file->int_from_hex_value("stm.fmc144_ext_slice_length");
	...
	uint ext_sliceNum = xml_file->int_from_hex_value("stm.fmc144_ext_slice_num");
	...
	uint extIntDelay = xml_file->int_from_hex_value("stm.fmc144_ext_int_delay");
	...
	uint int_trigNum = xml_file->int_from_hex_value("stm.fmc144_int_trig_num");
	hw->write("Buffers.Test_beam.int_trig_num", int_trigNum);
	//  cout << "int_trigNum = " << HEX(int_trigNum,2) << endl;

	// Set internal mode ADC offset
	uint int_ADCoffset = xml_file->int_from_hex_value("stm.fmc144_int_ADC_offset");
	...
	uint int_sliceLength = xml_file->int_from_hex_value("stm.fmc144_int_slice_length");
	...
	uint int_sliceNum = xml_file->int_from_hex_value("stm.fmc144_int_slice_num");
	...
	uint ext_trigTimeout = xml_file->int_from_hex_value("stm.fmc144_ext_trig_timeout");
...
uint32_t HWcomms::getDDR4addr(IPBusManager* hw)
{
	...
	uint32_t temp =
	    hw->read("Buffers.Debug_RAM_Status.Status_trig_and_wr_address_return");
