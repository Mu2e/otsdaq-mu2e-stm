...
int FMC120_adc::adc_init(IPBusManager* hw)
{
	...
	spi_120.spi_write(
	    hw, ADC_SELECT_BOTH, 0x11, 0x80);  //select master page of analog bank
	...
	spi_120.spi_write(
	    ...
	    ADC_SELECT_BOTH,
	    0x004F,
	    0x00);  //DC coupling enable Bit 0 = off,  1 = Enabled shifts VCM at adc down ~ 200mV ***
	spi_120.spi_write(
	    hw, ADC_SELECT_BOTH, 0x0026, 0x40);  //IGNORE inputs on power down pin
	...
	{  //First Nyquist zone
		spi_120.spi_write(
		    hw, ADC_SELECT_BOTH, 0x6842, dword);  //Set nyquist Zone 0 0-500Mhz
	...
	{  //Second Nyquist zone
		spi_120.spi_write(hw,
		                  ADC_SELECT_BOTH,
		                  0x6842,
		                  dword);  //Set nyquist Zone  zone 2 500 to 1000MHz
		spi_120.spi_write(hw, ADC_SELECT_BOTH, 0x684E, 0x80);  //Enable nyquist correction
	...
	{  //Third Nyquist zone
		spi_120.spi_write(hw,
		                  ADC_SELECT_BOTH,
		                  0x6842,
		                  dword);  //Set nyquist Zone 3 = zone 3 1000 to 1500MHz
		spi_120.spi_write(hw, ADC_SELECT_BOTH, 0x684E, 0x80);  //Enable nyquist correction
	...
	{
		spi_120.spi_write(
		    hw, ADC_SELECT_BOTH, 0x4005, 0x01);  //enable single channel writes
		spi_120.spi_write(
		    hw, ADC_SELECT_BOTH, 0x4004, 0x68);  //Upper byte of page address
		spi_120.spi_write(hw, ADC_SELECT_BOTH, 0x4003, 0x00);  //middle byte
		spi_120.spi_write(hw, ADC_SELECT_BOTH, 0x4002, 0x00);  //middle byte
		spi_120.spi_write(
		    hw, ADC_SELECT_BOTH, 0x4001, 0x00);  //lower byte of 32bit page address
		spi_120.spi_write(
		    hw,
		    ADC_SELECT_BOTH,
		    0x604E,
		    0x20);  //for CH-A, write to register address 0x4E for a feature Update
		spi_120.spi_write(
		    hw,
		    ADC_SELECT_BOTH,
		    0x704E,
		    0x20);  //for CH-B, write to register address 0x4E for a feature Update
	...
	spi_120.spi_write(
	    hw, ADC_SELECT_BOTH, 0x6007, 0x08);  //set internal defaults JESDV and subclass V1
	...
	if(1)
	{
		...
		spi_120.spi_write(
		    hw, ADC_SELECT_BOTH, 0x4005, 0x01);  // enable single channel writes
		spi_120.spi_write(
		    hw, ADC_SELECT_BOTH, 0x4004, 0x61);  // Upper byte of page address
		spi_120.spi_write(hw, ADC_SELECT_BOTH, 0x4003, 0x00);  // middle byte
		spi_120.spi_write(hw, ADC_SELECT_BOTH, 0x4002, 0x00);  // middle byte
		spi_120.spi_write(
		    hw, ADC_SELECT_BOTH, 0x4001, 0x00);  // lower byte of 32bit page address
		...
		// Read Modify Write
		dword = static_cast<uint16_t>(spi_120.spi_read(
		    hw, ADC0_SELECT, 0x6068));  // Read reset value on FOVR Threshold
		dword = dword | 0x80;
		spi_120.spi_write(
		    hw,
		    ADC_SELECT_BOTH,
		    0x6068,
		    dword);  // for CH-A, write to register address 68 data 0x82 in page 68000 Setting bit 7 Freezes DC correction
		// clearing bit 7 enables DC offset correction, 0x02 enables DC correction. Bits D6-D3 set Bandwidth value
		std::cout << "Working ..." << std::endl;
		// Read Modify Write
		dword = static_cast<uint16_t>(spi_120.spi_read(
		    hw, ADC0_SELECT, 0x7068));  // Read reset value on FOVR Threshold
		dword = dword | 0x80;
		spi_120.spi_write(
		    hw,
		    ADC_SELECT_BOTH,
		    0x7068,
		    dword);  // for CH-B, write to register address 68 data 0x82 in page 680000 Setting bit 7 Freezes DC correction.
		// clearing bit 7 enables DC offset correction, 0x02 enables DC correction. Bits D6-D3 set Bandwidth value
		std::cout << "Working ..." << std::endl;
