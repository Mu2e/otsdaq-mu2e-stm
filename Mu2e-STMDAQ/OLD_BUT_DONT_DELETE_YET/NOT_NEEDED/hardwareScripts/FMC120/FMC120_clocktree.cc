...
int FMC120_clocktree::clocktree_init(IPBusManager* hw, uint clockmode)
{
	...
	spi_120.spi_write(hw,
	                  LMK_SELECT,
	                  0x148,
	                  0x33);  //CLKIN_SEL0_MUX Configured as LMK MISO Push Pull Output
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x002, 0x00);  //POWERDOWN Disabled (Normal operation);
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x103,
	    0x05);  //ANA_DLY_DCLK2 No Analog Delay, Half step duty cycle correction, 50% Duty Cycle!  (DAC Spec id 40 -/ 60% MAX);
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x104,
	    0x62);  //DIG_DLY_SCLK1 DCLK halfstep=-0.5, SCLK sourced from DCLK,  2 vco clock cycle delay on sysref out
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x105, 0x00);  //ANA_DLY_SCLK1 SCLK analog delay disabled
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x106,
	    0xB0);  //PD_CLK1_0 !DIG_DLY, GLITCHLESS Half Step ON, !ANAGLITCH, !ANA_DLY, EN_DCLK, ACTIVE, EN_SCLK
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x107, 0x55);  //Dclk =LVPECL, Sclk = LVPECL, !DCLK_INV, !SCLK_INV
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x10C,
	    0x62);  //DIG_DLY_SCLK3  DCLK halfstep=-0.5, SCLK sourced from DCLK,  2 vco clock cycle delay on sysref out
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x10D, 0x0A);  //ANA_DLY_SCLK3  SCLK analog delay disabled
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x10E,
	    0xB0);  //PD_CLK2/3 !DIG_DLY, GLITCHLESS Half Step ON, !ANAGLITCH, !ANA_DLY, EN_DCLK, ACTIVE, EN_SCLK
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x10F,
	    0x11);  //FMT_CLK3_3 Dclock = LVPECL, Sclock = LVPECL !DCLK_INV, !SCLK_INV
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x113,
	    0x05);  //ANA_DLY_DCLK4 No Analog Delay, Half step duty cycle correction, 50% Duty Cycle!  (DAC Spec id 40 -/ 60% MAX);
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x114,
	    0x62);  //DIG_DLY_SCLK5 DCLK halfstep=-0.5, SCLK sourced from DCLK,  2 vco clock cycle delay on sysref out
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x115, 0x00);  //ANA_DLY_SCLK5 SCLK analog delay disabled
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x116,
	    0xB0);  //PD_CLK5_4 !DIG_DLY, GLITCHLESS Half Step ON, !ANAGLITCH, !ANA_DLY, EN_DCLK, ACTIVE, EN_SCLK
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x117,
	    0x57);  //DIV_CLKOUT5_4 Dclk =LVPECL, Sclk =LVPECL, !DCLK_INV, !SCLK_INV
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x11B,
	    0x05);  //ANA_DLY_DCLK6 No Analog Delay, Half step duty cycle correction, 50% Duty Cycle!  (DAC Spec id 40 -/ 60% MAX);
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x11C,
	    0x62);  //DIG_DLY_SCLK7 DCLK halfstep=-0.5, SCLK sourced from DCLK,  2 vco clock cycle delay on sysref out
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x11D, 0x00);  //ANA_DLY_SCLK7 SCLK analog delay disabled
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x11E,
	    0xB0);  //PD_CLK7_6 !DIG_DLY, GLITCHLESS Half Step ON, !ANAGLITCH, !ANA_DLY, EN_DCLK, ACTIVE, EN_SCLK
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x11F,
	    0x57);  //DIV_CLKOUT7_6 Dclk =LVPECL, Sclk =LVPECL, !DCLK_INV, !SCLK_INV
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x123,
	    0x05);  //ANA_DLY_DCLK8 No Analog Delay, Half step duty cycle correction, 50% Duty Cycle!  (DAC Spec id 40 -/ 60% MAX);
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x124,
	    0x62);  //DIG_DLY_SCLK9 DCLK halfstep=-0.5, SCLK sourced from DCLK,  2 vco clock cycle delay on sysref out
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x125, 0x00);  //ANA_DLY_SCLK9 SCLK analog delay disabled
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x126,
	    0xB7);  //PD_CLK9_8 !DIG_DLY, GLITCHLESS Half Step ON, !ANAGLITCH, !ANA_DLY, EN_DCLK, ACTIVE, EN_SCLK
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x12B,
	    0x05);  //ANA_DLY_DCLK10 No Analog Delay, Half step duty cycle correction, 50% Duty Cycle!  (DAC Spec id 40 -/ 60% MAX);
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x12C,
	    0x62);  //DIG_DLY_SCLK11 DCLK halfstep=-0.5, SCLK sourced from DCLK,  2 vco clock cycle delay on sysref out
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x12D, 0x00);  //ANA_DLY_SCLK11 SCLK analog delay disabled
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x12E,
	    0xB7);  //PD_CLK11_10 !DIG_DLY, GLITCHLESS Half Step ON, !ANAGLITCH, !ANA_DLY, EN_DCLK, ACTIVE, EN_SCLK
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x130,
	    0x63);  //DIV_CLKOUT13_12   // DIV_BY_3 = 1GHz, IDL/ODL= 1  ==>In/Out Drive level normal
	spi_120.spi_write(hw, LMK_SELECT, 0x131, 0x22);  //DIG_DLY_DCLK0 Digital Delay
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x133,
	    0x05);  //ANA_DLY_DCLK12 No Analog Delay, Half step duty cycle correction, 50% Duty Cycle!  (DAC Spec id 40 -/ 60% MAX);
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x134,
	    0x62);  //DIG_DLY_SCLK13    SCLK analog = 2 VCO Clock Cycles (666pS << from FMC144 );
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x135, 0x00);  //ANA_DLY_SCLK13 SCLK analog delay disabled
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x136,
	    0xB7);  //PD_CLK13_12 !DIG_DLY, GLITCHLESS Half Step ON, !ANAGLITCH, !ANA_DLY, EN_DCLK, ACTIVE, SCLK Disabled at VCM
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x137,
	    0x00);  //Sclk Off, Dclock On = LVPECL<< set to 0x00 to turn off Externl Clock option
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x138,
	    0x20);  //VCO_OSC_OUT VCO01 (High Speed);, PLL1_FB=OSCin,OSCOUT=input (Drivers powerd down);
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x139,
	    0x03);  //SYSREF_MUX SYSREF output source = Normal Sync start with 0 reprogram when turning on sysref

	//SYSREF Divider (SYSREF = LINE_RATE / (10 * F * K ););
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x13A, 0x01);  //SYSREF_DIV(MS); SYSREF Divider, 300 MHz
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x13B, 0x40);  //SYSREF_DIV(LS); SYSREF Divider, 300 MHz
	...
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x13C, 0x00);  //SYSREF_DDLY(MS); SYSREF Digital Delay  - Not Used
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x13D, 0x08);  //SYSREF_DDLY(LS); SYSREF Digital Delay  - Not Used

	spi_120.spi_write(
	    hw, LMK_SELECT, 0x13E, 0x00);  //SYSREF_PULSE_CNT 8 Pulses - Not Used
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x13F,
	    0x00);  //FB_CTRL PLL2_FB=prescaler, PLL1_FB=OSCIN   This is default for internal Oscillator, this changes on EXT osc
	spi_120.spi_write(hw,
	                  LMK_SELECT,
	                  0x140,
	                  0x01);  //OSCIN_SYSREF_PD Active= PLL1, LDO, VCO, OSCIN, SYSREF
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x141, 0x00);  //DIG_DLY_REG Disable all digital delays
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x142, 0x00);  //DIG_DLY_STEP_CNT No Adjustment of Digital Delay
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x143,
	    0x70);  //SYNC_SYSREF SYNC functionality enabled, prevent SYNC pin and DLD flags from generating SYNC event
	//DCLK12, DCLK10, DCLK8 do not re-sync during a sync event
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x144,
	    0xFF);  //DISABLE_DCLK_SYNC Prevent SYSREF clocks from synchronizing << set to 0x00 to turn off Externl Clock option
	spi_120.spi_write(hw, LMK_SELECT, 0x145, 0x7F);  //FIXED Always 0x7F
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x146,
	    0x00);  //CLKIN_SRC No AutoSwitching of clock inputs, all 3 CLKINx pins are set t0 Bipolar,
	...
	if(clockmode == CLOCKTREE_CLKSRC_EXTERNAL_REF)
	{
		...
		spi_120.spi_write(
		    ...
		    LMK_SELECT,
		    0x147,
		    0x1A);  // CLKin_SEL_MODE = CLKin1 Manual, CLKin1_OUT_MUX = Fin, CLKin0_OUT_MUX = PLL1
	}
	...
	if(clockmode == CLOCKTREE_CLKSRC_EXTERNAL)
	{
		...
		spi_120.spi_write(
		    ...
		    LMK_SELECT,
		    0x147,
		    0x12);  // CLKin_SEL_MODE = CLKin1 Manual, CLKin1_OUT_MUX = Fin, CLKin0_OUT_MUX = PLL1
	}
	...
	if(clockmode == CLOCKTREE_CLKSRC_INTERNAL)
	{
		spi_120.spi_write(
		    ...
		    LMK_SELECT,
		    0x147,
		    0x2A);  // CLKIN_MUX CLKIN = clkin1 (External Reference connector);, !INVERT, CLKIN1=PLL1, CLKIN0=SYSREF MUX
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x149,
	    0x00);  //CLKIN_SEL1_MUX SPI SDIO_readback = PUSH-PULL !! CLKIN_SEL1=input

	spi_120.spi_write(hw, LMK_SELECT, 0x14A, 0x00);  //RESET_MUX RESET Pin=Input
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x14B, 0x05);  //Holdover mode off Manual DAC Enabled
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x14C, 0xFF);  //MANUAL_DAC Force DAC to midscale 0x01FF
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x14D, 0x00);  //DAC_TRIP_LOW Min Voltage to force HOLDOVER
	spi_120.spi_write(hw,
	                  LMK_SELECT,
	                  0x14E,
	                  0x00);  // DAC_TRIP_HIGH Mult=4 Max Voltage to force HOLDOVER
	...
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x155, 0x00);  //CLKIN1_DIV (MS);      100MHz / 1000 = 100KHz
	...
	{  //100 KHzDefault PLL Frequency
		...
		spi_120.spi_write(hw,
		                  LMK_SELECT,
		                  0x157,
		                  0x03);  //CLKIN2_DIV (MS); 100MHz / 1000 = 100KHz 1000d = 0x03E8
		...
		spi_120.spi_write(hw,
		                  LMK_SELECT,
		                  0x159,
		                  0x13);  //PLL1_NDIV (MS)  PLL1 Ndivider = 5000 for 100HHz PDF
	...
	{  //500 KHz
		...
		//PLL1 CLKIN2 Divider, divides onboard 100MHz Ref Osc output down to Phase Detector Frequency
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x157, 0x00);  //CLKIN2_DIV (MS) R divide = 200 = 500KHz
		spi_120.spi_write(hw, LMK_SELECT, 0x158, 0xC8);  //CLKIN2_DIV (LS)
		//Onboard 500MHz VCSO at LMK OSCIN pins
		//PLL1 N divider, Divide 500MHz VCSO down to PDF
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x159, 0x03);  //PLL1_NDIV (MS)  N divide = 1000 = 500MHz PDF
	...
	{  //1 MHz
		...
		//PLL1 CLKIN2 Divider, divides onboard 100MHz Ref Osc output down to Phase Detector Frequency
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x157, 0x00);  //CLKIN2_DIV (MS) R divide = 0x64 = 100 = 1MHz
		spi_120.spi_write(hw, LMK_SELECT, 0x158, 0x64);  //CLKIN2_DIV (LS)
		//Onboard 500MHz VCSO at LMK OSCIN pins
		//PLL1 N divider, Divide 500MHz VCSO down to PDF
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x159, 0x01);  //PLL1_NDIV (MS)  N divide =0x1f4 = 500 = 1 MHz
	...
	{  //10 MHz
		...
		//PLL1 CLKIN2 Divider, divides onboard 100MHz Ref Osc output down to Phase Detector Frequency
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x157, 0x00);  //CLKIN2_DIV (MS) R divide = A = 10 = 10MHz
		spi_120.spi_write(hw, LMK_SELECT, 0x158, 0x0A);  //CLKIN2_DIV (LS)
		//Onboard 500MHz VCSO at LMK OSCIN pins
		//PLL1 N divider, Divide 500MHz VCSO down to PDF
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x159, 0x00);  //PLL1_NDIV (MS)   N divide =0x32 = 50 = 10 MHz
	...
	{  //25 MHz
		...
		//PLL1 CLKIN2 Divider, divides onboard 100MHz Ref Osc output down to Phase Detector Frequency
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x157, 0x00);  //CLKIN2_DIV (MS) R divide = 4 = 4 = 25MHz
		spi_120.spi_write(hw, LMK_SELECT, 0x158, 0x04);  //CLKIN2_DIV (LS)
		//Onboard 500MHz VCSO at LMK OSCIN pins
		//PLL1 N divider, Divide 500MHz VCSO down to PDF
		spi_120.spi_write(
		    hw, LMK_SELECT, 0x159, 0x00);  //PLL1_NDIV (MS)   N divide =0x14 = 20 = 25 MHz
	...
	{  //33,333.333 KHz
		...
		spi_120.spi_write(hw,
		                  LMK_SELECT,
		                  0x157,
		                  0x00);  //CLKIN2_DIV (MS) R divide = 3 = 3 = 33.333333 MHz
		...
		spi_120.spi_write(hw,
		                  LMK_SELECT,
		                  0x159,
		                  0x00);  //PLL1_NDIV (MS)   N divide =0xF = 15 = 33.333333 MHz
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x15B,
	    0xDF);  //PLL1_SETUP,  Dlig lock det window 43ns, PLL active, Pos Slope, max pump current
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x15C,
	    0x20);  //PLL1_LOCK_CNT (MS)    Lock detector window,  must be valid for 1024 cycles
	spi_120.spi_write(hw, LMK_SELECT, 0x15D, 0x00);  //PLL1_LOCK_CNT (LS)
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x15E, 0x00);  //PLL1_DLY   // not applicable keep at 0
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x15F, 0x0B);  //STATUS_LD1_MUX == LD1 Push-Pull Output
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x160,
	    0x00);  //PLL2_RDIV (MS) PLL2 Reference Divider = 4 refference frequency = 125MHz
	spi_120.spi_write(hw, LMK_SELECT, 0x161, 0x04);  //PLL2_RDIV (LS)
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x162,
	    0xD0);  //PLL2_PRESCALE PRE=6,  >255 to 500MHz  range  amp off, doubler off
	...
	spi_120.spi_write(
	    ...
	    LMK_SELECT,
	    0x168,
	    0x04);  //PLL2_NDIV (LOW) Cal after writing this register     >>P = 3, N = 8  (24 * 125Mhz_ref = 3G)
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x169,
	    0x49);  //PLL2_SETUP Window 3.7nS,  I(cp)=1.6mA, Pos Slope, CP ! Tristate, Bit 0 always 1
	...
	spi_120.spi_write(hw,
	                  LMK_SELECT,
	                  0x16B,
	                  0x20);  //PLL2_LOCK_CNT (LS)  PD must be in lock for 16 cycles
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x16C,
	    0x00);  //PLL2_LOOP_FILTER_R Disable Internal Resistors<< Uses externla Loop Filter
	// R3 = 200 Ohms  R4 = 200 Ohms
	spi_120.spi_write(
	    hw,
	    LMK_SELECT,
	    0x16D,
	    0x00);  //PLL2_LOOP_FILTER_C Disable Internal Caps     << uses externla loop filter
	//C3 = 10pF  C4 = 10pF
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x16E, 0x12);  //STATUS_LD2_MUX LD2=Locked   Push Pull Output
	spi_120.spi_write(
	    hw, LMK_SELECT, 0x173, 0x00);  //PLL2_MISC PLL2 Active, normal opperation
	...
	if(clockmode == CLOCKTREE_CLKSRC_EXTERNAL)
	{
		...
		spi_120.spi_write(
		    ...
		    LMK_SELECT,
		    0x147,
		    0x03);  //Clock Input Select CLKIN0 Manual, CLKin0_Buffer-> SYSREF Mux CLKin1 = Fin
	}
