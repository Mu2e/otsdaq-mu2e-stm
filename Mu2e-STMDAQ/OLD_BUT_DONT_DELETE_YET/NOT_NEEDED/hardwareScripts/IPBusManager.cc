...
IPBusManager::IPBusManager(std::string conn_file, std::string dev_id)
    : connection_file(conn_file), device_id(dev_id), manager(), hw()
{
	...
	{
		std::cout << "uhal error occurred" << std::endl;
	}
}

ConnectionManager* IPBusManager::getConnectionManager() const { return manager.get(); }

HwInterface* IPBusManager::getHwInterface() const { return hw.get(); }
...
std::vector<uint32_t> IPBusManager::readBlock(std::string node)
{
	...
	try
	{
		ValVector<uint32_t> blockVal = getHwInterface()->getNode(node).readBlock(
		    getHwInterface()->getNode(node).getSize());
		getHwInterface()->dispatch();
