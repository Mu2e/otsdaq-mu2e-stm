...
// Return run number
int BinaryFile::run() const { return _run; }

// Return subrun number
int BinaryFile::subrun() const { return _subrun; }

// Return filesize
unsigned int BinaryFile::fsize() const { return _fsize; }

// Return filename
std::string BinaryFile::filename() const { return _filename; }
...
// Set the run number
void BinaryFile::set_run(int run) { _run = run; }

// Set the subrun number
void BinaryFile::set_subrun(int subrun) { _subrun = subrun; }
...
void BinaryFile::set_raw_file_version(int iversion)
{  // 0 for old format without unix timestamp, 1 with unix time stamp
...
{
	std::stringstream ss;
	ss << path << "_run" << std::setfill('0') << std::setw(7) << run << "_subrun"
	   << std::setfill('0') << std::setw(5) << subrun << ".bin";
	_filename = ss.str();
	_path     = path;
	_run      = run;
	_subrun   = subrun;
...
void BinaryFile::read_data(int16_t* data, unsigned int N, unsigned int offset)
{
	...
	else
	{
		std::cout << "Requested to read subset of file that goes beyond EOF, offset = "
		          << offset << " bytes and asked to read " << 2 * N
		          << " bytes and filesize = " << _fsize << " bytes" << std::endl;
		exit(-1);
	}
}
...
peaks* BinaryFile::read_MWD_data()
{
	...
	_ipfile.read(reinterpret_cast<char*>(&HEADER),
	             sizeof(uint32_t));  // SHOULD BE 0xCAFE AC1D
	...
	std::cout << std::hex << HEADER << std::dec << " " << trigger_num << " "
	          << trigger_type << " " << npeaks << " # adc values " << nadc_values
	          << std::endl;
...
// Write integer value
void BinaryFile::write_int(int value) { _opfile.write((char*)&value, sizeof(value)); }
...
void BinaryFile::write_raw_data(int16_t* data, unsigned int size)
{  // might need some const bollocks here
	if(size >= INT32_MAX / 2)
	{
		std::cout
		    << " You are trying to write an array > 2 Gb -- problems ... EXITING ... "
		    << std::endl;
		exit(-1);
	}
...
void BinaryFile::write_MWD_data(peaks* peak_data)
{
	...
	{
		std::cout << "write_MWD_data: trigger_type is not 0 or 1, it is :: "
		          << peak_data->trigger_type << std::endl;
	}
	else
	{
