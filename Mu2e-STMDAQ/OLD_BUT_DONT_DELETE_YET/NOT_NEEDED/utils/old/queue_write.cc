...
// Constructor
template<typename T>
queue_write<T>::queue_write(std::string date_time)
{
...
template<typename T>
int queue_write<T>::try_push(int chan, T* data, int n, int index)
{
	...
	if(increment(current_tail[chan], retval) < current_tail[chan])
	{
		...
		memcpy(&buffer[chan][current_tail[chan]], &data[index], end * sizeof(data[0]));
	...
	else
	{
		...
		memcpy(
		    &buffer[chan][current_tail[chan]], &data[index], (retval * sizeof(data[0])));
	}
...
// Push data to queue
template<typename T>
void queue_write<T>::push(int chan, T* data, int n)
{
...
// Try to write data
template<typename T>
int queue_write<T>::try_write_file(int chan)
{
	// Initialise retval
	int retval = 0;
	...
	if(new_file[chan][subrun_buff_index[chan]])
	{
		...
		std::cout << "try_write_file: New file for subrun " << subrun_buff_index[chan]
	...
	if(increment(current_head[chan], retval) < current_head[chan])
	{
		...
		bf[chan][subrun_buff_index[chan]].write(
		    (char*)&buffer[chan][current_head[chan]],
		    end * sizeof(buffer[chan][current_head[chan]]));

		// Write data from start of buffer
		bf[chan][subrun_buff_index[chan]].write((char*)&buffer[chan][0],
...
template<typename T>
void queue_write<T>::write_file(bool* timeout, int chan)
{
	int retval = 0;
...
// Get a new file name
template<typename T>
std::string queue_write<T>::get_file_name(int chan, int subrun)
{
	// Open binary file
	std::string filename = "/home/stm_mu2e/STMDAQ-TestBeam/data/" + file_date_time + "_" +
	                       (chan == 0 ? "HPGe" : "LaBr") + "_" + std::to_string(subrun) +
	                       ".bin";
...
template<typename T>
void queue_write<T>::monitor_files(int chan)
{
	...
	for(uint file = 0; file < file_num; file++)
	{
		...
		if(new_file[chan][file].load())
		{
			...
			std::string file_name =
			    get_file_name(chan, (current_subrun_buff[chan] + 1) * file_num + file);
			// Store file name
			bf_name[chan][file] = file_name;
			// Open file
			bf[chan][file].open(file_name, std::ios::out | std::ios::binary);
...
template<typename T>
void queue_write<T>::write_monitor_func(int chan, UDPsocket& udp, queue_write<T>& writeq)
{
	while(!udp.timeout)
		monitor_files(chan);
}
...
// Write data to file
template<typename T>
void queue_write<T>::write_data_func(int chan, UDPsocket& udp, queue_write<T>& writeq)
{
	// Infinte loop
	while(1)
	{
