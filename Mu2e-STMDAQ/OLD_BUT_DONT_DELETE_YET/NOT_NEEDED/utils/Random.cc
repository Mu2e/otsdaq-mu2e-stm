...
void Random::Init(unsigned long random_seed)
{
	...
	stringstream ss;
	ss << "[ Random :: Init ] with user specified seed = " << _random_seed;
...
void Random::Init()
{
	...
	stringstream ss;
	ss << "[ Random :: Init ] with random/chrono seed = " << _random_seed;
	Logger::Instance()->write(Logger::INFO, ss.str());
	_init();
}
...
void Random::Init_Zig()
{
	...
	stringstream ss;
	ss << "Initialised ziggurat algorithm";
	;
	Logger::Instance()->write(Logger::INFO, ss.str());
}
...
int Random::DiscreteValue(int* distribution, int n)
{
	...
	for(int i = 0; i < n; i++)
	{
		distribution_vect.push_back(distribution[i]);
	}
	std::discrete_distribution<int> d_dist(distribution_vect.begin(),
	                                       distribution_vect.end());
...
Random* Random::Instance()
{
	if(!_instance)
	{
		Logger::Instance()->write(
		    Logger::ERROR,
		    "[ Random :: Instance ] You need to initialise Random() interface once using "
		    "Random::Init() or Random::Init(seed)");
		exit(-1);
	}
	return _instance;
}
