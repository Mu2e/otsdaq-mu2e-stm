...
DBase::DBase(std::string host, int port, int read_write)
{
	...
	std::stringstream ss;
	ss << "dbname = stm user = " << _username << " password = " << _password
	   << " hostaddr = " << _host << " port = " << _port;
	std::cout << "Connecting to database with connection string: " << ss.str()
	          << std::endl;
	try
	{
...
pqxx::connection* DBase::con() { return _c; }
...
void DBase::write_hpge_baseline(int run, int subrun, float mean, float rms)
{
	...
	{
		std::stringstream ss;
		ss << "insert into hpge_baseline(run,subrun,mean,rms) values (" << run << ","
		   << subrun << "," << mean << "," << rms << ")";
...
{
	std::stringstream ss;
	ss << "update hpge_baseline set mean = " << mean << ", rms = " << rms
	   << " where run = " << run << " and subrun =  " << subrun;
...
{
	std::stringstream ss;
	ss << "select * from hpge_baseline where run = " << run << " and subrun = " << subrun;
...
{
	std::cout << "Write sub run time for run = " << run << " subrun = " << subrun
	          << " time = " << timex << " mode = " << mode << std::endl;
	std::stringstream ss;
	...
	if(mode == 0)
	{
		ss << "insert into run_subrun_times(run,subrun,start_time) values (" << run << ","
		   << subrun << ",'" << timestamp << "')";
	}
	if(mode == 1)
	{
		ss << "update run_subrun_times set end_time = '" << timestamp
		   << "' where run = " << run << " and subrun = " << subrun;
	}
...
void DBase::write_runinfo_start(int run, std::string comment)
{
	...
	comment.erase(std::remove(comment.begin(), comment.end(), '\''), comment.end());
	std::stringstream ss;
	ss << "insert into runinfo(run,start_time,comment) values (" << run << ",'"
	   << timestamp << "','" << comment << "')";
	try
	{
		pqxx::work W(*_c);
...
void DBase::write_runinfo_stop(int run)
{
	...
	std::string       timestamp = dt.Format(7);
	std::stringstream ss;
	ss << "update runinfo set end_time = '" << timestamp << "' where run = " << run;
	try
	{
		pqxx::work W(*_c);
		std::cout << "Running SQL query: " << ss.str() << std::endl;
