...
std::vector<int16_t> processData::readFile(char* fname)
{
	...
	for(long unsigned int i = 0; i < vec.size(); i++)
	{
		...
		if(atHdr)
		{
			...
			if(!first_header)
			{
				...
				{
					std::cout << "Event " << event_number << " has length "
					          << event_length << std::endl;
					std::cout << "Must contain dropped packets. Cutting event..."
					          << std::endl;
				}
				// Else store event
				else
	...
	if(event_length < expected_elength)
	{
		std::cout << "Event " << event_number << " has length " << event_length
		          << std::endl;
