...
void E_K(std::string pathfile) {
  ...
  TF1*Fit = new TF1("Fit", "[0]*TMath::Gaus(x,[1],[2])", 1430, 1470);
  Fit->SetParameters(100,1450,3.5);
  h1->Fit(Fit,"0","", 1430, 1470);
  Fit->SetLineColor(kViolet+1);
  Fit->SetLineStyle(2);
  Fit->Draw("same");

  TLatex T1;
  //T1.DrawLatexNDC(.4,.7, "1460.8 keV");
  T1.DrawLatexNDC(.4,.7, "1460.8-E_{binding}(Ge) keV");
  ...
  //c1->Print("FitK_Spectrum_E-bindE_peak.png","png");
}
