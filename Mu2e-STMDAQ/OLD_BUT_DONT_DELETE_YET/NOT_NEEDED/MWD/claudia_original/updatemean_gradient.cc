...
int main(int narg, char* arg[])
{
	...
	std::cout << "Read data: # ADC values =  " << n << " in time "
	...
	std::cout << "Using mean " << mean << " threshold " << threesigmas << " M " << M
	          << " L " << L << std::endl;
	...
	for(int i = M; i < n; i++)
	{
		...
		else if(l[i] > (mean - threesigmas))
		{
			...
			std::cout << "Peak: " << counterpeak << " Time: " << timeaux * 1e-3 << " us"
			          << " Energy (ADC counts): " << energy[counterpeak] << std::endl;
			auxlow = 0.;
			counterpeak++;
		}
