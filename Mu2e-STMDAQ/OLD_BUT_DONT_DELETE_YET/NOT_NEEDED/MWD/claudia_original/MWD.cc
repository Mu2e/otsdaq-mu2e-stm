...
MWD::MWD(Xml* xml_file)
{
	...
	fixed_cut_parameter =
	    (double)xml_file->float_value("stm.mwd_fixed_cut_parameter", -1000.0);
}
...
peaks* MWD::find_peaks(double baseline_mean, double baseline_rms, double time_offset)
{
	...
	{
		std::cout << "ERROR:: find_peaks() : unknown cut mode .... " << cut_mode
		          << std::endl;
		exit(-1);
	}

	if(cut_mode == 1)
		std::cout << "MWD::find_peaks()---SIGMA CUT : baseline mean " << baseline_mean
		          << " rms = " << baseline_rms << " num_sigmas = " << nsigma_cut
		          << " threshold cut value = " << threshold_cut << " and M = " << M
		          << " L = " << L << " and MWD cut mode = " << cut_mode << std::endl;
	if(cut_mode == 2)
		std::cout << "MWD::find_peaks()---FIXED CUT : baseline mean " << baseline_mean
		          << " rms = " << baseline_rms
		          << " threshold cut value = " << threshold_cut << " and M = " << M
		          << " L = " << L << " and MWD cut mode = " << cut_mode << std::endl;
...
void MWD::mwd_algorithm(data* adc_values)
{  // This fill the double[l] array ie sets the private l*
