             ...
             double      tau)
{
	...
	bool simulation =
	    true;  //if this is true: use simulation configuration parameters, if this is false: use data (Run109 and 110) configuration parameters
	bool fnalheaders =
	    false;  //if this is true, we need to skip headers from the data in the binary files

	bool
	    calculatemean_and_rms;  //if this is true mean_baseline and rms_baseline are calculated using MWD::calculate_baseline(), if this is false, mean_baseline and rms_baseline are inputs
	...
	bool
	    printtime_peaks;  // true for printing in the screen time and energy of peaks, false not printing it
	// MWD::MWD
	//double tau; //ns
	...
	//Calibration
	double
	    calibration;  //1 for 1ADC=0.57keV, 2 for Liverpool Calibration M=400, L=200, 3 for ELBE calibration M=400, L=200
	// ************************************************************
  ...
  */
	...
	MWD* mwd = new MWD(
	    M, L, tau, nsigma_cut, thresholdgrad, fADC, cut_mode, fixed_cut_parameter);
	mwd->mwd_algorithm(d);
	//l value
	...
	if(calculatemean_and_rms == true)
	{
		...
		std::cout << "Calculated mean: " << mean_baseline
		          << " Calculated rms: " << rms_baseline << std::endl;
	}
	...
	std::cout << "MWD Algorithm computing time = "
	...
	if((savepeaks_toROOTfile == true) || (readpeaks_fromROOTfile == true))
	{
		...
		rootname = "MWD_" + filename.substr(filename.find("100kHz_Flash_job"), 18) +
		           "_M" + std::to_string(M) + "_L" + std::to_string(L) +
		           "_energypeaks.root";
  ...
  */
	...
	if(Draw_option == 1)
	{
		...
		for(int i = 0; i < npeaks; i++)
		{
			if(readpeaks_fromROOTfile == true)
			{
				peakADC = peakROOT.at(i);
			}
			if(readpeaks_fromROOTfile == false)
			{
				peakADC = peak_data->peak_heights.at(i);
			}
			if(calibration == 1)
			{
		...
		if(tau < 40000)
		{
			fitmean = ((50000 - tau) / 1000) + fitmean;
		}
		else if(tau > 59000)
		{
			fitmean = fitmean - (tau - 50000) / 4000;
		}
		else
		{
			fitmean = fitmean;
		...
		std::cout << "Fit using range: " << fitx << " and " << fity
		          << " and mean: " << fitmean << std::endl;
		...
		//Mean
		std::cout << "Mean energy (keV) and in ADC Counts from original histogram: "
		          << std::endl;
		std::cout << fitmean << " keV, " << hADC->GetMean() << "+-"
		          << hADC->GetMeanError() << " ADC Counts" << std::endl;
		//Standard deviation
		...
		double countsRaw =
		    hraw->Integral(hraw->FindFixBin(countx), hraw->FindFixBin(county), "");
		double efficiency       = countnumber / peakssim;
		double efficiency_error = efficiency / sqrt(countnumber);
		std::cout << "Efficiency npeaks in photopeak/npeaks simulated and error: "
		          << std::endl;
		...
		std::cout << "Number of peaks in the photopeak (using integral, depends on "
		             "binning), between "
		          << countx << " and " << county << ": " << countsRaw << std::endl;
		std::cout
		    << "Number of peaks in the photopeak (using number of entries): between "
		    << countx << " and " << county << ": " << countnumber << std::endl;
		...
		std::cout << "St.dev: " << p2 << " +- " << e2
		          << " Error using sigma/sqrt(2Ncounts)= " << p2 / sqrt(2 * countnumber)
		          << std::endl;
		...
		if(savepeaks_toROOTfile == true)
		{
			std::cout << "new ROOT file created: " << rootname << std::endl;
		}
		if(savepeaks_toROOTfile == false)
		{
			std::cout << "ROOT file not created" << std::endl;
		}
		if(readpeaks_fromROOTfile == true)
		{
			std::cout << "Reading peaks from ROOT file: " << rootname << std::endl;
		}
		if(readpeaks_fromROOTfile == false)
		{
			std::cout << "Not reading peaks from ROOT file, read them from MWD after "
			             "reading binary file"
			          << std::endl;
		}
	}
	...
	if(Draw_option == 2)
	{
		...
		{
			TLine* line =
			    new TLine(xx1[0], fixed_cut_parameter, xx1[1], fixed_cut_parameter);
	...
	if(Draw_option == 3)
	{
		...
		std::string roundedsigma =
		    fitsigma_string.substr(0, fitsigma_string.find(".") + 3);
	...
	if(printtime_peaks)
	{
		for(int i = 0; i < npeaks; i++)
		{
			if(readpeaks_fromROOTfile == false)
			{
				...
				{
					std::cout << "Peak: " << i
					          << ", Time: " << peak_data->peak_times.at(i) << " us,"
					          << " ADC Counts: " << peakADC
					          << ", Energy: " << peakADC * (-0.57) << " keV" << std::endl;
				}
				if(calibration == 2)
				{
					std::cout << "Peak: " << i
					          << ", Time: " << peak_data->peak_times.at(i) << " us,"
					          << " ADC Counts: " << peakADC
					          << ", Energy: " << (peakADC - 1.738) / (-1.767) << " keV"
					          << std::endl;
				}
				if(calibration == 3)
				{
			...
			if(readpeaks_fromROOTfile == true)
			{
				...
				if(calibration == 1)
				{
					std::cout << "Peak: " << i << ", ADC Counts: " << peakADC
					          << ", Energy: " << peakADC * (-0.57) << " keV" << std::endl;
				}
				if(calibration == 2)
				{
