...
void ZP_DataStored_res_eff(int print){
  ...
  //filespath[0] = "/work/cgarcia/DATA/Claudia/GenDataehHPGeSim/1836keV_0.32mV/MWDfilesM400L200.txt"; //index 0
  //filespath[1] = "/work/cgarcia/DATA/Claudia/GenDataehHPGeSim/1836keV_0.32mV/SuppresedFiles_tbefore2us_tafter5us_newcodev3/MWDfilesM400L200.txt"; //index 1
  //filespath[2] = "/work/cgarcia/DATA/Claudia/GenDataehHPGeSim/1836keV_0.32mV/SuppresedFiles_tbefore2us_tafter3us_newcodev3/MWDfilesM400L200.txt"; //index 2
  //filespath[3] = "/work/cgarcia/DATA/Claudia/GenDataehHPGeSim/1836keV_0.32mV/SuppresedFiles_tbefore1us_tafter2us_newcodev3/MWDfilesM400L200.txt"; //index 3
  //filespath[4] = "/work/cgarcia/DATA/Claudia/GenDataehHPGeSim/675keV_0.32mV/SuppresedFiles_tbefore1us_tafter1us_newcodev3/MWDfilesM400L200.txt"; //index 4
  
 
  filespath[0] = "/work/mu2e/data1/cgarcia/DATA/Claudia/GenDataehHPGeSim/662keV_0.32mV/MWDFilesM400L200.txt"; //index 0 
  filespath[1] = "/work/mu2e/data1/cgarcia/DATA/Claudia/GenDataehHPGeSim/662keV_0.32mV/SuppresedFiles_tbefore1us_tafter2us_newcodev3/MWDFilesM400L200.txt"; //index 1            
  ...
  double res[5], res_error[5], res_error_calc[5], eff[5], eff_error[5], chi2[5], meanE[5], meanE_error[5], bytes[5];
  vector<double> resolutionv[5], res_errorv[5], eff_errorv[5], efficiencyv[5], chi2v[5], meanEv[5], meanE_errorv[5], res_error_calcv[5], bytesv[5], fractionkeptv, fractionkept_errorv;
  std::string val;
  ...
  std::cout<<"File Size Raw: "<<file_names[0].size()<<std::endl;
  std::cout<<"File Size 7us: "<<file_names[1].size()<<std::endl;
  std::cout<<"File Size 5us: "<<file_names[2].size()<<std::endl;
  std::cout<<"File Size 3us: "<<file_names[3].size()<<std::endl;
  std::cout<<"File Size 2us: "<<file_names[4].size()<<std::endl;
  ...
  for(int i=0;i<N;i++){
	...
	if(i==0){ //raw
	  if (countline==37){ ss >> bytes[i];}
	}
	if(i==1){ //ZS
	  if (countline==36){ ss >> bytes[i];}
	}
	countline++;
      }
      ...
      std::cout<<"Number of bytes in file: "<<bytes[i]<<std::endl;
      myfile[i].close();
    }
 ...
 grRes[0]->SetMarkerColor(kRed+1);
 grRes[0]->SetMarkerStyle(21);
 grRes[0]->SetMarkerSize(1.2);
 grRes[0]->Draw("same,p");

 grRes[1]->SetMarkerColor(kBlue+1);
 ...
 legend->AddEntry(grRes[0],"Raw","p");
 //legend->AddEntry(grRes[1],"Suppressed 7#mus","p");
 //legend->AddEntry(grRes[2],"Suppressed 5#mus","p");
 legend->AddEntry(grRes[1],"Suppressed 3#mus","p");
 //legend->AddEntry(grRes[4],"Suppressed 2#mus","p");
 legend->Draw("same");
 ...
 if(print==1){
   ...
   grEff[0]->SetMarkerColor(kRed+1);
   grEff[0]->SetMarkerStyle(21);
   grEff[0]->SetMarkerSize(1.2);
   grEff[0]->Draw("same,p");

   grEff[1]->SetMarkerColor(kBlue+1);
   ...
   auto legend = new TLegend(0.1,0.7,0.48,0.9);
   legend->AddEntry(grEff[0],"Raw","p");
   //legend->AddEntry(grEff[1],"Suppressed 7#mus","p");
   //legend->AddEntry(grEff[2],"Suppressed 5#mus","p");
   legend->AddEntry(grEff[1],"Suppressed 3#mus","p");
   //legend->AddEntry(grEff[4],"Suppressed 2#mus","p");
   legend->Draw("same");
 ...
 if(print==2){
   ...
   grE[0]->SetMarkerColor(kRed+1);
   grE[0]->SetMarkerStyle(21);
   grE[0]->SetMarkerSize(1.2);
   grE[0]->Draw("same,p");

   grE[1]->SetMarkerColor(kBlue+1);
   ...
   auto legend = new TLegend(0.1,0.7,0.48,0.9);
   legend->AddEntry(grE[0],"Raw","p");
   //legend->AddEntry(grE[1],"Suppressed 7#mus","p");
   //legend->AddEntry(grE[2],"Suppressed 5#mus","p");
   legend->AddEntry(grE[1],"Suppressed 3#mus","p");
   //legend->AddEntry(grE[4],"Suppressed 2#mus","p");
   legend->Draw("same");
 ...
 if(print==3){
   ...
   for(int i=0;i<n;i++){
     //need to account for the number of jobs in 1,5 and 10kHz in the raw data, the suppressed data size is for the full 20 jobs
     if(i<3){
       fractionkeptv.push_back(bytesv[1].at(i)/(bytesv[0].at(i)*Njobs));
       std::cout<<"Suppressed bytes: "<<bytesv[1].at(i)<<" Raw bytes: "<<bytesv[0].at(i)*Njobs<<" Fraction kept: "<<fractionkeptv.at(i)<<std::endl;
     }
     else{
       fractionkeptv.push_back(bytesv[1].at(i)/bytesv[0].at(i));
       std::cout<<"Suppressed bytes: "<<bytesv[1].at(i)<<" Raw bytes: "<<bytesv[0].at(i)<<" Fraction kept: "<<fractionkeptv.at(i)<<std::endl;
     }
     fractionkept_errorv.push_back(0);
   }


   TGraphErrors *grSup = new TGraphErrors(n,&rate[0],&fractionkeptv[0],&rate_error[0],&fractionkept_errorv[0]);
   grSup->SetMarkerColor(kViolet-8);
   grSup->SetMarkerStyle(21);
   grSup->Draw("same,p");

   auto legend = new TLegend(0.1,0.7,0.48,0.9);
   legend->AddEntry(grSup,"Suppressed 3#mus","p");
   //legend->Draw("same");
