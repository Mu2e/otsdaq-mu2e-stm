...
GeUserDetectorConstruction::GeUserDetectorConstruction()
    ...
    , stepLimit(0)
    ,  //fMaterial(0),
    worldLength(100 * CLHEP::m)
    , fTargetLength(0)
{
}

GeUserDetectorConstruction::~GeUserDetectorConstruction() { delete stepLimit; }
...
G4VPhysicalVolume* GeUserDetectorConstruction::Construct()
{
	...
	double HPGe_rot_angley, VD_rmax_cm, VD_halflength_cm, VD_xcenter_cm, VD_ycenter_cm,
	    VD_zcenter_cm;

	double Poly_rmax_cm, Poly_halflength_cm, Poly_xcenter_cm, Poly_ycenter_cm,
	    Poly_zcenter_cm;

	double AlWindow_rmax_cm, AlWindow_halflength_cm, AlWindow_xcenter_cm,
	    AlWindow_ycenter_cm, AlWindow_zcenter_cm;
	...
	while(std::getline(input_file, line))
	{
		...
		while(ss >> element)
		{
			if(element.compare("bool_doHPGe:") == 0)
			{
				...
				{
					Ge_detector = true;
					std::cout << "Build Ge detector" << std::endl;
				}
			}
			...
			if(element.compare("bool_doPoly:") == 0)
			{
				...
				{
					Poly_detector = true;
					std::cout << "Build Poly detector" << std::endl;
				}
			}
			...
			if(element.compare("bool_doKClPot:") == 0)
			{
				...
				{
					KCl_detector = true;
					std::cout << "Build KCl detector" << std::endl;
				}
			}
			...
			if(element.compare("bool_doVDplane:") == 0)
			{
				...
				{
					VDplane_STM = true;
					std::cout << "Build VD detector" << std::endl;
				}
			}
			...
			{
				ss >> bool_doAlWindow;
				if(bool_doAlWindow == 1)
				{
					Al_Window = true;
					std::cout << "Build Al Window" << std::endl;
				}
			}

			if(element.compare("HPGe_rot_angley:") == 0)
				ss >> HPGe_rot_angley;

			if(element.compare("VD_rmax_cm:") == 0)
				ss >> VD_rmax_cm;
			if(element.compare("VD_halflength_cm:") == 0)
				ss >> VD_halflength_cm;
			if(element.compare("VD_xcenter_cm:") == 0)
				ss >> VD_xcenter_cm;
			if(element.compare("VD_ycenter_cm:") == 0)
				ss >> VD_ycenter_cm;
			if(element.compare("VD_zcenter_cm:") == 0)
				ss >> VD_zcenter_cm;

			if(element.compare("Poly_rmax_cm:") == 0)
				ss >> Poly_rmax_cm;
			if(element.compare("Poly_halflength_cm:") == 0)
				ss >> Poly_halflength_cm;
			if(element.compare("Poly_xcenter_cm:") == 0)
				ss >> Poly_xcenter_cm;
			if(element.compare("Poly_ycenter_cm:") == 0)
				ss >> Poly_ycenter_cm;
			if(element.compare("Poly_zcenter_cm:") == 0)
				ss >> Poly_zcenter_cm;

			if(element.compare("AlWindow_rmax_cm:") == 0)
				ss >> AlWindow_rmax_cm;
			if(element.compare("AlWindow_halflength_cm:") == 0)
				ss >> AlWindow_halflength_cm;
			if(element.compare("AlWindow_xcenter_cm:") == 0)
				ss >> AlWindow_xcenter_cm;
			if(element.compare("AlWindow_ycenter_cm:") == 0)
				ss >> AlWindow_ycenter_cm;
			if(element.compare("AlWindow_zcenter_cm:") == 0)
				ss >> AlWindow_zcenter_cm;
		}
	}
	input_file.close();
  ...
  */
	...
	if(Ge_detector == true)
	{
		...
		G4Tubs* solidTarget1 =
		    new G4Tubs("Cyl1", 0, R_det, frontcyl_HalfTargetLength, 0, CLHEP::twopi);
		G4Tubs* solidTarget2 =
		    new G4Tubs("Cyl2", R_hole, R_det, fHalfTargetLength, 0, CLHEP::twopi);
		...
		G4double center_nohole_cyl =
		    center_cylhole - fHalfTargetLength - frontcyl_HalfTargetLength;

		G4double centercyl2 = center_cylhole - center_nohole_cyl;

		G4ThreeVector zTrans(0, 0, centercyl2);

		solidTarget = new G4UnionSolid(
		    "Cylinder1+Cylinder2", solidTarget1, solidTarget2, 0, zTrans);

		logicTarget = new G4LogicalVolume(solidTarget, TargetMater, "Target", 0, 0, 0);

#ifdef debug
		std::cout << "fTargetLength: " << G4BestUnit(fTargetLength, "Length")
		          << " fHalfTargetLength: " << G4BestUnit(fHalfTargetLength, "Length")
		          << " Rot y: " << HPGe_rot_angley << std::endl;
		std::cout << "frontcyl_TargetLength: "
		          << G4BestUnit(frontcyl_TargetLength, "Length")
		          << " frontcyl_HalfTargetLength: "
		          << G4BestUnit(frontcyl_HalfTargetLength, "Length") << std::endl;
		std::cout << "total_length: " << G4BestUnit(total_length, "Length") << std::endl;
		std::cout << "center_cylhole: " << G4BestUnit(center_cylhole, "Length")
		          << std::endl;
		std::cout << "center_nohole_cyl: " << G4BestUnit(center_nohole_cyl, "Length")
		          << std::endl;
	...
	if(Al_Window == true)
	{
...
#endif
		std::cout << "Rot y: " << HPGe_rot_angley << " deg, rmax: " << AlWindow_rmax_cm
		          << " cm, half length:  " << AlWindow_halflength_cm
		          << " cm , (x,y,z)center cm: (" << AlWindow_xcenter_cm << ", "
		          << AlWindow_ycenter_cm << ", " << AlWindow_zcenter_cm << ")"
		          << std::endl;
		...
		G4Tubs* solidAlWindow = new G4Tubs("AlWindow",
		                                   0,
		                                   AlWindow_rmax_cm * CLHEP::cm,
		                                   AlWindow_halflength_cm * CLHEP::cm,
		                                   0,
		                                   CLHEP::twopi);  //R, half length
		logicAlWindow =
		    new G4LogicalVolume(solidAlWindow, AlWindowMaterial, "AlWindow", 0, 0, 0);
		new G4PVPlacement(RotationAl_Window,
		                  G4ThreeVector(AlWindow_xcenter_cm * CLHEP::cm,
		                                AlWindow_ycenter_cm * CLHEP::cm,
		                                AlWindow_zcenter_cm * CLHEP::cm),
		                  logicAlWindow,
		                  "AlWindow",
		                  logicWorld,
		                  false,
...
#endif
		std::cout << "rmax: " << Poly_rmax_cm
		          << " cm, half length:  " << Poly_halflength_cm
		          << " cm , (x,y,z)center cm: (" << Poly_xcenter_cm << ", "
		          << Poly_ycenter_cm << ", " << Poly_zcenter_cm << ")" << std::endl;

		//10cm poly
		G4Tubs* solidPoly = new G4Tubs("PolyAbsorber",
		                               0,
		                               Poly_rmax_cm * CLHEP::cm,
		                               Poly_halflength_cm * CLHEP::cm,
		                               0,
		                               CLHEP::twopi);  //R, half length
		logicPoly = new G4LogicalVolume(solidPoly, PolyMaterial, "PolyAbsorber", 0, 0, 0);
		new G4PVPlacement(0,
		                  G4ThreeVector(Poly_xcenter_cm * CLHEP::cm,
		                                Poly_ycenter_cm * CLHEP::cm,
		                                Poly_zcenter_cm * CLHEP::cm),
		                  logicPoly,
		                  "PolyAbsorber",
		                  logicWorld,
		                  false,
	...
	if(KCl_detector == true)
	{
		...
		G4int       ncomponents;
		G4Material* KCl =
		    new G4Material("KCl", density, ncomponents = 2);  //2 is n components
		...
		G4Tubs* solidKClTarget = new G4Tubs(
		    "KClTarget", 0, Saltcyl_R, Saltcyl_HalfTargetLength, 0, CLHEP::twopi);
	...
	if(VDplane_STM == true)
	{
		...
		std::cout << "Rot y: " << HPGe_rot_angley << " deg, rmax: " << VD_rmax_cm
		          << " cm, half length: " << VD_halflength_cm << "cm, (x,y,z)center cm: ("
		          << VD_xcenter_cm << ", " << VD_ycenter_cm << ", " << VD_zcenter_cm
		          << ")" << std::endl;
		...
		//Circle VD
		G4Tubs* solidplane = new G4Tubs("VacuumTarget",
		                                0,
		                                VD_rmax_cm * CLHEP::cm,
		                                VD_halflength_cm * CLHEP::cm,
		                                0,
		                                CLHEP::twopi);  //radius, half z
		//Plane VD
		...
		new G4PVPlacement(RotationVD,
		                  G4ThreeVector(VD_xcenter_cm * CLHEP::cm,
		                                VD_ycenter_cm * CLHEP::cm,
		                                VD_zcenter_cm * CLHEP::cm),
		                  logicVacuum,
		                  "VacuumTarget",
		                  logicWorld,
		                  false,
    ...
    */
	...
	if(KCl_detector == true)
	{
		logicKCl->SetSensitiveDetector(aTrackerSD);
	}
	if(Poly_detector == true)
	{
		logicPoly->SetSensitiveDetector(aTrackerSD);
	}
	if(VDplane_STM == true)
	{
		logicVacuum->SetSensitiveDetector(aTrackerSD);
	}
	if(Al_Window == true)
	{
		logicAlWindow->SetSensitiveDetector(aTrackerSD);
	}
