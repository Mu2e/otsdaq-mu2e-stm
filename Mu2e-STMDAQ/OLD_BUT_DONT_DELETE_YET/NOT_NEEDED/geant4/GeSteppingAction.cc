...
// Collect energy and track length step by step
...
#ifdef debug
	std::cout << "PreSteppos: " << PreSteppos / CLHEP::cm
	          << "cm  PostSteppos: " << PostSteppos / CLHEP::cm << " cm" << std::endl;
#endif
...
#ifdef debug
	std::cout << "Pre Momentum vector: (" << pre_px / CLHEP::keV << ","
	          << pre_py / CLHEP::keV << "," << pre_pz / CLHEP::keV << ") keV"
	          << std::endl;
	std::cout << "Post Momentum vector: (" << post_px / CLHEP::keV << ","
	          << post_py / CLHEP::keV << "," << post_pz / CLHEP::keV << ") keV"
	          << std::endl;
#endif
	//Particle Step
	...
	if(volume->GetName() == "Target")
	{
...
#ifdef debug
		std::cout << "Pre Position: (" << prex << "," << prey << "," << prez << ") cm"
		          << std::endl;
		std::cout << "Post Position: (" << postx << "," << posty << "," << postz << ") cm"
		          << std::endl;
#endif
		//Momentum direction
...
#ifdef debug
		std::cout << "Pre Momentum direction: (" << predir_px << "," << predir_py << ","
		          << predir_pz << "), normalised momentum vector" << std::endl;
#endif
...
#ifdef debug
		std::cout << "Post Momentum direction: (" << postdir_px << "," << postdir_py
		          << "," << postdir_pz << "), normalised momentum vector" << std::endl;
#endif
		//Momentum value
		auto pre_px = step->GetPreStepPoint()->GetMomentum().x();
		auto pre_py = step->GetPreStepPoint()->GetMomentum().y();
...
#ifdef debug
		std::cout << "PreStep Time: " << step_initialtime / CLHEP::ns << " ns"
		          << std::endl;
		std::cout << "PostStep Time: " << step_finaltime / CLHEP::ns << " ns"
		          << std::endl;
#endif
		...
		if(PDGID == 22)
		{
			...
			massSigma     = emCalculator.ComputeCrossSectionPerVolume(
                            fEkin, particle, procName, material) /
			            density;
#ifdef debug
			G4cout << "Process: " << procName << " with geant4 Cross Section= "
			       << G4BestUnit(massSigma, "Surface/Mass") << G4endl;
...
#ifdef debug
			G4cout << "Difference between cross-section as an input(geant4) and "
			          "simulated(calculated): "
			       << diffcrosssect << std::endl;
#endif
		...
		samplingeion =
		    G4lrint(G4RandGamma::shoot(meaneion * invFanoFactor, invFanoFactor));
  ...
  G4RunManager::GetRunManager()->AbortEvent();}*/
	...
	if((volume->GetName() == "KClTarget") || (volume->GetName() == "PolyAbsorber") ||
	   (volume->GetName() == "VacuumTarget") || (volume->GetName() == "AlWindow"))
	{
		//Event ID
		auto eventid = G4RunManager::GetRunManager()->GetCurrentEvent()->GetEventID();
#ifdef debug
