...
void plotGen_Poly_VD(std::string filename){

  //double Nphot_gen = 100000;
  //double Nphot_gen = 10000;
  double Nphot_gen = 1000;
  ...
  //VD
  TH2D* hyx_VD = new TH2D("hyx_VD", "xy position of the hit in each step VD", 1000, -30, 30, 1000, -30, 30);
  hyx_VD->GetXaxis()->SetTitle("x_{#gamma} (detected VD) [cm]");
  hyx_VD->GetYaxis()->SetTitle("y_{#gamma} (detected VD) [cm]");
  
  TH1F* hE_phot = new TH1F("hE_phot", "E photon in event", 100, 0, 1000000);
  ...
  //Poly
  TH2D* hzy = new TH2D("hzy", "zy position of the hit in each step Poly", 1000, 0, 26, 1000, -10, 10);
  hzy->GetXaxis()->SetTitle("z_{#gamma} (Polyethylene) [cm]");
  hzy->GetYaxis()->SetTitle("y_{#gamma} (Polyethylene) [cm]");
  TH3D* hgeom=new TH3D("hgeom", "Poly geom; x; z; y", 100, -10, 10, 100, 1.4, 19.6, 100, -10, 10);
