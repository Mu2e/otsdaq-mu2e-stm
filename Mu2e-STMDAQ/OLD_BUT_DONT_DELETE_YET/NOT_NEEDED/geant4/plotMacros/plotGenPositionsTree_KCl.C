...
void plotGenPositionsTree_KCl(std::string filename){
  ...
  //KCl detector
  TH2D* hzx = new TH2D("hzx", "zx position of the hit in each step", 1000, -18, -2, 1000, -8, 8);
  hzx->GetXaxis()->SetTitle("z [cm]");
  hzx->GetYaxis()->SetTitle("x [cm]");
  TH2D* hzy = new TH2D("hzy", "zy position of the hit in each step", 1000, -18, -2, 1000, -10, 10);
  ...
  TH1F* htracklengthevent_phot = new TH1F("htracklengthevent_phot", "Total Track Length of photons depositing energy in KCl", 100, -1, 30);

  //htracklengthevent_phot->GetXaxis()->SetTitle("Total #gamma Track Length in KCl detector [cm]");
  htracklengthevent_phot->GetXaxis()->SetTitle("Total #gamma Track Length (#neq0) in KCl pot [cm]"); 
  ...
  htracklengthevent_phot->SetLineColor(kRed+1);
  ...
  c1->Print("HitsKCldetector(0,0,-10)_TracklengthKCl_0.png");
}
