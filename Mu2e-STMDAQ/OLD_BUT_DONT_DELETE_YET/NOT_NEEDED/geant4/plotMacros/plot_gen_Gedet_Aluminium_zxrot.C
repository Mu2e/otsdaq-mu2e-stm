...
void plot_gen_Gedet_Aluminium_zxrot(std::string filename, std::string HPGe_rotation){
  ...
  TH2D* hyx_gen = new TH2D("hyx_gen", "xy position of the hit in each step", 1000, 0, 4, 1000, -2, 2);
  ...
  TH2D* hxz_gen = new TH2D("hxz_gen", "xz position of the hit in each step", 1000, -12, 10, 1000, -10, 10);
  hxz_gen->GetXaxis()->SetTitle("z_{gen #gamma collimator} [cm]");
  hxz_gen->GetYaxis()->SetTitle("x_{gen #gamma collimator} [cm]");
  ...
  TH2D* hyz_stm = new TH2D("hyz_stm", "yz position of the hit in each step", 1000, -12, 10, 1000, -10, 10);
  hyz_stm->GetXaxis()->SetTitle("z_{#gamma STM} [cm]");
  hyz_stm->GetYaxis()->SetTitle("y_{#gamma STM} [cm]");
  TH2D* hyz_al = new TH2D("hyz_al", "yz position of the hit in each step", 1000, -12, 10, 1000, -10, 10);
  ...
  TH2D* hyz_vd = new TH2D("hyz_vd", "yz position of the hit in each step", 1000, -1, 10, 1000, -10, 10);
  hyz_vd->GetXaxis()->SetTitle("z_{#gamma STM} [cm]");
  hyz_vd->GetYaxis()->SetTitle("y_{#gamma STM} [cm]");
  TH2D* hxz_al = new TH2D("hxz_al", "xz position of the hit in each step", 1000, -12, 10, 1000, -10, 10);
  hxz_al->GetXaxis()->SetTitle("z_{#gamma STM} [cm]");
  hxz_al->GetYaxis()->SetTitle("x_{#gamma STM} [cm]");
  ...
  TTree* tree4=(TTree*)input->Get("StepsAlWindow");
  ...
  for(unsigned long i=0; i<entries4; i++)
    {
      tree4->GetEntry(i);
      double costheta = pre_pz/sqrt(pre_px*pre_px + pre_py*pre_py + pre_pz*pre_pz);
      //if((preYpos > 2.06)&&(costheta>0)){
      //if(EventID==946577){
      if((PDGID==22)&&(EdepStep!=0)){
      std::cout<<"EventID: "<<EventID<<" PDGID: "<<PDGID<<" preXpos: "<<preXpos<<" preYpos: "<<preYpos<<" preZpos: "<<preZpos<<" pre_px: "<<pre_px<<" pre_py: "<<pre_py<<" pre_pz: "<<pre_pz<<" EdepStep: "<<EdepStep<<" StepLengthStep: "<<StepLengthStep<<" costheta: "<<costheta<<std::endl;
	}
      hxz_al->Fill(preZpos, preXpos);
      hyz_al->Fill(preZpos, preYpos);
    }
  ...
  hxz_gen->SetFillStyle(3001);
  hxz_gen->SetLineColor(kBlack);
  hxz_gen->SetLineWidth(1);
  hxz_gen->SetFillColor(kBlue);
  hxz_gen->Draw("HIST,SAMES");

  hxz_al->SetFillStyle(3001);
  hxz_al->SetLineColor(kBlack);
  hxz_al->SetLineWidth(1);
  hxz_al->SetFillColor(kBlue);
  hxz_al->Draw("HIST,SAMES");
  ...
  hyz_al->SetFillStyle(3001);
  hyz_al->SetLineColor(kBlack);
  hyz_al->SetLineWidth(1);
  hyz_al->SetFillColor(kCyan-3);
  hyz_al->Draw("HIST,SAMES");
