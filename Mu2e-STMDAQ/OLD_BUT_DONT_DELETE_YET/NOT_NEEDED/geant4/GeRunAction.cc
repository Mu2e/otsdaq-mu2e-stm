...
// set printing event number per each event
	...
	analysisManager->CreateH1("StepLength-Step",
	                          "Step Length by electrons+photons  in each step",
	                          100,
	                          0.,
	                          1.5);
	...
	analysisManager->CreateH1(
	    "diffcrosssectE", "Difference between Cross Sections in each step", 100, -10, 10);
	analysisManager->SetH1XAxisTitle(
	    9, "#sigma_{input Geant4}-(1/(#rho_{Ge}*steplength)) (g/cm^{2})");
	analysisManager->CreateH1("NehEvent", "", 100, 0, 300000);
	analysisManager->SetH1XAxisTitle(
	    10, "#e^{-}-hole pairs created in each event by eIoni+eBrem+msc");
	// fill histograms H2  STEP
	analysisManager->CreateH2(
	    "ZXpos", "zx position of the hit in each step", 1000, 0., 20, 1000, -20., 20);
	analysisManager->SetH2XAxisTitle(0, "z (cm)");
	analysisManager->SetH2YAxisTitle(0, "x (cm)");
	analysisManager->CreateH2(
	    "ZYpos", "zy position of the hit in each step", 1000, 0., 20, 1000, -20., 20);
...
{
	G4cout << "GeRunAction::BeginOfRunAction: Run #" << aRun->GetRunID() << " start."
	       << G4endl;
