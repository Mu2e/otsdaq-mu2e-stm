...
GeSensitiveDetector::GeSensitiveDetector(G4String name) : G4VSensitiveDetector(name) {}
...
//GeRunAction* runActionCur=(GeRunAction *)runManager->GetUserRunAction();
	...
	G4cout << "Detector hit:" << pointPre->GetPosition() / CLHEP::cm << " cm  --> "
	       << pointPost->GetPosition() / CLHEP::cm << " cm by " << particleCurName
	       << " with Epost_step=" << kineticEnergyCur / CLHEP::keV << "keV inside "
	       << volumeCurName << G4endl;

	G4cout << "Particle created:  " << particleCur->GetParticleName() << " PDGID("
	       << particleCur->GetPDGEncoding() << ") " << trackCur->GetTrackID()
	       << " Pos start: " << pointPre->GetPosition() / CLHEP::cm
	       << "cm  Pos end: " << pointPost->GetPosition() / CLHEP::cm
	       << "cm Step Length: " << aStep->GetStepLength() / CLHEP::cm << " cm"
	       << " Energy Deposit: " << (aStep->GetTotalEnergyDeposit()) / CLHEP::keV
	       << " keV"
	       << " Mass: " << particleCur->GetPDGMass() << G4endl;
	G4cout << "PreStep-----Ekin= " << (pointPre->GetKineticEnergy()) / CLHEP::keV
	       << " keV"
	       << " with Etot= " << (pointPre->GetTotalEnergy()) / CLHEP::keV << " keV"
	       << G4endl;
	G4cout << "PostStep-----Ekin= " << (pointPost->GetKineticEnergy()) / CLHEP::keV
	       << " keV"
	       << " with Etot= " << (pointPost->GetTotalEnergy()) / CLHEP::keV << " keV"
	       << G4endl;
#endif
