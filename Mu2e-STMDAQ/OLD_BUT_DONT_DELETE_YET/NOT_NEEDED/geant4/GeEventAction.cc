...
void GeEventAction::EndOfEventAction(const G4Event* event)
{
...
#ifdef debug
	std::cout << "Egen: " << Egen << " pxgen: " << pxgen << " pygen: " << pygen
	          << " pzgen: " << pzgen << " MeV" << std::endl;
#endif
  ...
  */
	...
	G4cout << "---> End of event: " << eventID
	       << "   Total energy deposit: " << std::setw(7) << fEnergyAbs << " keV "
	       << "       Total track length by e+,e-: " << std::setw(7) << fTrackLAbs
	       << " cm       Total track length by phot: " << fTrackgamma << " cm" << G4endl;
