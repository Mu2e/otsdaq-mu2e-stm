...
GePrimaryGeneratorAction::~GePrimaryGeneratorAction() { delete particleGun; }
...
void GePrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
	if(anEvent->GetEventID() == 0)
	{
		...
		while(std::getline(input_file, line))
		{
			...
			while(ss >> element)
			{
				if(element.compare("pdgid:") == 0)
					ss >> pdgid;
				if(element.compare("momMeV:") == 0)
					ss >> momMeV;
				if(element.compare("thetamin:") == 0)
					ss >> thetamin;
				if(element.compare("initpos_x_cm:") == 0)
					ss >> initpos_x_cm;
				if(element.compare("initpos_y_cm:") == 0)
					ss >> initpos_y_cm;
				if(element.compare("initpos_z_cm:") == 0)
					ss >> initpos_z_cm;

				if(element.compare("bool_dodisk_pos:") == 0)
				{
					ss >> bool_dodisk_pos;
					if(bool_dodisk_pos == 1)
					{
						dodisk_pos = true;
						std::cout << "do disk initial pos" << std::endl;
					}
				}

				if(element.compare("init_rmax_cm:") == 0)
					ss >> init_rmax_cm;
				...
				if(element.compare("bool_docyl_pos:") == 0)
				{
					...
					{
						docyl_pos = true;
						std::cout << "do cyl initial pos" << std::endl;
					}
				}
				...
				{
					ss >> bool_dofixed_pos;
					if(bool_dofixed_pos == 1)
					{
						dofixed_pos = true;
						std::cout << "do fixed initial pos" << std::endl;
					}
				}
			}
		}
		...
		std::cout << "PDG id: " << pdgid << ", Initial pos (x,y,z)cm: (" << initpos_x_cm
		          << "," << initpos_y_cm << "," << initpos_z_cm << "), p=" << momMeV
		          << "MeV, thetamin: " << thetamin << std::endl;
	}
  ...
  */
	...
	std::cout << "Generated Pos= " << G4ThreeVector(x, y, z) / CLHEP::cm << " cm"
	          << std::endl;
	std::cout << "Initial Energy: " << G4BestUnit(energy, "Energy") << std::endl;
	std::cout << "Initial Momentum Vector: "
	          << particleGun->GetParticleMomentumDirection() << " Initial Momentum Mag: "
	          << anEvent->GetPrimaryVertex()->GetPrimary()->GetTotalMomentum() /
	                 CLHEP::keV
	          << " keV Etot energy: "
	          << anEvent->GetPrimaryVertex()->GetPrimary()->GetTotalEnergy() << std::endl;
	std::cout << "cosTheta: " << cosTheta << std::endl;
	std::cout << "Number of particle generated: " << particleGun->GetNumberOfParticles()
	          << std::endl;
