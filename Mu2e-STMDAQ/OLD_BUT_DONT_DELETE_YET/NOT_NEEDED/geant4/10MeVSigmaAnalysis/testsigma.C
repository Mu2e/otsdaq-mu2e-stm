...
void testsigma(){
  ...
  TH1F*hedepevent = new TH1F("hedepevent", "Edep by electrons in each event", 100, 9985., 10000);
  ...
  TH1F*hsigma = new TH1F("hsigma","", 100, 0, 0.01); //0.0001 kev binning 
  hsigma->GetXaxis()->SetTitle("#sigma_{E} (keV)");
  hsigma->SetTitle("10 MeV photons");

  //Histogram to store the meanenergy of each file
  TH1F*hmeanE = new TH1F("hmeanE","", 150, 9985, 10000); // 0.1 kev binning 
  hmeanE->GetXaxis()->SetTitle("Mean E_{Reco} (keV)");
  hmeanE->SetTitle("10 MeV photons");


  //Abrimos el txt
  fstream readfile;
  readfile.open("10MeVruns.txt",ios::in);
  string name;
  ...
  */
  ...
  double rate_photoelectric = hedepevent->Integral(hedepevent->FindFixBin(9985), hedepevent->FindFixBin(10000), "");
  ...
  double rate_compton = hedepevent->Integral(hedepevent->FindFixBin(0), hedepevent->FindFixBin(9985), "");
