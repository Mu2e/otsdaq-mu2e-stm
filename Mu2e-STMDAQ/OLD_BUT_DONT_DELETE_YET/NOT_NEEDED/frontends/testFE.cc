...
void read_data(int socket)
{
	...
	recvSpeed = readCount * (pSize * 8 * 1e-9) /
	            (timeNano * 1e-9);  // Packet size [bytes] * Gbits in byte / time in secs
}
...
void write_data()
{
	...
	// Enter infinite loop until exit FE is triggered in exit thread
	while(!exitFE)
	{  // or (rData.packetTimeout && writeCount != readCount)){
...
int main()
{
	...
	if(readCount == numberOfPackets)
	{
		cout << "Read efficiency = " << readCount << "/" << numberOfPackets << " = "
		     << eff << " %. "
		     << "Receive speed = " << recvSpeed << " Gbit/s." << endl;
	}
	else
	{
		cout << "Read efficiency = " << readCount << "/" << numberOfPackets << " = "
	...
	cout << "Write efficiency = " << double(writeCount) / numberOfPackets * 100
	     << " %. Written a total of " << totalDataSize * 1e-6 << " Mb" << endl;
