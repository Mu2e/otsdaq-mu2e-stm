...
int begin_of_run(int run_number)
{
	...
	{
		binary_file[i] =
		    xml_file->value("stm.raw_binary_filename") + UDPin[i].get_channel_name(i);
...
{
	// Notify user
	cout << "In " << UDPin[chan].get_channel_name(chan) << " receive data thread..."
	     << endl;
...
int main()
{
	...
	for(uint i = 0; i < chNum; i++)
	{
		cout << "Channel " << i << " [" << UDPin[i].get_channel_name(i)
		     << "]: received  = " << recvCount[i] << ", pulled = " << pullCount[i]
