...
void NewDropPackets(std::string filename, int percentage_dropped)
{
	...
	std::cout << "Size of the packet in time: " << packetLen * (1. / fadc) << " us"
	          << std::endl;
	...
	for(int i = 0; i < (numPackets + 1); i++)
	{
		...
		if(countercopy + counterdrop + packetLen > n)
		{
	...
	std::cout << "Total elements: " << counterdrop + countercopy
	          << ", should  be the same as: " << n << std::endl;
	...
	std::cout << "We are skipping: " << 100 * total_droppedpackets / numPackets << " %"
	          << std::endl;
...
void FractionalDropPackets(std::string filename, int num, int den)
{
	...
	for(int i = 0; i < (numPackets + 1); i++)
	{
		...
		{
			memcpy(&ADC_droppackets[countercopy],
			       &ADC[counter],
			       (den - num) * (2 * packetLen));
	...
	std::string output_filename =
	    "/work/cgarcia/DATA/Claudia/GenData/MWDEfficiency_SimPoisson/DropPackets/" +
	    filename.substr(filename.find("Genda \
ta"),
	                    18) +
	    "_frac1droppedpacket_" + percentage + "percent" + ".bin";
...
int main(int argc, char* argv[])
{
	...
	int mode =
	    0;  //mode = 0 probability (input percentage), mode = 1 (input integer number)
