...
void CreateBinary(int16_t**         data,
                  unsigned long int dataNumTot,
                  int               packetNum,
                  ...
                  uint              tHdrLen)
{
	...
	for(int packetn = 0; packetn < packetNum; packetn++)
	{
		...
		if(packetn == 0)
		{
			sizep = packetLen - pHdrLen;
		}
		else
		{
			sizep = packetLen - pHdrLen - tHdrLen;
		}
		while(i < sizep)
		{
			...
			{
				//Keep trigger header
				for(unsigned int k = 0; k < (packetLen - pHdrLen); k++)
				{
					bin_data[h] = data[packetn][j];
					cout << "j= " << j << " i= " << i << " data stored= " << bin_data[h]
					     << endl;
					j++;
					i++;
					h++;
			...
			else
			{
				...
				for(unsigned int k = 0; k < (packetLen - pHdrLen - tHdrLen); k++)
				{
					bin_data[h] = data[packetn][j];
					cout << "j= " << j << " i= " << i << " data stored= " << bin_data[h]
					     << endl;
...
int main(void)
{
	...
	for(int p = 0; p < 3; p++)
	{
		...
		for(int count = 0; count <= 41; count++)
		{
			...
			{
				data[p][count] = 1;
				std::cout << data[p][count] << std::endl;
			}
			if(count > 2 && count <= 18)
			{
				data[p][count] = 2;
			...
			if(count > 26 && count <= 29 && p == 0)
			{
				data[p][count] = 3;
				std::cout << data[p][count] << std::endl;
			}
			if(count > 26 && count <= 29 && p == 1)
			{
				data[p][count] = 4;
				std::cout << data[p][count] << std::endl;
			}
			if(count > 26 && count <= 29 && p == 2)
			{
				data[p][count] = 5;
				std::cout << data[p][count] << std::endl;
			}
			...
			if(count > 37 && count <= 40 && p == 0)
			{
				data[p][count] = 3;
				std::cout << data[p][count] << std::endl;
			}
			if(count > 37 && count <= 40 && p == 1)
			{
				data[p][count] = 4;
				std::cout << data[p][count] << std::endl;
			}
			if(count > 37 && count <= 40 && p == 2)
			{
				data[p][count] = 5;
				std::cout << data[p][count] << std::endl;
			}
