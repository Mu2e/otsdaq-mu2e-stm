...
int16_t** getData(struct headerInfo trigInfo)
{
	...
	long unsigned int dataNumTot =
	    sampleNum + sliceTot * sHdrLen + packetNum * (pHdrLen + tHdrLen);
	...
	for(int i = 0; i < pulseNum; i++)
	{
		...
		if(timeIndex + pulseLength > sampleNum)
			max = sampleNum;
	...
	for(int p = 0; p < packetNum; p++)
	{
		...
		uint16_t* triggerHeader = tHeader(
		    macroCount, macroTime, mode, microCount, channel, ADCoffset, sliceNum);
		...
		for(int s = 0; s < sliceNum; s++)
		{
			...
			if(extSampleTime <= macroWidth)
				ext_int_delay = extSampleTime;
			if(extSampleTime > macroWidth)
				ext_int_delay = extSampleTime - macroWidth;

			double intSampleTot =
			    macroGapWidth - ext_int_delay - int_trig_num * int_ADC_offset;
			...
			if(mode == 0)
			{
				adc1 = (macroPeriod * macroCount + sliceNo * sliceTime) * 1000;
			...
			if(mode == 1)
			{
				...
				adc1 = ((macroCount + 1) * macroWidth + (macroCount + 1) * ext_int_delay +
				        microCount * intTrigLen + (microCount + 1) * int_ADC_offset +
				        sliceNo * sliceTime) *
				       1000;
			}
			...
			uint32_t sliceTm =
			    sliceTime / tadc;  //Number of ADC values in each slice, constant
...
int main()
{
	...
	double totSampleTime = extSampleTime + ext_int_delay + ext_ADC_offset +
	                       int_trig_num * (ext_ADC_offset + intSampleTime);
	if(totSampleTime > macroPeriod)
	{
		cout << "\nERROR! Summed sampling times are larger than macropulse period: "
		     << endl;
		cout << "-" << endl;
		...
		cout << "---" << endl;
		cout << "Total summed sampling time per macropulse = " << totSampleTime << " us"
		     << endl;
	...
	while(macroCount < 4)
	{
		...
		// Calculate total length of data array including headers
		dataNumTot =
		    sampleNum + sliceTot * sHdrLen + packetNum * (pHdrLen + tHdrLen);  //Need
		// Calculat array length of each packet
		...
		struct headerInfo extMode = {
		    macroCount, macroTime, mode, microCount, channel, ext_ADC_offset};
		...
		{
			cout << "Error: External ADC offset is within one order of magnitude of the "
			        "marcopulse width!"
			     << endl;
			cout << "Exiting...\n" << endl;
			exit(0);
		...
		auto extStart =
		    std::chrono::high_resolution_clock::now();  // Time before ext data
		int16_t** extData = getData(extMode);
		auto   extEnd = std::chrono::high_resolution_clock::now();  // Time after ext data
		double extTime =
		    std::chrono::duration<double, std::milli>(extEnd - extStart).count() *
		    1e3;  // Ext data time
		...
		{
			cout << "\nERROR! Summed internal sampling times are larger than remaining "
			        "time in macropulse gap: "
			     << endl;
			cout << "-" << endl;
			...
			cout << "Macropulse gap = " << macroGapWidth << " us" << endl;
			cout << "---" << endl;
			cout << "Total internal sampling time per macropulse gap = "
			     << int_trig_num * intSampleTime << " us" << endl;
			cout << "Reamining time available for sampling in macropulse gap = "
			     << intSampleTot << " us" << endl;
			cout << "Difference = " << intSampleTot - int_trig_num * intSampleTime
			     << " us\n"
			     << endl;
			exit(0);
		}
		...
		for(int t = 0; t < int_trig_num; t++)
		{
			...
			// Fill struct with header info
			struct headerInfo intMode = {
			    macroCount, macroTime, mode, intTrigCount, channel, int_ADC_offset};

			// Check ADC offset isn't within one order of magnitude of interal trigger length
			if(int_ADC_offset >= 0.1 * intTrigLen)
			{
				cout << "Error: Internal ADC offset is within one order of magnitude of "
				        "the trigger window!"
				     << endl;
				cout << "Exiting...\n" << endl;
				exit(0);
			...
			double    intTime =
			    std::chrono::duration<double, std::milli>(intEnd - intStart).count() *
			    1e3;
		...
		double elapsed_time_ms =
		    std::chrono::duration<double, std::milli>(macroEnd - macroStart).count();
