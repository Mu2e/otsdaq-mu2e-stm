...
void PlotAcceptance_E(std::string ArtFiles_location, double acceptance_VD10_15, double acceptance_VD10_15_error, double acceptance_VD101_8990, double acceptance_VD101_8990_error, double xmin, double xmax, double ymin, double ymax, string Xtitlest, string Ytitlest, int VDnumber, double cutVD10, double branch){
  ...
  double E_error[21];
  double acceptance_phot[21], acceptance[21], gammasdet_stm[21];
  double acceptance_error[21], gammasdet_stm_error[21];  
  //study acceptance error for now set it to 0
  ...
  for(int i = 0; i < 21; i++){
    ...
    acceptance_VD15_101 = p[0] + 1./(p[1]*E[i]) - sqrt(1./(p[2]*E[i]) + 1./(p[3]*E[i]*E[i]));

    if(VDnumber == 10){
      acceptance_phot[i] = acceptance_VD10_15 * acceptance_VD15_101 *acceptance_VD101_8990;
    }
    //else if(VDnumber == 15){
    //acceptance_phot[i] = acceptance_VD15_101 *acceptance_VD101_8990;
    //}
    else{std::cout<<"The initial VD introduced is not valid...exit..."; exit(0);}
  ...
  TF1*Fit = new TF1("Fit", "[0] + (1./([1]*x)) - sqrt( (1./([2]*x)) + (1./([3]*x*x)) )", 1, xmax);
  ...
  //TFitResultPtr fitptr = gracc->Fit(Fit,"0S","", xmin, 1);  
  //TFitResultPtr fitptr = gracc->Fit(Fit,"0S","", 1, xmax);
  ...
  double p_fitvalues[4] = { 6.748e-7 , 5.923e5 , 4.455e11, 3.506e11 };
  ...
  /*
  if(drawacc_weight==true){

    double acc_weighted[21];
    for (int i = 0; i < 21; i++){

      acc_weighted[i] = sumbincontent[i]*acceptance[i];
      std::cout<<"Acc: "<<acceptance[i]<<" Acc weighted : "<<acc_weighted[i]<<std::endl;
    }
    
    TGraphErrors *gracc = new TGraphErrors( N, E, acc_weighted, E_error, acceptance_error );
    gracc->Print("");
    gracc->SetMarkerStyle(21);
  ...
  */
  ...
  //c1->Print("MDC2020MuonsEle_FlashPhotonsEdistrVD10cut_Acc_1_0.0032_80MeVlogscale_newfit.png");
  //c1->Print("AcceptanceVD10_VD8990_E_1_0.0032_VD10_nocut.pdf");
}


//================================================================

void Acceptance_Rate(std::string ArtFiles_location, double acceptance_VD10_15, double acceptance_VD10_15_error, double acceptance_VD101_8990, double acceptance_VD101_8990_error, int VDnumber, double cutVD10){
  
  PlotAcceptance_E("", acceptance_VD10_15, acceptance_VD10_15_error, acceptance_VD101_8990,  acceptance_VD101_8990_error, 0, 6, 0, 0.00000035, "E_{#gamma} [MeV]", "Acceptance VD10 - VD89,90", VDnumber, cutVD10, 0);
  ...
  //PlotAcceptance_E(ArtFiles_location, acceptance_VD10_15, acceptance_VD101_8990,  0, 6, 0, 0.02, "E_{#gamma} [MeV]", "Acceptance(VD10,cut-VD89,90)#timesWeight", VDnumber, cutVD10, 1);  
}
