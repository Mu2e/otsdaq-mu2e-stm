...
void PlotAcceptance_E(std::string ArtFiles_location, double acceptance_VD10_15, double acceptance_VD10_15_error, double acceptance_VD101_8990, double acceptance_VD101_8990_error, double xmin, double xmax, double ymin, double ymax, string Xtitlest, string Ytitlest, int VDnumber, double cutVD10, double branch){
  ...
  TH1D*h1 = new TH1D("h1","", Nbins_in, xlow, xlimit);
  TH1D*h2 = new TH1D("h2","", Nbins_in, xlow, xlimit);
  //TH1D*h3 = new TH1D("h3","", Nbins_in, 0, 0.003);
  TH1D*h3 = new TH1D("h3","", Nbins_in, 0, 0.05); 
  ...
  for(int i = 0; i < Nbins; i++){
    ...
    weight[i] = bincontent;
      ...
      //Introduce the number of electrons in costheta vs. the number of electrons in the full range from MDCD2020 as energy dependent
      /*if(energyMeV<0.5){
	nparticles_md2020_costhetarange = 10.0;
	nparticles_md2020_full = 252224.0;
      }
      else if((energyMeV>=0.5)&&(energyMeV<1)){
	nparticles_md2020_costhetarange = 14.0;
	nparticles_md2020_full = 556930.0;
      }
      else if((energyMeV>=1)&&(energyMeV<5)){
	nparticles_md2020_costhetarange = 787.0;
	nparticles_md2020_full = 13761400.0;
      }
      else if((energyMeV>=5)&&(energyMeV<25)){
	nparticles_md2020_costhetarange = 621.0;
	nparticles_md2020_full = 10193300.0;
      }
      else{
	nparticles_md2020_costhetarange = 171.0;
	nparticles_md2020_full = 3059940.0;
	}*/

    nparticles_md2020_costhetarange = 1603;
    nparticles_md2020_full = 27823794;


      //This acceptances in ElectronBeamStudyARTGeant.pdf
      if(pdgidpart == 11){      
	//if(energyMeV<50) { acceptance_VD15_101 = 2.3/10000; }
	//else if((energyMeV>=50)&&(energyMeV<100)) { acceptance_VD15_101 = 13.0/10000; }
	//else { acceptance_VD15_101 = 3979.0/10000; }
	//Calculate a weighted acceptance for number of particles <25 and E>25MeV:  UPPER LIMIT
	acceptance_VD15_101 = 0.00025 ;
	//std::cout<<"weighted acceptance: "<<acceptance_VD15_101<<std::endl;
      }//if pdgidpart == 11
      ...
      if(pdgidpart == 22){
	//if(energyMeV<5) { acceptance_VD15_101 = 2.3/10000; }
	//else if((energyMeV>=5)&&(energyMeV<25)){ acceptance_VD15_101 = 18.0/10000; }
	//else if((energyMeV>=25)&&(energyMeV<50)){ acceptance_VD15_101 = 97.0/10000; }
	//else if((energyMeV>=50)&&(energyMeV<100)){ acceptance_VD15_101 = 489.0/10000; }
	//else{ acceptance_VD15_101 = 43854.0/10000; }
	//Calculate a weighted acceptance for number of particles <25 and E>25MeV:  UPPER LIMIT
	acceptance_VD15_101 = 0.0024 ;
        //std::cout<<"weighted acceptance: "<<acceptance_VD15_101<<std::endl;
    }


      h3->Fill(acceptance_VD15_101, weight[i]);
     ...
     if(VDnumber == 10){
       if(bincenter > 0){
	 acceptance_phot[i] = acceptance_VD10_15 * acceptance_VD15_101 * acceptance_VD101_8990;
       }
       else { acceptance_phot[i] =0; }
     }

     
     std::cout<<"Acceptance small solid angle: "<<acceptance_phot[i]<<std::endl;
     
     acceptance[i] = Acc*acceptance_phot[i];
  ...
  if(drawhisto ==true){
    ...
    h2->GetYaxis()->SetTitleOffset(1.85);
  
    gPad->SetLogy();

    //Integral of the histogram to get the rate
    double integral =  h2->Integral(h2->FindFixBin(xlow), h2->FindFixBin(xlimit), "");
    std::cout<<"Integral of the histogram "<<xlow<<"-"<<xlimit<<" MeV: "<<integral<<std::endl;
    double integral_width =  h2->Integral(h2->FindFixBin(xlow), h2->FindFixBin(xlimit), "width");
    std::cout<<"Integral of the histogram (multiplying by bin width) "<<xlow<<"-"<<xlimit<<" MeV: "<<integral_width<<std::endl;
    ...
    for(int i = 0; i < Nbinsh2; i++){
      ...
      if((bincenter>xlow)&&(bincenter<xlimit)){sum=sum+bincontent;}
    }
    std::cout<<"Integral of the histogram by summing the bin content "<<xlow<<"-"<<xlimit<<" MeV: "<<sum<<std::endl;
  ...
  //c1->Print("MDC2020MuonsEle_PhotonsfromEleEdistrVD10cut_Acc_1_0.0032_80MeVlogscale_ConvFactorcte_legend.png");
...
void Acceptance_Rate_ConvFactor_Ele(std::string ArtFiles_location, double acceptance_VD10_15, double acceptance_VD10_15_error, double acceptance_VD101_8990, double acceptance_VD101_8990_error, int VDnumber, double cutVD10){

  
  //PlotAcceptance_E(ArtFiles_location, acceptance_VD10_15, acceptance_VD10_15_error, acceptance_VD101_8990,  acceptance_VD101_8990_error, 0, 6, 0, 0.0000005, "E_{e^{-}} [MeV]", "Electron Counts #times Acceptance(VD10-VD89,90)", VDnumber, cutVD10, 1);


  //PlotAcceptance_E(ArtFiles_location, acceptance_VD10_15, acceptance_VD10_15_error, acceptance_VD101_8990,  acceptance_VD101_8990_error, 0, 6, 0, 0.0000005, "E_{e^{-}} [MeV]", "Counts #times Acceptance(VD10-VD89,90)", VDnumber, cutVD10, 1);
  ...
  //PlotAcceptance_E(ArtFiles_location, acceptance_VD10_15, acceptance_VD10_15_error, acceptance_VD101_8990,  acceptance_VD101_8990_error, 0, 6, 0, 0.0000005, "Photon Acceptance(VD15-VD101) from Ele-Beam, weighted by energy", "Counts", VDnumber, cutVD10, 1); 
}
