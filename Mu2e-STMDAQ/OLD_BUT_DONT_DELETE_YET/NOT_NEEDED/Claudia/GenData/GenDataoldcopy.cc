...
int16_t* tHeader(int16_t triggernumber,
                 int64_t triggertime,
                 int16_t triggertype_channel_datatype,
                 int32_t triggertimeoffset,
                 int16_t baselinemean,
                 int16_t baselineRMS)
{
...
void GenDataHeaders()
{
	...
	double rate =
	    ratekHz *
	    1e-6;  // 1kHz in us --------------(I think this is 1e+3/ratekHz = us)--------------------
	// Average number of pulses in the sample
	int pulseNum = rate * sampleTime;

	// Print variables to screen
	cout << "Rate in detector = " << rate << " kHz"
	     << endl;  //I think this is ratekHz-------
	...
	for(int i = 0; i < pulseNum; i++)
	{
		...
		for(uint j = index; j < max; j++)
		{
			...
			ADC[j] += -(twiceA / (1 + exp(-(time - xshift) * invtaufall))) *
			          (1.0 - (1.0 / (1 + exp(-(time - xshift) * invtaudecay))));
		}
	}
	...
	for(int i = 0; i < sliceTot; i++)
	{
		...
		if(newPacket)
		{
			...
			int16_t* triggerHeader             = tHeader(trignumber,
                                             trigtime,
                                             trigtype_channel_datatype,
                                             trigtimeoffset,
                                             baselinemean,
                                             baselineRMS);
