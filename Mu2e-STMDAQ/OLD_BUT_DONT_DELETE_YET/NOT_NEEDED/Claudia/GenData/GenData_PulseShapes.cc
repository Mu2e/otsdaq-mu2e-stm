...
void Gendata_PulseShapes(int         argc,
                         char*       argv[],
                         std::string rate_stringkHz,
                         double      noiseSD,
                         std::string stringoutputpath)
{
	//Read 10 pulses shapes
	fstream readfile;
	readfile.open("/work/cgarcia/DATA/Claudia/PulseShape_Analysis/Shapes/pulses.txt",
	              ios::in);
	std::string         name;
	...
	for(int i = 0; i < pulseNum; i++)
	{
		...
		std::cout << "Peak " << i << " at " << timeIndex * tadc
		          << " us, pulse type: " << iRand << std::endl;
		...
		for(unsigned long int j = timeIndex; j < max; j++)
		{
			...
			if(j < lasttime_index)
			{
				ADC[j] += pulse[iRand].at(j - timeIndex);
			}
			// Subtract noise (the signal from data already has 0.32 mV noise) and add pulse
			else
			{
				ADC[j] += pulse[iRand].at(j - timeIndex) - track_noise[j];
			}
	...
	std::cout << "Real number of pulses in binary file: " << realpulses_binary
	          << std::endl;
	std::cout << "Rate: " << (realpulses_binary / timeSample) * 1000 << " kHz"
	          << std::endl;
