...
int16_t pulseCalc(double x, double twiceA)
{
	...
	if(x < xshift)
	{
		pulse = -(twiceA / (1 + exp(-(x - xshift) * invtaufall)));
	}
	if(x >= xshift)
	{
		pulse = -twiceA * (1.0 - (1.0 / (1 + exp(-(x - xshift) * invtaudecay))));
	}
	return pulse;
}
             ...
             std::string stringoutputpath)
{
	...
	std::cout << "Number of pulses generated (real number of simulated pulses at the end "
	             "of the file): "
	          << pulseNum << std::endl;
	std::cout << "Simulated Pulse Height: " << twiceA / 2
	          << " ADC Counts = " << (twiceA / 2) * 0.57 << " keV" << std::endl;
	...
	std::cout << "NoiseSD: " << noiseSD << " mV = " << sigma_noise_ADC
	          << " ADC Counts, xshift= " << xshift << std::endl;
	...
	for(int i = 0; i < pulseNum; i++)
	{
		...
		//Need to add this
		if((timeIndex + xshift / tadc) >= sampleNum)
		{
			continue;
		}
