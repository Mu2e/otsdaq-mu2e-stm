...
int main(int argc, char* argv[])
{
	...
	{
		outrootname = "Signal_BackgroundTrueReco_" + sigmarange[0].str() + "_" +
		              sigmarange[1].str() + "_" + sigmarange[2].str() + "sigma.root";
	...
	for(int i = 0; i < Njobs; i++)
	{
		...
		rootname = input_path +
		           "/UnbinnedLogLike_1.00kHz_TimeSim_10s_seed_0_0.0010MeV_50Runs_Job_" +
		           std::to_string(i) + "_Background_ShapeNominal.root";
		...
		std::cout << "1 vector: " << treeread->GetEntries()
		          << " of entries: " << nexperiments << std::endl;
		...
		for(int j = 0; j < nexperiments; j++)
		{
			...
			for(int k = 0; k < backsize; k++)
			{
				...
				recobrems[k] =
				    fbrems->Integral(frange_sigma[k][0], frange_sigma[k][1]) / bin_width;
				//Integrate this histogram in range to get True background + signal
				trueSB[k]    = hSTM->Integral(hSTM->FindFixBin(frange_sigma[k][0]),
                                           hSTM->FindFixBin(frange_sigma[k][1]),
                                           "");
				...
				std::cout << "---- True Background: " << sigmarange[k].str()
				          << "sigma: " << TrueBack_[k] << std::endl;
				std::cout << "Reco Background: " << sigmarange[k].str()
				          << "sigma: " << RecoBack_[k] << std::endl;
			}
			...
			if(plot_1stCovmatrix_element == true)
			{
				...
				std::cout << "Cov matrix element: " << _Covmatrix[0][0]
				          << " Reco sigma: " << reco_sigma << " Accuracy: " << accuracy
				          << std::endl;
				covMatrixHisto->Fill(reco_sigma);
			}

			std::cout << "---- True Signal: " << TrueSignal_
			          << ", Reco Signal: " << RecoSignal_ << std::endl;
		}  //nexperiments
