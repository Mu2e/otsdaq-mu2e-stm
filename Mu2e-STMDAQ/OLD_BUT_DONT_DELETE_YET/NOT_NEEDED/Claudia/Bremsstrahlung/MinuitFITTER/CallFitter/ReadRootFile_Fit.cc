...
void ReadRootFile_Fit(int         argc,
                      char*       argv[],
                      std::string rootname,
                      int         FitOption,
                      double      xfit_low,
                      ...
                      std::string outpath)
{
	...
	{
		std::string logfile = outpath + "/ReadRootFile_Fitrange_" +
	...
	{
		rootoutfile = outpath + "/ReadRootFile_Fitrange_" + stream_xfit_low.str() + "_" +
		              stream_xfit_max.str() + "_Timesim_" + std::to_string(int(TimeSim)) +
		              "s_" + rate_str + "kHz_" + resolution_str + "MeV.root";
	...
	fitter->Histo_Data(
	    fit_range,
	    histo);  //Get nbins_fit, _binning, bincontent, bincontent_error, bincenter
	...
	if(store_dataROOT == true)
	{
		...
		char*       char_BestFitPars_fitrange =
		    const_cast<char*>(str_BestFitPars_fitrange.c_str());
		...
		TMatrixD* BestFitPars_newFit =
		    new TMatrixD(NP, 1, _BestFitPars_fitrange.GetMatrixArray(), "");
	...
	std::string nameplot_png = outpath + "/ReadRootFile_Fitrange_" +
	                           stream_xfit_low.str() + "_" + stream_xfit_max.str() +
	                           "_Timesim_" + std::to_string(int(TimeSim)) + "s_" +
	                           rate_str + "kHz_" + resolution_str + "MeV.png";
