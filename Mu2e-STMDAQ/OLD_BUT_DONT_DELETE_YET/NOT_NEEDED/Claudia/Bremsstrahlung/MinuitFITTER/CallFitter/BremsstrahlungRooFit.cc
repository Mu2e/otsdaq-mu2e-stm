...
int main(int argc, char* argv[])
{
	...
	std::string path =
	    "/data1/cgarcia/SignaltoBackground/MinuitFit_1kHz_100000s_347keV_1keVres/"
	    "1000JOBS/";
	...
	for(int i = 0; i < njobs; i++)
	{
		std::string file_name =
		    path + "BinnedLoglike_NOIntegral_1kHz_TimeSim_100000s_seed_0_50Runs_Job_" +
		...
		std::cout << "1 vector: " << treeread->GetEntries()
		          << " of entries: " << TrueSignal->size() << std::endl;
		...
		for(int j = 0; j < nexperiments; j++)
		{
			...
			std::cout << "Data size in fit range (signal + brems): " << datasize
			          << std::endl;
			...
			//Signal
			double fs_var =
			    (best_fitpar[0] / bin_width) /
			    datasize;  //here best_fitpar[0] is the amplitude for a given binning, we need to provide the normalised parameter so we need to divide by the data size
