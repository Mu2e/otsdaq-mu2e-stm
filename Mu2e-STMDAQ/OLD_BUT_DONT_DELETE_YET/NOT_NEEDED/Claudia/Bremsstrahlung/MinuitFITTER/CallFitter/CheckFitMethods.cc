    ...
    int argc, char* argv[], std::string rootname, int FitOption, int nsample, double seed)
{
	...
	fitter->Histo_Data(
	    fit_range,
	    hSTM1);  //Get nbins_fit, _binning, bincontent, bincontent_error, bincenter
	...
	{
		rootfile =
		    "Genfx_seed_" + std::to_string(int(seed)) + "_" + name_method + ".root";
	...
	std::string nameplot_png =
	    "Genfx_seed_" + std::to_string(int(seed)) + "_" + name_method + ".png";
	char*       char_nameplot_png = const_cast<char*>(nameplot_png.c_str());
	std::string nameplot_pdf =
	    "Genfx_seed_" + std::to_string(int(seed)) + "_" + name_method + ".pdf";
