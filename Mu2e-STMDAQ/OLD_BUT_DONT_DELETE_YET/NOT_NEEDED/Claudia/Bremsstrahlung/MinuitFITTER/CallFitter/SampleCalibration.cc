...
void SampleCalibration(int         argc,
                       char*       argv[],
                       std::string rootname,
                       int         FitOption,
                       double      seed,
                       ...
                       std::string outpath)
{
	...
	bool store_dataROOT =
	    false;  //if this is true it doesn't plot the histogram in screen
	bool plot_data = true;
	...
	for(int loop = 0; loop < nsample; loop++)
	{
		...
		for(int i = 0; i < NP_; i++)
		{
			...
			std::cout << "Sampled values for calib " << i << ": " << sampvalues[i]
			          << std::endl;
		}
		...
		for(int i = 0; i < NP_; i++)
		{
			...
			if(i == 0)
			{
				fgenerated[loop]->SetParameter(i, sampvalues[i] - 1.03);
			}
			else
			{
