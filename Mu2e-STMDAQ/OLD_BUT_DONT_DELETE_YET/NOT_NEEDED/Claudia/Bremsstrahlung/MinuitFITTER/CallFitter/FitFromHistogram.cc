...
void FitFromHistogram(int argc, char *argv[], std::string rootname, int FitOption, double xfit_low, double xfit_max, double meanE_fit, double seed, std::string outpath)
{
  ...
  std::string logpngname = "/CalibrationSignalFactor200345_Fitrange_"+stream_xfit_low.str()+"_"+stream_xfit_max.str()+"_Timesim_"+std::to_string(int(TimeSim))+"s_"+rate_str+"kHz_"+resolution_str+"MeV";
  ...
  */
  ...
  fitter->Histo_Data(fit_range, hEnewCalib); //Get nbins_fit, _binning, bincontent, bincontent_error, bincenter
...
int main(int argc, char *argv[]) {
  ...
  FitFromHistogram(argc, argv, rootname, FitOption, xfit_low, xfit_max, meanE_fit, seed, outpath);
