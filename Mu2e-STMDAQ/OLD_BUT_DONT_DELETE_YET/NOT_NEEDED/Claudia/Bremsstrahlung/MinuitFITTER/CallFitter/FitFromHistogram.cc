...
void FitFromHistogram(int         argc,
                      char*       argv[],
                      std::string rootname,
                      int         FitOption,
                      double      xfit_low,
                      ...
                      std::string outpath)
{
	...
	std::string logpngname = "/CalibrationSignalFactor200345_Fitrange_" +
	                         stream_xfit_low.str() + "_" + stream_xfit_max.str() +
	                         "_Timesim_" + std::to_string(int(TimeSim)) + "s_" +
	                         rate_str + "kHz_" + resolution_str + "MeV";
	//std::string logpngname = "/CalibrationBackgroundShape3_Fitrange_"+stream_xfit_low.str()+"_"+stream_xfit_max.str()+"_Timesim_"+std::to_string(int(TimeSim))+"s_"+rate_str+"kHz_"+resolution_str+"MeV":

	if(store_dataLOG == true)
	{
		std::string logfile = outpath + logpngname + ".log";
  ...
  */
	...
	std::cout << "Time simulation: " << TimeSim << " Rate  simulation: " << RateSim
	          << " Resolution simulation: " << ResolSim << std::endl;
	...
	TMatrixD* SeedFitPars = (TMatrixD*)infile->Get("SeedFitPars");
	TH1D*     hEnewCalib  = (TH1D*)infile->Get("hEnewCalib");

	//Covmatrix->Print();
	//BestFitPars->Print();
	//histo->Draw("");

	int    Nentries  = hEnewCalib->GetEntries();
	int    Nbins     = hEnewCalib->GetNbinsX();
	double bin_width = hEnewCalib->GetBinWidth(0);

	std::cout << "Entries: " << Nentries << " Bins: " << Nbins
	...
	fitter->Histo_Data(
	    fit_range,
	    hEnewCalib);  //Get nbins_fit, _binning, bincontent, bincontent_error, bincenter

	//Return nbins for the fit
	unsigned long int nbins_fit = fitter->return_nbinsfit();
	//Return dof
	unsigned long int dof = fitter->return_dof();
	std::cout << "Number of bins for the fit: " << nbins_fit
	          << " Degrees of freedom (ndof): " << dof << std::endl;
	...
	par_fit[1] = meanE_fit;  //new mean energy with new calibration, it's not 0.347
	...
	std::cout << "Sigma Signal: sqrt(cov matrix element)/bin width: "
	          << sqrt(_Covmatrix_newFit[0][0]) << "/" << bin_width << "=" << reco_sigma
	          << std::endl;
	...
	//double plot_x_low = 0.3;
	//double plot_x_max = 0.4;
	...
	histo->GetXaxis()->SetRangeUser(plot_x_low, plot_x_max);
	ymax = histo->GetMaximum() + 50;
	histo->GetXaxis()->SetRangeUser(xfit_low, xfit_max);

	/*
  hEnewCalib->GetXaxis()->SetRangeUser(plot_x_low, plot_x_max);
  ymax = hEnewCalib->GetMaximum()+50;
  hEnewCalib->GetXaxis()->SetRangeUser(xfit_low, xfit_max);
  */
	...
	std::string sim_legend_str = "#splitline{Nominal Signal Shape }{(rate= " + rate_str +
	                             " kHz, #sigma= " + resolution_str + " MeV)}";
	char* sim_legend = const_cast<char*>(sim_legend_str.c_str());

	auto legend = new TLegend(0.57, 0.6, 0.9, 0.9);
	legend->AddEntry("histo", sim_legend, "f");
	legend->AddEntry("hEnewCalib", "Non-Linear Signal Calibration", "f");
	legend->AddEntry("ffit", "Fit function", "l");
	legend->Draw("same");
  ...
  */
	...
	std::cout << "output path: " << outpath << std::endl;
	std::string nameplot_png = outpath + logpngname + ".png";
...
int main(int argc, char* argv[])
{
	...
	double      meanE_fit = std::stod(argv[5]);
	double      seed      = std::stod(argv[6]);
	std::string outpath   = argv[7];

	FitFromHistogram(
	    argc, argv, rootname, FitOption, xfit_low, xfit_max, meanE_fit, seed, outpath);
