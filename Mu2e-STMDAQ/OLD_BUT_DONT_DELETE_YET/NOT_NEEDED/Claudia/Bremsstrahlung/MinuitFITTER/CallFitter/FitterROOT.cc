...
void FitterROOT(int                  argc,
                char*                argv[],
                double               Bremsrate_kHz,
                double               TimeSim,
                double               meanE,
                double               sigma,
                int                  FitOption,
                ...
                int                  jobNum,
                std::string          outpath,
                std::vector<double>& TrueSignal,
                std::vector<double>& RecoSignal,
                std::string          txtBackgroundParameters)
{
	...
	bool store_dataROOT =
	    true;  // store signal+brems gen data and fit cov matrix in a ROOT file
	...
	bool store_all_peaksROOT =
	    false;  //this can only be true if store_dataROOT is true (signal and background together)
	bool store_separate_peaksROOT =
	    false;  //this can only be true if store_dataROOT is true (signal and background separate)
	...
	if(useintegral && usebin_chi2)
	{
		name_string = outpath + "/BinnedChi2_Integral_" + rate_string + "kHz_TimeSim_" +
		              TimeSim_string + "s_seed_" + seed_string + "_" +
		              stream_sigma.str() + "MeV";
	}
	if(!useintegral && usebin_chi2)
	{
		name_string = outpath + "/BinnedChi2_NOIntegral_" + rate_string + "kHz_TimeSim_" +
		              TimeSim_string + "s_seed_" + seed_string + "_" +
		              stream_sigma.str() + "MeV";
	}
	if(useintegral && usebin_loglike)
	{
		name_string = outpath + "/BinnedLoglike_Integral_" + rate_string +
		              "kHz_TimeSim_" + TimeSim_string + "s_seed_" + seed_string + "_" +
		              stream_sigma.str() + "MeV";
	}
	if(!useintegral && usebin_loglike)
	{
		name_string = outpath + "/BinnedLoglike_NOIntegral_" + rate_string +
		              "kHz_TimeSim_" + TimeSim_string + "s_seed_" + seed_string + "_" +
		              stream_sigma.str() + "MeV";
	}
	if(useunbin_loglike)
	{
		name_string = outpath + "/UnbinnedLogLike_" + rate_string + "kHz_TimeSim_" +
		              TimeSim_string + "s_seed_" + seed_string + "_" +
		              stream_sigma.str() + "MeV";
	}
	...
	std::string   BremsBackground_txt =
	    "/work/mu2e/data1/cgarcia/SignaltoBackground_Mu2eAvRunI/HPGe/"
	    "SystematicsBackgroundShapes/backgroundshapes/" +
	    txtBackgroundParameters;
	...
	if(background_num_str.compare(bckshapenom) == 0)
	{
		background_num = 0;
	}
	else
	{
		background_num = std::stod(background_num_str);
	}
	...
	while(std::getline(input_file, line))
	{
		...
		while(ss >> element)
		{
			if(element.compare("p0=") == 0)
				ss >> p0;
			if(element.compare("p1=") == 0)
				ss >> p1;
			if(element.compare("p2=") == 0)
				ss >> p2;
  ...
  */
	...
	std::cout << "Brems parameters before normalisation: p0= " << p0 << " p1= " << p1
	          << " p2= " << p2 << " p3= " << p3 << std::endl;
	...
	std::cout << "Brems parameters after normalisation: p0= " << p0 << " p1= " << p1
	          << " p2= " << p2 << " p3= " << p3 << std::endl;
	...
	if(store_dataROOT == true)
	{
		...
		rootfile = name_string + "_" + std::to_string(nexperiments) + "Runs_Job_" +
		           std::to_string(jobNum) + "_Background_" + background_type + ".root";
		...
		{
			output = new TFile(rootfile.c_str(), "update");
		}
	}
	...
	if((store_all_peaksROOT == true) && (store_separate_peaksROOT == true))
	{
		std::cout << "This option stores the data twice, set one of the 2 to false..."
		          << std::endl;
		exit(0);
	}
	...
	if(debugout == true)
	{
		std::cout << "Theoretical pulses in Bremsstrahlung background: " << pulseNumBrems
		          << std::endl;
		std::cout << "--Sample number of pulses with a Poisson distribution---"
		          << std::endl;
	}
	...
	if(debugout == true)
	{
		std::cout << "Number of pulses in Bremsstrahlung background: " << pulseNumBrems
		          << std::endl;
	}
	...
	{
		if((dataBrems[i] > photopeak_347_low) && (dataBrems[i] < photopeak_347_high))
		{
			counts_bremsphotopeak++;
		}
	}
	...
	for(unsigned long int i = 0; i < pulseNumBrems; i++)
	{
		...
		if((store_dataROOT == true) && (store_separate_peaksROOT == true))
		{
			PeaksBackground.push_back(dataBrems[i]);
		}
	}
  ...
  */
	...
	for(int i = 0; i < dimpeaks; i++)
	{
		if(debugout == true)
		{
			std::cout << "Theoretical pulses in Signal: " << pulseNumXrays[i]
			          << std::endl;
		}
		...
		if(debugout == true)
		{
			std::cout << "--Sample number of pulses with energy: " << mean_E[i]
			          << "MeV using a Poisson distribution---" << std::endl;
			std::cout << "Number of pulses in X-Ray peak: " << pulseNumXrays[i]
			          << std::endl;
		}
		data_size += pulseNumXrays[i];
	}

	std::cout << "Number of Flash  photons in range " << photopeak_347_low << ", "
	          << photopeak_347_high << " MeV: " << counts_bremsphotopeak
	          << " and X-rays: " << pulseNumXrays[0] << std::endl;

	double ratioXrayFlash = 1.0 * pulseNumXrays[0] / counts_bremsphotopeak;

	double ratioXrayFlash_error =
	    ratioXrayFlash * sqrt((sqrt(pulseNumXrays[0]) / pulseNumXrays[0]) *
	                              (sqrt(pulseNumXrays[0]) / pulseNumXrays[0]) +
	                          (sqrt(counts_bremsphotopeak) / counts_bremsphotopeak) *
	                              (sqrt(counts_bremsphotopeak) / counts_bremsphotopeak));

	std::cout << "Xrays/Flash(347keV): " << ratioXrayFlash << " +- "
	          << ratioXrayFlash_error << std::endl;

	//Initialise signal+brems dataset
	if(debugout == true)
	{
		std::cout << "Data size: " << data_size << " counts (brems+signal in range)"
		          << std::endl;
	}
	...
	for(unsigned long int j = 0; j < pulseNumXrays[0]; j++)
	{
		...
		if((store_dataROOT == true) && (store_separate_peaksROOT == true))
		{
			PeaksSignal.push_back(dataSignal[j]);
		}
    ...
    */
		...
		std::cout << "min FCN: " << min_fcn << " error definition: " << error_def
		          << std::endl;
		...
		std::cout << "Sigma Signal: sqrt(cov matrix element)/bin width: "
		          << sqrt(_Covmatrix[0][0]) << "/" << bin_width << "=" << reco_sigma
		          << std::endl;
		...
		std::cout << "Time for accuracy=0.1 = " << time_acc << " +- " << time_acc_error
		          << " s" << std::endl;
		...
		if(store_dataROOT == true)
		{
			std::string str_SeedFitPars =
			    "SeedFitPars" + std::to_string(count_experiment);
			...
			TMatrixD* SeedFitPars =
			    new TMatrixD(NP, 1, _SeedFitPars.GetMatrixArray(), "");
			...
			std::string str_BestFitPars =
			    "BestFitPars" + std::to_string(count_experiment);
			...
			TMatrixD* BestFitPars =
			    new TMatrixD(NP, 1, _BestFitPars.GetMatrixArray(), "");
	...
	if(useunbin_loglike)
	{
		...
		std::cout << "Peaks recovered from Unbinned LogLikelihood fit: " << reco_peaks
		          << std::endl;
		TMatrixD _Covmatrix(7, 7, Covmatrix->GetMatrixArray(), "");
	...
	if(check_ROOTfit == true)
	{
		if(usebin_loglike)
		{
			std::cout << "****************************************************"
			          << std::endl;
			std::cout << "******ROOT: FITTING WITH BINNED LOG-LIKELIHOOD******"
			          << std::endl;
			std::cout << "****************************************************"
			          << std::endl;
			if(useintegral == true)
			{
				histo->Fit(fbrems, "LIRN", "", fit_range[0], fit_range[1]);
			}
			else
			{
				histo->Fit(fbrems, "LRN", "", fit_range[0], fit_range[1]);
			}
			double chi2_rootfit = fbrems->GetChisquare();
			double dof_rootfit  = fbrems->GetNDF();
			double prob_rootfit = fbrems->GetProb();
			std::cout << "Root fit chi2: " << chi2_rootfit << ", dof: " << dof_rootfit
			          << ", chi2/dof: " << chi2_rootfit / dof_rootfit
			          << ", prob: " << prob_rootfit << std::endl;
		...
		if(usebin_chi2)
		{
			std::cout << "**********************************************************"
			          << std::endl;
			std::cout << "******ROOT: FITTING WITH BINNED LEAST SQUARES - CHI2******"
			          << std::endl;
			std::cout << "**********************************************************"
			          << std::endl;
			if(useintegral == true)
			{
				histo->Fit(fbrems, "IRN", "", fit_range[0], fit_range[1]);
			}
			else
			{
				histo->Fit(fbrems, "RN", "", fit_range[0], fit_range[1]);
			}
			double chi2_rootfit = fbrems->GetChisquare();
			double dof_rootfit  = fbrems->GetNDF();
			double prob_rootfit = fbrems->GetProb();
	...
	if(check_MINOS == true)
	{
		...
		if(usebin_loglike)
		{
			...
			for(int i = 0; i < npoints; i++)
			{
				...
				if((loglike_min < (min_fcn + error_def + 0.1)) &&
				   (loglike_min > (min_fcn + error_def - 0.1)))
				{
					std::cout << "Loglike 1sigma: " << loglike_min
					          << " p0 range: " << p_minos[0]
					          << " error: " << (p_minos[0] - p_minuit[0]) << std::endl;
				}

				grMinos->SetPoint(i, p_minos[0], loglike_min);
			}
			auto minimum =
			    *std::min_element(loglike_vector.begin(), loglike_vector.end());
		...
		if(usebin_chi2)
		{
			...
			for(int i = 0; i < npoints; i++)
			{
				...
				if((chi2_min < (min_fcn + error_def + 0.1)) &&
				   (chi2_min > (min_fcn + error_def - 0.1)))
				{
					std::cout << "Least-Squares 1sigma: " << chi2_min
					          << " p0 range: " << p_minos[0]
					          << " error: " << (p_minos[0] - p_minuit[0]) << std::endl;
				}
	...
	if(drawplot == true)
	{
		std::string nameplot_png = name_string + "_" + std::to_string(nexperiments) +
		                           "Runs_Job_" + std::to_string(jobNum) + "_Background_" +
		                           background_type + ".png";
...
int main(int argc, char* argv[])
{
	...
	//Run Config
	int nexperiments =
	    std::atoi(argv[7]);  //It will do more than 1 experiment with no pop up plot
	...
	{
		std::string logfile = outpath + "/seed_" + std::to_string(seed) + "_" +
		                      std::to_string(nexperiments) + "Runs_Job_" +
		                      std::to_string(jobNum) + "_Timesim_" +
		                      std::to_string(int(TimeSim)) + "_BremsRate_" + argv[1] +
		                      "kHz_resol_" + argv[4] + "MeV_" + background_type +
		                      "_FitOpt_" + std::to_string(FitOption) + ".log";
		char* char_logfile = const_cast<char*>(logfile.c_str());

		gSystem->RedirectOutput(char_logfile, "w");
	}

	std::cout << "INPUT CONFIG, Bremsrate(kHz): " << Bremsrate_kHz
	          << ", TimeSim(s): " << TimeSim << ", meanE(MeV): " << meanE
	          << ", sigma(MeV): " << sigma << ", seed: " << seed
	          << ", FitOption: " << FitOption << ", nexperiments: " << nexperiments
	          << std::endl;
	...
	for(int i = 0; i < nexperiments; i++)
	{
		...
		std::cout << "--------Call Function for experiment #: " << count_experiment
		          << std::endl;
