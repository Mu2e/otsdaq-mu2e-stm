...
void SampleBackground_Fit(int         argc,
                          char*       argv[],
                          std::string rootname,
                          int         FitOption,
                          double      xfit_low,
                          ...
                          std::string outpath)
{
	...
	std::cout << "Entries(S+B): " << Nentries << " Entries(S): " << Nentries_signal
	          << " Entries(B): " << Nentries_back << " Bins: " << Nbins
	...
	{
		std::string logfile = outpath + "/SampleBackground_Fit_Fitrange_" +
		                      stream_xfit_low.str() + "_" + stream_xfit_max.str() +
		                      "_Timesim_" + std::to_string(int(TimeSim)) + "s_" +
		                      rate_str + "kHz_" + resolution_str + "MeV.log";
	...
	{
		rootoutfile = outpath + "/SampledBackground_FitData_Timesim_" +
		              std::to_string(int(TimeSim)) + "s_" + rate_str + "kHz_" +
		              resolution_str + "MeV.root";
	...
	TH1D* hDeltaSignal =
	    new TH1D("#DeltaSignal", "", 200, xdeltasignal_min, xdeltasignal_max);
	...
	for(int loop = 0; loop < nsample; loop++)
	{
		...
		std::cout << "Sampled values for brems: " << values[0] << " " << values[1] << " "
		...
		fitter->Histo_Data(
		    fit_range,
		    hSTM[loop]);  //Get nbins_fit, _binning, bincontent, bincontent_error, bincenter
		...
		BestFitPars_newFit =
		    new TMatrixD(NP, 1, _BestFitPars_newFit.GetMatrixArray(), "");
		...
		std::cout << "Sigma Signal: sqrt(cov matrix element/bin width): "
		          << sqrt(_Covmatrix_newFit[0][0]) << "/" << bin_width << "="
		          << reco_sigma << std::endl;
		accuracy = reco_sigma / reco_peaks;
		std::cout << "Accuracy (sigma/signal): " << accuracy << std::endl;
		diff_reco_true_bestfit = reco_peaks - true_peaks;
		...
		if(store_dataROOT == true)
		{
			...
			std::string bestfitpar_name =
			    "BestFitPar_fromSampling" + std::to_string(loop);
	...
	std::cout << "RECO SIGNAL HISTOGRAM: mean: " << meanRecoSignal_fromexperiments
	          << " stdev: " << stdevRecoSignal_fromexperiments << std::endl;
	std::cout << "DIFF RECO-TRUE HISTOGRAM: mean: " << meanDiffSignal_fromexperiments
	          << " stdev: " << stdevDiffSignal_fromexperiments << std::endl;
	...
	if(plot_data == true)
	{
		...
		string Bremsrate_kHz_str =
		    "#splitline{347 keV peak (resolution=" + resolution_str_keV + " keV)+}{\n" +
		...
		legend1->AddEntry(fgenerated_brems_cov[0],
		                  "#splitline{\nEstimated Bremsstrahlung}{Background}",
		                  "l");
	...
	std::string nameplot_png = "SampleBackground_seed_" + std::to_string(int(seed)) +
	                           "_" + rate_str + "kHz_" + resolution_str +
	                           "MeV_SigmaRecoSignal.png";
	char*       char_nameplot_png = const_cast<char*>(nameplot_png.c_str());
	std::string nameplot_pdf      = "SampleBackground_seed_" + std::to_string(int(seed)) +
	                           "_" + rate_str + "kHz_" + resolution_str +
	                           "MeV_SigmaRecoSignal.pdf";
...
int main(int argc, char* argv[])
{
	...
	SampleBackground_Fit(
	    argc, argv, rootname, FitOption, xfit_low, xfit_max, seed, nsample, outpath);
