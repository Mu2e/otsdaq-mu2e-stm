...
void CheckFitMethods_GenData(
    int argc, char* argv[], std::string rootname, int FitOption, int nsample, double seed)
{
	...
	{
		rootfile =
		    "GenData_seed_" + std::to_string(int(seed)) + "_" + name_method + ".root";
	...
	hp[5] =
	    new TH1D("hp5", "", 100, limhisto[11], limhisto[10]);  //since negative parameters
	...
	for(int loop = 0; loop < nsample; loop++)
	{
		...
		std::cout << "Sampled values: " << values[0] << " " << values[1] << " "
  ...
  */
	...
	std::string nameplot_png =
	    "GenData_seed_" + std::to_string(int(seed)) + "_" + name_method + ".png";
	char*       char_nameplot_png = const_cast<char*>(nameplot_png.c_str());
	std::string nameplot_pdf =
	    "GenData_seed_" + std::to_string(int(seed)) + "_" + name_method + ".pdf";
