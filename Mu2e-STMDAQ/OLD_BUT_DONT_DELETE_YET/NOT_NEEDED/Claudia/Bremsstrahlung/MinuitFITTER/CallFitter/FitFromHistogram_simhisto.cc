...
void FitFromHistogram_simhisto(int         argc,
                               char*       argv[],
                               int         FitOption,
                               double      xfit_low,
                               double      xfit_max,
                               ...
                               double      backgroundpeaks)
{
	...
	readfile_reco.open(
	    ...
	    "FlashXrayOverlappedSim/SUM_Flash12HzXrays/" +
	        rate + "kHz/MWDM400L200_Noise/" + rate + "kHz_FlashReco.txt",
	    ios::in);
	...
	fitter->Histo_Data(
	    fit_range,
	    hreco);  //Get nbins_fit, _binning, bincontent, bincontent_error, bincenter
  ...
  */
	...
	std::string sim_legend_str_fit =
	    "#splitline{# fitted signal events = }{" + stream_reco_peaks.str() + " #pm " +
	    stream_reco_sigma.str() + " from 595 347-keV X-rays}";
...
int main(int argc, char* argv[])
{
	...
	FitFromHistogram_simhisto(argc,
	                          argv,
	                          FitOption,
	                          xfit_low,
	                          xfit_max,
