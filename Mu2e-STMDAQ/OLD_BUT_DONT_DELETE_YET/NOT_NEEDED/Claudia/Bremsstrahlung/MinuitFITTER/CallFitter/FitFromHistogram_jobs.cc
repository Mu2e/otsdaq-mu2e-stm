...
void FitFromHistogram_jobs(int         argc,
                           char*       argv[],
                           std::string rootpath,
                           int         FitOption,
                           double      xfit_low,
                           ...
                           std::string outpath)
{
	...
	if(store_ROOTfile == true)
	{
		if(FitOption == 1000)
		{
			outrootname = "Signal_BackgroundTrueReco_" + TimeSim_ + "s_" + RateSim +
			              "kHz_" + ResolSim + "MeV_BinnedChi2_20000bins.root";
		}
		if(FitOption == 2000)
		{
			outrootname = "Signal_BackgroundTrueReco_" + TimeSim_ + "s_" + RateSim +
			              "kHz_" + ResolSim + "MeV_BinnedLoglike_20000bins.root";
		}
	...
	for(int i = 0; i < Njobs; i++)
	{
		...
		std::string rootname = rootpath +
		                       "/BinnedLoglike_NOIntegral_1.00kHz_TimeSim_2000s_seed_0_0."
		                       "0010MeV_50Runs_Job_" +
		...
		for(int j = 0; j < nexperiments; j++)
		{
			...
			std::cout << "Number of peaks simulated: " << hSignal->GetEntries()
			          << std::endl;
			...
			fitter->Histo_Data(
			    fit_range,
			    hSTM);  //Get nbins_fit, _binning, bincontent, bincontent_error, bincenter
			...
			std::cout << "Time for accuracy=0.1 = " << time_acc << " +- "
			          << time_acc_error << " s" << std::endl;
	...
	if(FitOption == 2000)
	{
		nameplot_png = "Signal_BackgroundTrueReco_" + TimeSim_ + "s_" + RateSim + "kHz_" +
		               ResolSim + "MeV_BinnedLoglike_20000bins.png";
	}
	if(FitOption == 1000)
	{
		nameplot_png = "Signal_BackgroundTrueReco_" + TimeSim_ + "s_" + RateSim + "kHz_" +
		               ResolSim + "MeV_BinnedChi2_20000bins.png";
	}
...
int main(int argc, char* argv[])
{
	...
	FitFromHistogram_jobs(
	    argc, argv, rootpath, FitOption, xfit_low, xfit_max, seed, outpath);
