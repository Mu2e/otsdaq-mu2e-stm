...
void Plot_RatesTime( std::string input_path , std::string resolution ){
  ...
  std::string PlotLabel_log = "ResolutionsAndTimes_Rate"+resolution+"kHz.log";
  ...
  double timesimulation[Nfiles], timesimulation_error[Nfiles], sigmaS[Nfiles], SReco[Nfiles], accuracy[Nfiles], sigma_sigmaS[Nfiles], time_acc[Nfiles], time_acc_error[Nfiles], rate[Nfiles], rate_error[Nfiles], accuracy_error[Nfiles];
  ...
  for(int i = 0 ; i < Nfiles ; i++) {
    ...
    */
    ...
    pos1 = rootname.find("_0") + 1;
    pos2 = rootname.find("keV_");
    diff= pos2-pos1;
  ...
  TGraphErrors *grres = new TGraphErrors( Nfiles, rate, time_acc, rate_error, time_acc_error );

  grres->SetMarkerStyle(20);
  grres->GetYaxis()->SetTitle("Time (#sigma_{S}/S=0.1) [s]");
  grres->GetXaxis()->SetTitle("Rates [kHz]");
  std::string PlotLabel_title = "Resolution: "+resolution+" MeV";
  char* PlotLabel_char_title = const_cast<char*>(PlotLabel_title.c_str());
  grres->SetTitle(PlotLabel_char_title);
  grres->SetMarkerStyle(20);
  grres->SetMarkerColor(kOrange+3);
  grres->GetXaxis()->SetRangeUser(0, 1500);
  //grres->GetYaxis()->SetRangeUser(0, 5000);
  grres->GetYaxis()->SetRangeUser(0, 10000);
  ...
  //Fit the graph
  TF1* fres = new TF1("fres","[0]+[1]*x", 0, 1550);
  fres->SetParameters(10000,10000); 

  fres->SetNpx(300000);
  grres->Fit(fres, "0", "", 0, 1550);
  ...
  std::string PlotLabel_png = "RatesAndTimes_Resol"+resolution+"MeV_invaxis_zoom.png";
  std::string PlotLabel_pdf = "RatesAndTimes_Resol"+resolution+"MeV_invaxis_zoom.pdf";
