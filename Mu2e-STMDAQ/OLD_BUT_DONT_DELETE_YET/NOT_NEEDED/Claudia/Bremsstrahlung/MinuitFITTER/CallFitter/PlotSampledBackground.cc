...
void PlotSampledBackground(int argc, char* argv[], std::string rootname)
{
	...
	for(int j = 0; j < nexperiments; j++)
	{
		...
		std::cout << "BestFitPar[0]: " << best_fitpar[0] << " bin width: " << bin_width
		          << std::endl;
		std::cout << "Peaks recovered from Binned fit: " << reco_peaks << std::endl;
		reco_sigma = sqrt(_Covmatrix[0][0]) / bin_width;
		std::cout << "_Covmatrix[0][0]: " << _Covmatrix[0][0]
		          << " bin_width: " << bin_width << " reco_sigma: " << reco_sigma
		          << std::endl;
	...
	fit->SetParameters(
	    hDeltaSignal->GetMaximum(), hDeltaSignal->GetMean(), hDeltaSignal->GetRMS());
	...
	std::cout << "mean: " << stream_mean.str() << " +- " << stream_meanerror.str()
	          << std::endl;
	std::cout << "rms: " << stream_rms.str() << " +- " << stream_rmserror.str()
	          << std::endl;

	char*       char_latex1;
	TLatex      latex1;
	std::string str_latex1;
	str_latex1 = "#color[2]{#splitline{Mean = " + stream_mean.str() + " #pm " +
	             stream_meanerror.str() + "}{Std Dev = " + stream_rms.str() + " #pm " +
	             stream_rmserror.str() + "}}";
	...
	std::string nameplot_png =
	    "SampleBackground_seed_0_1kHz_0.0030MeV_SigmaRecoSignal_sigma.png";
