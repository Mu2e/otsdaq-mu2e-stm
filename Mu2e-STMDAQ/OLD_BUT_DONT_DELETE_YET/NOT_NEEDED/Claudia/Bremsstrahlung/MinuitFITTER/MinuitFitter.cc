...
double Bremsfunc(double* x, double* par) { return Bremsfunc_fit(x[0], par); };
...
double BremsfuncNORM_fit(double x, double* par)
{
	...
	double I_0 =
	    ((-1) * par[0] * TMath::Log(1 + par[2] * TMath::Exp((-1) * par[1] * frange[0]))) /
	        (par[1] * par[2]) +
	    (par[3] * frange[0]);
	double I_1 =
	    ((-1) * par[0] * TMath::Log(1 + par[2] * TMath::Exp((-1) * par[1] * frange[1]))) /
	        (par[1] * par[2]) +
	    (par[3] * frange[1]);
	double norm = I_1 - I_0;
...
double Signalfunc(double* x, double* par) { return Signalfunc_fit(x[0], par); };
...
double SBfunc(double* x, double* par) { return SBfunc_fit(x[0], par); };
...
double SBfuncNORM_fit(double x, double* par)
{
	...
	double f =
	    (fs * SignalfuncNORM_fit(x, parS)) + ((1. - fs) * BremsfuncNORM_fit(x, parB));
	delete parS;
	delete parB;
...
double SBfuncNORM_cte_fit(double x, double* par)
{
	...
	//Rescale with a cte
	double f = par[7] * ((fs * SignalfuncNORM_fit(x, parS)) +
	                     ((1. - fs) * BremsfuncNORM_fit(x, parB)));
	delete parS;
	delete parB;
	return f;
};

double SBfuncNORM_cte(double* x, double* par) { return SBfuncNORM_cte_fit(x[0], par); };
...
void fitter_fcn_wrapper(int& npar, double* gin, double& f, double* par, int iflag)
{
	...
	{
		Logger::Instance()->write(Logger::ERROR,
		                          "Unitialised Minuit function pointer - EXITING..");
		exit(-1);
	}
...
MinuitFitter::MinuitFitter(int _Option, unsigned long _seed)
{
	...
	{
		std::cout << "ERROR::: bad constructor FIT MODE" << std::endl;
		exit(0);
	}
	...
	{
		std::cout << "*Using BIN CENTER********************************" << std::endl;
	}
	...
	{
		std::cout << "*Using Default Minuit SETS***********************" << std::endl;
	}
...
void MinuitFitter::InitSB_Data(unsigned long int pulses) { dataSB = new double[pulses]; };
...
void MinuitFitter::Histo_Data(double* fit_range, TH1D* histo)
{
	...
	std::cout << "Fit range: [" << fit_range[0] << ", " << fit_range[1] << "]"
	          << std::endl;
	std::cout << "histo->GetNbinsX(): " << histo->GetNbinsX() << " " << (int)nbins
	          << std::endl;
	std::cout << "bin_min: " << bin_min << " bin_max: " << bin_max << " size: " << size
	          << std::endl;
	...
	if(histo->GetNbinsX() != (int)nbins)
	{
		std::cout << "ERROR::: bad Nbins in histogram" << std::endl;
		exit(0);
	}
	else
	{
...
void MinuitFitter::MinuitfcnLogLike(
	...
	for(unsigned long int i = 0; i < nbins_fit; i++)
	{
		if(bincontent[i] > 0)
		{
			...
			if(useintegral == true)
			{
				...
				log = (y_fit2 - bincontent[i]) +
				      bincontent[i] * (TMath::Log(bincontent[i]) - TMath::Log(y_fit2));
			}
			else
			{
				//log  = (y_fit1-bincontent[i])+bincontent[i]*(ROOT::Math::Util::EvalLog(bincontent[i])-ROOT::Math::Util::EvalLog(y_fit1));
				log = (y_fit1 - bincontent[i]) +
				      bincontent[i] * (TMath::Log(bincontent[i]) - TMath::Log(y_fit1));
			}
...
void MinuitFitter::MinuitfcnBinnedChisq(
    int& npar, double* gin, double& f, double* par, int iflag)
{
	double chisq = 0;
	double delta;
...
void MinuitFitter::MinuitfcnUnBinnedLogLike(
    int& npar, double* gin, double& f, double* par, int iflag)
{
	f = 0;
	for(unsigned long int i = 0; i < _data_size; i++)
	{
		...
		if(L > 0.)
			f -= TMath::Log(L);  //f = ROOT::Math::Util::EvalLog(L);
		else
		{
			f = HUGE_VAL;
			return;
...
void MinuitFitter::fit(double* par, int Ninput_par)
{
	...
	if(defaultminuit == false)
	{
		...
		val = 2.0;
		_minuit->mnexcm("SET STR", &val, 1, ierr);
		// Accuracy of floating point in function calculation (we use doubles..)
		val = 1.e-12;
		_minuit->mnexcm("SET EPS", &val, 1, ierr);
	}
	...
	{
		stringstream s2;
		s2 << "Fitter(): Minuit::MIGRAD failed : error code = " << ierr;
	...
	if(!useunbin_loglike)
	{
		...
		{
			stringstream s2;
			s2 << "Fitter(): Minuit::MINOS failed : error code = " << ierr;
			Logger::Instance()->write(Logger::WARNING, s2.str());
		}
	}
