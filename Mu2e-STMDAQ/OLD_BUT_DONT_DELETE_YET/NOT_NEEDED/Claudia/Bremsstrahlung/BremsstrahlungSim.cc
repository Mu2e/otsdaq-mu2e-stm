...
std::vector<double> BremsstrahlungSim(int               argc,
                                      char*             argv[],
                                      ...
                                      double            useweight)
{
	...
	if(debugout == true)
	{
		std::cout << "Bremsstrahlung function: value_0: " << value_0
		          << ", value_1: " << value_1 << ", value_2: " << value_2 << std::endl;
	}
	...
	else if(useweight == 1)
	{
		...
		if(debugout == true)
		{
			std::cout << "Reading input file with weights: " << input_path << std::endl;
		}
	...
	else
	{
		...
		if(debugout == true)
		{
			std::cout << "File generated with pulseNumBrems_aux (number of pulses used "
			             "to fill the histogram) and "
			          << nbins << " weights: " << output_path << std::endl;
		}
		writefile.close();
	}
	...
	if(ART_geom == false)
	{
		prot_per_maininjectorcycle =
		    spills_maininjector * microbunch_spill * POT_microbunch;
	...
	unsigned long int pulseNumXrays66 = 0, pulseNumXrays347 = 0, pulseNumXrays844 = 0,
	                  pulseNumXrays1809 = 0;
	...
	if(debugout == true)
	{
		...
		std::cout << "Histogram from: " << xmin_histo << " to: " << xmax_histo
		          << " MeV, nbins: " << nbins << ", binning: " << binning << " MeV"
		          << std::endl;

		std::cout << "Protons per main injector cycle: " << prot_per_maininjectorcycle
		          << std::endl;
		//std::cout<<""<<std::endl;
		std::cout << "STM acceptance: " << STM_accept << " prot per s: " << rate_prot_s
		          << " stopped muons per second: " << rate_stoppedmuons_s << std::endl;
		//std::cout<<""<<std::endl;
		std::cout << "rate_66keV_s: " << rate_66keV_s
		          << " rate_347keV_s: " << rate_347keV_s
		          << " rate_844keV_s: " << rate_844keV_s
		          << " rate_1809keV_s: " << rate_1809keV_s << std::endl;
		//std::cout<<""<<std::endl;
		std::cout << "rate_66keV_s_STM: " << rate_66keV_s_STM
		          << " rate_347keV_s_STM: " << rate_347keV_s_STM
		          << " rate_844keV_s_STM: " << rate_844keV_s_STM
		          << " rate_1809keV_s_STM: " << rate_1809keV_s_STM << std::endl;
	}

	if(mean == 0.066)
	{
		pulseNumXrays = rate_66keV_s_STM * TimeSim;
		if(debugout == true)
		{
			std::cout << "66keV XRay Rate in theory: " << pulseNumXrays << " pulses"
			          << std::endl;
		}
	}
	else if(mean == 0.347)
	{
		pulseNumXrays = rate_347keV_s_STM * TimeSim;
		if(debugout == true)
		{
			std::cout << "347keV XRay Rate in theory: " << pulseNumXrays << " pulses"
			          << std::endl;
		}
	}
	else if(mean == 0.844)
	{
		pulseNumXrays = rate_844keV_s_STM * TimeSim;
		if(debugout == true)
		{
			std::cout << "844keV XRay Rate in theory: " << pulseNumXrays << " pulses"
			          << std::endl;
		}
	}
	else if(mean == 0.1809)
	{
		pulseNumXrays = rate_1809keV_s_STM * TimeSim;
		if(debugout == true)
		{
			std::cout << "1809keV XRay Rate in theory: " << pulseNumXrays << " pulses"
			          << std::endl;
		}
	}
	else if(mean == 0)
	...
	{
		std::cout << "XRay energy not valid" << std::endl;
		exit(0);
	}
	...
	if(debugout == true)
	{
		std::cout << "Number of pulses in X-Ray peak: " << pulseNumXrays << std::endl;
	}
	...
	//Generated histos
	double numberCountsXray =
	    hXray->Integral(hXray->FindFixBin(xmin_histo), hXray->FindFixBin(xmax_histo), "");
	//Before adding Xrays to bremsstrahlung
	double numberCountsBrems = hBrems->Integral(
	    hBrems->FindFixBin(xmin_histo), hBrems->FindFixBin(xmax_histo), "");
	...
	for(int i = 0; i < nbins; i++)
	{
		source[i] = hcBrems->GetBinContent(i + 1);
	}
	s->Background(source,
	              nbins,
	              10,
	...
	//Output histos
	double numberCountsBackSignal =
	    hBrems->Integral(hBrems->FindFixBin(mean - sigmacut * sigma),
	                     hBrems->FindFixBin(mean + sigmacut * sigma),
	                     "");
	double numberCountsSignal =
	    hcBrems->Integral(hcBrems->FindFixBin(mean - sigmacut * sigma),
	                      hcBrems->FindFixBin(mean + sigmacut * sigma),
	                      "");
	double numberCountsBack     = d->Integral(d->FindFixBin(mean - sigmacut * sigma),
                                          d->FindFixBin(mean + sigmacut * sigma),
                                          "");
	...
	if(debugout == true)
	{
		std::cout << "Number of counts in Full generated Xray peak histogram: "
		          << numberCountsXray << std::endl;
		std::cout << "Number of counts in Full generated Bremsstrahlung histogram: "
		          << numberCountsBrems << std::endl;
		std::cout << "Using: mean: " << mean << " sigma: " << sigma << " mean-"
		          << sigmacut << "sigmas: " << mean - sigmacut * sigma << " mean+"
		          << sigmacut << "sigmas: " << mean + sigmacut * sigma << std::endl;
		std::cout << "Number of counts in Xray+brems +-" << sigmacut
		          << "sigmas (using fitted histogram): " << numberCountsBackSignal
		          << std::endl;
		std::cout << "Number of counts in Xray peak after background substraction +-"
		          << sigmacut << "sigmas: " << numberCountsSignal << std::endl;
		std::cout << "Number of counts in background: " << Brems << " = "
		          << numberCountsBack << std::endl;
	}
  ...
  */
	...
	legend->AddEntry(
	    hBrems, "#splitline{^{27}Al energy spectrum +}{\nBremsstrahlung}", "f");
...
int main(int argc, char* argv[])
{
	...
	for(unsigned long int i = 1; i <= num_experiments; i++)
	{
		...
		std::vector<double> SignalBackground = BremsstrahlungSim(argc,
		                                                         argv,
		                                                         Bremsrate_kHz,
		                                                         TimeSim,
		                                                         nbins,
		...
		if(i == num_experiments)
		{
			...
			{
				sumsquared_Xraysreco =
				    sumsquared_Xraysreco +
				    ((Signal.at(j) - mean_Xraysreco) * (Signal.at(j) - mean_Xraysreco));
				sumsquared_Xraystrue =
				    sumsquared_Xraystrue + ((TrueXrayCounts.at(j) - mean_Xraystrue) *
				                            (TrueXrayCounts.at(j) - mean_Xraystrue));
			...
			std::cout << "Real mean Xrays simulated in the peak+-rms: " << mean_Xraystrue
			          << std::endl;
			...
			std::cout << "Brems computing time = "
			          << boost::chrono::duration_cast<boost::chrono::milliseconds>(t2 -
			                                                                       t1)
			          << " ms" << std::endl;
