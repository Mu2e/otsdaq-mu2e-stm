...
int main()
{
	...
	for(unsigned long j = 0; j < npeaks; j++)
	{
		...
		for(unsigned long i = 0; i < n; i++)
		{
			...
			ADC = -(twiceA / (1 + exp(-(x[i] - xshift) * invtaufall))) *
			          (1.0 - (1.0 / (1 + exp(-(x[i] - xshift) * invtaudecay)))) +
			      Anoise * sin(x[i]);
	...
	unsigned long int Available_Bytes_packet =
	    npacketsbytes - sizepacketheader - sizetriggerheader - Nslices * sizesliceheader;
  ...
  for(int16_t i=0;i<100;i++){
	...
	int count = 0;
	std::cout << "Press enter to start sending data when all the clients are ready"
	          << std::endl;
	getchar();
	...
	while(1)
	{
		...
		std::cout << count << " Number of bytes requested from client: " << byteschunk
		          << " Number of bytes sent: " << nbytes << std::endl;
		...
		if(elementsread >= totalADCvalues)
		{
			break;
		}
		count++;
