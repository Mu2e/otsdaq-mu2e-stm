...
void HLS_ZS(uint16_t      allchunk[chunk],
            out_stream_t& output,
            uint64_t&     ntriggers,
            uint32_t&     zp_size)
{
...
#pragma HLS interface ap_fifo port     = output
	...
	//Read header
	uint64_t WORD0[4];
	WORD0[0] = (uint64_t)allchunk[infifo_hdr_lastZStrigstored1] << 0 &
	           (uint64_t)0x000000000000FFFF;
	WORD0[1] = (uint64_t)allchunk[infifo_hdr_lastZStrigstored2] << 16 &
	           (uint64_t)0x00000000FFFF0000;
	WORD0[2] = (uint64_t)allchunk[infifo_hdr_lastZStrigstored3] << 32 &
	           (uint64_t)0x0000FFFF00000000;
	WORD0[3] = (uint64_t)allchunk[infifo_hdr_lastZStrigstored4] << 48 &
	           (uint64_t)0xFFFF000000000000;
	last_triggerstored = (uint64_t)(WORD0[3] | WORD0[2] | WORD0[1] | WORD0[0]);
	std::cout << "Check Last ZS trigger from previous chunk: " << last_triggerstored
	          << std::endl;
	...
	while(k < ntriggers_chunk)
	{
		...
		uint64_t WORD1[4];
		WORD1[0] = (uint64_t)allchunk[infifo_thdr_TrigTime1 + dataindex] << 0 &
		           (uint64_t)0x000000000000FFFF;
		WORD1[1] = (uint64_t)allchunk[infifo_thdr_TrigTime2 + dataindex] << 16 &
		           (uint64_t)0x00000000FFFF0000;
		WORD1[2] = (uint64_t)allchunk[infifo_thdr_TrigTime3 + dataindex] << 32 &
		           (uint64_t)0x0000FFFF00000000;
		WORD1[3] = (uint64_t)allchunk[infifo_thdr_ChTime4 + dataindex] << 48 &
		           (uint64_t)0xFFFF000000000000;
		...
		if(k == 0)
		{
			chunkstart = trigstart;
		}

		uint64_t WORD2[4];
		WORD2[0] = (uint64_t)allchunk[infifo_thdr_TrigNum1 + dataindex] << 0 &
		           (uint64_t)0x000000000000FFFF;
		WORD2[1] = (uint64_t)allchunk[infifo_thdr_TrigNum2 + dataindex] << 16 &
		           (uint64_t)0x00000000FFFF0000;
		WORD2[2] = (uint64_t)allchunk[infifo_thdr_TrigNum3 + dataindex] << 32 &
		           (uint64_t)0x0000FFFF00000000;
		WORD2[3] = (uint64_t)allchunk[infifo_thdr_ADCoffset1 + dataindex] << 48 &
		           (uint64_t)0xFFFF000000000000;
		...
		uint64_t WORD3[4];
		WORD3[0] = (uint64_t)allchunk[infifo_thdr_evtwtag1 + dataindex] << 0 &
		           (uint64_t)0x000000000000FFFF;
		WORD3[1] = (uint64_t)allchunk[infifo_thdr_evtwtag2 + dataindex] << 16 &
		           (uint64_t)0x00000000FFFF0000;
		WORD3[2] = (uint64_t)allchunk[infifo_thdr_evtwtag3 + dataindex] << 32 &
		           (uint64_t)0x0000FFFF00000000;
		WORD3[3] = (uint64_t)allchunk[infifo_thdr_evtmode1 + dataindex] << 48 &
		           (uint64_t)0xFFFF000000000000;

		EvtwdTag = (uint64_t)(WORD3[2] | WORD3[1] | WORD3[0]);

		uint32_t WORD4[2];
		WORD4[0] = (uint32_t)allchunk[infifo_thdr_evtmode2 + dataindex] << 0 &
		           (uint32_t)0x0000FFFF;
		WORD4[1] = (uint32_t)allchunk[infifo_thdr_DRingMarkerevtmode3 + dataindex] << 16 &
		           (uint32_t)0xFFFF0000;
	...
	for(uint64_t j = 0; j < ntriggers; j++)
	{
		...
		while(l <= postnumADCstoredaux)
		{
			...
			{
				cout << "This value is not stored, index: " << int(realtrigger_abs[j]) + l
				     << endl;
			}
			if(checkprenum < 0)
			{
	...
	if(mode == 0)
	{
		...
		for(uint64_t j = 0; j < ntriggers_chunkSPILL; j++)
		{
			...
			while(i < triglength)
			{
				...
				{
					std::cout << "Header: trignum: " << trigger_num[j]
			...
			if(j < (ntriggers_chunkSPILL - 1))
			{
				triglength = triglength + trigger_len[j + 1];
			}
		}  //for ntriggers
	}      //IF MODE 0
