...
int main()
{
	...
	if(mode == 0)
	{
		std::cout << "Size of the chunk in ADC values (headers+ADC+ADC=0): " << chunk
		          << std::endl;
		std::cout << "Number of triggers per chunk to send: " << ntriggers_chunkSPILL
		          << std::endl;
		std::cout << "Number of ADC values per trigger: " << nADC_triggerSPILL
		          << std::endl;
	...
	if(mode == 1)
	{
		...
		if(ntriggers_chunkGAP < 1)
		{  //gap triggers go over different chunks
			ntriggers_chunk = 1;
		}
	}
	...
	if((infifo_hdr_Len + ntriggers_chunk * (infifo_thdr_Len + ADC_hardtrig)) > auxchunk)
	{
		std::cout << "Length of data written bigger than input FIFO size----EXIT----"
		          << std::endl;
		exit(0);
	}

	//Percentage where the overlapped chunk has to start at least
	double overlapped_chunk =
	    double((nADC_SPILLchunk - 1) - postnumADCstored + 1 - prenumADCstored) /
	    nADC_SPILLchunk;
	...
	std::cout << "Start of overlapping pulse with respect to the previous one: "
	          << overlapped_chunk << " Next chunk start in: " << next_chunk_start
	          << std::endl;
	...
	if(mode == 0)
	{
		...
		std::cout << "From " << ntriggers_chunkSPILL
		          ...
		          << next_trigger_start
		          << " which is trigger number: " << next_trigger_startTrigNum
		          << std::endl;
	}
	...
	if(mode == 1)
	{
		...
		std::cout << "The next chunk starts in ADC value: " << next_chunk_startGAP
		          << std::endl;
		...
		while(endchunk < nADC_triggerGAP)
		{
			...
			std::cout << "Start of chunk number: " << nchunkspertriggerGAP << ": "
			          << startchunk << ", end chunk: " << endchunk << std::endl;
	...
	//Through warning
	if(prenumADCstored >
	   ((nADC_SPILLchunk * (1 - overlapped_chunk)) - (postnumADCstored - 1)))
	{
	...
	for(uint64_t i = 0; i < nchunks; i++)
	{
		...
		if(mode == 1)
		{
			...
			{
				numADCtoZS = numelements_lasttrigGAP;
			}
			else
			{
				numADCtoZS = ADC_hardtrig;
			}
		}
	...
	std:
		cout << "Chunk goes from ADC index: " << chunkstart << " to "
		     << chunkstart + (ntriggers_chunk * ADC_hardtrig) - 1 << std::endl;
		//outFILE<<"Chunk goes from: "<<setprecision(9)<<chunkstart<<" to "<<setprecision(9)<<chunkstart+(nADC_chunk-1)<<endl;

		uint64_t aux = chunkstart + infifo_hdr_Len +
		               ntriggers_chunk * (infifo_thdr_Len + numADCtoZS);
		if(aux >= n)
		{
		...
		for(uint32_t k = 0; k < ntriggers_chunk; k++)
		{
			if(mode == 0)
			{
				TrigNum  = trigstart / ADC_hardtrig;
				EvtwdTag = evttag_counter;
			}
			...
			std::cout << "BEEF: " << std::hex << allchunk[infifo_thdr_1] << std::dec
			          << std::endl;
		...
		std::cout << "chunk: " << auxchunk << " datawritten: " << datawritten
		          << " rest0ADC: " << rest0ADC << std::endl;
		if(rest0ADC != 0)
		{
		...
		uint64_t lasttrigstored3 =
		    (uint64_t)alldata[0] << 0 & (uint64_t)0x000000000000FFFF;
		uint64_t lasttrigstored2 =
		    (uint64_t)alldata[1] << 16 & (uint64_t)0x00000000FFFF0000;
		uint64_t lasttrigstored1 =
		    (uint64_t)alldata[2] << 32 & (uint64_t)0x0000FFFF00000000;
		uint64_t lasttrigstored0 =
		    (uint64_t)alldata[3] << 48 & (uint64_t)0xFFFF000000000000;
		last_triggerstored = (uint64_t)(lasttrigstored0 | lasttrigstored1 |
