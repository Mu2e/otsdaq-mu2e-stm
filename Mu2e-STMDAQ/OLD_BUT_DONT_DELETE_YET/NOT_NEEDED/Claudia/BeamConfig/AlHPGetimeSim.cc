...
void AlHPGetimeSim(int                        argc,
                   char*                      argv[],
                   std::default_random_engine gen,
                   double                     spillRatekHz /*kHz*/,
                   int                        n_maininjector_cycles)
{
	...
	int print =
	    3;  //print=0 print HPGe arrival time histogram, print=1 print Gap rate as a function of gap number, print=2 histogram gap rate, print=3 print Al energy spectrum
	bool savepeaks_toROOTfile = false;  //run the simulation and store variables in a tree
	bool readpeaks_fromROOTfile =
	    true;  //don't run the simulation, read variables from tree
	...
	std::cout << "Number of main injector cycles to simulate: " << n_maininjector_cycles
	          << std::endl;

	//Number of peaks in first beam mode at this rate
	long int pulseNum = spillsize * spillRateHz;
	std::cout << "Number of peaks per micropulse (" << spillsize << " s): " << pulseNum
	          << std::endl;
	std::cout << "Beam on and off (8 micropulses, 7 gaps) size in Spill Mode: "
	          << beam_on_period << " s" << std::endl;
	std::cout << "Number of peaks in first Beam-on mode (" << beam_on_size
	          << " s): " << pulseNum * nspills << std::endl;
	...
	std::cout << "Pulses arriving at Al can: DIO with a prob of " << DIO_muons
	          << "%, be captured " << captured_muons << "%, 3d->2p transition "
	          << dp_muons << "% or 2p->1s " << ps_muons << "%" << std::endl;
	//start 3d->2p
	...
	std::cout << " If muons are stopped in 3d orbital all of them decay to 2p and then "
	             "to 1s, once in 1s, they can: DIO with a prob of "
	          << DIO_3d << "% or be captured " << capture_3d << "%" << std::endl;
	//start 2p->1s
	...
	std::cout << " If muons are stopped in 2p, they decay to 1s,once in 1s, they can: "
	             "DIO with a prob of "
	          << DIO_2p << "% or be captured " << capture_2p << "%" << std::endl;
	//Xray Probs
	distribution[3] = {excitedAl_muons, Mgbetadecay_muons, noxrays};
	std::cout << "Once muons are captured, they can: Emit 1809 keV X-Rays (excited Al): "
	          << excitedAl_muons
	          << "%, emit 844 keV X-Rays (beta decay): " << Mgbetadecay_muons
	          << "% or nothing: " << noxrays << "%" << std::endl;
	...
	std::exponential_distribution<double> Xray844keV_time(
	    log(2) / betadecay_time_us);  // MHz (rate)
	std::exponential_distribution<double> Xray1809keV_time(
	    log(2) / groundstateAl_lifetime_us);  //MHz (rate)
	...
	if(readpeaks_fromROOTfile == false)
	{
		for(int k = 0; k < n_maininjector_cycles; k++)
		{
			...
			std::cout << "--------------------------Main injector Cyle num " << k
			          << " starts in " << arrivalAl_time_init
			          << " us--------------------------" << std::endl;
			...
			for(int j = 0; j < nspills; j++)
			{
				...
				std::cout << "Start of micropulse " << j << ": " << arrivalAl_time
				          << " us" << std::endl;
				...
				for(int i = 0; i < pulseNum; i++)
				{
					...
					std::cout << "Stopped process: " << start_mustop_process
					          << ", arriving to Al target: " << arrivalAl_time << " us"
					          << std::endl;
					...
					if(start_mustop_process == 1)
					{
						...
						if(xray == 0)
						{
							...
							arrivalHPGe_time = arrivalAl_time + muAl_lifetime_us +
							                   xray_exptime; /*excited Al*/
							std::cout
							    << "1809keV X-Rays: theoretical time excited Al decay "
							    << groundstateAl_lifetime_us << " us" << std::endl;
							std::cout
							    << "Lifetime in Al: " << muAl_lifetime_us
							    << " us plus excited Al decay exponential sampling time: "
							    << xray_exptime << " us" << std::endl;
							std::cout << "Arrival HPGe time: " << arrivalHPGe_time
							          << " us" << std::endl;
						...
						else if(xray == 1)
						{
							...
							arrivalHPGe_time = arrivalAl_time + muAl_lifetime_us +
							                   xray_exptime; /*beta decay*/
							std::cout << "844keV X-Rays: theoretical time beta decay "
							          << betadecay_time_us << " us" << std::endl;
							std::cout << "Lifetime in Al: " << muAl_lifetime_us
							          << " us plus beta decay exponential sampling time: "
							          << xray_exptime << " us" << std::endl;
							std::cout << "Arrival HPGe time: " << arrivalHPGe_time
							          << " us" << std::endl;
							HPGe_timesv.push_back(arrivalHPGe_time);
						...
						{
							std::cout << "No X-Rays produced" << std::endl;
							continue;
						}
					}
					...
					if(start_mustop_process == 2)
					{
						...
						std::cout << "347keV X-Rays: " << prompt_time_us << " us"
						          << std::endl;
						std::cout << "Arrival HPGe time: " << arrivalHPGe_time << " us"
						          << std::endl;
						HPGe_timesv.push_back(arrivalHPGe_time);
						...
						if(start_mustop_process == 1)
						{
							std::cout << "Muon captured" << std::endl; /*Muon captured*/
							xray = distribution[3](gen);
							if(xray == 0)
							{
								xray_exptime     = Xray1809keV_time(gen);
								arrivalHPGe_time = arrivalHPGe_time + muAl_lifetime_us +
								                   xray_exptime; /*excited Al*/
								std::cout << "1809keV X-Rays: theoretical time excited "
								             "Al decay "
								          << groundstateAl_lifetime_us << " us"
								          << std::endl;
								std::cout << "Lifetime in Al: " << muAl_lifetime_us
								          << " us plus excited Al decay exponential "
								             "sampling time: "
								          << xray_exptime << " us" << std::endl;
								std::cout << "Arrival HPGe time: " << arrivalHPGe_time
								          << " us" << std::endl;
					...
					if(start_mustop_process == 3)
					{
						...
						arrivalHPGe_time = arrivalAl_time + prompt_time_us;
						std::cout << "66keV X-Rays: " << prompt_time_us << " us"
						          << std::endl;
						std::cout << "Arrival HPGe time: " << arrivalHPGe_time << " us"
						          << std::endl;
						HPGe_timesv.push_back(arrivalHPGe_time);
		...
		for(unsigned long int k = 0; k < max_gap; k++)
		{
			...
			{
				if((HPGe_timesv.at(i) > startgap) && (HPGe_timesv.at(i) < endgap))
				{
					count_integral.at(k) = count_integral.at(k) + 1;
				}
			}
			std::cout << "Gap (main injector cycle) number: " << k
			          << " starting at: " << startgap << " us and finishing: " << endgap
			          << std::endl;
	...
	if(readpeaks_fromROOTfile == true)
	{
		...
		for(unsigned long int i = 0; i < size2; i++)
		{
			...
			std::cout << "Gap rate: " << gap_ratev.at(i) << " Gap num: " << gap_numv.at(i)
			          << std::endl;
		}
	...
	std::cout << "HPGe times size: " << HPGe_timesv.size()
	          << ", Gap rate size: " << gap_ratev.size()
	          << ", Number of gaps: " << gap_numv.size()
	          << ", Number of Al XRays: " << XRay_energiesv.size() << std::endl;
	...
	if(print == 3)
	{
		...
		HPGeenergies->GetYaxis()->SetTitle(
		    "Al-Xrays / (T_{SPILL} #upoint #MI cycles #upoint R_{SPILL})");
	...
	if(print == 4)
	{
		...
		for(int i = 0; i < n_maininjector_cycles; i++)
		{
			...
			gaprate = N0 * (exp(exp1) - exp(exp2)) * ((exp(exp3) - 1) / (exp(exp4) - 1)) /
			          gapsize;
