...
void supalg(in_stream_t&   input,
            out_stream_t&  output,
            ulongint_data& ntriggers,
            ulongint_data& last_triggerstored,
            ulongint_data  chunkstart,
...
#pragma HLS INTERFACE axis port = output
	...
	int16_data ADC
	    [nADC_chunk];  //It has to know the size at compile time (#define) for the synthesis process
	...
	while(j < (nADC_chunk - window))
	{
		...
		if((j + n_average) > (nADC_chunk - window))
		{
			n_average = (nADC_chunk - window) - j;
		}
		av_gradient = 0;
		av_ADCtime  = 0;
	...
	for(unsigned long int i = 0; i < triggercounter; i++)
	{
		...
		if((trigger_vect[i] <= last_triggerstored) ||
		   ...
		   (trigger_vect[i] + postnumADCstored >= chunkstart + nADC_chunk))
		{
			continue;
		}
		else
	...
	for(int j = 0; j < realtrignum; j++)
	{
		...
		while(k <= postnumADCstored)
		{
			...
			{
				cout << "This value is not stored, index: " << realtrigger_abs[j] + k
				     << ", value: " << suppressed_data[realtrigger_abs[j] + k] << endl;
			}
			k++;
		}
		//print=true;
		...
		while(i < triglength)
		{
			...
			if(increasecounter == true)
			{
				cout << "Header: trignum: " << sendsupdata[b]
				     << " Nslices: " << sendsupdata[b + 1]
				     << " Start data: " << sendsupdata[b + 2]
				     << " Number of sup values: " << sendsupdata[b + 3] << endl;
				b             = b + 4 + valueswritten;
