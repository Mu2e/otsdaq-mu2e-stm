#include "supalg.h"
void supalg(int16_t            allchunk[chunk],
            out_stream_t&      output,
            unsigned long int& ntriggers,
            int&               zp_size,
...
#pragma HLS interface ap_fifo port     = output
    ...
    unsigned long int chunknum4 =
        (unsigned long int)allchunk[1] << 0 & (unsigned long int)0x000000000000FFFF;
    unsigned long int chunknum3 =
        (unsigned long int)allchunk[2] << 16 & (unsigned long int)0x00000000FFFF0000;
    unsigned long int chunknum2 =
        (unsigned long int)allchunk[3] << 32 & (unsigned long int)0x0000FFFF00000000;
    unsigned long int chunknum1 =
        (unsigned long int)allchunk[4] << 48 & (unsigned long int)0xFFFF000000000000;
    chunknum = (unsigned long int)(chunknum1 | chunknum2 | chunknum3 | chunknum4);
    cout << "Check Chunk Number: " << chunknum << endl;
    unsigned long int chunkstart8 =
        (unsigned long int)allchunk[5] << 0 & (unsigned long int)0x000000000000FFFF;
    unsigned long int chunkstart7 =
        (unsigned long int)allchunk[6] << 16 & (unsigned long int)0x00000000FFFF0000;
    unsigned long int chunkstart6 =
        (unsigned long int)allchunk[7] << 32 & (unsigned long int)0x0000FFFF00000000;
    unsigned long int chunkstart5 =
        (unsigned long int)allchunk[8] << 48 & (unsigned long int)0xFFFF000000000000;
    chunkstart =
        (unsigned long int)(chunkstart5 | chunkstart6 | chunkstart7 | chunkstart8);
    cout << "Check Chunk starts in: " << chunkstart << " ADC values" << endl;
    unsigned long int trignumstart12 =
        (unsigned long int)allchunk[9] << 0 & (unsigned long int)0x000000000000FFFF;
    unsigned long int trignumstart11 =
        (unsigned long int)allchunk[10] << 16 & (unsigned long int)0x00000000FFFF0000;
    unsigned long int trignumstart10 =
        (unsigned long int)allchunk[11] << 32 & (unsigned long int)0x0000FFFF00000000;
    unsigned long int trignumstart9 =
        (unsigned long int)allchunk[12] << 48 & (unsigned long int)0xFFFF000000000000;
    trignumstart = (unsigned long int)(trignumstart9 | trignumstart10 | trignumstart11 |
                                       trignumstart12);
    cout << "Check Trig Num start: " << trignumstart << endl;
    unsigned long int lasttrigstored16 =
        (unsigned long int)allchunk[13] << 0 & (unsigned long int)0x000000000000FFFF;
    unsigned long int lasttrigstored15 =
        (unsigned long int)allchunk[14] << 16 & (unsigned long int)0x00000000FFFF0000;
    unsigned long int lasttrigstored14 =
        (unsigned long int)allchunk[15] << 32 & (unsigned long int)0x0000FFFF00000000;
    unsigned long int lasttrigstored13 =
        (unsigned long int)allchunk[16] << 48 & (unsigned long int)0xFFFF000000000000;
    last_triggerstored = (unsigned long int)(lasttrigstored13 | lasttrigstored14 |
                                             lasttrigstored15 | lasttrigstored16);
    ...
    cout << "Check number of ADC values to ZS in this chunk: " << numADCtoZS << endl;
    int16_t ADC[nADC_chunk];
    //gradient vector
    ...
    unsigned long int lasttrig    = 0;
    std::cout << "Average of gradient taken with: " << n_average << " values"
              << std::endl;
    ...
    std::cout << "fadc= " << fADC << " tadc= " << tadc << " MHz, window: " << window
              << " threshold " << threshold << " tbefore: " << tbefore << " us ("
              << prenumADCstored << " ADC Counts)"
              << " tafter " << tafter << " us (" << postnumADCstored << " ADC Counts)"
              << std::endl;
    ...
    unsigned long int
        realtrigger_vect[nADC_chunk];  //no more triggers than adc values per chunk
    unsigned long int realtrigger_abs[nADC_chunk];
    ...
    for(unsigned long int i = 0; i < triggercounter; i++)
    {
        ...
        if((trigger_vect[i] <= last_triggerstored) ||
           (trigger_vect[i] - prenumADCstored <= last_triggerstored) ||
           (trigger_vect[i] + postnumADCstored >= chunkstart + numADCtoZS))
        {
            continue;
        }
        else
    ...
    int16_t suppressed_data[nADC_chunk];
    ...
    for(unsigned long int j = 0; j < ntriggers; j++)
    {
        ...
        while(k <= postnumADCstored)
        {
            ...
            {
                cout << "This value is not stored, index: " << int(realtrigger_abs[j]) + k
                     << endl;
            }
            if(checkprenum < 0)
            {
                std::cout << "Not prenum ADC values to store in this chunk for this ZS "
				                 "trigger, not all values required are stored for this "
				                 "trigger---wrong energy expected"
                          << std::endl;
            }
            k++;
        }
    ...
    int16_t sendsupdata[nADC_chunk];
    ...
    if(mode == 0)
    {
        ...
        for(unsigned long int j = 0; j < ntriggers_chunkSPILL; j++)
        {
            ...
            while(i < triglength)
            {
                ...
                {
                    b = b - 7;
                    cout << "Removed previous header" << std::endl;
                }
    ...
    if(mode == 1)
    {
        int16_t numstart_data     = 0;
        int16_t numstart_dataplus = 0;
        int16_t valueswritten =
            0;  //This could give an error if we write all the trigger (37000 ADC values) because int16 range is 32000 values
        int16_t slices          = 0;
        bool    increasecounter = false;
        bool    datadelay       = false;
        //Last ZS trigger found in this chunk is the 4 first elements of the output array
        ...
        while(i < numADCtoZS)
        {
            ...
            if(increasecounter == true)
            {
                cout << "Header: trignum: " << trignumstart
                     << " Nslices: " << sendsupdata[b + 4]
                     << " Start data: " << sendsupdata[b + 5]
                     << " Number of sup values: " << sendsupdata[b + 6] << endl;
                b = b + 7 + valueswritten;
            }
            while((suppressed_data[i] == init) && (i < numADCtoZS))
            {
