...
int main()
{
	...
	//std::cout<<"size of ulongint_data "<<sizeof(ulongint_data)<<std::endl;
	//std::cout<<std::numeric_limits<ulongint_data>::max()<<std::endl;
	...
	//ulongint_data nADC_chunk, restchunk;
	...
	double_data overlapped_chunk =
	    double_data((nADC_chunk - 1) - postnumADCstored + 1 - prenumADCstored) /
	    nADC_chunk;
	...
	cout << "Start of overlapping pulse with respect to the previous one: "
	     << overlapped_chunk << " Next chunk start in: " << next_chunk_start << endl;
	...
	ulongint_data next_trigger_startTrigNum = 0;
	ulongint_data next_chunk_startGAP       = 0;
	...
	if(mode == 0)
	{
		for(int i = 0; i < ntriggers_chunkSPILL; i++)
		{
			...
			if(next_trigger_aux < next_chunk_start)
			{
				next_trigger_start = next_trigger_aux;
			}
		}
		next_trigger_startTrigNum =
		    next_trigger_start / ADC_hardtrig;  //divide by trigger size
		cout << "From " << ntriggers_chunkSPILL
		     << " triggers in the chunk, the next chunk starts in ADC value:  "
		     << next_trigger_start
		     << " which is trigger number: " << next_trigger_startTrigNum << endl;
	}

	ulongint_data nchunkspertriggerGAP = 1000;  //intitialise to a value !=0
	ulongint_data endchunk             = 0;
	ulongint_data startchunk           = 0;
	ulongint_data numelements_lasttrigGAP;
	if(mode == 1)
	{
		...
		while(endchunk < nADC_triggerGAP)
		{
			...
			cout << "Start of chunk number: " << nchunkspertriggerGAP << ": "
			     << startchunk << ", end chunk: " << endchunk << endl;
			numelements_lasttrigGAP = nADC_triggerGAP - startchunk;
			startchunk              = startchunk + next_chunk_startGAP;
		}
		std::cout << "One trigger (" << nADC_triggerGAP << " ADC values) extends over "
		          << nchunkspertriggerGAP << " chunks" << std::endl;
		std::cout << "Number of elements to write in the last chunk of the trigger "
		          << numelements_lasttrigGAP << std::endl;
	}

	//Through warning
	if(prenumADCstored > ((nADC_chunk * (1 - overlapped_chunk)) - (postnumADCstored - 1)))
	{
		std::cout << "WARNING::::::: In the limit case where trigger is in "
		             "(chunk-post-1), this trigger would be stored in next chunk but pre "
		             "adc values chosen exceeds the start of the next chunk, PRE values "
		             "stored must be lower::::::: ERROR"
		          << std::endl;
		exit(1);
	}

	ulongint_data trignumstart       = 0;
	...
	for(ulongint_data i = 0; i < nchunks; i++)
	{
		...
		if(mode == 1)
		{
			...
			{
				cout << "INCREASE TRIGGER COUNTER FOR GAP" << std::endl;
				trignumstart++;
				chunkstart = trignumstart * nADC_triggerGAP;
				auxtrig    = 0;
			}
			if(auxtrig == (nchunkspertriggerGAP - 1))
			{
				numADCtoZP = numelements_lasttrigGAP;
				//if(numelements_lasttrigGAP>32767){std::cout<<""<<std::endl; std::cout<<"ERROR: The number of elements to zero suppress is: "<<numelements_lasttrigGAP<<" bigger than the int16_t range (variable in the input header, change numADCtoZP to ulongint_data)..."<<std::endl;}
			}
			else
			{
				numADCtoZP = nADC_chunk;
			}
		}
		std::cout << "" << endl;
		cout << "CHUNK NUMBER: " << i << endl;
		cout << "Chunk goes from: " << chunkstart << " to "
		     << chunkstart + (nADC_chunk - 1) << endl;
		...
		std::cout << "Last ZS trigger from previous chunk: " << last_triggerstored
		          << std::endl;
		//Number of ADC values to ZP in this chunk
		allchunk[17] = numADCtoZP & 0x0000FFFF;
		allchunk[18] = numADCtoZP >> 16;
		std::cout << "Number of ADC values to ZS in this chunk: " << numADCtoZP
		          << std::endl;
		...
		ulongint_data lasttrigstored3 =
		    (ulongint_data)alldata[0] << 0 & (ulongint_data)0x000000000000FFFF;
		ulongint_data lasttrigstored2 =
		    (ulongint_data)alldata[1] << 16 & (ulongint_data)0x00000000FFFF0000;
		ulongint_data lasttrigstored1 =
		    (ulongint_data)alldata[2] << 32 & (ulongint_data)0x0000FFFF00000000;
		ulongint_data lasttrigstored0 =
		    (ulongint_data)alldata[3] << 48 & (ulongint_data)0xFFFF000000000000;
		last_triggerstored = (ulongint_data)(lasttrigstored0 | lasttrigstored1 |
		                                     lasttrigstored2 | lasttrigstored3);
