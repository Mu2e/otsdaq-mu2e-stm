...
void supalg(int16_data     allchunk[chunk],
            out_stream_t&  output,
            ulongint_data& ntriggers,
            int&           zp_size,
            ulongint_data  ADC_hardtrig)
{
...
#pragma HLS interface ap_fifo port = output
  ...
  */
	...
	ulongint_data chunknum4 =
	    (ulongint_data)allchunk[1] << 0 & (ulongint_data)0x000000000000FFFF;
	ulongint_data chunknum3 =
	    (ulongint_data)allchunk[2] << 16 & (ulongint_data)0x00000000FFFF0000;
	ulongint_data chunknum2 =
	    (ulongint_data)allchunk[3] << 32 & (ulongint_data)0x0000FFFF00000000;
	ulongint_data chunknum1 =
	    (ulongint_data)allchunk[4] << 48 & (ulongint_data)0xFFFF000000000000;
	chunknum = (ulongint_data)(chunknum1 | chunknum2 | chunknum3 | chunknum4);
	cout << "Check Chunk Number: " << chunknum << endl;
	ulongint_data chunkstart8 =
	    (ulongint_data)allchunk[5] << 0 & (ulongint_data)0x000000000000FFFF;
	ulongint_data chunkstart7 =
	    (ulongint_data)allchunk[6] << 16 & (ulongint_data)0x00000000FFFF0000;
	ulongint_data chunkstart6 =
	    (ulongint_data)allchunk[7] << 32 & (ulongint_data)0x0000FFFF00000000;
	ulongint_data chunkstart5 =
	    (ulongint_data)allchunk[8] << 48 & (ulongint_data)0xFFFF000000000000;
	...
	ulongint_data trignumstart12 =
	    (ulongint_data)allchunk[9] << 0 & (ulongint_data)0x000000000000FFFF;
	ulongint_data trignumstart11 =
	    (ulongint_data)allchunk[10] << 16 & (ulongint_data)0x00000000FFFF0000;
	ulongint_data trignumstart10 =
	    (ulongint_data)allchunk[11] << 32 & (ulongint_data)0x0000FFFF00000000;
	ulongint_data trignumstart9 =
	    (ulongint_data)allchunk[12] << 48 & (ulongint_data)0xFFFF000000000000;
	trignumstart =
	    (ulongint_data)(trignumstart9 | trignumstart10 | trignumstart11 | trignumstart12);
	cout << "Check Trig Num start: " << trignumstart << endl;
	ulongint_data lasttrigstored16 =
	    (ulongint_data)allchunk[13] << 0 & (ulongint_data)0x000000000000FFFF;
	ulongint_data lasttrigstored15 =
	    (ulongint_data)allchunk[14] << 16 & (ulongint_data)0x00000000FFFF0000;
	ulongint_data lasttrigstored14 =
	    (ulongint_data)allchunk[15] << 32 & (ulongint_data)0x0000FFFF00000000;
	ulongint_data lasttrigstored13 =
	    (ulongint_data)allchunk[16] << 48 & (ulongint_data)0xFFFF000000000000;
	last_triggerstored = (ulongint_data)(lasttrigstored13 | lasttrigstored14 |
	...
	int16_data ADC
	    [nADC_chunk];  //It has to know the size at compile time for the synthesis process
	...
	while(j < (numADCtoZP - window))
	{
		...
		if((j + n_average) > (numADCtoZP - window))
		{
			n_average = (numADCtoZP - window) - j;
		}
		av_gradient = 0;
		av_ADCtime  = 0;
	...
	for(unsigned long int i = 0; i < triggercounter; i++)
	{
		...
		if((trigger_vect[i] <= last_triggerstored) ||
		   ...
		   (trigger_vect[i] + postnumADCstored >= chunkstart + numADCtoZP))
		{
			continue;
		}
		else
	...
	for(int j = 0; j < ntriggers; j++)
	{
		...
		while(k <= postnumADCstored)
		{
			...
			if(checkprenum < 0)
			{
				std::cout << "Not prenum ADC values to store in this chunk for this ZP "
	...
	if(mode == 0)
	{
		...
		for(ulongint_data j = 0; j < ntriggers_chunkSPILL; j++)
		{
			...
			while(i < triglength)
			{
				...
				{
					cout << "Header: trignum: " << sendsupdata[b]
