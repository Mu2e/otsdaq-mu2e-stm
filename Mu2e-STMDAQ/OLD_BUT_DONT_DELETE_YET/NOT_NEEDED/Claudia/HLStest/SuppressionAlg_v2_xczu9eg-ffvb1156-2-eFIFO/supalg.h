...
const double_data chunkdoub=NRAMblocks*ADCRAM;
const ulongint_data chunk=NRAMblocks*ADCRAM;



//Number of triggers per chunk to send in Spill
const ulongint_data ntriggers_chunkSPILL=(chunkdoub/fADC)/hardwareclockSPILL;

//Number of ADC values per trigger in Spill
const ulongint_data nADC_triggerSPILL=fADC*hardwareclockSPILL;  

//Number of ADC values per chunk according to triggers in Spill
const ulongint_data nADC_chunk=nADC_triggerSPILL*ntriggers_chunkSPILL; 

//Free ADC spaces per chunk for ADC=0 in Spill and Gap
const ulongint_data restchunk=chunkdoub-nADC_chunk-headersize; 

//Number of ADC values per trigger in Gap
const ulongint_data nADC_triggerGAP=fADC*hardwareclockGAP;
