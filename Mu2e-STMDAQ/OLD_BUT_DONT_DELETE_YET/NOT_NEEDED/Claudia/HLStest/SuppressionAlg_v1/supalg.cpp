...
void supalg(in_stream_t&   input,
            ...
            double_data    topcalls,
            ulongint_data  nADC_chunk,
            int16_data     ADC_hardtrig[])
{
...
#pragma HLS INTERFACE axis port = output
	...
	int16_data ADC
	    [chunk];  //size a bit bigger than nADC_chunk, so that it knows the size at compile time (#define) for the synthesis process
	...
	for(unsigned long int i = 0; i < triggercounter; i++)
	{
		...
		if((trigger_vect[i] <= last_triggerstored) ||
		   (trigger_vect[i] - window <= last_triggerstored) ||
		   (trigger_vect[i] + postnumADCstored >= (topcalls + 1) * nADC_chunk))
		{
			continue;
		}
		else
	...
	if(print == true)
	{
		for(int j = 0; j < nADC_chunk; j++)
		{
			cout << j << " " << suppressed_data[j] << endl;
		}
	}
