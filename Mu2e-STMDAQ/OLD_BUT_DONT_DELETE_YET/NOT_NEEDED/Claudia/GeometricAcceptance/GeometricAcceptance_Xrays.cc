...
void GeometricAcceptanceXrays(int         argc,
                              char*       argv[],
                              ...
                              std::string output_path)
{
	...
	std::cout << "Area STM: " << area_STM << "cm2"
	          << " center STM (x,y,z): (" << x_centerSTM << ", " << y_centerSTM << ", "
	          << z_STM << ")mm"
	          << " r STM: " << r_STM << "mm" << std::endl;
    ...
    */
		...
		if(row % 1000000 == 0)
		{
			...
			std::cout << row << " px: " << px << " py: "
			          << " " << py << " pz: " << pz << " xST: " << x_ST
			          << " yST: " << y_ST << " zST: " << z_ST << " theta: " << theta
			          << " phi: " << phi << " STM: x: " << x_STM << " y: " << y_STM
			          << std::endl;
		}

		//Negative pz particles will never hit the STM
		if((sqrt((x_STM - x_centerSTM) * (x_STM - x_centerSTM) +
		         (y_STM - y_centerSTM) * (y_STM - y_centerSTM)) <= r_STM) &&
		   (pz >= 0))
		{
			writefile << row << " ST: px: " << px << " py: " << py << " pz: " << pz
			          << " p: " << p << " x: " << x_ST << " y: " << y_ST << " z: " << z_ST
			          << " -hit STM: x: " << x_STM << " y: " << y_STM
	...
	std::cout << "" << std::endl;
	std::cout << "Reading and compute acceptance time for " << row << " values= "
	          << boost::chrono::duration_cast<boost::chrono::milliseconds>(t2 - t1)
	          << std::endl;
	writefile << "Reading and compute acceptance time for " << row << " values= "
	          << boost::chrono::duration_cast<boost::chrono::milliseconds>(t2 - t1)
	          << std::endl;
