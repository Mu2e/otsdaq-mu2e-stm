...
int main(int narg, char* arg[])
{
	...
	std::string rootname = filename.substr(filename.find("run"), 8) +
	                       "_suppressedsignal_" +
	                       filename.substr(filename.find("bin"), 6) + ".root";
	...
	std::string   output_filename = filename.substr(filename.find("run"), 8) +
	                              "_suppressedsignal_" +
	                              filename.substr(filename.find("bin"), 6) + ".bin";
	...
	cout << "Threshold ADC: " << mean << "  Sigma: " << sigma
	     << "  3*Sigma: " << threesigmas << endl;
