...
int main(int argc, char* argv[])
{
	...
	std::cout << "Number of elements: " << ADC.size() << " in time "
	          << boost::chrono::duration_cast<boost::chrono::milliseconds>(t2 - t1)
	          << std::endl;

	std::cout << "Uisng PRE: " << PRE << " POST: " << POST
	          << " WINDOW_LEN: " << WINDOW_LEN << " THRESHOLD: " << THRESHOLD
	          << " fadc: " << fadc << std::endl;
	t1 = boost::chrono::high_resolution_clock::now();
  ...
  */
	...
	for(int i = WINDOW_LEN + 2; i < n; i++)
	{
		...
		{
			out_value = ADC.at(
			    i - PRE - 2);  //Primer valor de ADC en la window en la que guardamos data
		}
		...
		if(printcout == true)
		{
			...
			std::cout << "variance*threshold^2: " << variance * THRESHOLD * THRESHOLD
			          << std::endl;
		...
		if((height * height > variance * THRESHOLD * THRESHOLD) &
		   ...
		   (height <= 0) & (new_values[2] < new_values[1]) &
		   (new_values[1] < new_values[0]) & (last_trigger < i - PRE - POST))
		{
			last_trigger = i;
		}
		...
		variance += (new_values[0] - old_value) *
		            (new_values[0] + old_value - mean - old_mean) / 128.0;
	...
	{
		std::cout << "Trigger " << i << " in: " << storetrigger.at(i) << " clock ticks "
		          << storetrigger.at(i) * tadc << " us" << std::endl;
	}
	...
	if(condition == true)
	{
		...
		for(int i = 0; i < limitplot; i++)
		{
			...
			grvariancethreshold->SetPoint(
			    i, (i + WINDOW_LEN + 2) * tadc, variancev.at(i) * THRESHOLD * THRESHOLD);
			grheight->SetPoint(
			    i, (i + WINDOW_LEN + 2) * tadc, heightv.at(i) * heightv.at(i));
			if(heightv.at(i) * heightv.at(i) > 5000)
			{
				std::cout << "signal (new_value[0]): " << ADC.at(i + WINDOW_LEN)
				          << " variance: " << variancev.at(i)
				          << " variance*th2: " << variancev.at(i) * THRESHOLD * THRESHOLD
				          << " height2: " << heightv.at(i) * heightv.at(i)
				          << " clock tick: " << i + WINDOW_LEN + 2
				          << " time: " << (i + WINDOW_LEN + 2) * tadc << " us"
				          << std::endl;
			}
		}
	...
	if(signalsup == true)
	{
		...
		for(int i = startplot; i < limitplot; i++)
		{
			...
			grvariancethreshold->SetPoint(
			    j, (i + WINDOW_LEN + 2) * tadc, variancev.at(i));
			grheight->SetPoint(
			    j, (i + WINDOW_LEN + 2) * tadc, heightv.at(i) * heightv.at(i));
			j++;
		}
