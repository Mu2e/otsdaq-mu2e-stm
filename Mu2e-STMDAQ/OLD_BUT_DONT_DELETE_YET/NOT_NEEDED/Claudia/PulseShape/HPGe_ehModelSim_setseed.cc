...
void HPGe_ehModelSim(int                        argc,
                     char*                      argv[],
                     std::string                rate_stringkHz,
                     double                     Energy,
                     double                     noiseSD,
                     double                     timeSample,
                     ...
                     std::string                output_path)
{
	...
	std::cout << "Number of pulses generated (real number of simulated pulses at the end "
	             "of the file): "
	          << pulseNum << std::endl;
	std::cout << "Simulated Pulse Energy: " << Energy
	          << " keV = " << Energy / fehDriftFunctions.ADC_toE << " ADC Counts"
	          << std::endl;
	...
	std::cout << "NoiseSD: " << noiseSD << " mV = " << sigma_noise_ADC << " ADC Counts"
	          << std::endl;
	...
	std::poisson_distribution<int> pulseTime(
	    1 /
	    rate);  //mean of timings distribution in us, gives the time separation between pulses in us following a Poisson distribution
	...
	if(!myFile_int.is_open())
		throw std::runtime_error("Could not open file");
	if(!myFile_freq.is_open())
		throw std::runtime_error("Could not open file");

	std::string line;
	double      val;
	bool        csv =
	    false;  //this is always false, activate it in the loop if first input Energy == 0
	...
	std::piecewise_constant_distribution<> dist_EnergyFlash_STM(
	    intervals.begin(), intervals.end(), freq.begin());
	...
	{
		std::string rootfile =
		    output_path + "EnergySampled_" + rate_stringkHz + "kHz.root";
	...
	for(int i = 0; i < pulseNum; i++)
	{
		...
		std::cout << "***********************NEW EVENT: " << i << " with " << n_compt_phot
		          << " comptons***********************" << std::endl;
		//total number of eh pairs in the whole event
		double Nehevent = 0;
		//Get the distribution of the energy for each compton
		std::vector<double> energiescomptons =
		    fehDriftFunctions.Distribute_energies(Energy, n_compt_phot, gen);
		...
		if(timeIndex >= sampleNum)
		{
			...
			std::cout << "-----Peak generated at: " << timeIndex * tadc << " us "
			          << " outside sample lenght of: " << timeSample
			          << " us -----END LOOP-----" << std::endl;
			continue;
		}
		...
		std::cout << "Peak " << i << " at " << double(timeIndex * tadc) << " us"
		          << std::endl;
		...
		for(int c = 0; c < n_compt_phot; c++)
		{
			...
			std::cout << "---------New compton at: " << r0 << " cm, energy: " << Ei
			          << " keV, Neh: " << Nehi << " ---------" << std::endl;
			...
			for(unsigned long int j = timeIndex; j < max; j++)
			{
				...
				double time =
				    (j + 1 - timeIndex) * tadc;  //starting in tadc us not in 0 us
	...
	*/
		...
		for(unsigned long int j = timeIndex; j < max; j++)
		{
			...
			{  //Need to add the +1 or -1 because when converting to int we can get a difference of +-1 between var1 and var2
				...
				{
					Voltage[j] += fehDriftFunctions.HighPassFilter(
					    timeV, ts, Nehevent); /*std::cout<<"hi1"<<std::endl;*/
				}
			}  // if var1==var2
	...
	std::cout << "Real number of pulses stored in binary file: " << realpulses_binary
	          << std::endl;
...
int main(int argc, char* argv[])
{
	...
	HPGe_ehModelSim(
	    argc, argv, rate_stringkHz, Energy, noiseSD, timeSample, gen, output_path);
