...
void E_res_eff(int print){
  gROOT->SetStyle("ATLAS");

  int N=4; //number of files to read 

  int Etrue[4]={347,662,844,1809};
  std::string Estring[4];
  char* Echar[4]; 
  ...
  filespath[0] = "/data1/cgarcia/DATA/Claudia/GenDataehHPGeSim/"+std::to_string(Etrue[0])+"keV_0.32mV/MWDFilesM"+std::to_string(M)+"L"+std::to_string(L)+".txt"; //index 0 
  filespath[1] = "/data1/cgarcia/DATA/Claudia/GenDataehHPGeSim/"+std::to_string(Etrue[1])+"keV_0.32mV/MWDFilesM"+std::to_string(M)+"L"+std::to_string(L)+".txt"; //index 1
  filespath[2] = "/data1/cgarcia/DATA/Claudia/GenDataehHPGeSim/"+std::to_string(Etrue[2])+"keV_0.32mV/MWDFilesM"+std::to_string(M)+"L"+std::to_string(L)+".txt"; //index 2
  filespath[3] = "/data1/cgarcia/DATA/Claudia/GenDataehHPGeSim/"+std::to_string(Etrue[3])+"keV_0.32mV/MWDFilesM"+std::to_string(M)+"L"+std::to_string(L)+".txt"; //index 3
  ...
  for(int i=0;i<N;i++){
    ...
    std::cout<<"File Size E= "+std::to_string(Etrue[i])+", M= "+std::to_string(M)+"L= "+std::to_string(L)+": "<<file_names[0].size()<<std::endl;
  }
  ...
  for(int i=0;i<N;i++){
	...
	countline++;
      ...
      meanEv[i].push_back(((Etrue[i]-meanE[i])/Etrue[i]));
      meanE_errorv[i].push_back(abs((Etrue[i]-meanE[i])/Etrue[i])*(meanE_error[i]/meanE[i]));
      ...
      std::cout<<"Eshift= (Etrue[i]-meanE[i])/Etrue[i]= "<<((Etrue[i]-meanE[i])/Etrue[i])<<std::endl;
      myfile[i].close();
    }
 ...
 grRes[3]->SetMarkerColor(kPink+10);
 ...
 for(int i=0;i<N;i++){
   Estring[i] = "E="+std::to_string(Etrue[i])+" keV";
   Echar[i] = const_cast<char*>(Estring[i].c_str());
 }

 auto legend = new TLegend(0.1,0.7,0.5,0.9);
 legend->AddEntry(grRes[0],Echar[0],"p");
 legend->AddEntry(grRes[1],Echar[1],"p");
 legend->AddEntry(grRes[2],Echar[2],"p");
 legend->AddEntry(grRes[3],Echar[3],"p");
 legend->Draw("same");
 ...
 if(print==1){
   ...
   grEff[3]->SetMarkerColor(kPink+10);
   ...
   auto legend = new TLegend(0.1,0.7,0.5,0.9);
   legend->AddEntry(grEff[0],Echar[0],"p");
   legend->AddEntry(grEff[1],Echar[1],"p");
   legend->AddEntry(grEff[2],Echar[2],"p");
   legend->AddEntry(grEff[3],Echar[3],"p");
   legend->Draw("same");
 ...
 if(print==2){
   ...
   auto legend = new TLegend(0.1,0.7,0.5,0.9);
   legend->AddEntry(grE[0],Echar[0],"p");
   legend->AddEntry(grE[1],Echar[1],"p");
   legend->AddEntry(grE[2],Echar[2],"p");
   legend->AddEntry(grE[3],Echar[3],"p");
   legend->Draw("same");
