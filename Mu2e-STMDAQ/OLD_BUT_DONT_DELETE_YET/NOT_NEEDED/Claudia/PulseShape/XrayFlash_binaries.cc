...
int main(int argc, char* argv[])
{
	...
	for(int i = ncsv; i < limit; i++)
	{
		...
		for(int j = 0; j < NFlash_per_Xray; j++)
		{
			if(nbinfiles < 10)
			{
				filenameFlash = bin_path + "genData0keV_" + bin_rate + "kHz_job0" +
				                std::to_string(nbinfiles) + ".bin";
			}
			else
			{
				filenameFlash = bin_path + "genData0keV_" + bin_rate + "kHz_job" +
				                std::to_string(nbinfiles) + ".bin";
			}
			...
			unsigned long int n =
			    st.st_size /
			    2;  // get size of file (in bytes) and set number of ADC values (each value is 2 bytes)
    ...
    */
		...
		std::string output_filename =
		    bin_rate + "kHz_Flash_job0" + std::to_string(i) + ".bin";
