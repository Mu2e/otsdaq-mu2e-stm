...
#include "/work/cgarcia/STMDAQ-TestBeam/Claudia/PulseShape/ehDriftFunctions.h"
                     ...
                     std::string output_path)
{
	...
	std::cout << "NoiseSD: " << fehDriftFunctions.noiseSD << " mV = " << sigma_noise_ADC
	          << " ADC Counts" << std::endl;
	...
	for(int i = 0; i < pulseNum; i++)
	{
		...
		std::vector<double> energiescomptons =
		    fehDriftFunctions.Distribute_energies(Energy, n_compt_phot);
		...
		for(int c = 0; c < n_compt_phot; c++)
		{
			...
			{
				r0 = fehDriftFunctions.PosCompPhot(InitPos, Ei);
			}
...
{
	//argv[0]=program, argv[1]=rate(kHz), argv[2]=Energy(keV), argv[3]=noiseSD(mV), argv[4]=timeSample(us), argv[5]=output path
