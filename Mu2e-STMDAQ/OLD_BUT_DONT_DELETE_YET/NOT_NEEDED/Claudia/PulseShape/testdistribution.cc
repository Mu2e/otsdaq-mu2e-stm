...
int main()
{
	...
	std::default_random_engine
	    generator;  //with this generator get always the same result for the distribution

	//construct a trivial random generator engine from a time-based seed:
	unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
	std::default_random_engine generator2(
	    seed);  //with this generator get a different distribution every time we run it
	...
	distribution[7] = {
	    40, 110, 160, 171, 227, 154, 74, 38, 14, 11, 1, 0};  //800keV X-rays
	distribution[8] = {
	    32, 124, 167, 187, 178, 157, 81, 49, 11, 10, 4, 0};  //900keV X-rays
	distribution[9] = {
	    45, 125, 153, 186, 165, 146, 101, 43, 25, 6, 2, 3};  //1000keV X-rays
