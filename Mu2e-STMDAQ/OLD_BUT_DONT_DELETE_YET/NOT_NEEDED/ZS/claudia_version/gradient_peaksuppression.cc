...
int main(int narg, char* arg[])
{
...
#ifdef PRINT_COUT
	std::cout << "fadc= " << fADC << "MHz, window: " << window << " threshold "
	          << threshold << "tbefore: " << tbefore << "us tafter " << tafter << " us"
	          << std::endl;
#endif
	...
	for(unsigned long int i = 0; i < (n - window); i++)
	{
		...
		if((gradient[i] < threshold) && (peak == true))
		{
			continue;
		}
...
#ifdef PRINT_COUT
			std::cout << "Trigger number: " << triggercounter << ": " << trigger
			          << " Triggertime: " << trigger * tadc << std::endl;
#endif

			triggercounter++;

			for(int k = prenumADCstored; k > 0; k--)
			{
				suppressed_data[counter] = ADC[trigger - k];
				peak                     = true;
				counter++;
			}

			for(int j = 0; j < postnumADCstored; j++)
			{
				suppressed_data[counter] = ADC[trigger + j];
				peak                     = true;
				counter++;
			}
		}
	...
	//string s= "/work/cgarcia/DATA/MWD_Analysis/RUN109/SuppresedFiles_tbefore5us_tafter10us/Suppressed_run109"+filename.substr(filename.find(".bin_"), 7);
	std::string s    = "hi";
	int         size = s.length();
