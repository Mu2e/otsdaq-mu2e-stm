...
void CallZS(int argc, char* argv[], std::string filename)
{
	...
	if(mode == 0)
	{
		...
		if(zp.output)
		{
			std::cout << "From " << ntriggers_chunkSPILL << " triggers in the chunk, "
			          << "the next chunk starts in ADC value: " << next_trigger_start
	...
	for(int64_t i = 0; i < nchunks; i++)
	{
		...
		// Form the input FIFO
		int16_t* inputFIFO = zp.Form_InputFIFO(
		...
		if(zp.output)
		{
			std::cout << "Zero suppressed file size in ADC Counts: " << zp_size
			          << std::endl;
		...
		unsigned long int lasttrigstored3 =
		    (unsigned long int)outputFIFO[0] << 0 & (unsigned long int)0x000000000000FFFF;
		unsigned long int lasttrigstored2 = (unsigned long int)outputFIFO[1] << 16 &
		                                    (unsigned long int)0x00000000FFFF0000;
		unsigned long int lasttrigstored1 = (unsigned long int)outputFIFO[2] << 32 &
		                                    (unsigned long int)0x0000FFFF00000000;
		unsigned long int lasttrigstored0 = (unsigned long int)outputFIFO[3] << 48 &
