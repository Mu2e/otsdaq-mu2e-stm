...
int main(int argc, char* argv[])
{
	...
	unsigned long int n =
	    st.st_size /
	    2;  // get size of file (in bytes) and set number of ADC values (each vlaue is 2 bytes)
	std::ifstream myFile;
	...
	std::cout << "Number of elements: " << n << " in time "
	...
	for(int i = 0; i < triggercounter; i++)
	{
		...
		if(i == 0)
		{
			...
			if(trigger_vect[i] - prenumADCstored < 0)
			{
				memcpy(&suppressed_data[0],
				       &ADC[0],
				       2 * (trigger_vect[i] + postnumADCstored));

#ifdef PRINT_COUT
				std::cout << "Not " << prenumADCstored << " ADC values before 1st trigger"
				          << std::endl;
#endif
				...
				memcpy(
				    ...
				    &time[0],
				    8 * (trigger_vect[i] +
				         postnumADCstored));  //be careful they are unsigned long int so 8 bytes

#ifdef PRINT_COUT
				std::cout << "Trigger num " << i << " at " << trigger_vect[i]
				          << " clock ticks " << trigger_vect[i] * tadc << " us"
				          << std::endl;
				std::cout << "Copied values from ADC data from index: 0 to "
				          << trigger_vect[i] + postnumADCstored << std::endl;
				std::cout << "Copied to suppressed data array from index: 0 to "
				          << trigger_vect[i] + postnumADCstored << std::endl;
				std::cout << "Time in ADC counts from: 0 to "
				          << time[trigger_vect[i] + postnumADCstored] << std::endl;
#endif
			...
			{
				memcpy(&suppressed_data[0],
				       &ADC[trigger_vect[i] - prenumADCstored],
				       2 * totalnumADCstored);
				memcpy(&suppressed_time[0],
				       &time[trigger_vect[i] - prenumADCstored],
				       8 * totalnumADCstored);
				...
				std::cout << "Trigger num " << i << " at " << trigger_vect[i]
				          << " clock ticks " << trigger_vect[i] * tadc << " us"
				          << std::endl;
				std::cout << "Copied values from ADC data from index: "
				          << trigger_vect[i] - prenumADCstored << " to "
				          << trigger_vect[i] + postnumADCstored << std::endl;
				std::cout << "Copied to suppressed data array from index: 0 to "
				          << totalnumADCstored << std::endl;
				std::cout << "Time in ADC counts from: "
				          << time[trigger_vect[i] - prenumADCstored] << " to "
				          << time[trigger_vect[i] + postnumADCstored] << std::endl;
#endif
		...
		else
		{
			...
			if(trig_dif < totalnumADCstored)
			{
				...
				if(trigger_vect[i] + postnumADCstored > n)
				{
					...
					memcpy(&suppressed_data[last_triggerstored_index +
					                        (trig_dif - prenumADCstored)],
					       &ADC[trigger_vect[i] - prenumADCstored],
					       2 * (prenumADCstored + remainADC));

#ifdef PRINT_COUT
					std::cout << "Last trigger exceeds " << postnumADCstored
					          << " ADC values" << std::endl;
#endif

					memcpy(&suppressed_time[last_triggerstored_index +
					                        (trig_dif - prenumADCstored)],
					       &time[trigger_vect[i] - prenumADCstored],
					       8 * (prenumADCstored + remainADC));
					...
					std::cout << "Copied values from ADC data from index: "
					          << trigger_vect[i] - prenumADCstored << " to "
					          << trigger_vect[i] + remainADC << std::endl;
					std::cout << "Copied to suppressed data array from index:"
					          << last_triggerstored_index + (trig_dif - prenumADCstored)
					          << " to " << last_triggerstored_index + trig_dif + remainADC
					          << std::endl;
					std::cout << "Time in ADC counts from: "
					          << time[trigger_vect[i] - prenumADCstored] << " to "
					          << time[trigger_vect[i] + remainADC - 1]
					          << std::endl;  //Don't understand why I have to put this -1
#endif
				...
				std::cout << "Copied to suppressed data array from index:"
				          << last_triggerstored_index + (trig_dif - prenumADCstored)
				          << " to "
				          << last_triggerstored_index + trig_dif + postnumADCstored
				          << std::endl;
				std::cout << "Time in ADC counts from: "
				          << time[trigger_vect[i] - prenumADCstored] << " to "
				          << time[trigger_vect[i] + postnumADCstored] << std::endl;
			...
			if(trig_dif >= totalnumADCstored)
			{
				...
				{
					unsigned long int remainADC = n - trigger_vect[i];
					memcpy(&suppressed_data[last_triggerstored_index + postnumADCstored],
					       &ADC[trigger_vect[i] - prenumADCstored],
					       2 * (prenumADCstored + remainADC));
					...
					memcpy(&suppressed_time[last_triggerstored_index + postnumADCstored],
					       &time[trigger_vect[i] - prenumADCstored],
					       8 * (prenumADCstored + remainADC));

					size_sup = last_triggerstored_index + postnumADCstored +
					           prenumADCstored + remainADC;
					...
					std::cout << "Copied values from ADC data from index: "
					          ...
					          << trigger_vect[i] + remainADC << std::endl;
					std::cout << "Copied to suppressed data array from index:"
					          << last_triggerstored_index + postnumADCstored << " to "
					          << last_triggerstored_index + totalnumADCstored + remainADC
					          << std::endl;
					std::cout << "Time in ADC counts from: "
					          << time[trigger_vect[i] - prenumADCstored] << " to "
					          << time[trigger_vect[i] + remainADC - 1]
				...
				size_sup =
				    last_triggerstored_index + postnumADCstored + totalnumADCstored;
				...
				std::cout << "Copied to suppressed data array from index:"
				          << last_triggerstored_index + postnumADCstored << " to "
				          << last_triggerstored_index + postnumADCstored +
				                 totalnumADCstored
				          << std::endl;
				std::cout << "Time in ADC counts from: "
				          << time[trigger_vect[i] - prenumADCstored] << " to "
	...
	t2 = boost::chrono::high_resolution_clock::now();
	std::cout << "" << std::endl;
	std::cout << "Zero Suppression Algorithm computing time = "
	          << boost::chrono::duration_cast<boost::chrono::milliseconds>(t2 - t1)
	          << std::endl;
	...
	for(unsigned long int i = 0; i < limitplot; i++)
	{
		...
		grad->SetPoint(i, tadc * avADCtime[i], avgradient[i]);
		//plot gradient
		//grad->SetPoint(i,timeus[i],gradient[i]);
		//plot raw signal
		gsignal->SetPoint(i, timeus[i], ADC[i]);
		//std::cout<<timeus[i]<<" "<<ADC[i]<<std::endl;
