...
int main(int argc, char* argv[]){
  ...
  unsigned first = filename.find("genData662keV_") + 14;
  unsigned last = filename.find("kHz");
  std::string strNew = filename.substr (first,last-first);
  std::cout << strNew << "," << (double)peak_count/10000.0*100 << "," << speed << "," << 100-(double)sup_data_len/(double)n*100 <<  std::endl;
