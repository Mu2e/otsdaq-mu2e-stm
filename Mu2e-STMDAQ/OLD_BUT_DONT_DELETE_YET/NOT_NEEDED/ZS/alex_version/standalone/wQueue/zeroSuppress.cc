...
daqZS::daqZS()
{
	...
	for(int i = 0; i < CHNUM; i++)
	{
		...
		{
			buffer_free[i][j].store(true);
		}
	}
}

// ZS algorithm
std::tuple<int16_t*, uint64_t, uint64_t, uint64_t, int64_t, int64_t> daqZS::do_ZS(
    int chan, uint64_t n, int16_t*& data)
{
