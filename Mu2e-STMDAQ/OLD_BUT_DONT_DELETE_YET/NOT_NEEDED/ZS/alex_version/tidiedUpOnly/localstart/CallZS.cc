...
void CallZS(int argc, char* argv[], std::string filename)
{
	...
	unsigned long int nADC_chunk =
	    zsup.nADC_chunk_(nADC_triggerSPILL, ntriggers_chunkSPILL);
	...
	if(mode == 1)
	{
		...
		if(zsup.output)
		{
			...
			std::cout << "Size of the chunk in ADC values (headers+ADC+ADC=0): " << chunk
			          << std::endl;
			std::cout << "Number of ADC values per trigger: " << nADC_triggerGAP
			          << std::endl;
			std::cout << "Number of ADC values per chunk: " << nADC_chunk << std::endl;
		}
	...
	double overlapped_chunk =
	    double((nADC_chunk - 1) - postnumADCstored + 1 - prenumADCstored) / nADC_chunk;
	...
	if(mode == 0)
	{
		...
		next_trigger_startTrigNum =
		    next_trigger_start / ADChardtrig;  //divide by trigger size
	...
	if(mode == 1)
	{
		...
		if(zsup.output)
		{
			std::cout << "One trigger (" << nADC_triggerGAP
			          << " ADC values) extends over " << nchunkspertriggerGAP << " chunks"
			          << std::endl;
	...
	if(prenumADCstored > ((nADC_chunk * (1 - overlapped_chunk)) - (postnumADCstored - 1)))
	{
		if(zsup.output)
		{
			std::cout << "WARNING::::::: In the limit case where trigger is in "
			             "(chunk-post-1), this trigger would be stored in next chunk but "
			             "pre adc values chosen exceeds the start of the next chunk, PRE "
			             "values stored must be lower::::::: ERROR"
			          << std::endl;
		}
	...
	for(int64_t i = 0; i < nchunks; i++)
	{
		...
		if(zsup.output)
		{
			...
			std::cout << "Chunk goes from: " << chunkstart << " to "
			          << chunkstart + (nADC_chunk - 1) << std::endl;
		...
		int16_t* allchunk = zsup.Form_InputFIFO(
		    nADC, mode, i, chunkstart, trignumstart, last_triggerstored, numADCtoZS);
		...
		std::cout << "Number of elements supressed: " << numADCtoZS << " in time "
		          << boost::chrono::duration_cast<boost::chrono::nanoseconds>(t2 - t1)
		          << std::endl;
		...
		if(zsup.output)
		{
			std::cout << "Zero suppressed file size in ADC Counts: " << zsup_size
			          << std::endl;
		...
		unsigned long int lasttrigstored3 =
		    (unsigned long int)alldata[0] << 0 & (unsigned long int)0x000000000000FFFF;
		unsigned long int lasttrigstored2 =
		    (unsigned long int)alldata[1] << 16 & (unsigned long int)0x00000000FFFF0000;
		unsigned long int lasttrigstored1 =
		    (unsigned long int)alldata[2] << 32 & (unsigned long int)0x0000FFFF00000000;
		unsigned long int lasttrigstored0 =
		    (unsigned long int)alldata[3] << 48 & (unsigned long int)0xFFFF000000000000;
		last_triggerstored = (unsigned long int)(lasttrigstored0 | lasttrigstored1 |
		                                         lasttrigstored2 | lasttrigstored3);

		if(zsup.output)
		{
			std::cout << "Received last trigger stored: " << last_triggerstored
			          << std::endl;
		}
