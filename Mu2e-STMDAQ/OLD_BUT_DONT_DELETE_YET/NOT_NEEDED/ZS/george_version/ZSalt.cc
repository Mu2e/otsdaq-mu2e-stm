...
int16_t* ZS::Form_InputFIFO(int16_t*          nADC,
                            int16_t           _mode,
                            ...
                            int32_t           _numADCtoZS)
{
	...
	if(output)
	{
		...
		std::cout << "Last ZS trigger from previous chunk: " << _last_triggerstored
		          << std::endl;
		std::cout << "Number of ADC values to ZS in this chunk: " << _numADCtoZS
		          << std::endl;
	}
...
void ZS::ReadInputHeader(int16_t* allchunk)
{
	...
	if(output)
	{
		...
		std::cout << "Check Chunk starts in: " << chunkstart << " ADC values"
		          << std::endl;
		std::cout << "Check Trig Num start: " << trignumstart << std::endl;
		std::cout << "Check last trigger stored: " << last_triggerstored << std::endl;
		std::cout << "Check number of ADC values to ZS in this chunk: " << numADCtoZS
		          << std::endl;
	}
...
void ZS::supalg(int16_t* data)
{
	...
	while(j < (numADCtoZS - window))
	{
		...
		if(avg_gradient < threshold && peak == false)
		{
			...
			if((peak_trig_time > last_triggerstored) &&  // (a)
			   (peak_trig_time - prenumADCstored_(tadc_()) >
			    last_triggerstored) &&  // (b)
			   (peak_trig_time + postnumADCstored_(tadc_()) < chunkstart + numADCtoZS))
			{  // (c)
				...
				while(k <= postnumADCstored_(tadc_()))
				{
					...
					if(checkprenum > limit && checkprenum >= 0)
					{
						...
						suppressed_data[(uint)avg_time + k] =
						    data[headersize + (uint)avg_time + k];
					...
					if(checkprenum < 0)
					{
						...
						if(output)
						{
							// Print for user
							std::cout
							    << "Not prenum ADC values to store in this chunk for "
							       "this ZS trigger, not all values required are stored "
							       "for this trigger ---wrong energy expected"
							    << std::endl;
						}
					}
		...
		if(mode == 0)
		{
			...
			{
				if(std::adjacent_find(ans[i].begin(),
				                      ans[i].end(),
				                      std::not_equal_to<>()) == ans[i].end())
				{
					...
					// Number of slices
					outputFIFO[b * i + 4] = slices;
					// Start of data relative to the hardware trigger
					outputFIFO[b * i + 5] = numstart_data;
	...
	//std::cout << i << " ";
      ...
      std::cout << std::endl;
		...
		{
			// Print for user
			std::cout << "fadc= " << fADC << " tadc_()= " << tadc_()
			          << " MHz, window: " << window << " threshold " << threshold
			          << " tbefore: " << tbefore << " us (" << prenumADCstored_(tadc_())
			          << " ADC Counts)"
			          << " tafter " << tafter << " us (" << postnumADCstored_(tadc_())
			          << " ADC Counts)" << std::endl;
		}
		...
		{
			std::cout << "Number of new triggers found in chunk: " << trig_count
			          << std::endl;
			std::cout << "Last trigger stored updated: " << last_triggerstored
			          << std::endl;
	...
	int16_t* ZS::Form_OutputFIFO()
	{
		...
		if(mode == 0)
		{
			...
			for(unsigned long int j = 0; j < ntriggers_chunkSPILL; j++)
			{
				...
				while(i < triglength)
				{
					//std::cout << suppressed_data[i] << " " << suppressed_data[triglength-i] << "\n";
					// If printing info...
					if(output)
					{
						// Print for user
			...
			// if(d!=0 || g !=0 || h!=0) std::cout << "d= " << d << "  g= " << g << "  h= " << h << "\n";
		}  // End iF MODE 0
		//std::cout << "q = " << q << "  r= " << r << "\n";
		// If off-spill
		if(mode == 1)
