...
static const double spillWidth =
    4 * onEventCycleNum * onEventWidth1 + onEventCycleNum * onEventWidth2;
...
static const uint totOffEventsMIcycle =
    7 * offEventsRegGap +  // 7 regular gaps per MI cycle
...
static const struct modeConfig pM[totSamplePeriods] = {
    ...
    {15,
     offMode,
     spillGap8to1 / 5,
     offEventsGap8to1 / 5},  // Gap 8 --> 1 [part1], off-spill
    {16,
     offMode,
     spillGap8to1 / 5,
     offEventsGap8to1 / 5},  // Gap 8 --> 1 [part2], off-spill
    {17,
     offMode,
     spillGap8to1 / 5,
     offEventsGap8to1 / 5},  // Gap 8 --> 1 [part3], off-spill
    {18,
     offMode,
     spillGap8to1 / 5,
     offEventsGap8to1 / 5},  // Gap 8 --> 1 [part4], off-spill
    {19,
     offMode,
     spillGap8to1 / 5,
     offEventsGap8to1 / 5}};  // Gap 8 --> 1 [part5] off-spill
...
int16_t* HPGeSim::genPulses(int16_t* ADCtmp, expConfig exp, modeConfig md)
{
	...
	if(!ADCemulate)
	{
		...
		for(int pn = 0; pn < pulseNum; pn++)
		{
			...
			if((timeIndex + pulseLen) >= sampleLen[p])
				continue;
...
void HPGeSim::fillPackets(int16_t* ADC, pStruct& pData, expConfig exp, modeConfig md)
{
	...
	for(uint i = 0; i < packetNum; i++)
	{
		...
		memcpy(
		    pData.pack[i].data + count, packetHeader, fw_pHdr_Len * sizeof *packetHeader);
		// Account for the packet header length
		...
		while(1)
		{
			...
			if(fw_tHdr_Len >= leftInPacket)
			{
				...
				if(leftInPacket != 0)
				{
					cout << "Error 1! Packet remainder should be zero. Exiting..."
					     << endl;
					exit(0);
				}
			...
			memcpy(pData.pack[i].data + count,
			       triggerHeader,
			       fw_tHdr_Len * sizeof *triggerHeader);
			// Account for the trigger header length
			leftInPacket -= fw_tHdr_Len;
			// Increment data counter
			count += fw_tHdr_Len;
			...
			if(eventToAdd > leftInPacket)
			{
				...
				// Get the end point of the event to add
				uint ADCend = eOrigin[p][eventCount][eLen[p][eventCount] - eventToAdd +
				                                     leftInPacket - 1];
				// Find the length of the data to add
				uint dataLen = ADCend - ADCstart + 1;
				...
				if(leftInPacket != 0)
				{
					cout << "Error 2! Packet remainder should be zero. Exiting..."
					     << endl;
					exit(0);
				}
				...
				if(count != packetLen)
				{
					cout << "Error 3! Packet counter not the same size as the packet. "
					        "Exiting..."
					     << endl;
					exit(0);
				}
			...
			else
			{
				...
				if(eventToAdd != 0)
				{
					cout << "Error 4! Packet remainder should be zero. Exiting..."
					     << endl;
					exit(0);
				}
...
void HPGeSim::simMIcycle(pStruct* pData, expConfig exp, bool output)
{
	...
	for(uint i = 0; i < totSamplePeriods; i++)
	{
		...
		if(i == totSamplePeriods - 1)
		{
			...
			double cycleTime =
			    chrono::duration<double, micro>(MIcycleEnd - MIcycleStart).count();
			if(output)
				cout << "\tSimulated MI cycle (send) time = " << cycleTime << " us"
				     << endl;
		}
	}
...
void HPGeSim::genData(int16_t** ADCtmp, pStruct& pData, expConfig exp, modeConfig md)
{
	...
	// Get pulse data
	if(!ADCemulate)
		ADCtmp[p] = genPulses(ADCtmp[p], exp, md);
...
void HPGeSim::runSim(expConfig exp, bool output)
{
	...
	while(1)
	{
		...
		{
			sendThread =
			    new thread(&HPGeSim::simMIcycle, this, MIcycle[sendInd], exp, output);
			if(writeSimData)
				writeThread = new thread(&HPGeSim::writeToFile, this, MIcycle[sendInd]);
		}
		...
		for(uint i = 0; i < totSamplePeriods; i++)
		{
			...
			dataThread[i] = new thread(&HPGeSim::genData,
			                           this,
			                           ADC[genInd],
			                           ref(MIcycle[genInd][i]),
			                           exp,
			                           pM[i]);
		...
		string cycPrint = "MI cycle " + to_string(MIcount) +
		                  ": noise + pulse generation and packet allocation";
		if(output)
			calcTime(cycPrint, dataStart);
...
void HPGeSim::initSim(expConfig exp, bool output)
{
	...
	for(int i = 0; i < thArrayNum; i++)
		ADC[i] = new int16_t*[totSamplePeriods]();
	// The MI cycle pStruct
	MIcycle = new pStruct*[thArrayNum];
	for(int i = 0; i < thArrayNum; i++)
		MIcycle[i] = new pStruct[totSamplePeriods];

	// Form MI cycle events for Mu2e smimulation
	if(output)
		cout << "Forming events and packets..." << endl;
	for(uint i = 0; i < totSamplePeriods; i++)
	{
		// Form events
	...
	for(uint i = 0; i < totSamplePeriods; i++)
	{
		...
		for(int j = 0; j < thArrayNum; j++)
			ADC[j][i] = new int16_t[sampleLen[i]]();
...
void HPGeSim::setupSim(Xml* xml_file, expConfig exp, bool output, int run_number)
{
	...
	int          Mbytes = 1048576;
	unsigned int max_binary_size =
	    (unsigned int)max_binary_file_size * (unsigned int)Mbytes;
	...
	{
		std::cout
		    << "Max binary file size has to be less than 2 Gb, else int32 issues, EXITING"
		    << std::endl;
		exit(-1);
	}
