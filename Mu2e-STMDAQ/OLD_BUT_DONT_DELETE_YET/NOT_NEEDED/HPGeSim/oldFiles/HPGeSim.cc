...
int16_t* HPGeSim::genPulseData(struct expConfig  exp,
                               double            rateHz,
                               double            sampleTime,
                               unsigned long int sampleLen)
{
	...
	for(int i = 0; i < pulseNum; i++)
	{
		...
		if(timeIndex + pulseLength / exp.adc_time_clock_period > sampleLen)
			max = sampleLen;
...
void HPGeSim::getData(struct expConfig exp, struct headerInfo trigInfo)
{
	...
	unsigned int packetLen =
	    fw_pHdr_Len + fw_tHdr_Len + slicesPerPacket * (fw_sHdr_Len + sliceLen);
	...
	long unsigned int dataLen =
	    sampleLen + sliceNum * fw_sHdr_Len + packetNum * (fw_pHdr_Len + fw_tHdr_Len);
		...
		for(uint s = 0; s < slicesPerPacket; s++)
		{
			...
			for(uint j = 0; j < sliceLen; j++)
			{
				...
				if(ADCemulate)
				{
					...
					if(adcCount == INT16_T_MAX * (int16MAXcount + 1))
						int16MAXcount += 1;
				}
				else
				{
...
void HPGeSim::simSuperCycle(expConfig exp, bool output)
{
	...
	double extTime =
	    std::chrono::duration<double, std::milli>(extEnd - macroStart).count() * 1e3;

	// Now, find the time to wait before switching to internal mode
	double extWait = 0;
	// Subtract the time to simulate data from the wait time...
	if(extTime < timeInMacropulse)
		extWait = timeInMacropulse - extTime;
	// Wait
	usleep(extWait);
	...
	double intTime =
	    std::chrono::duration<double, std::milli>(intEnd - macroStart).count() * 1e3;
	...
	{
		cout << "Error! Simulation takes longer than the given accelerator cycle period!"
		     << endl;
	...
	if(output)
	{
		cout << "Macropulse " << macroCount << " (" << exp.macropulse_freq
		     << " Hz) simulation period =  " << elapsed_time_ms << " ms" << endl;
	}
...
void HPGeSim::setupSim(Xml* xml_file, expConfig exp, bool output, int run_number)
{
	...
	if(output)
	{
		cout << "Accepting micropulse trigger " << microStart << " out of "
		     << microPerMacro << " micropulses in first macropulse..." << endl;
	}
