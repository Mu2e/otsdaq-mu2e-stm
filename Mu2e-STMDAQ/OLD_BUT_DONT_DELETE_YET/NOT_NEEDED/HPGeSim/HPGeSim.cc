	...
	while(1)
	{
		...
		if(!firstLoop)
		{
			sendThread = new thread(
			    &HPGeSim::simMIcycle, this, formPack.MIcycle[sendInd], exp, output);
			if(writeSimData)
				writeThread =
				    new thread(&HPGeSim::writeToFile, this, formPack.MIcycle[sendInd]);
		}
		...
		for(uint i = 0; i < totSamplePeriods; i++)
		{
			...
			dataThread[i] = new thread(
			    &emulateADC::genData, ref(emADC), emADC.ADC[genInd], exp, pM[i]);
		}
		...
		for(uint i = 0; i < totSamplePeriods; i++)
		{
			...
			formPack.fillPackets(emADC,
			                     MIcount,
			                     emADC.ADC[genInd][i],
			                     formPack.MIcycle[genInd][i],
			                     exp,
			                     pM[i]);
		}
