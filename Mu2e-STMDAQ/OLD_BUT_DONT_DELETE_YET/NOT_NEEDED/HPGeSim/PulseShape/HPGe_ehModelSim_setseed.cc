...
void ehDriftFunctions::write_to_file(std::string output_path)
{
...
int16_t* ehDriftFunctions::gen_pulse(double            Energy,
                                     unsigned long int pulseStart,
                                     unsigned long int pulseEnd)
{
	...
	if(output)
		std::cout << "***********************NEW EVENT: " << realpulses_binary << " with "
		          << n_compt_phot << " comptons***********************" << std::endl;
	...
	for(unsigned long int j = pulseStart; j < pulseEnd; j++)
	{
		...
		{
			Voltage[j - pulseStart] +=
			    Qpulse[j - pulseStart]; /*if (output) std::cout << "hi2" << std::endl;*/
		}
...
void ehDriftFunctions::HPGe_ehModelSim(std::string rate_stringkHz,
                                       double      Energy,
                                       double      noiseSD,
                                       double      timeSample)
{
...
void ehDriftFunctions::gen_noise(double noiseSD)
{
	...
	if(output)
		std::cout << "NoiseSD: " << noiseSD << " mV = " << sigma_noise_ADC
		          << " ADC Counts" << std::endl;
...
void ehDriftFunctions::init_sim(expConfig   exp,
                                ...
                                double      timeSample)
{
	...
	if(output)
		std::cout << "Real time of the sample: " << sampleNum * tadc << " us"
		          << std::endl;
	...
	if(output)
		std::cout << "Number of pulses generated (real number of simulated pulses at the "
		             "end of the file): "
		          << pulseNum << std::endl;
	if(output)
		std::cout << "Simulated Pulse Energy: " << Energy << " keV = " << Energy / ADC_toE
		          << " ADC Counts" << std::endl;
