...
int16_t** emulateADC::genPulseArray(expConfig exp)
{
	...
	for(int i = 0; i < genPulseNum; i++)
	{
		...
		if(simPulse)
		{
			...
			cout << "\r"
			     << "Simulated " << i + 1 << "/" << genPulseNum << " pulses"
			     << std::flush;
		}
		// Else if just using simple pulse...
		else
...
int16_t* emulateADC::genPulses(int16_t* ADCtmp, expConfig exp, modeConfig md)
{
	...
	if(simPulse)
	{
		...
		for(int i = 0; i < pulseNum; i++)
			pulseIndex[i] = int(r4_uni(seed) * genPulseNum);
	}
	...
	if(!ADCemulate)
	{
		...
		for(int pn = 0; pn < pulseNum; pn++)
		{
			...
			timeIndex +=
			    int(Random::Instance()->PoissValue(rate) / exp.adc_time_clock_period);
