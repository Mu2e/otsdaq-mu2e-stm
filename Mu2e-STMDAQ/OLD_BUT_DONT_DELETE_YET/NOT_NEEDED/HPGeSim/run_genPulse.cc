...
int main(int argc, char* argv[])
{
	...
	for(int i = 0; i <= loops; i++)
	{
		...
		if(i > 0)
		{
			...
			if(t_rem_hours >= 1)
			{
				std::cout << "Roughly " << t_rem_hours << " hours remaining..."
				          << std::endl;
			}
			else
			{
				std::cout << "Roughly " << t_rem_minutes << " minutes remaining..."
				          << std::endl;
			}
		}
		...
		std::cout << "HPGe Simulation computing time = " << t_ms << " millseconds"
		          << std::endl;
	...
	std::cout << "Genereated a total of " << HPGe_pulse.total_pulses << " pulses"
	          << std::endl;
