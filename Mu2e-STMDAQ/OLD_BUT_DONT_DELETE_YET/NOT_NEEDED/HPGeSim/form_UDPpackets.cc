...
void formPackets::fillPackets(emulateADC emADC,
                              uint       MIcount,
                              int16_t*   ADC,
                              pStruct&   pData,
                              expConfig  exp,
	...
	for(uint i = 0; i < packetNum; i++)
	{
		...
		while(1)
		{
			...
			if(eventToAdd > leftInPacket)
			{
				...
				uint ADCstart =
				    emADC.eOrigin[p][eventCount][emADC.eLen[p][eventCount] - eventToAdd];
				// Get the end point of the event to add
				uint ADCend = emADC.eOrigin[p][eventCount][emADC.eLen[p][eventCount] -
				                                           eventToAdd + leftInPacket - 1];
...
void formPackets::createPackets(emulateADC emADC,
                                pStruct&   pData,
                                expConfig  exp,
                                modeConfig md)
{
	...
	for(uint i = 0; i < eventNum; i++)
	{
		...
		while(eventToAdd != 0)
		{
			...
			if(eventToAdd > leftInPacket)
			{
				...
				// Get the end point of the event to add
				uint ADCend =
				    emADC.eOrigin[p][i][emADC.eLen[p][i] - eventToAdd + leftInPacket - 1];
				// Find the length of the data to add
				uint dataLen = ADCend - ADCstart + 1;
...
void formPackets::initPackets(emulateADC emADC, expConfig exp, bool output)
{
	...
	for(int i = 0; i < emADC.genArrayNum; i++)
