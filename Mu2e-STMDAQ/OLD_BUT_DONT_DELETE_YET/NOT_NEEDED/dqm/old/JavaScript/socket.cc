...
void disconnect() { cm_disconnect_experiment(); }
...
int main(int argc, char* argv[])
{
	...
	{
		std::cout
		    << "Could not retrieve the environment variables. Exiting with error code "
		    << status << std::endl;
		disconnect();
		return 1;
	}
  ...
  */
	...
	if(status != CM_SUCCESS)
	{
		std::cout << "Failed to connect to experiment " << exp_name_str << " on "
		          << host_name_str << " . Exiting with error code " << status
		          << std::endl;
		disconnect();
		return 1;
	}
	...
	if(host_name_str != "")
	{
		std::cout << "Connected to experiment " << exp_name_str << " running on "
		          << host_name_str << "." << std::endl;
	}
	else
	{
	...
	std::cout << "Connected to buffer " << EVENT_BUFFER_NAME << " of size "
	          << DEFAULT_BUFFER_SIZE << " bytes (" << DEFAULT_BUFFER_SIZE / (1024 * 1024)
	          << " MB)." << std::endl;

	// Request events to be sent over
	// TODO - change the sampling type to GET_ALL and make it work that way.
	INT event_id;
	status = bm_request_event(
	    online_buffer, EVENTID_ALL, TRIGGER_ALL, GET_NONBLOCKING, &event_id, NULL);
	// Check if the event request was successful
	if(status != BM_SUCCESS)
	{
		std::cout << "Failed to setup event requests. Exiting with error code " << status
		          << std::endl;
		disconnect();
		return 1;
	}
	...
	if(status != CM_SUCCESS)
	{
		std::cout << "Error in requesting starting transitions in MIDAS. Exiting with "
		             "error code "
		          << status << "." << std::endl;
		disconnect();
		return 1;
	...
	{
		std::cout
		    << "Error in requesting ending transitions in MIDAS. Exiting with error code "
		    << status << "." << std::endl;
		disconnect();
		return 1;
	}
	...
	if(status != CM_SUCCESS)
	{
		std::cout << "Failed to retrieve the ODB handle. Exiting with error code "
		          << status << std::endl;
		disconnect();
		return 1;
	}
	...
	status = db_get_value(ODB_handle,
	                      0,
	                      "/Runinfo/Run number",
	                      &current_run_number,
	                      &INT_SIZE,
	...
	{
		std::cout << "The current run number is " << current_run_number << "."
		          << std::endl;
	}
	...
	std::string buffer_ODB_path =
	    "/Experiment/Buffer sizes/" + static_cast<std::string>(EVENT_BUFFER_NAME);
	status = db_get_value(ODB_handle,
	                      0,
	                      buffer_ODB_path.c_str(),
	                      &buffer_size,
	                      &DWORD_SIZE,
	                      TID_DWORD,
	                      FALSE);
	...
	{
		std::cout
		    << "Could not retrieve the current buffer size. Exiting with error code "
		    << status << std::endl;
		disconnect();
		return 1;
	}
	...
	if(status == CM_SUCCESS)
	{
		std::cout << "Found a transition." << std::endl;
	}
	...
	while(true)
	{
		...
		{
			std::cout
			    << "Could not receive an event from this socket. Exiting with error code "
			    << status << std::endl;
			disconnect();
			break;
		}
		...
		{
			std::cout << "An error has occurred with reading the data. Have expected a "
			             "minimum size of "
		...
		status =
		    bk_find(p_bank, bank_name.c_str(), &bank_length, &bank_type, &bank_void_data);
		if(status != CM_SUCCESS)
		{
			std::cout << "Bank " << bank_name << " not found in header." << std::endl;
			break;
		}
		std::vector<uint16_t> data_elements(
		    bank_data,
		    bank_data + (rpc_tid_size(bank_type) / sizeof(char)) * bank_length);
