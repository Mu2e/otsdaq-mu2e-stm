...
void Logger::write(unsigned int                       level_,
                   ...
                   const std::lock_guard<std::mutex>& lock_)
{
	...
	if(level_ <= _logLevel)
	{
		...
		if(level_ == Logger::ERROR)
			levelStr = "ERROR";
		else if(level_ == Logger::INFO)
			levelStr = "INFO";
		else if(level_ == Logger::WARNING)
			levelStr = "WARNING";
		else if(level_ == Logger::NOTE)
			levelStr = "NOTE";
		else if(level_ == Logger::DEBUG)
			levelStr = "DEBUG";
		else
		{
			std::stringstream ss;
			ss << "[ Logger :: write ] Invalid log level " << level_
...
void Logger::setLogLevel(const unsigned int logLevel)
{
	...
	{
		std::stringstream ss;
		ss << "[ Logger :: setLogLevel ] Invalid log level " << logLevel
		   << " chosen, must be ERROR, INFO, WARNING, NOTE or DEBUG";
...
void Logger::setStylePlain() { this->setStyle(0); }
...
void Logger::setStyle(unsigned int style_)
{
	...
	{
		std::stringstream ss;
		ss << "[ Logger :: setStyle ] Invalid style " << style_
...
void Logger::LogToFile(string fileName_)
{
	...
	{
		stringstream ss;
		ss << "[ Logger :: LogToFile ] Already logging to file";
		CloseLogFile();
		//this->write(Logger::ERROR,ss.str(),lock);
		//return;
	}
	...
	if(_opFile)
	{
		_logToFile = true;
		stringstream ss;
		ss << "[ Logger :: LogToFile ] Logging to file: " << _opFileName;
		this->write(Logger::INFO, ss.str(), lock);
	}
	else
	{
		stringstream ss;
		ss << "[ Logger :: LogToFile ] Failed to open file: " << _opFileName << std::endl;
		this->write(Logger::ERROR, ss.str(), lock);
	}
}
...
void Logger::CloseLogFile()
{
	...
	if(_logToFile)
	{
		...
		stringstream ss;
		ss << "[ Logger :: CloseLogFile ] Closing log file: " << _opFileName;
	...
	{
		stringstream ss;
		ss << "[ Logger :: CloseLogFile ] Cannot close log file as not currently logging "
		      "to file";
