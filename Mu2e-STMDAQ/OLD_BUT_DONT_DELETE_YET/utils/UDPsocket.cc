...
int UDPsocket::createSocket(int CHAN, int FW_or_SW)
{
	...
	Logger::Instance()->write(
	    1,
	    "CREATED" + type + "SERVER: Channel = " + std::to_string(CHAN) + " [" +
	        get_channel_name(CHAN) + "]: IP = " + ip +
	        ", PORT = " + std::to_string(port) + ", socket = " + std::to_string(sock));
...
int UDPsocket::setupClient(int CHAN, int FW_or_SW)
{
	...
	Logger::Instance()->write(
	    1,
	    "CREATED" + type + "CLIENT: Channel = " + std::to_string(CHAN) + " [" +
	        get_channel_name(CHAN) + "]: IP = " + ip +
	        ", PORT = " + std::to_string(port) + ", socket = " + std::to_string(sock));
...
int UDPsocket::set_SO_RCVBUF(int socket, uint64_t size)
{
	...
	Logger::Instance()->write(1,
	                          "Setting SO_RCVBUF size to " + std::to_string(size) +
	...
	Logger::Instance()->write(
	    1,
	    "Calculated RECVMMSG_NUM =  " + std::to_string(RECVMMSG_NUM) +
	        " * 8198 byte UDP packets = " +
	        std::to_string((RECVMMSG_NUM * MAX_PACKET_SIZE) * 1e-9) + " Gbytes");
...
int UDPsocket::set_SO_SNDBUF(int socket, uint64_t size)
{
	...
	Logger::Instance()->write(1,
	                          "Setting SO_SNDBUF size to " + std::to_string(size) +
	...
	Logger::Instance()->write(
	    1,
	    "Calculated SENDMMSG_NUM =  " + std::to_string(SENDMMSG_NUM) +
	        " * 8198 byte UDP packets = " +
	        std::to_string((SENDMMSG_NUM * MAX_PACKET_SIZE) * 1e-9) + " Gbytes");
...
int UDPsocket::recv(int16_t* data, int socket, int CHAN)
{
	...
	// Set the timeout
	struct timespec timeout = {read_timeout.tv_sec, read_timeout.tv_usec * 100000};
