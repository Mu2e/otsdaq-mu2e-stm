...
const int16_t fw_tHdr::HdrStart_data[fw_tHdr::HdrStart_len] = {
...
expConfig dataVars::getXMLvalues(expConfig exp)
{
	...
	exp.trigger_time_clock = xml_file->double_value("stm.trigger_time_clock", 13);  // MHz
	// Macropulse width
	exp.macropulse_width = xml_file->double_value("stm.macropulse_width", 1) * 1e3;  // us
	// Macropulse frequency
	exp.macropulse_freq = xml_file->double_value("stm.macropulse_freq", 1);  // Hz

	// ADC clock frequency
	exp.adc_time_clock = xml_file->double_value("stm.adc_time_clock", 320.0);  // MHz
	// Sample period of ADC (microsec)
	exp.adc_time_clock_period = 1 / exp.adc_time_clock;
	;  // us
	// ADC offset clock frequency
	exp.adc_offset_time_clock =
	    xml_file->double_value("stm.adc_offset_time_clock", 125.0);
	;  // MHz

	// ADC prescale
	exp.prescale = xml_file->int_from_hex_value("stm.fmc144_prescale");

	// External mode ADC offset
	exp.ext_ADC_offset = xml_file->int_from_hex_value("stm.fmc144_ext_ADC_offset");
	// Slice length - external mode
	exp.ext_slice_length = xml_file->int_from_hex_value(
	    "stm.fmc144_ext_slice_length");  // Number of int16_t values
	// Number of slices - external mode
	exp.ext_slice_num = xml_file->int_from_hex_value("stm.fmc144_ext_slice_num");

	// The time T after a trigger to wait for a new trigger
	// before switching to external mode
	exp.ext_trig_timeout = xml_file->int_from_hex_value("stm.fmc144_ext_trig_timeout");

	// The offset in time to wait after external mode
	// before sending the first internal trigger.
	exp.ext_int_delay =
	    xml_file->int_from_hex_value("stm.fmc144_ext_int_delay");  // ticks

	// The number of internal triggers to generate.
	exp.int_trig_num = xml_file->int_from_hex_value("stm.fmc144_int_trig_num");
	// Internal mode ADC offset
	exp.int_ADC_offset = xml_file->int_from_hex_value("stm.fmc144_int_ADC_offset");
	// Slice length - internal mode
	exp.int_slice_length = xml_file->int_from_hex_value(
	    "stm.fmc144_int_slice_length");  // Number of int16_t values
	// Number of slices - internal mode
	exp.int_slice_num = xml_file->int_from_hex_value("stm.fmc144_int_slice_num");
...
int dataVars::checkExpConfig(expConfig exp)
{
	...
	{
		cout << "Error: Total external samping period is longer than macropulse width / "
		        "bunch period!"
		     << endl;
	...
	double intPeriod =
	    ...
	    -
	    (intADCoffset +
	     exp.int_sample_time);  // minus a single internal trigger period (effeictively int_ext_delay)
	...
	{
		cout << "Error: Total internal samping time is longer than remianing internal "
		        "period!"
		     << endl;
	...
	{
		cout << "Error: Number of internal triggers exceeds remaining internal period!"
		     << endl;
		...
		cout << "\tMaximum number of internal triggers allowed in reamining time = "
		     << floor(intPeriod /
		              (exp.int_trig_num * (intADCoffset + exp.int_sample_time)))
		     << endl;
		return 0;
	}
...
uint16_t dataVars::check_dead_beef(int16_t* data,
                                   uint64_t packet_start,
                                   uint16_t leftInPacket)
{
	// Get the packet number
	uint32_t pnum = (uint16_t)data[packet_start + 1] << 16 | (uint16_t)data[packet_start];
	...
	while(leftInPacket != 0)
	{
		...
		if((uint16_t)(data[loc] & 0xFFFF) != BEEF and
		   (uint16_t)(data[loc + 1] & 0xFFFF) != DEAD)
		{
			Logger::Instance()->write(0,
			                          "ERROR! End of packet " + std::to_string(pnum) +
