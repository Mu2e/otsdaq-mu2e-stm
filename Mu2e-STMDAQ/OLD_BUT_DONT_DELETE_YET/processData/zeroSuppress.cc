...
daqZS::daqZS(uint64_t buffer_len)
{
	// Check that the max data stored from the previous event
	// isn't larger than an on-spill event
	std::cout << "Max data stored in previous event is " << prev_data_max
	          << " ADC values = " << (double)prev_data_max * tadc << " us" << std::endl;
	if((double)prev_data_max * tadc > 1.675)
	{
		std::cout << "ERROR in ZS.hh! Maximum data stored from previous event is larger "
		             "than an on-spill event"
		          << std::endl;
		exit(0);
	}
	...
	{
		suppressed_data[i] = new int16_t[buffer_len];
	}
}

// ZS algorithm
std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, int64_t, int64_t> daqZS::do_ZS(
    int chan, uint64_t n, int16_t*& data, queue_buffer* pushq)
{
	...
	while(i < (n - window))
	{
		...
		while(j < event_end)
		{
			...
			if((j + n_average) > (n - window))
			{
				...
				memcpy(prev_data_end[chan],
				       &data[event_end - prev_data_max],
			...
			for(int k = 0; k < n_average; k++)
			{
				...
				// Store start of data
				if(adc_count < total_peak_max)
				{
					start_data[chan][adc_count] = grad_low;
				}
				...
				{
					// Store data
					suppressed_data[chan][sup_data_len] = grad_low;
			...
			if(avg_gradient < threshold && peak_found[chan] == false)
			{
				...
				if(peak_loc < i)
				{
					...
					memcpy(&suppressed_data[chan][sup_data_len],
					...
					for(int k = 0; k < navg_after_peak_loc; k++)
					{
						...
						if(peak_loc_in_prev + k < prev_data_max)
						{
							...
							suppressed_data[chan][sup_data_len] =
							    prev_data_end[chan][peak_loc_in_prev + k];
						}
						...
						else
						{
							...
							suppressed_data[chan][sup_data_len] =
							    data[peak_loc_in_prev - prev_data_max + k];
						}
						// Increment sup_data_len
						sup_data_len++;
						// Decrement left_to_store
				...
				else
				{
					...
					{
						// Store data
						suppressed_data[chan][sup_data_len] = data[peak_loc + k];
						// Increment sup_data_len
						sup_data_len++;
						// Decrement left_to_store
						left_to_store[chan]--;
	...
	// If we've stored any zs'd data
	//  if (sup_data_len > 0){
	// Push data to the queue
	//pushq.push(chan,suppressed_data,sup_data_len);
	// Memcpy the start of the ZS data to the dqm array
	//memcpy(dqm_pulse[chan],suppressed_data[chan],total_peak_max*sizeof(int16_t));
	//    }
	//  else{
	// Memcpy the start of the input data (noise) to the dqm array
	memcpy(dqm_pulse[chan], start_data[chan], total_peak_max * sizeof(int16_t));
	//  }

	// Calculate rate
	uint64_t rate = peak_count / (adc_count * tadc * 1e-6);

	return {sup_data_len, peak_count, rate, baseline_count, baseline_mean, baseline_rms};
}
