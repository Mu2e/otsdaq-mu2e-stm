...
public:
  ...
  // The suppressed data array
  int16_t* suppressed_data[CHNUM];

  // The pulses to be displayed by the DQM
  int16_t *dqm_pulse[CHNUM] = {new int16_t [total_peak_max] (),
			       new int16_t [total_peak_max] ()};

  // Start of data for noise in case of no pulses   
  int16_t *start_data[CHNUM] = {new int16_t [total_peak_max] (),
				new int16_t [total_peak_max] ()};
  ...
  int64_t b_mean[CHNUM] = {};
  // Baseline rms per channel
  int64_t b_rms[CHNUM] = {};
  // Baseline count per channel
  uint64_t b_count[CHNUM] = {};

  // Updated suppression factor (%)
  double sup_factor[CHNUM] = {};
