...
void checkData::check_packets(int chan, uint64_t data_len, int16_t* data)
{
	...
	if(data_len % MAX_PACKET_LEN != 0)
	{
		Logger::Instance()->write(
		    ...
		    "checkData::check_packets : Packet sizes incorrect! Channel = " +
		        udp.get_channel_name(chan) + ". Data size = " + std::to_string(data_len) +
		        " % MAX_PACKET_LEN = " + std::to_string(data_len % MAX_PACKET_LEN));
	}
	...
	while(count < data_len)
	{
		...
		uint32_t packet_num =
		    (uint16_t)data[packet_start + 1] << 16 | (uint16_t)data[packet_start];
...
void checkData::check_dropped_packets(int chan, uint32_t packet_num, uint32_t last_packet)
{
	...
	if(diff < 0)
	{
		Logger::Instance()->write(
		    0,
		    "Packet number mismatch! Channel = " + udp.get_channel_name(chan) +
		        ", previous packet number = " + std::to_string(last_packet) +
		        ", previous packet number = " + std::to_string(packet_num));
		std::cout << "Packet number mismatch! Channel = " << udp.get_channel_name(chan)
		          << ", previous packet number = " << std::to_string(last_packet)
		          << ", previous packet number = " << std::to_string(packet_num) << "\n";
	}
	...
	if(diff > 1)
	{
		...
		for(int j = 1; j < diff; j++)
		{
			...
			Logger::Instance()->write(
			    2,
			    "Dropped UDP packet warning: Channel = " + udp.get_channel_name(chan) +
			        ", missing packet number = " + std::to_string(last_packet + j));
			std::cout << "Dropped UDP packet warning: Channel = "
...
void checkData::check_pHdr(uint32_t packet_num, int16_t pHdr_end)
{
	...
	{
		// Log the dropped packet
		std::cout << "ERROR! Packet number " << std::to_string(packet_num)
		          << " has pHdrEnd " << std::to_string((uint16_t)pHdr_end)
		          << " instead of 65518 (0xFFEE)"
		          << "\n";

		Logger::Instance()->write(
		    0,
		    "ERROR! Packet number " + std::to_string(packet_num) + " has pHdrEnd " +
		        std::to_string((uint16_t)pHdr_end) + " instead of 65518 (0xFFEE)");
	}
}
		...
		if(data[hdr_start_loc] != fw_tHdr::HdrStart_data[fw_tHdr::HdrStart_len - 1])
		{
			Logger::Instance()->write(
			    ...
			    "ERROR! Event/trigger header mismatch!!\nPacket number = " +
			        std::to_string(pnum) + ". Event number = " + std::to_string(evNum));
		}
...
uint64_t checkData::check_fw_tHdr(int chan, int16_t* data, uint64_t hdr_index)
{
	...
	if(channel != chan)
	{
		...
		for(uint i = 0; i < fw_tHdr_Len / 2; i++)
		{
			...
			if((uint16_t)(data[hdr_index + i * 2] & 0xFFFF) == BEEF and
			   (uint16_t)(data[hdr_index + i * 2 + 1] & 0xFFFF) == DEAD)
			{
				// is 0xDEADBEEF
				deadbeef[chan] = true;
			}
		...
		Logger::Instance()->write(
		    0,
		    "ERROR! Channel number " + std::to_string(channel) +
	...
	if(EvNum != EWT)
	{
		Logger::Instance()->write(0,
		                          "ERROR! Event number " + std::to_string(EvNum) +
		...
		if(data[hdr_index + tHdr_vars.HdrStart_start + i] != tHdr_vars.HdrStart_data[i])
		{
			Logger::Instance()->write(
			    ...
			    "ERROR! Event number " + std::to_string(EvNum) +
			        ...
			        " instead of " + std::to_string(tHdr_vars.HdrStart_data[i]) +
			        " for header index " + std::to_string(tHdr_vars.HdrStart_start + i));
		}
	}
