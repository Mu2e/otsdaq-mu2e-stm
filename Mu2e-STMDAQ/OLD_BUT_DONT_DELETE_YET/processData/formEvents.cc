...
uint64_t formEvents::get_events(int           chan,
                                uint64_t      data_len,
                                int16_t*&     data,
                                queue_buffer* pushq)
{
	...
	if(data_len % MAX_PACKET_LEN != 0)
	{
		Logger::Instance()->write(
		    ...
		    "formEvents::get_events : Packet sizes incorrect! Channel = " +
		        std::to_string(chan) + ". Data size = " + std::to_string(data_len) +
	...
	while(count < data_len)
	{
		...
		while(leftInPacket > fw_tHdr_Len)
		{
			...
			if((uint16_t)(data[hdr_start_loc] & 0xFFFF) == BEEF and
			   (uint16_t)(data[hdr_start_loc + 1] & 0xFFFF) == DEAD and
			   (uint16_t)(data[hdr_start_loc + 2] & 0xFFFF) == BEEF and
			   (uint16_t)(data[hdr_start_loc + 3] & 0xFFFF) == DEAD)
			{
				...
				pushq->push(
				    chan, event_data[chan], acc_event_size[chan] / sizeof(int16_t));
			...
			if(first_event[chan])
			{
				...
				memcpy(current_event_data[chan], &data[hdr_start_loc], fw_tHdr_Size);
...
void formEvents::store_event(int chan)
{
	...
	uint16_t event_len_check =
	    (current_event_size[chan] - fw_tHdr_Size) / sizeof(int16_t);
	...
	if(event_len_check != current_event_len[chan])
	{
		Logger::Instance()->write(
		    0,
		    "ERROR in formEvents::get_events! Chan = " + std::to_string(chan) +
		        ". Event number = " + std::to_string(current_event_num[chan]) +
		        ". Stored event length = " + std::to_string(event_len_check) +
		        ". Given event length = " + std::to_string(current_event_len[chan]) +
		        ".");
	}
	...
	if(hdr_event_len != current_event_len[chan])
	{
		Logger::Instance()->write(
		    ...
		    "ERROR in formEvents::get_events! Chan = " + std::to_string(chan) +
		        ". Event number = " + std::to_string(current_event_num[chan]) +
		        ". Stored event length = " + std::to_string(event_len_check) +
		        ". Given event length = " + std::to_string(hdr_event_len) + ".");
	}
	...
	uint64_t copy_loc =
	    (acc_event_size[chan] - current_event_size[chan]) / sizeof(int16_t);
	// memcpy current event array to all event array
	memcpy(
	    &event_data[chan][copy_loc], current_event_data[chan], current_event_size[chan]);
