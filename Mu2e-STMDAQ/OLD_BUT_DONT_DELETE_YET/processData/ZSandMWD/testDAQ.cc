...
void allocation()
{
	...
	std::cout << "Using " << mem_percent_per_core * 100
	          << "% of available memory = " << free_mem * 1e-9 << " Gbytes" << std::endl;

	// Calculate rough data struct size with 1 off-spill event
	static const int    event_num_per_struct = 10;
	static const double struct_size_int16 =
	    event_num_per_struct * (off_spill_len + fw_tHdr_Len) * sizeof(int16_t);
	static const double struct_size_double =
	    event_num_per_struct *
	    (off_spill_len * sizeof(double) + fw_tHdr_Len * sizeof(int16_t));
	std::cout << "Each int16_t data struct has rough size " << struct_size_int16 * 1e-9
	          << " Gbytes" << std::endl;
	std::cout << "Each double data struct has rough size " << struct_size_double * 1e-9
	          << " Gbytes" << std::endl;

	// Get the ratio of double/int16_t struct sizes
	double double_int_ratio = struct_size_double / struct_size_int16;
	std::cout << "Ratio of double/int16_t struct sizes = " << double_int_ratio
	          << std::endl;
	...
	for(int i = 0; i < process_max; i++)
	{
		if(process[i])
		{
			...
			{
				std::cout << "Do not recognise queue type: " << queue_type[i]
				          << std::endl;
			}
		}
	}
	...
	double double_buffer_size =
	    double_qs * double_int_ratio / queue_factor * free_mem / double_qs;
	std::cout << "Allocating " << int_qs << " int16_t queues of size "
	          << int_buffer_size * 1e-9 << " GB" << std::endl;
	std::cout << "Allocating " << double_qs << " double queues of size "
	          << double_buffer_size * 1e-9 << " GB" << std::endl;

	// Create queues
	for(int i = 0; i < q_num; i++)
	{
		if(queue_type[i] == "int16_t")
			queues[i] = new queue_zs<int16_t>(event_num_per_struct, int_buffer_size);
		if(queue_type[i] == "double")
			queues[i] = new queue_zs<double>(event_num_per_struct, double_buffer_size);
	}
}
...
void do_orig_algs(int16_t* ADC)
{
	...
	std::cout << "MWD baseline = " << baseline.first << " ± " << baseline.second
	          << std::endl;
	...
	std::cout << "Data size after ZS: " << sup_data_len * sizeof(int16_t) * 1e-9
	          << " Gbytes" << std::endl;
	std::cout << "Number of triggers/peaks found: ZS = " << peak_count
	          << ", MWD = " << mwd_peaks1.npeaks << std::endl;
	std::cout << "ZS + MWD Algorithm computing time = " << time_ms << " milliseconds"
	          << std::endl;
	std::cout << "ZS + MWD computing speed = " << speed << " Gbit/s" << std::endl;
}
