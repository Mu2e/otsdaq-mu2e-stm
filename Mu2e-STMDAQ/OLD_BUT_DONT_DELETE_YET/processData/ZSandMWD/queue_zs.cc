...
// Constructor
template<typename T>
queue_zs<T>::queue_zs(int event_num, int buffer_size)
{
	size                    = buffer_size;
...
template<typename T>
bool queue_zs<T>::try_push(int chan, data_struct<T> data)
{
...
// Push data to queue
template<typename T>
void queue_zs<T>::push(int chan, data_struct<T> data)
{
	// Wait until pushed..
...
template<typename T>
bool queue_zs<T>::try_pull(int chan, data_struct<T>& data)
{
	// Initialise retval
	uint64_t retval = 0;
...
template<typename T>
int queue_zs<T>::pull(bool* timeout, int chan, data_struct<T>& data)
{
	int retval = 0;
