...
void MWD::deconv_thread(int                chan,
                        queue_zs<int16_t>& pullq,
                        queue_zs<double>&  pushq,
                        bool*              timeout)
{
...
void MWD::diff_thread(int               chan,
                      queue_zs<double>& pullq,
                      queue_zs<double>& pushq,
                      bool*             timeout)
{
...
data_struct<double> MWD::deconvolute(int chan, data_struct<int16_t>& datas)
{
	...
	for(uint64_t i = start; i < n; i++)
	{
		...
		double a_double =
		    data[i] - (1 - (T0 / tau)) * last_deconv_data[chan] + last_a[chan];
...
// Find peaks
MWD_peaks MWD::find_peaks(
    uint64_t n, double* l, double baseline_mean, double baseline_rms, double time_offset)
{
