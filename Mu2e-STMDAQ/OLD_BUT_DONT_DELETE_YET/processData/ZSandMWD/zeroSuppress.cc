...
void daqZS::pulse_thread(int                chan,
                         queue_zs<int16_t>& pullq,
                         queue_zs<int16_t>& pushq,
                         bool*              timeout)
{
...
void daqZS::suppress_thread(int                chan,
                            queue_zs<int16_t>& pullq,
                            queue_zs<int16_t>& pushq,
                            bool*              timeout)
{
...
// ZS algorithm
data_struct<int16_t> daqZS::split_headers(int chan, uint64_t n, int16_t*& data)
{
	...
	if(!first_pass[chan])
	{
		...
		// Copy in the data
		memcpy(
		    event_data.adc_data, prev_data_end[chan], prev_num[chan] * sizeof(int16_t));
	...
	memcpy(
	    prev_data_end[chan], &data[n - prev_num[chan]], prev_num[chan] * sizeof(int16_t));
...
// ZS algorithm
uint64_t daqZS::find_peaks(int chan, data_struct<int16_t>& datas)
{
	// The adc length
...
// Suppress data
uint64_t daqZS::suppress_data(int chan, data_struct<int16_t>& datas)
{
	// The adc length
	uint64_t n = datas.data_len;
		...
		else
		{
			...
			else
			{
				...
				if(loc_copy_from < 0)
				{
					...
					// Memcpy the data
					memcpy(&suppressed_data[sup_data_len],
					       &prev_peak_overlap[chan][before_peak_max - to_copy],
	...
	memcpy(prev_peak_overlap[chan],
	       &data[n - before_peak_max],
	       before_peak_max * sizeof(int16_t));

	// Delete suppressed data
	//  delete suppressed_data;
