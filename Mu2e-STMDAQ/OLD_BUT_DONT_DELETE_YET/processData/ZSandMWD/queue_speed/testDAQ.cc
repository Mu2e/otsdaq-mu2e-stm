...
int main(int argc, char* argv[])
{
	...
	std::cout << "Each data struct has rough size " << struct_size * 1e-9 << " Gbytes"
	          << std::endl;
	...
	std::cout << "Setting queue sizes to " << mem_percent_per_core * 100
	          << "% of available memeory" << std::endl;
	uint64_t mem_per_queue = free_mem / q_num * mem_percent_per_core;

	std::cout << q_num << " queues = " << mem_per_queue * 1e-9 << " Gbytes per queue"
	          << std::endl;
	...
	std::cout << "Available system memory after queue allocation  = " << free_mem * 1e-9
	          << " Gbytes" << std::endl;

	// Start thread
	std::thread* start_thread = new std::thread(
	    start, 0, event_data, event_array_len, std::ref(*queues[0]), event_num);
	...
	for(int i = 0; i < q_num - 1; i++)
	{
		threads[i] = new std::thread(
		    queue_thread, 0, i, std::ref(*queues[i]), std::ref(*queues[i + 1]), timeout);
	}
