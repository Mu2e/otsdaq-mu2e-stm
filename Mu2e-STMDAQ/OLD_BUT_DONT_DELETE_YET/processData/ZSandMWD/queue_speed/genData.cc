...
{
	// Print for user
	std::cout << "\nForming data into events..." << std::endl;
	...
	for(uint i = 0; i < eventNum; i++)
	{
		...
		// Form event header
		int16_t* tH_event = form_tHdr(
		    c, DTCclock, ADCclock, eventNum, EWT, EM, DRTDC, eventStart, eventLength);
	...
	{  // only once
		std::cout << "Number of events = "
		          << " = " << numberhb << std::endl;
		std::cout << "Event length = "
		          << " = " << deltahb << " = " << deltahb_ns << " ns = " << deltahb_s
		          << " s"
