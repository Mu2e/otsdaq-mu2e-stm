...
public:
  ...
  data_struct<int16_t> data_buffer[CHNUM][buffer_num];
  ...
  void pulse_thread(int chan, queue_zs<int16_t> &pullq, queue_zs<int16_t> &pushq, bool *timeout);

  // Thread to suppress data
  void suppress_thread(int chan, queue_zs<int16_t> &pullq, queue_zs<int16_t> &pushq, bool *timeout);
  
  // Split the headers from the data
  data_struct<int16_t> split_headers(int chan, uint64_t data_len, int16_t* &data);

  // Find the peaks in the data
  uint64_t find_peaks(int chan, data_struct<int16_t> &data);

  // Zero suppress the data
  uint64_t suppress_data(int chan, data_struct<int16_t> &data);
