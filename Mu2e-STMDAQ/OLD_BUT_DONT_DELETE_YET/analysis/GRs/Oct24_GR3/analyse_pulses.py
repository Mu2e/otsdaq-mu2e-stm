...
def getEventHdr(header):
    ...
    # Get event number
    EN = np.uint64(0)
    EN_0 = (nheader[4]).astype(np.uint16);
    EN_1 = (nheader[5]).astype(np.uint16);
    EN_2 = (nheader[6]).astype(np.uint16);
    EN = EN_2 << 32 | EN_1 << 16 | EN_0;    
    if (output): print("Event Number = ",EN)

    # Get event window tag
    EWT = np.uint64(0)
    EWT_0 = (nheader[7]).astype(np.uint16);
    EWT_1 = (nheader[8]).astype(np.uint16);
    EWT_2 = (nheader[9]).astype(np.uint16);
