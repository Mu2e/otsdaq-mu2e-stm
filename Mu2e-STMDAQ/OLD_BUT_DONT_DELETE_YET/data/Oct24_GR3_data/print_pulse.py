...
def getEventHdr(header):
    ...
    # print("%d = %d << 40 | %d << 24 | %d << 8 | %d" % (TM40,TM40_3,TM40_2,TM40_1,TM40_0))
    # if (EN == 2): exit(0)
    # if (TM40 < 0):
    #     print (TM40_0,int(TM40_0) & 0xFFFF)
    #     print("%d = %d << 48 | %d << 32 | %d << 16 | %d" % (TM40,TM40_3,TM40_2,TM40_1,TM40_0))
    #     exit(0)
...
def getADCdata(data):
    ...
    while(data_count < len(data)):
        ...
        # Get event start in adc counts
        e_start.append(adc_count)
    ...
    return adc_data,eHdr_data,e_start
...
event_starts = output_data[2]
output_data = output_data[0]
print(len(event_starts))
for i in range(len(event_starts)):
    count_diff = event_starts[i] - event_starts[i-1]
    dtc_diff = (output_headers[i][-1] - output_headers[i-1][-1])*1e-3
    adc_diff = (output_headers[i][0] - output_headers[i-1][0])*1e-3
    if (i != 0):
        #print(i,count_diff/(300e6)*1e6,dtc_diff,adc_diff)
        if (dtc_diff < 0):
            print(output_headers[i][-1],output_headers[i-1][-1])
            exit(0);
        if (adc_diff < 0):
            print(output_headers[i][0],output_headers[i-1][0])
            exit(0);
...
for i in range(len(output_data)):
    EWT = output_headers[event_count][1]
    print(i,"EWT = ",EWT,output_data[i])
    if (output_data[i] > 20000):
        avg_i += i
        avg_count += 1
        #print(i,i/(300e6)*1e6,output_data[i])
    else:
        if (avg_i != 0):
            avg_time = avg_i/avg_count
            pulse_sep = avg_time-old_avg
            if (pulse_num < 30):
                print(pulse_num,event_count,EWT,avg_time,pulse_sep,pulse_sep/(300e6)*1e6,)
                # if (event_time_count > 75):
                #     print(pulse_num,EWT,(avg_time-event_starts[event_count])/(300e6)*1e6,pulse_sep/(300e6)*1e6,)
            pulse_num += 1
            old_avg = avg_time
        avg_i = 0
        avg_count = 0
    event_time_count += 1/(300e6)*1e6 
    if (i == event_starts[event_count]):
        event_count += 1
        event_time_count = 0
        #if (pulse_num < 30): print(event_count, ": EWT = ", EWT, " (at ", event_starts[event_count], " adc clock ticks)")
