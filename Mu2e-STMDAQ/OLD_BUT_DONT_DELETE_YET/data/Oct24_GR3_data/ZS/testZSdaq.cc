...
// ZS queue class
queue_zs event_queue;

// ZS queue class
queue_zs pulse_queue;
...
void pull_from_zs(int chan, int16_t*& data)
{
	std::cout << "In test pull thread" << std::endl;

	// Data buffer
	data_struct data_call;
	...
	{
		// Pull data from queue
		retval = sup_queue.pull(timeout, chan, data_call);
...
int main(int argc, char* argv[])
{
	...
	std::thread* push_zs_thread = new std::thread(push_to_zs, 0, suppressed_data_2);
	...
	// Start test pull
	std::thread* pull_zs_thread =
	    new std::thread(pull_from_zs, 0, std::ref(suppressed_data_2));
	...
	for(int i = 0; i < sup_data_len; i++)
	{
		...
		{
			std::cout << i << " /  " << sup_data_len << "   " << suppressed_data_1[i]
			          << "   " << suppressed_data_2[i] << std::endl;
			fail_count++;
			success = false;
		}
	}
	...
	if(!success)
	{
		std::cout << fail_count << "/" << sup_data_len << " ("
		          << (double)fail_count / (double)sup_data_len * 100
		          << "%) incorrect elements." << std::endl;
