...
ZeroSuppress::ZeroSuppress()
{
	...
	logger->log("ZeroSuppress: ADC sampling frequency = " + std::to_string(fADC) +
	                " MHz (" + std::to_string(tADC) + " us per ADC value).",
	            1);
	...
	logger->log("ZeroSuppress: Storing " + std::to_string(before_peak_time) + " us / " +
	                std::to_string(before_peak) + " ADC values / " +
	                std::to_string(before_peak * sizeof(int16_t)) + " bytes before peak.",
	            1);
	...
	logger->log("ZeroSuppress: Storing " + std::to_string(after_peak_time) + " us / " +
	                std::to_string(after_peak) + " ADC values / " +
	                std::to_string(after_peak * sizeof(int16_t)) + " bytes after peak.",
	            1);
	...
	logger->log("ZeroSuppress: Storing a total of " + std::to_string(total_peak / fADC) +
	                " us / " + std::to_string(total_peak) + " ADC values / " +
	                std::to_string(total_peak * sizeof(int16_t)) + " bytes per peak.",
	            1);
	...
	logger->log(
	    "ZeroSuppress: HPGe decay time constant = " + std::to_string(hpge_decay_time) +
	        " us / " + std::to_string(hpge_decay) + " ADC values.",
	    1);

	// Gradient threshold
	threshold = cfg.getValue<int>("stm.zs.hpge.grad_threshold");
	logger->log("ZeroSuppress: Gradient threshold = " + std::to_string(threshold) + ".",
	            1);

	// Gradient window
	window = cfg.getValue<int>("stm.zs.hpge.grad_window");
	logger->log(
	    "ZeroSuppress: Gradient window = " + std::to_string(window) + " ADC values.", 1);

	// Gradient average
	n_average = cfg.getValue<int>("stm.zs.hpge.n_avg");
	logger->log("ZeroSuppress: Gradient average number = " + std::to_string(n_average) +
	                " ADC values.",
	            1);
	...
	logger->log("ZeroSuppress: Storing " + std::to_string(max_noise_time) + " us / " +
	                std::to_string(max_noise_len) +
	                " ADC values of noise data per buffer.",
	            1);
}
...
void ZeroSuppress::find_peaks(std::shared_ptr<DataStruct>& buffer)
{
	...
	buffer->baseline_rms_current =
	    std::sqrt((baseline_sum2 * inv_count) - baseline_mean * baseline_mean);
	...
	{
		std::string error =
		    "ERROR in ZeroSuppress::find_peaks. For last EWT in buffer = " +
		    std::to_string(last_EWT) + ", total data size (" +
		    std::to_string(buffer->raw_len) +
		    ") != accumulated data size in header map (" + std::to_string(total) + ")";
		logger->log(error, 0);
	}
...
void ZeroSuppress::suppress_data(std::shared_ptr<DataStruct>& buffer)
{
	...
	for(int i = 0; i < peak_num; i++)
	{
		...
		else
		{
			...
			else
			{
				...
				if(i > 0)
				{  // Avoid overlapping peaks for simplicity
					if(peak_sep > max_sep)
					{
						max_sep       = peak_sep;
						max_sep_index = i;
	...
	std::copy(
	    data->begin() + (n - before_peak), data->begin() + n, prev_peak_data.begin());
