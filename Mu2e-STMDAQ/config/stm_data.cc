...
// Constructor
STMdata::STMdata(Config& cfg_, const std::shared_ptr<AsyncLogger>& logger_)
    : cfg(cfg_)
    , logger(logger_)
    , master_config(cfg, logger)
    ,  // STMDAQ master config
    fw_config(cfg, logger)
    ,  // firmware config
    mu2e_config(cfg, logger, fw_config)
    ,  // mu2e config
    udp_config(cfg, logger, master_config)
    ,  // udp config
    zs_config(cfg, logger, master_config, fw_config, mu2e_config)
    ,  // zero suppression config
    prescale_config(cfg, logger)
    ,  // prescale config
    write_config(cfg, logger)
    ,  // file writing config
    buffer_config(cfg, logger, fw_config, mu2e_config, zs_config, MAX_PACKET_SIZE)
    ,  // buffer config
    baseline_config(
        cfg, logger, fw_config, buffer_config, MAX_PACKET_LEN - pHdr_Len - fw_eHdr_len)
    ,  // adc baseline config
    mwd_config(cfg, logger, fw_config, baseline_config)
    ,  // mwd config
    pulseheight_config(cfg, logger, fw_config, baseline_config)
    ,  // pulse height config
    tcp_config(cfg, logger, master_config)
    ,                                   // tcp config
                                  ...
                                  uint16_t leftInPacket)
{
	...
	while(leftInPacket != 0)
	{
		   ...
		   (uint16_t)(data[loc + 1] & 0xFFFF) != DEAD)
		{
			if(logger)
			{
				logger->log("ERROR! End of packet " + std::to_string(pnum) +
			...
			{
				std::cout << "ERROR! End of packet " << pnum
