...
int main()
{
	...
	if(host == stm->fw_config.ctrl_srvr)
	...
	if(!stop::should_stop())
		om = std::make_shared<OperationManager>(cfg, logger, stm, signal);
	...
	if(om->class_num() > 0)
	{
		...
		if(!stop::should_stop())
			pool = std::make_shared<BufferPool>(cpu, logger, stm, om);

		// Instance of thread manager
		if(!stop::should_stop())
			std::shared_ptr<ThreadManager> tm =
			    std::make_shared<ThreadManager>(cpu, logger, stm, signal, pool, om, hw);
	}
