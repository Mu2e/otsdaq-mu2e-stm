...
struct HIDDEN HardwareManager::Impl
{
	    ...
	    : pb_adaptor(py::module::import("sys"))
	{
		...
		if(!fs::exists(hw_dir))
		{
			logger_->log("HardwareManager::Impl: hardware directory (" + hw_dir +
			                 ") does not exist!",
			             0);
			std::this_thread::sleep_for(std::chrono::seconds(1));
			return;
		...
		{
			logger_->log("HardwareManager::Impl: hardware path (" + hw_dir +
		...
		{
			logger_->log("HardwareManager::Impl: pybind adaptor script (" + hw_dir + "/" +
			                 pb + ".py" + ") does not exist!",
			             0);
			std::this_thread::sleep_for(std::chrono::seconds(1));
			return;
		}
		...
		run_and_get = pb_adaptor.attr(
		    stm_->fw_config.python.pybind.second.c_str());  // hardware/run_and_get.py
	}
    ...
    , impl(std::make_unique<Impl>(logger_, stm_))
{
	...
	if(!stop::should_stop() && !stm->master_config.use_sw_sim)
	{
		...
		{
			logger->log(
			    "HardwareManager::ps_mem: PS memory not ready, you will need to reboot "
			    "the board PetaLinux!. Exiting...",
			    0);
			std::this_thread::sleep_for(std::chrono::seconds(1));
			return;
		}
		...
		{
			logger->log(
			    "HardwareManager::init_adc: ADC initialisation failed!. Exiting...", 0);
			std::this_thread::sleep_for(std::chrono::seconds(1));
			return;
		}
...
bool HardwareManager::load_firmware()
{
	...
	const std::string ssh_addr =
	    stm->fw_config.load_fw.petalinux_user + "@" + stm->fw_config.load_fw.petalinux_ip;
	// Get remote petalinux ssh address
	...
	{
		logger->log(
		    "HardwareManager::load_firmware: Failed to start command (system() error)",
		    2);
		return false;
	}
	...
	if(WIFEXITED(rc))
	{
		...
		{
			logger->log("HardwareManager::load_firmware: Command failed, exit code " +
			                std::to_string(code),
			            2);
			return false;
		}
	...
	{
		logger->log("HardwareManager::load_firmware: Command killed by signal " +
