...
#include "Mu2e-STMDAQ/processing/operation_manager.hh"  // include the concrete class here
  ...
  buffer_pool_total(buffer_pool_size * thread_num)
{
  ...
  for (size_t i = 0; i < thread_num; i++) op_names[i] = selectedOperations[i].first;
