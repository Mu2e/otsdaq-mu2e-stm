...
template <typename T>
DQMBlock<T>::DQMBlock(const std::string& shm_name, size_t shm_size)
  : shm_name_(shm_name), shm_size_(shm_size) {

  // Remove existing segment and recreate
  bip::shared_memory_object::remove(shm_name_.c_str());  
  shm_ = std::make_unique<bip::shared_memory_object>(
	bip::open_or_create, shm_name.c_str(), bip::read_write);
...
void DQM::registerBlock(DQMPageType type, const std::string& shm_name, size_t size) {
  ...
  blocks_[type] = std::make_unique<DQMBlock<T>>(shm_name, size);
  logger->log("Registered DQM shared memory block: " + shm_name +
