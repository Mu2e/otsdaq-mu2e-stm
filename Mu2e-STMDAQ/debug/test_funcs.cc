...
// Constructor
TestFuncs::TestFuncs(const std::shared_ptr<SignalHandler>& signal_) : signal(signal_)
{
	// FOR TESTING ONLY
	...
	register_operation("mod_prev_buffer", [this](auto& b) { mod_prev_buffer(b); });
	register_operation("read_mod_buffer", [this](auto& b) { read_mod_buffer(b); });
	register_operation("genData", [this](auto& b) { genData(b); });
	register_operation("addOne", [this](auto& b) { addOne(b); });
	register_operation("confirmData", [this](auto& b) { confirmData(b); });
	register_operation("do_nothing", [this](auto& b) { do_nothing(b); });
	register_operation("print", [this](auto& b) { print(b); });
	register_operation("printPackets", [this](auto& b) { print_packets(b); });
	register_operation("check_form_events", [this](auto& b) { check_form_events(b); });
	register_operation("check_prep_zs", [this](auto& b) { check_zs(b); });
	register_operation("check_zs", [this](auto& b) { check_zs(b); });
}

// FOR TESTING ONLY - Generate data
void TestFuncs::mod_prev_buffer(std::shared_ptr<DataStruct>& buffer)
{
	std::cout << "mod_buffer_count = " << mod_buffer_count << std::endl;
	...
	{
		++mod_buffer_count;
		prev_buffer_tot += prev_buffer->raw_len;
		string msg =
		    "mod_buffer_count = " + std::to_string(mod_buffer_count) +
		    ", prev_buffer->raw_len = " + std::to_string(prev_buffer->raw_len) +
		    ", prev_buffer_tot = " + std::to_string(prev_buffer_tot) +
		    ", this_buffer->raw_len = " + std::to_string(n) +
		    ", this_buffer_tot = " + std::to_string(this_buffer_tot) +
		    ", difference = " + std::to_string(this_buffer_tot - prev_buffer_tot) +
		    " =? " + std::to_string(n);
		std::cout << msg << std::endl;
		prev_buffer->peak_finding_finished.store(true, std::memory_order_release);
	}

	prev_buffer      = buffer;
	first_buffer_mod = false;
}

// FOR TESTING ONLY - Generate data
void TestFuncs::read_mod_buffer(std::shared_ptr<DataStruct>& buffer)
{
	std::cout << "read_buffer_count = " << read_buffer_count << std::endl;

	bool write = true;
	// Wait find_peaks finishes writing
	while(!buffer->peak_finding_finished.load(std::memory_order_acquire) &&
	      !stop::should_stop())
	{
		if(write == true)
		{
			std::cout << "waiting for boolean switch" << std::endl;
			write = false;
		}
		// simple spin, or use backoff/yield/sleep if needed
	...
	{
		std::cout << "Boolean switch flipped" << std::endl;
	}
...
void TestFuncs::confirmData(std::shared_ptr<DataStruct>& buffer)
{
	for(size_t i = 0; i < buffer->raw_len; ++i)
	{
		...
		{
			std::cout << "BUFFER 1 ERROR: " << i << "," << buffer->raw[i]
			          << " != " << test << std::endl;
			exit(0);
		}
		...
		{
			std::cout << "BUFFER 2 ERROR: " << buffer->zs[i] << " != " << test2
			          << std::endl;
			exit(0);
		}
		int16_t test3 = static_cast<int16_t>(static_cast<double>(test2) / 4.0);
		if(buffer->ph[i] != test3)
		{
			std::cout << "BUFFER 3 ERROR: " << buffer->ph[i] << " != " << test3
			          << std::endl;
			exit(0);
		}
	}
...
// FOR TESTING ONLY
void TestFuncs::do_nothing(std::shared_ptr<DataStruct>& buffer) {}
...
void TestFuncs::print_packets(std::shared_ptr<DataStruct>& buffer)
{
	...
	if(buffer->buffer_num == 0)
	{
		...
		std::cout << "-------------------------------" << std::endl;
		std::cout << "Printing first " << print_num << " elements of first " << packet_num
		          << " packets..." << std::endl;
...
void TestFuncs::print(std::shared_ptr<DataStruct>& buffer)
{
	...
	std::cout << " Packets = " << buffer->packet_count
	          << " (dropped = " << buffer->dropped_packet_count << ")\n";
	...
	// }
	std::cout << " Events = " << buffer->EWT_count
	          << " (lost = " << buffer->lost_EWT_count << ")\n";
...
void TestFuncs::check_form_events(std::shared_ptr<DataStruct>& buffer)
{
	...
	//       sw_event_header hdr = std::get<hdrMap_hdrData>(raw);
	//       if (i == 0) std::cout << "---" << std::endl;
	//       std::cout << std::get<hdrMap_EWT>(raw)
	//                 << "  " << std::get<hdrMap_event_start>(raw)
	//                 << "  " << std::get<hdrMap_event_len>(raw);
	//       std::cout << "\n";
	//       if (i == buffer->raw_header_num-1) std::cout << "---" << std::endl;
	//     }
	//   }
	//   max += std::get<hdrMap_event_len>(raw);
	// }
...
void TestFuncs::check_prep_zs(std::shared_ptr<DataStruct>& buffer)
{
	...
	//     std::array<int16_t,fw_eHdr_len> zs_hdr = std::get<hdrMap_hdrData>(zs);
	//     if (i < buffer->raw_header_num){
	// 	std::tuple raw = buffer->raw_header_map[i];
	// 	std::array<int16_t,fw_eHdr_len> raw_hdr = std::get<hdrMap_hdrData>(raw);
	// 	std::cout << "raw: " << std::get<hdrMap_EWT>(raw)
	// 		  << "  " << std::get<hdrMap_adcIndex>(raw)
	// 		  << "  " << std::get<hdrMap_dataLen>(raw)
	// 		  << "  " << raw_hdr[fw_eHdr.EvStart]
	// 		  << "  " << raw_hdr[fw_eHdr.EvInPacket]
	// 		  << "  " << raw_hdr[fw_eHdr.EvLen]
	// 		  << ",  zs: " << std::get<hdrMap_EWT>(zs)
	// 		  << "  " << std::get<hdrMap_adcIndex>(zs)
	// 		  << "  " << std::get<hdrMap_dataLen>(zs)
	// 		  << "  " << zs_hdr[fw_eHdr.EvStart]
	// 		  << "  " << zs_hdr[fw_eHdr.EvInPacket]
	// 		  << "  " << zs_hdr[fw_eHdr.EvLen];
	//     }
	//     else{
	// 	std::cout << "raw: " << "  -  "
	// 		  << "  " << "  -  "
	...
	// 		  << "  " << zs_hdr[fw_eHdr.EvLen];
	//     }
	//     // std::cout << " " << hdr[fw_eHdr.EvStart] << " " << hdr[fw_eHdr.EvInPacket] << " " << hdr[fw_eHdr.EvLen];
	//     std::cout << "\n";
	//   }
...
void TestFuncs::check_zs(std::shared_ptr<DataStruct>& buffer)
{
	...
	for(int i = 0; i < n - buffer->zs_overflow_num; i++)
	{
		...
		if((*data)[i] != low_check)
		{
			...
			{
				std::cout << "LOW DOESN'T MATCH: i = " << i << ", data = " << (*data)[i]
				          << ", check = " << low_check << std::endl;
				low_print_count++;
			}
			else
			{
		...
		if((*data)[i + window] != high_check)
		{
			...
			{
				std::cout << "HIGH DOESN'T MATCH: i = " << i << ", data = " << (*data)[i]
				          << ", check = " << high_check << std::endl;
				high_print_count++;
			}
			else
			{
