...
#include "Mu2e-STMDAQ/sim/gen_eventData.hh"
// Include gen data code
#include "Mu2e-STMDAQ/sim/send_TCP.hh"
...
void send_thread(const std::shared_ptr<BufferQueue<std::vector<int16_t>>>& input_queue,
                 const std::atomic<bool>& add_headers_finished,
                 size_t                   nevents)
{
	// The buffer number
	//size_t buffer_num = 0;
...
// Add headers thread
void hdr_TCP_thread(
    const std::shared_ptr<BufferQueue<std::vector<int16_t>>>& input_queue,
...
int main(int argc, char* argv[])
{
	...
	std::shared_ptr<SignalHandler> signal =
	    std::make_shared<SignalHandler>(nullptr, nullptr);
	...
	// Print to user
	std::cout << "Executing STMDAQ data simulation with the following " << argc
	          << " inputs: " << std::endl;

	if(argc != 4)
	{
		std::cout << "Incorrrect argument input. Please re-run with the following list:"
		          << std::endl;
		std::cout << "./build/bin/simdaq.exe [number of events] [event length (ns)] "
		             "[raw_prescale]"
		          << std::endl;
		return 0;
	}
...
*/
	...
	// Generate data
	std::shared_ptr<GenEvents> gen = std::make_shared<GenEvents>(
	    stm, event_num, event_len, prescale, adcCountsAroundPeakZS);
	...
	// Add header thread
	std::thread header_thread(hdr_TCP_thread,
	                          std::ref(queue_gen_to_hdr),
	                          std::ref(queue_hdr_to_send),
	                          std::ref(gen_events_finished),
	                          std::ref(add_headers_finished));

	// Sender thread
	std::thread sender_thread(send_thread,
	                          std::ref(queue_hdr_to_send),
	                          std::ref(add_headers_finished),
	                          event_num);
