             ...
             const std::atomic<bool>&                                  finished)
{
	...
	// Print sending speed
	std::cout << "Sent " << std::fixed << std::setprecision(2) << gbytes << " Gbytes ("
	          << packet_num << " packets) at " << std::fixed << std::setprecision(2)
...
int main(int argc, char* argv[])
{
	...
	// Print to user
	std::cout << "Executing packet generation programme with the following " << argc
	          << " inputs: " << std::endl;
	...
	else
	{
		std::cout << "Incorrrect argument input. Please re-run with the following list:"
		          << std::endl;
		std::cout << "./run.exe [number of events] [event length (ns)] [input binary "
		             "file (OPTIONAL)]"
		          << std::endl;
		return 0;
	}
	...
	int CHAN = cfg.getValue<int>("channel");

	// Generate data
	std::shared_ptr<GenData> gen =
	    std::make_shared<GenData>(CHAN, event_num, event_len, filename);
	...
	std::thread consumer_thread(
	    consume, std::ref(udp), std::ref(queue), std::ref(finished));
