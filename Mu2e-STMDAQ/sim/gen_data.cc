...
// Constructor
GenData::GenData(size_t             channel_,
                 size_t             event_num_,
                 size_t             event_period_,
    ...
    , event_len((double)event_period * 1e-9 * fADC * 1e6)
{
	...
	// Notify user
	std::cout << "Event len = " << event_period << " ns = " << event_len
	          << " ADC values = " << event_size << " bytes." << std::endl;
	std::cout << "Event num = " << event_num << " events = " << data_len
	          << " ADC values = " << (double)data_len * sizeof(int16_t) * 1e-9 << " GB."
	          << std::endl;
	...
	else
	{
		...
		double events_in_file =
		    static_cast<double>(data_size) / static_cast<double>(event_size);
		// Calculate repetition number of file
		double nfiles_in_data = static_cast<double>(data_len * sizeof(int16_t)) /
		                        static_cast<double>(data_size);
		std::cout << "There are " << events_in_file << " " << event_len
		          << " ns events in filepath" << std::endl;
		std::cout << "Choose from the following two options:" << std::endl;
		std::cout << "A) Send the user input number of events: " << event_num
		          << " events = " << nfiles_in_data << " * the input file." << std::endl;
		...
		while(true)
		{
			...
			if(input == 'a' or input == 'A')
			{
				...
				std::cout << "Input file will be repeated " << nfiles_in_data << " times."
				          << std::endl;
				break;
			}
			else if(input == 'b' or input == 'B')
			{
				...
				while(true)
				{
					std::cout << "Input the number times the file be repeated:"
					          << std::endl;
					std::string n;
					std::cin >> n;
					try
				...
				data_len = event_len * event_num;
				std::cout << "Event number changed to " << event_num << " events."
				          << std::endl;
				// Notify user
				std::cout << "Event len = " << event_period << " ns = " << event_len
				          << " ADC values = " << event_size << " bytes." << std::endl;
				std::cout << "Event num = " << event_num << " events = " << data_len
				          << " ADC values = " << (double)data_len * sizeof(int16_t) * 1e-9
				          << " GB." << std::endl;
				break;
			}
			else
	...
	std::cout << print_string << " data, size = " << data_size * 1e-9 << " GB."
	          << std::endl;

	// Make sure the data size is larger than the event size
	if(data->size() < event_len)
	{
		std::cout << "ERROR: Please ensure generated data length (" << data->size()
		          << " ADC values) is >= the event length " << event_len
		          << " ADC values). Exiting..." << std::endl;
		exit(0);
	}
}
                          ...
                          std::atomic<bool>&                    finished)
{
	...
	{
		// Empty packet
		std::shared_ptr<std::vector<int16_t>> packet =
		    std::make_shared<std::vector<int16_t>>(MAX_PACKET_LEN);
		...
		while(left_in_packet > 0)
		{
			...
			// Create event header
			std::vector<int16_t> header = form_event_header(
			    channel, EWT, 1, this_event_len, event_start, event_in_packet);
			...
			if(data_count + event_in_packet == data->size())
				copy_end = data->size();
			...
			if(data_count == data_len)
			{
				...
				{
					// Create event header
					std::vector<int16_t> header =
					    form_event_header(channel, 0, 0, 0, 0, 0);
		...
		// Print final EWT
		if(EWT == std::ceil(event_num))
			std::cout << "Final EWT = " << EWT << std::endl;
	...
	if(new_packet_for_nullhb)
	{
		...
		// Create event header
		std::vector<int16_t> header = form_event_header(channel, 0, 0, 0, 0, 0);
	...
	// Notify user
	std::cout << "Generated " << EWT << " events (" << packet_num << " packets)."
	          << std::endl;
}
                                                ...
                                                size_t event_in_packet)
{
	...
	// Copy header anchor
	std::copy(
	    std::begin(fw_eHdr.anchor_data), std::end(fw_eHdr.anchor_data), header.begin());
	// ADC Clock 0-3
