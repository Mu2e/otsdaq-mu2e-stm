...
// Constructor
GenEvents::GenEvents(const std::shared_ptr<STMdata>& stm_,
                     size_t                          event_num_,
                     size_t                          event_period_,
                     size_t                          prescale_,
                     int                             adcCountsAroundPeakZS_)
    : stm(stm_)
    , fADC(stm->fw_config.fADC * 1e6)
    ,  // Hz
    tADC(stm->fw_config.tADC)
    ,  // s
...
//calculate peak
double GenEvents::calculate_peak(
    uint16_t time, uint16_t Amp, uint16_t tshift, uint16_t tfall, uint16_t tdecay)
{
    ...
    std::atomic<bool>&                                        finished)
{
	...
	{
		const sw_event_header hdr_temp =
		    stm->create_sw_eHdr(EWT, ADCclock, Ch_DTCclk, EM);

		std::shared_ptr<std::vector<int16_t>> event_data =
		    std::make_shared<std::vector<int16_t>>(
		        sw_eHdr.len + event_len + 2 * maxpeaks +
		            (adcCountsAroundPeakZS + 2) * maxpeaks,
		        0);
		...
		if(max_events_per_struct >= event_num)
			max_events_requested = event_num;
		else
			max_events_requested = max_events_per_struct;
		...
		{
			std::uniform_int_distribution<size_t> di{sw_eHdr.len + adc_count,
			                                         sw_eHdr.len + adc_count + event_len};
			peakposes[i] = di(gen);
			//
		...
		for(int p = 0; p < npeaks; p++)
		{
			...
			for(int i = 0; i < event_len; i++)
			{
				...
				// Get peak sample
				double peak =
				    GenEvents::calculate_peak(sw_eHdr.len + i, peakAmp, tshift, 10, 1000);
		...
		for(int p = 0; p < npeaks; p++)
		{
			...
			std::cout << "gen peakposes (us): " << peakposes[p] * 10.0 / 3000
			          << std::endl;
			;
			++event_data_count;
			// Pulse height
