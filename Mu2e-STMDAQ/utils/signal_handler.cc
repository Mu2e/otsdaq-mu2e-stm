...
void SignalHandler::handleSignal(int signal)
{
	...
	{
		instance->logger->log("SignalHandler: Signal " + std::to_string(signal) +
		                          " received. Awaiting user confirmation.",
		                      1);
	}
}
...
void SignalHandler::listenForSignals()
{
	...
	while(!stop::should_stop())
	{
		...
		std::cout << "\nCaught interrupt. Are you sure you want to exit? (y/n): "
		          << std::endl;

		// Log the confirmation request (if logging is enabled)
		if(instance && instance->logger)
		{
			instance->logger->log("SignalHandler: Awaiting user confirmation for exit.",
			                      1);
		}

		char choice;
		auto start_time =
		    std::chrono::steady_clock::now();  // Start timer for timeout mechanism
		...
		{
			std::this_thread::sleep_for(
			    std::chrono::milliseconds(100));  // Small sleep to prevent CPU overuse
		}
		...
		if(choice == 'y' || choice == 'Y')
		{
			...
			{
				instance->logger->log(
				    "SignalHandler: User confirmed exit. Stopping program.", 1);
			}
		...
		else
		{
			...
			{
				instance->logger->log(
				    "SignalHandler: User denied exit. Resuming program.", 1);
			}
			exit_requested.store(
			    false);  // Reset flag so future signals can trigger exit confirmation again
		}
	}
