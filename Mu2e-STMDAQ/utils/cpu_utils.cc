...
const std::vector<int> cpu_utils::socket1_cores = {24, 25, 26, 27, 28, 29, 30, 31,
...
void cpu_utils::log(const std::string& msg, int level)
{
	...
	if(logger)
		...
		pending_logs.emplace_back(msg, level);
}
...
void cpu_utils::set_logger(std::shared_ptr<AsyncLogger> l)
{
	std::lock_guard<std::mutex> lock(log_mutex);
	logger = l;
	for(const auto& [msg, level] : pending_logs)
		logger->log(msg, level);
	pending_logs.clear();
}
    ...
    : cfg(cfg_), role(role_), next_core_id(0)
{
	...
	{
		socket_id =
		    EnvVars::expand("${HOSTNAME}") == cfg.getValue<std::string>("stm.ch0_host")
		        ? 1
		        : 0;
	...
	log("CPU utils initialised: Role: " + role_name +
	        ...
	        ",  Managed cores: " + std::to_string(allowed_cores.size()),
	    1);
}
...
void cpu_utils::pin_thread_to_core(size_t core_id, const std::string& name)
{
	...
	if(result != 0)
	{
		log("CPU utils: Error! Failed to pin " + name + " to core " +
		        std::to_string(core_id) + " (errno " + std::to_string(result) + ")",
		    0);
	}
	else
	{
