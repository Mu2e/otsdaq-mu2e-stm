...
template<typename T>
void LoggerSTM::registerBlock(DQMPageType        type,
...
T* LoggerSTM::get(DQMPageType type)
{
	...
	if(!ptr)
	{
		throw std::runtime_error(
		    "Requested LoggerSTM SHM block not found in SHMmanager.");
	}
	return ptr;
}
...
void LoggerSTM::write(unsigned int                       level_,
                      ...
                      const std::lock_guard<std::mutex>& lock_)
{
	...
	if(level_ <= _logLevel)
	{
		...
		if(level_ == LoggerSTM::ERROR)
			levelStr = "ERROR";
		else if(level_ == LoggerSTM::INFO)
			levelStr = "INFO";
		else if(level_ == LoggerSTM::WARNING)
			levelStr = "WARNING";
		else if(level_ == LoggerSTM::NOTE)
			levelStr = "NOTE";
		else if(level_ == LoggerSTM::DEBUG)
			levelStr = "DEBUG";
		else
		{
			std::stringstream ss;
			ss << "[ LoggerSTM :: write ] Invalid log level " << level_
			   << " chosen, must be ERROR, INFO, WARNING, NOTE or DEBUG";
		...
		if(_style > 2)
			_style = 0;  //Default to 0 if invalid style set
		if(_style == 0)
		{
		...
		else if(_style == 2)
		{
			...
			msg << levelStr << " : " << date << " : "
			    << message_;  //Add message level and date
		}
		...
		{
			if(level_ == LoggerSTM::ERROR)
				stop::trigger_critical_stop();  //throw CriticalError();
		}
...
void LoggerSTM::setLogLevel(const unsigned int logLevel)
{
	...
	{
		std::stringstream ss;
		ss << "[ LoggerSTM :: setLogLevel ] Invalid log level " << logLevel
		   << " chosen, must be ERROR, INFO, WARNING, NOTE or DEBUG";
...
void LoggerSTM::setStylePlain() { this->setStyle(0); }
...
void LoggerSTM::setStyle(unsigned int style_)
{
	...
	{
		std::stringstream ss;
		ss << "[ LoggerSTM :: setStyle ] Invalid style " << style_
		   << " chosen, must be in range 0-2";
		this->write(LoggerSTM::ERROR, ss.str(), lock);
		_style = 0;
	}
	else
...
void LoggerSTM::LogToFile(string fileName_)
{
	...
	{
		std::stringstream ss;
		ss << "[ LoggerSTM :: LogToFile ] Already logging to file";
		CloseLogFile();
		//this->write(LoggerSTM::ERROR,ss.str(),lock);
		//return;
	}
	...
	if(_opFile)
	{
		_logToFile = true;
		std::stringstream ss;
		ss << "[ LoggerSTM :: LogToFile ] Logging to file: " << _opFileName;
		this->write(LoggerSTM::INFO, ss.str(), lock);
	}
	else
	{
		std::stringstream ss;
		ss << "[ LoggerSTM :: LogToFile ] Failed to open file: " << _opFileName
		   << std::endl;
		this->write(LoggerSTM::ERROR, ss.str(), lock);
	}
}
...
void LoggerSTM::CloseLogFile()
{
	...
	if(_logToFile)
	{
		...
		std::stringstream ss;
		ss << "[ LoggerSTM :: CloseLogFile ] Closing log file: " << _opFileName;
	...
	{
		std::stringstream ss;
		ss << "[ LoggerSTM :: CloseLogFile ] Cannot close log file as not currently "
		      "logging to file";
...
void LoggerSTM::initSHM(bool make_shm, unsigned int max_alarms)
{
	if(make_shm)
	{
		...
		size_t alarm_bytes =
		    sizeof(dqm_data_alarm) +
		    (max_alarms - 1) *
		        sizeof(alarm_entry) +  // -1 for array initialised with size 1
		    sizeof(uint64_t);

		// Create persistent SHM block
		registerBlock<dqm_data_alarm>(
		    DQMPageType::ALARMS, "/dqm_alarm_data", alarm_bytes, true);
		_logToSHM = true;

		std::stringstream ss;
		ss << "[ LoggerSTM::initSHM ] Registered SHM block for alarms"
...
void LoggerSTM::logToSHM(unsigned int level_, const std::string& message_)
{
	...
	// Increment number
	if(alarm_data->num_alarms != alarm_data->max_alarms)
		alarm_data->num_alarms += 1;
	...
	// Complete safe write
	gen_end_ptr->store(alarm_data->gen_start.load(std::memory_order_acquire),
