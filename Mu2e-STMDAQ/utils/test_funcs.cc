...
TestFuncs::TestFuncs()
{
	...
	functionMap["print"] = [this](std::shared_ptr<DataStruct>& buffer) { print(buffer); };
...
void TestFuncs::confirmData(std::shared_ptr<DataStruct>& buffer)
{
	for(size_t i = 0; i < buffer->raw_len; ++i)
	{
		...
		{
			std::cout << "BUFFER 3 ERROR: " << buffer->mwd[i] << " != " << test3
			          << std::endl;
			exit(0);
		}
	}
...
void TestFuncs::check_form_events(std::shared_ptr<DataStruct>& buffer)
{
	...
	for(int i = 0; i < buffer->raw_header_num; i++)
	{
		...
		if(print)
		{
			if(i == 0 or i == 1 or i == buffer->raw_header_num - 2 or
			   i == buffer->raw_header_num - 1)
			{
				...
				std::cout << std::get<hdrMap_EWT>(raw) << "  "
				          ...
				          << std::get<hdrMap_eventLen>(raw) << "  " << hdr[eHdr.EvStart]
				          << "  " << hdr[eHdr.EvInPacket] << "  " << hdr[eHdr.EvLen];
				std::cout << "\n";
				if(i == buffer->raw_header_num - 1)
					std::cout << "---" << std::endl;
			}
		}
	...
	{
		std::cout << "ERROR: Buffer header data count = " << std::to_string(max) << " != "
		          << " buffer->raw_len = " << std::to_string(buffer->raw_len)
		          << std::endl;
		std::cout << "EWT range = " << std::get<hdrMap_EWT>(buffer->raw_header_map[0])
		          << " --> "
		          << std::get<hdrMap_EWT>(
		                 buffer->raw_header_map[buffer->raw_header_num - 1])
		          << std::endl;
		//exit(0);
	}
	...
	for(int i = 0; i < buffer->raw_len; i++)
	{
		...
		if(buffer->raw[i] != counter_check)
		{
			...
			{
				std::cout << j << " " << counter_check + j << " " << buffer->raw[j]
				          << std::endl;
			}
			std::cout << i << "/" << buffer->raw_len
			          << " ERROR in TestFuncs::check_form_events, incrementing counter = "
			          << counter_check << ", data = " << buffer->raw[i] << std::endl;
			break;
			//    exit(0);
		}
		counter_check++;
