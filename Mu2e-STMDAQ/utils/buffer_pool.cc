    ...
    : cpu(cpu_)
{
	...
	{
		max_packets_per_call = std::dynamic_pointer_cast<UDP>(om->get_class("UDP"))
		                           ->get_max_packets_per_call();
	}

	// Calculate unused buffer size
	int buffer_remainder =
	    int(raw_buffer_size * 1e6) - int(max_packets_per_call * MAX_PACKET_SIZE);

	// Check buffer allocation is correct
	if(buffer_remainder < 0)
	{
		logger->log(
		    "BufferPool: UDP initialisation code error: more packets than allocated "
		    "buffer size.",
		    0);
	}
	...
	{
		zs_overflow = (double)(std::dynamic_pointer_cast<ZeroSuppress>(
		                           om->get_class("ZeroSuppress"))
		                           ->get_prev_data_max() *
		                       sizeof(int16_t) * 1e-6);
	}
	...
	std::string buffer_size_log =
	    "BufferPool: Raw/ZS buffer sizes = " + std::to_string(raw_buffer_size) + " MB (" +
	    print_opt + std::to_string(zs_overflow) + " MB ZS overflow).";
	...
	size_t max_events_per_struct = std::ceil(((double)raw_buffer_size * 1e6) /
	                                         ((double)min_event_len * sizeof(int16_t)));
	logger->log("BufferPool: Initialising data structs to hold a maximum of " +
	                std::to_string(max_events_per_struct) + " mu2e events.",
	            1);
	...
	if(om->check_class("ZeroSuppress"))
	{
		zs_overflow_event_max = (double)(std::dynamic_pointer_cast<ZeroSuppress>(
		                                     om->get_class("ZeroSuppress"))
		                                     ->get_overflow_event_max());
	}
	...
	logger->log("BufferPool: Initialising header map arrays of " +
	                ...
	                " byte header maps = " + std::to_string(hdrMapSize) + " MB.",
	            1);
	...
	{
		zs_max_noise_len =
		    (std::dynamic_pointer_cast<ZeroSuppress>(om->get_class("ZeroSuppress"))
		         ->get_max_noise_len());
	}
	...
	double struct_size =
	    (raw_buffer_size + zs_buffer_size + mwd_buffer_size + hdrMapSize) *
	    (double)(1 + (double)struct_extra / 100);
	logger->log(
	    "BufferPool: Estimating total struct size ~ " + std::to_string(struct_size) +
	        " MB = " + std::to_string(raw_buffer_size) + " MB (raw) + " +
	        std::to_string(zs_buffer_size) + " MB (ZS) + " + std::to_string(hdrMapSize) +
	        " MB (raw/zs header maps) + " + std::to_string(struct_extra) +
	        " MB (MWD) + " + std::to_string(mwd_buffer_size) + " % (metadata).",
	    1);
	...
	if(buffer_mem > FREE_MEM)
	{
		// Throw error
		std::string warning =
		    "ERROR. Intended total buffer memory = " + std::to_string(buffer_mem * 1e-9) +
		    " GB > available memory = " + std::to_string(FREE_MEM * 1e-9) + " GB";
		logger->log(warning, 0);
	}

	// Set the buffer pool size
	pool = std::make_unique<boost::lockfree::stack<std::shared_ptr<DataStruct>>>(
	    buffer_pool_total);

	// Set the reset queue size
	resetQueue =
	    std::make_unique<boost::lockfree::spsc_queue<std::shared_ptr<DataStruct>>>(
	        buffer_pool_total);

	// Create pool of data struct buffers
	for(size_t i = 0; i < buffer_pool_total; ++i)
	{
		pool->push(std::make_shared<DataStruct>(
...
void BufferPool::resetWorker()
{
	...
	while(running)
	{
		...
		if(resetQueue->pop(buffer))
		{
			...
			{
				std::cerr
				    << "[ERROR] Skipping nullptr in resetQueue. Possible corruption.\n";
				continue;
			}
...
void BufferPool::clearPool()
{
	...
	if(pool)
	{
		...
		std::cout << "Cleared BufferPool of " << count << " elements (expected "
		          << buffer_pool_total << ")." << std::endl;
	}
}
