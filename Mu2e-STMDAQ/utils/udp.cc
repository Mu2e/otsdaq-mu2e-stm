    ...
    , wait_time(0.0)
{
	...
	{
		if(logger)
			logger->log("socket creation failed", 0);
		exit(EXIT_FAILURE);
	}
	...
	if(logger)
		logger->log("Configuring UDP " + type + " for " + detector + " channel...", 1);
...
void UDP::get_mem_max()
{
	...
	if(logger)
		logger->log(
		    "/proc/sys/net/core/rmem_max = " + std::to_string(RMEM_MAX) + " bytes.", 1);
	...
	if(logger)
		logger->log(
		    "/proc/sys/net/core/wmem_max = " + std::to_string(WMEM_MAX) + " bytes.", 1);
}
...
void UDP::configure_server()
{
	...
	if(logger)
		logger->log("SO_RCVBUF set to: " + std::to_string(actual_rmem_max) + " (" +
	...
	{
		if(logger)
			logger->log("Unable to bind socket.", 0);  // Handle binding failure
		exit(EXIT_FAILURE);
	}
	...
	std::string size_msg = "Setting raw data buffer size user value of " +
	                       std::to_string(raw_buffer_size) + " MB.";
	if(logger)
		logger->log(size_msg, 1);

	// Calculate the maximum number of packets per call
	max_packets_per_call = raw_buffer_size * 1e6 / MAX_PACKET_SIZE;
	if(logger)
		logger->log("Maximum UDP receiving = " + std::to_string(max_packets_per_call) +
		                " * 8198 kB packets per call.",
		            1);
	...
	// Print to user
	if(logger)
		logger->log("CREATED UDP SERVER. IP = " + ip + ", PORT = " +
...
void UDP::setup_buffers()
{
	...
	for(size_t i = 0; i < max_packets_per_call; ++i)
	{
		...
		messages[i].msg_hdr.msg_iov =
		    &iovecs[i];  // Associate the iovec with the corresponding mmsghdr structure
		messages[i].msg_hdr.msg_iovlen =
		    1;  // Indicate that each mmsghdr describes one buffer
	}
}
...
void UDP::configure_client()
{
	...
	// Print the actual buffer size
	if(logger)
		logger->log("SO_SNDBUF set to: " + std::to_string(actual_wmem_max) + " (" +
	...
	// Print to user
	if(logger)
		logger->log("CREATED UDP CLIENT. IP = " + ip +
...
void UDP::receive_data(std::shared_ptr<DataStruct>& buffer)
{
	...
	while(!signal->shouldStop())
	{
		...
		while(received_packets < max_packets_per_call && !signal->shouldStop())
		{
			...
			{
				if(logger)
					logger->log("UDP::receive_data - poll failed!", 0);
			}
			...
			else if(poll_ret == 0)
			{
				...
				if(has_received_data)
				{
					...
					if(logger)
						logger->log(
						    "Stopped receiving from STM firmware. Waiting for data...",
						    2);
				...
				{
					if(logger)
						logger->log("Waiting for data from STM firmware...", 1);
					// Indicate that we are Waiting for data
					waiting_for_data = true;
				}
			}
			...
			else
			{
				...
				int retval = recvmmsg(socket_fd,
				                      &messages[received_packets],
				                      max_packets_per_call - received_packets,
				                      MSG_DONTWAIT,
				                      &timeout);
				...
				{
					if(logger)
						logger->log("UDP::receive_data - recvmmsg failed", 2);
				}
		...
		if(received_packets == max_packets_per_call)
			return;
