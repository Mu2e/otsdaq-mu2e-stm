...
int main(int argc, char* argv[])
{
	...
	// Print for user
	std::cout << "File size = " << static_cast<double>(file_size) * 1e-9 << " Gbytes"
	          << std::endl;
	...
	const double data_bits =
	    static_cast<double>(ADC.size()) * sizeof(int16_t) * 8.0;  // 16 bits/sample
	...
	std::cout << "Data size attributed per function: " << (data_bits / 8.0 / 1e6)
	          << " MB\n";
	std::cout << "fill_hist throughput: " << fill_gbit_per_s
	          << " Gbit/s  (time: " << fill_seconds << " s)\n";
	std::cout << "EM_algorithm throughput: " << em_gbit_per_s
	          << " Gbit/s  (time: " << em_seconds << " s)\n";

	std::cout << "Baseline Gaussian: weight = " << w0 << ", mean = " << mu0
	          << " sigma = " << sigma0 << std::endl;
	std::cout << "Pulses Exponential: weight = " << w1 << ", t = " << t
