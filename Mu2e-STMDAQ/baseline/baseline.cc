...
void Baseline::configure_baseline(const std::string& filename)
{
		...
		if(std::getline(iss, key, '='))
		{
			...
			if(std::getline(iss, value_str))
			{
				...
				try
				{
					...
					if(key == "nbins")
						nbins = std::stoi(value_str);
					else if(key == "min_adc")
						min_adc = std::clamp(
						    std::stoi(value_str),  // Minimum adc value
					...
					// Expectation-Maximization (EM) parameters
						...
						max_iters = std::stoi(value_str);
					else if(key == "tol")
						tol = std::stod(value_str);
					else if(key == "var_floor")
						var_floor = std::stod(value_str);
					else
					{
	...
	for(int i = 0; i < nbins; ++i)
		hist_bin_centres[i] = min_adc + (i + 0.5) * width;
...
std::pair<const std::vector<uint64_t>&, const std::vector<double>&> Baseline::fill_hist(
    const std::vector<int16_t>& ADC)
{
	// The adc length
...
Baseline::EM_algorithm()
{
	...
	if(!convergence)
	{
		std::cout << "[Baseline] EM (Gauss+Exp) did not converge in " << max_iters
		          << std::endl;
		exit(0);
	}
...
double Baseline::hist_percentile(double p01)
{
	...
	for(int i = 0; i < nbins; ++i)
	{
		...
		if(s >= target)
		{
			...
			const double frac =
			    (target - prev) / std::max(1e-12, static_cast<double>(hist_counts[i]));
