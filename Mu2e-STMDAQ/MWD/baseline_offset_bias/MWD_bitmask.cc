...
MWD::MWD(const std::string& config_filename)
{
	...
	else
	{
		...
		std::cout << "Baseline offset = " << baseline_offset
		          << ". Physical baseline = " << baseline_mean
		          << ", offset baseline = " << baseline_mean + baseline_offset
		          << ", new threshold value = " << threshold_cut << "." << std::endl;
	}
}
...
void MWD::configure_mwd(const std::string& filename)
{
	...
	while(std::getline(infile, line))
	{
		...
		if(std::getline(iss, key, '='))
		{
			...
			if(std::getline(iss, value_str))
			{
				...
				{
					if(key == "M")
						M = std::stoi(value_str);
					else if(key == "L")
						L = std::stoi(value_str);
					else if(key == "tau")
						tau = std::stod(value_str);
					else if(key == "fADC")
						fADC = std::stod(value_str);
					else if(key == "use_fixed_cut")
						use_fixed_cut = std::stoi(value_str);
					else if(key == "baseline_mean")
						baseline_mean = std::stod(value_str);
					else if(key == "baseline_rms")
						baseline_rms = std::stod(value_str);
					else if(key == "nsigma_cut")
						nsigma_cut = std::stod(value_str);
					else if(key == "thresholdgrad")
						thresholdgrad = std::stod(value_str);
					else if(key == "nsigma_baseline_offset")
						nsigma_baseline_offset = std::stod(value_str);
					else if(key == "fixed_cut_value")
						fixed_cut_value = std::stod(value_str);
					else if(key == "auxlow_init")
						auxlow_init = std::stod(value_str);
					else
					{
				...
				{
					std::cerr << "ERROR: Failed to parse value for key '" << key
					          << "': " << value_str << std::endl;
				}
			}
		}
	...
	std::cout << "used_fixed_cut         = " << (use_fixed_cut ? "true" : "false")
	          << std::endl;
...
mwd_peaks MWD::calc_mwd(MultiFileHandler& provider)
{
	...
	{
		std::cerr
		    << "Error: M and L must be powers of 2 for optimized ring buffer access."
		    << std::endl;
		std::exit(1);
	}
	...
	double                        samples_per_second =
	    static_cast<double>(samples.size()) / elapsed_seconds.count();
	double gbit_per_second = (samples_per_second * 16.0) / 1e9;  // 16 bits per sample
	std::cout << " - Deconvolution and differentiation speed: " << std::fixed
	          << std::setprecision(3) << gbit_per_second << " Gbit/s" << std::endl;
	...
	std::cout << " - Peak finding speed: " << std::fixed << std::setprecision(3)
	          ...
	          << " - Found peaks: " << peak_data.npeaks << std::endl;
...
// Deconvolution and differentiation
template<typename T>
void MWD::deconv_and_diff(const std::vector<int16_t>& samples, std::vector<T>& D)
{
	...
	if(!ai_file.is_open())
	{
		throw std::runtime_error("Failed to open output file for ai values: " +
		                         ai_output_filename);
	}
...
void MWD::averaging(const std::vector<T_D>& D, std::vector<T_avg>& avgs)
{
	...
	if(!avg_file.is_open())
	{
		throw std::runtime_error("Favgled to open output file for avg values: " +
		                         avg_output_filename);
	}
...
// The MWD algorithm
template<typename T>
void MWD::mwd_algorithm(const std::vector<int16_t>& samples, std::vector<T>& avgs)
{
	// Variables for deconvolution, differentiation, moving average calculation
	double prev_sample = 0.0;
	double prev_a      = 0.0;
