...
mwd_peaks MWD::calc_mwd(MultiFileHandler& provider)
{
	...
	while(provider.next(sample))
	{
		...
		if(time_since_last_print.count() >= 5.0)
		{
			...
			double gbit_per_second =
			    (samples_per_second * 16.0) / 1e9;  // 16 bits per sample

			size_t samples_remaining = (total_samples > 0) ? (total_samples - i) : 0;
			double seconds_remaining =
			    (samples_per_second > 0) ? (samples_remaining / samples_per_second) : 0;
	...
	std::cout << "MWD + peak finding complete: processed " << i << " samples, found "
	          << peak_data.npeaks << " peaks." << std::endl;
