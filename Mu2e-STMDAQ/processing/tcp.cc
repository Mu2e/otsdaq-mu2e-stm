...
{
	// Notify user
	logger->log("TCP: Configuring TCP client for " + stm->master_config.ch_name +
	                " channel: IP = " + ip + ", PORT = " + std::to_string(port) + ".",
	            1);
...
void TCP::get_mem_max()
{
	...
	{
		logger->log(
		    "TCP:get_mem_max: Error! wmem_max not found in /proc/sys/net/core/wmem_max. "
		    "Exiting...",
		    0);
	...
	// Log to user
	logger->log(
	    "TCP: /proc/sys/net/core/wmem_max = " + std::to_string(WMEM_MAX) + " bytes.", 1);
}
...
void TCP::log_disconnect()
{
	...
	if(now - last_disconnected_log < disconnected_log_period)
		return;
	last_disconnected_log = now;
	std::string msg =
	    "TCP: No server connection detected! errono = " + std::to_string(errno) + " (" +
...
void TCP::send_data(std::shared_ptr<DataStruct>& buffer)
{
	...
	for(size_t i = 0; i < n; ++i)
	{
		...
		if(len == 0)
		{
			logger->log("TCP::send_data: Event builder returned length = 0 for EWT = " +
			                std::to_string(EWT) + ". Skipping EWT...",
			            2);
			continue;
		}
	...
	if(dt >= rate_log_period)
	{
		...
		logger->log("TCP_SEND_CLIENT: send rate = " + std::to_string(gbps) +
		                " Gbit/s (avg over " + std::to_string(seconds) + " s)",
		            1);
...
bool TCP::send_all(const uint8_t* src, size_t nbytes)
{
	...
	if(socket_fd < 0 || !is_connected.load(std::memory_order_acquire))
		return false;
