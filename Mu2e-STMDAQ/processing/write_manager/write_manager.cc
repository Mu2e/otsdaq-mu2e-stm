    ...
    , shutdown_requested(false)
{
	// Read config values
	output_dir =
	    cfg.getValue<std::string>("stm.write_data.write_dir");  // The write directory
	file_buffer_depth = cfg.getValue<int>(
	    "stm.write_data.file_buffer_depth");  // Number of buffered files
	log_interval_sec = cfg.getValue<int>(
	    "stm.write_data.log_interval_sec");  // The write logging frequency
...
void WriteManager::write_vector(const std::string&          type,
                                std::shared_ptr<DataStruct> buffer)
{
	...
	if(!file || ctx.current_file_size + data_bytes > ctx.max_file_size)
	{
		...
		auto new_file =
		    std::atomic_exchange(&ctx.next_file, std::shared_ptr<std::ofstream>{});
		if(new_file)
		{
...
const std::vector<int16_t>& WriteManager::select_vector(
    const std::string& type, std::shared_ptr<DataStruct> buffer)
{
	if(type == "raw")
		return buffer->raw;
	if(type == "zs")
...
void WriteManager::file_manager_thread(const std::string& type)
{
	...
	while(!shutdown_requested.load())
	{
		...
		while(true)
		{
			...
			std::string filename =
			    generate_filename(ctx.prefix, type, timestamp, ctx.subrun++);
		...
		while(ctx.retired_files->pop(closing))
		{
			...
			{
				closing->close();
				std::cout << "[write_manager] Closed file: " << ctx.last_opened_path
				          << std::endl;
			}
		}
...
void WriteManager::logger_thread()
{
	...
	while(!shutdown_requested.load())
	{
		...
		logger->log("WriteManager: Speed: raw=" + std::to_string(raw_gbps) +
		                ...
		                " Gbit/s | mwd=" + std::to_string(mwd_gbps) + " Gbit/s.",
		            1);
		...
		double total_bps =
		    (cur_raw + cur_zs + cur_mwd - last_raw - last_zs - last_mwd) / dt;
		// If writing data
		if(total_bps > 0)
		{
			...
			// Log to user
			logger->log("WriteManager:  Disk: used=" + format_gb(used) +
			                " | available=" + format_gb(space.available) +
			                " | est. time until full: " + std::to_string(minutes) +
			                " min.",
			            1);
		}
...
void WriteManager::cleanup_unused_files(Context& ctx)
{
	...
	while(ctx.file_queue->pop(f))
	{
		...
		if(f && f->is_open())
		{
			...
			std::cout << "WriteManager: Deleted unused file: " << ctx.last_opened_path
			          << std::endl;
		}
	}
}
