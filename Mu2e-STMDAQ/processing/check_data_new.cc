...
void CheckData::check_packets(std::shared_ptr<DataStruct>& buffer)
{
	...
	{
		logger->log("CheckData::check_packets : Packet sizes incorrect!  Data size = " +
		                std::to_string(n) +
		                " % MAX_PACKET_LEN = " + std::to_string(n % MAX_PACKET_LEN),
		            0);
		return;
	}
	...
	if(firstPacket or is_null_hb)
	{
		...
		if(is_null_hb)
		{
			...
			logger->log("CheckData: storing first packet number after null_hb (" +
			                ...
			                ") as last detected packet number.",
			            2);
			logger->log("CheckData: storing first EWT after null_hb (" +
			                std::to_string(last_EWT) + ") as last detected EWT.",
			            2);
		}
	...
	while(count < n)
	{
		...
		uint32_t packet_num =
		    (uint16_t)data_ptr[packet_start + 1] << 16 | (uint16_t)data_ptr[packet_start];
	...
	while(leftInPacket > fw_eHdr_len)
	{
		...
		if(data_ptr[hdr_start_loc] != fw_eHdr.anchor_data[fw_eHdr.anchor_len - 1])
		{
			logger->log("CheckData: ERROR! Event header mismatch!!\nPacket number = " +
			                std::to_string(pnum) + ". EWT = " + std::to_string(EWT),
			            0);
			return;
		}
...
void CheckData::check_pHdr(uint32_t packet_num, int16_t pHdr_end)
{
	...
	if((uint16_t)pHdr_end != pHdr_check)
	{
		...
		logger->log("CheckData: ERROR! Packet number " + std::to_string(packet_num) +
                                      ...
                                      uint32_t                     last_packet)
{
	...
	if(diff < 0)
	{
		logger->log("CheckData: Packet number mismatch! Previous packet = " +
		                std::to_string(last_packet) +
		                ", new packet = " + std::to_string(packet_num),
		            0);
		return 0;
	}
	...
	if(diff > 1)
	{
		...
		// Log the dropped packet
		logger->log("CheckData: Dropped " + std::to_string(diff - 1) +
		                " UDP packets. Missing packet numbers = " +
		                std::to_string(last_packet + 1) + " --> " +
		                std::to_string(packet_num - 1),
		            2);
...
uint64_t CheckData::check_eHdr(std::shared_ptr<DataStruct>& buffer, uint64_t hdr_index)
{
	...
	if(channel != stm->master_config.ch_num)
	{
		...
		for(size_t i = 0; i < fw_eHdr_len / 2; i++)
		{
			...
			if((uint16_t)(data_ptr[hdr_index + i * 2] & 0xFFFF) == BEEF and
			   (uint16_t)(data_ptr[hdr_index + i * 2 + 1] & 0xFFFF) == DEAD)
			{
				// is 0xDEADBEEF
				deadbeef = true;
			}
		...
		if(deadbeef)
			return 0;
		logger->log(
		    "CheckData: ERROR! Event header channel = " + std::to_string(channel) +
		        " does not match expected channel number",
		    0);
		return 0;
	}
	...
	if(diff < 0 && !is_null_hb)
	{
		logger->log("CheckData: ERROR - EWT mismatch! Previous EWT = " +
		                std::to_string(last_EWT) + ", new EWT = " + std::to_string(EWT),
		            0);
		return 0;
	}
	if(diff_EvNum < 0 && !is_null_hb)
	{
		logger->log("CheckData: ERROR - Event number mismatch! Previous event number = " +
		                ...
		                ", new event number = " + std::to_string(EvNum),
		            0);
		return 0;
	}
	...
	if(diff > 1 && !is_null_hb)
	{
		...
		// Log the lost EWTs
		logger->log("CheckData: Lost " + std::to_string(diff - 1) +
		                " EWTs. Missing EWTs = " + std::to_string(last_EWT + 1) +
		                " --> " + std::to_string(EWT - 1),
		            2);
	...
	for(size_t i = 0; i < fw_eHdr.anchor_len; i++)
	{
		...
		{
			logger->log(
			    "CheckData: ERROR! EWT " + std::to_string(EvNum) +
			        " has event header end value " +
			        std::to_string(data_ptr[hdr_index + fw_eHdr.anchor_start + i]) +
			        " instead of " + std::to_string(fw_eHdr.anchor_data[i]) +
			        " for header index " + std::to_string(fw_eHdr.anchor_start + i),
			    0);
			return 0;
		}
	}
