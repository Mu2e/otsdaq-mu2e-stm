    ...
    , prev_data(std::vector<int16_t>(window + n_average, 0.0))
{
	...
	register_operation("find_peaks",
	                   [this](auto& b, auto& prev) { find_peaks(b, prev); });
                              ...
                              std::shared_ptr<DataStruct>& prev_buffer)
{
	...
	while(j < total - leftover)
	{
		...
		for(int k = 0; k < n_average; k++)
		{
			...
			{
				grad_low  = (idx_low < prev_num) ? prev_data[idx_low]
				                                 : data_ptr[idx_low - prev_num];
				grad_high = (idx_high < prev_num) ? prev_data[idx_high]
				                                  : data_ptr[idx_high - prev_num];
			}
		...
		if(avg_gradient < threshold && peak_found == false)
		{
			...
			if((first_buffer || !prev_buffer) && peak_start < 0)
				peak_start = 0;
	...
	// Copy overflow data
	std::copy(
	    buffer->raw.begin() + copy_from, buffer->raw.begin() + n, prev_data.begin());
...
void ZeroSuppress::suppress_data(std::shared_ptr<DataStruct>& buffer)
{
	...
	for(int i = 0; i < buffer->zs_data.size(); i++)
	{
		...
		if(!found_EWT)
		{
			// Critical Error
			logger->log(
			    "ZeroSupress::suppress_data: Error! zs data located outside of EWTs in "
			    "buffer!",
			    0);
			std::cout << i << " " << buffer->zs_data.size() << " " << start << " " << len
			          << " " << copy_end << " " << buffer->raw_len << std::endl;
	...
	{
		std::string error =
		    "ZeroSuppress::suppress_data: ERROR. Total ZS data len in buffer = " +
		    std::to_string(buffer->zs_len) +
		    ", total ZS data len in EWTs =" + std::to_string(EWT_adc_count) + "!";
		logger->log(error, 0);
		return;
	}
