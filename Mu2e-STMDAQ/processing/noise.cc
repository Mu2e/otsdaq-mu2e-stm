...
void Noise::check_noise(std::shared_ptr<DataStruct>& buffer)
{
	...
	double square_sum = std::inner_product(
	    noise_data->begin(), noise_data->end(), noise_data->begin(), 0.0);
	double mean_of_squares = square_sum / noise_len;
	baseline_rms           = std::sqrt(mean_of_squares);

	std::cout << "Noise " << std::to_string(avg_baseline) << " RMS "
	          << std::to_string(baseline_rms) << std::endl;
	...
	if(abs(avg_baseline) > max_allowed_baseline)
	{
		std::string error = "ERROR in Noise::check_noise. Mean baseline not within +/-" +
		                    std::to_string(max_allowed_baseline) +
		                    ", baseline_avg = " + std::to_string(avg_baseline);
		logger->log(error, 1);
	}
	if(abs(baseline_rms) > max_allowed_noise_rms)
	{
		std::string error = "ERROR in Noise::check_noise. Noise RMS greater than " +
		                    std::to_string(max_allowed_noise_rms) +
		                    ", baseline_rms = " + std::to_string(baseline_rms);
		logger->log(error, 1);
	}
...
void Noise::noise_fft(std::shared_ptr<DataStruct>& buffer)
{
	...
	for(int i = 0; i < noise_len; i++)
		noise_for_fft[i] = static_cast<double>((*noise_data)[i]);

	fftw_complex* fft_out =
	    (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * (noise_len / 2 + 1));
	fftw_plan plan =
	    fftw_plan_dft_r2c_1d(noise_len, noise_for_fft.data(), fft_out, FFTW_ESTIMATE);
	...
	// Write FFT to buffer?
	std::copy(fft_power.begin(), fft_power.end(), noise_data_fft->begin());
}
