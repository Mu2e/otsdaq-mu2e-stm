...
MWD::MWD(const std::shared_ptr<AsyncLogger>& logger_,
    ...
    , tau(stm->mwd_config.tau)
    , tau_norm(stm->mwd_config.tau_norm)
    ,
    //  Differentiation
    M(stm->mwd_config.M)
    , m_mask(stm->mwd_config.m_mask)
    , a_ring(std::vector<double>(M, 0.0))
    ,
    // Averaging
    L(stm->mwd_config.L)
    , inv_L(stm->mwd_config.inv_L)
    , l_mask(stm->mwd_config.l_mask)
    , D_ring(std::vector<double>(L, 0.0))
    ,
    // Peak finding
    peak_init(stm->mwd_config.peak_min_init)
    , peak_height(peak_init)
    , peak_time(0.0)
    , peak_zs_start(0)
    , peak_zs_len(0)
    , use_fixed_cut(stm->mwd_config.use_fixed_cut)
    , fixed_cut_value(stm->mwd_config.fixed_cut_value)
                     ...
                     std::shared_ptr<DataStruct>& prev_buffer)
{
	...
	for(size_t i = 0; i < n; ++i)
	{
		...
		if(pf_index >= M) [[likely]]
		{
			...
			if(peak_height != peak_init && avg > threshold_cut)
			{
				...
				EWT_to_store->ph.emplace_back(
				    static_cast<int16_t>(std::lround(peak_time)),
	...
	if(buffer->ph_len != count)
	{
		std::string error = "MWD::find_peaks: ERROR. Total ph data len in buffer = " +
		                    std::to_string(buffer->ph_len) +
		                    ", total ph data len in EWTs =" + std::to_string(count) + "!";
		logger->log(error, 0);
		return;
	}
