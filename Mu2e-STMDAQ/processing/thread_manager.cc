...
ThreadManager::ThreadManager(const std::shared_ptr<cpu_utils>&        cpu_,
    ...
    , avg_speed_num(thread_num, 0)
    , avg_speed_den(thread_num, 0)
    , baseline_window_buffers(stm->baseline_config.hist_window_buffers)
{
	...
	for(size_t i = 0; i < thread_num; ++i)
	{
		...
		// Initialise queues
		if(i != thread_num - 1)
			queues.emplace_back(std::make_shared<BufferQueue<DataStruct>>(
			    stm->buffer_config.queue_capacity));
		// Get input queue
		auto inq = (i == 0) ? nullptr : queues[i - 1];
		// Get output queue
		auto outq = (i == thread_num - 1) ? nullptr : queues[i];

		logger->log("ThreadManager: Creating " + thread_name[i] + " thread with index " +
		                std::to_string(i) + "...",
		            1);
		...
		threads.emplace_back(
		    &ThreadManager::worker_thread, this, i, inq, outq, operation);
	}
                                  ...
                                  const op_any operation)
{
	...
	std::visit(
	    [&](auto const& fn) {
		    ...
		    {
			    logger->log(
			        "ThreadManager::workerThread received an empty operation function.",
			        0);
	...
	while(!finished[0].load() || (thrd_idx > 0 && !finished[thrd_idx - 1].load()) ||
	      (inq && !inq->is_empty()))
	{
		// Dummy buffer
		//    std::shared_ptr<DataStruct> buffer = nullptr;
		...
		if(!buffer->do_not_use)
		{
			...
			else
			{
				...
				if(prev_buffer)
					push_buffer(thrd_idx, outq, prev_buffer);
			...
			if(first_buffer)
				thread_start_time = std::chrono::high_resolution_clock::now();
			first_buffer = false;
		}
	...
	{
		if(inq->is_empty())
			logger->log("Final queue emptied.", 1);
	}
}
...
// Log thread performance
void ThreadManager::log_performance(
    const size_t                                                      thrd_idx,
    const std::chrono::time_point<std::chrono::high_resolution_clock> thread_start_time)
{
	// End timing
	...
	if(thrd_idx != 0)
		while(!finished[thrd_idx - 1].load())
			continue;
	std::ostringstream performance;
	performance << "ThreadManager: " << thread_name[thrd_idx] << " processed "
	            << std::fixed << std::setprecision(2) << gbytes << " Gbytes ("
	            << buffer_count[thrd_idx]
	            << " buffers), average speed [Gbit/s] = " << std::fixed
	            << std::setprecision(2)
	            << (avg_speed_num[thrd_idx] / avg_speed_den[thrd_idx])
	            << " (operation) / " << (gbits / elapsed.count()) << " (worker thread).";
...
void ThreadManager::data_source_thread(const size_t thrd_idx)
{
	...
	{
		logger->log(
		    "ThreadManager::data_source_thread: Initialising software data setup...", 1);
	}
	else
	{
		logger->log(
		    "ThreadManager::data_source_thread: Initialising firmware data setup...", 1);
	}
	...
	else if(stm->fw_config.use_dtc_sim)
	{
		...
		{
			logger->log(
			    "ThreadManager::data_source_thread: Initialising DTC simulation...", 1);
			// Sleep for 3 seconds
			for(int i = 0; i < 3; i++)
			{
				std::this_thread::sleep_for(std::chrono::seconds(1));
	...
	else
	{
		...
		if(hw)
		{
			logger->log("ThreadManager::data_source_thread: Setting ROC for real DTC...",
			            1);
			...
			if(!dtc_set)
				logger->log(
				    "ThreadManager::data_source_thread: Error setting ROC to real DTC.",
				    1);
		}
	}
}
