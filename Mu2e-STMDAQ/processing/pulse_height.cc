    ...
    : cfg(cfg_), logger(logger_), stm(stm_)
{
	...
	functionMap["detectPulseCandidates"] = [this](std::shared_ptr<DataStruct>& current,
	                                              std::shared_ptr<DataStruct>& previous) {
		detectPulseCandidates(current, previous);
	};

	functionMap["processPulseCandidates"] = [this](
	                                            std::shared_ptr<DataStruct>& current,
	                                            std::shared_ptr<DataStruct>& previous) {
...
std::pair<float, float> PulseHeight::calculateBaseline(const std::vector<int16_t>& ADC,
                                                       int pulseStart)
{
                                   ...
                                   int16_t&                    raw_min_val)
{
	...
	const float y0 = sg[0] * d[i0 - 3] + sg[1] * d[i0 - 2] + sg[2] * d[i0 - 1] +
	                 sg[3] * d[i0] + sg[4] * d[i0 + 1] + sg[5] * d[i0 + 2] +
	                 sg[6] * d[i0 + 3];

	const float y1 = sg[0] * d[i1 - 3] + sg[1] * d[i1 - 2] + sg[2] * d[i1 - 1] +
	                 sg[3] * d[i1] + sg[4] * d[i1 + 1] + sg[5] * d[i1 + 2] +
	                 sg[6] * d[i1 + 3];

	const float y2 = sg[0] * d[i2 - 3] + sg[1] * d[i2 - 2] + sg[2] * d[i2 - 1] +
	                 sg[3] * d[i2] + sg[4] * d[i2 + 1] + sg[5] * d[i2 + 2] +
	                 sg[6] * d[i2 + 3];
                                        ...
                                        std::shared_ptr<DataStruct>& previousBuffer)
{
	...
	{
		auto [minIt, maxIt] = std::minmax_element(ADC.begin(), ADC.end());

		logger->log(
		    "RAW BUFFER:"
		    " size=" +
		        std::to_string(size) + " min=" + std::to_string(*minIt) +
		        " max=" + std::to_string(*maxIt),
		    1);

		for(int i = 0; i < std::min(50, size); ++i)
		{
			logger->log("ADC[" + std::to_string(i) + "]=" + std::to_string(ADC[i]), 1);
		}
	...
	if(previousBuffer && !previousBuffer->raw.empty())
	{
		...
		if(prevSize >= 2 && size >= 2)
		{
			...
			const bool monotonicFall =
			    prevLast1 < prevLast2 && curr0 < prevLast1 && curr1 < curr0;
			const bool thresholdCross =
			    prevLast1 < -cfg.threshold || curr0 < -cfg.threshold;
			...
			if(monotonicFall && thresholdCross && grad > cfg.slopeThreshold)
			{
				...
				while((localWindowDynamic + 1) < size &&
				      ADC[localWindowDynamic] <=
				          (ADC[localWindowDynamic - 1] + cfg.noiseTolerance) &&
	...
	while(pos + cfg.slopeWindow < size)
	{
		...
		while((pulseStart + localWindowDynamic + 1) < size &&
		      ADC[pulseStart + localWindowDynamic] <=
		          (ADC[pulseStart + localWindowDynamic - 1] + cfg.noiseTolerance) &&
		      localWindowDynamic < cfg.localWindowMax)
		{
			++localWindowDynamic;
		}
	...
	state.carryoverDeadSamples = static_cast<int>(std::max<int64_t>(
	    0,
	    lastConfirmedPulseEnd -
	        (state.globalProcessedSamples + static_cast<int64_t>(size))));
                                         ...
                                         std::shared_ptr<DataStruct>& previousBuffer)
{
	...
	for(const auto& candidate : currentBuffer->pulseCandidates)
	{
		...
		bool stitchedPulse =
		    candidate.stitchedCandidate && previousBuffer && !previousBuffer->raw.empty();
		...
		else
		{
			...
			if(!this_EWT)
			{
				logger->log(
				    "NULL EWT:"
				    " buffer=" +
				        std::to_string(currentBuffer->buffer_num) +
		...
		const bool contaminatedBaseline =
		    candidate.baselineStd > stm->pulseheight_config.contaminatedBaselineStd;
		...
		if(contaminatedBaseline)
		{
			...
			const int searchStart =
			    std::max(0, triggerIdx - static_cast<int>(cfg.localWindowMax));
		...
		const float dynamicMinPulseHeight =
		    contaminatedBaseline ? 30.0f : std::max(30.0f, 3.0f * candidate.baselineStd);
