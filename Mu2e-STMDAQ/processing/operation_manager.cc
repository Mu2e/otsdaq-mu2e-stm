    ...
    : cfg(cfg_), logger(logger_), stm(stm_), signal(signal_)
{
	...
	for(const auto& class_ : class_names)
	{
		...
		if(!isValidClass(class_.first))
		{
			...
			output << "Operation class key: \"" << class_.first << "\" in "
			       << cfg.getXMLpath() << " is NOT a valid class name.";
		...
		if(class_.first == "FormEvents")
			form_events = true;
		// If ZeroSuppress or MWD are on and FormEvents is off, switch them off
		if((class_.first == "ZeroSuppress" || class_.first == "MWD" ||
		    class_.first == "PulseHeight") &&
		   !form_events)
		{
			// Log class is off and continue
			logger->log("WARNING: " + class_.first +
			                " ON when FormEvents OFF. Switching off " + class_.first +
			                ".",
			            2);
		...
		std::pair<std::string, std::string> find_peaks_op = {"ZeroSuppress",
		                                                     "find_peaks"};
		if(class_.first == "Noise" &&
		   std::find(useOpsFlag.begin(), useOpsFlag.end(), find_peaks_op) ==
		       useOpsFlag.end())
		{
			logger->log("WARNING: " + class_.first + " ON when " + find_peaks_op.first +
		...
		{
			logger->log("WARNING: " + class_.first + " ON for " +
			                stm->master_config.ch_name + ", use PulseHeight instead" +
			                ". Switching off " + class_.first + ".",
			            2);
			logger->log(class_.first + " is OFF.", 1);
			continue;
		}
		...
		{
			logger->log("WARNING: " + class_.first + " ON for " +
			                stm->master_config.ch_name + ", use MWD instead" +
			                ". Switching off " + class_.first + ".",
			            2);
			logger->log(class_.first + " is OFF.", 1);
			continue;
		...
		{
			logger->log("WARNING: Switching off " + class_.first + " for " +
			                stm->master_config.ch_name + ".",
			            2);
			logger->log(class_.first + " is OFF.", 1);
			continue;
		}
		...
		for(const auto& op_ : op_names)
		{
			...
			if(!isValidOp(class_.first, op_.first))
			{
				// Log critical error to user
				std::stringstream output;
				output << "Operation key: \"" << class_.first << "::" << op_.first
				       << "\" in " << cfg.getXMLpath()
	...
	// Notify user
	logger->log("OperationManager initialised a total of " +
	                std::to_string(useOps.size()) + " operations",
	            1);
...
std::vector<std::pair<std::string, op_any>> OperationManager::getUseOps() const
{
	return useOps;
}
