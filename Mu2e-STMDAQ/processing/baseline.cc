...
void Baseline::calc_baseline(std::shared_ptr<DataStruct>& buffer)
{
	...
	if(buffer->hist_counts_window.size() != nbins)
	{
		logger->log("Baseline: Error! buffer->hist_window has " +
		                ...
		                " bins. Config bin num = " + std::to_string(nbins) +
		                ". Exiting...",
		            0);
	...
	std::memcpy(
	    buffer->hist_counts_all.data(), hist_counts_all.data(), nbins * sizeof(uint64_t));
	...
	buffer->baseline_window =
	    EM_algorithm(buffer->hist_counts_window, buffer->total_window);
	// Fit all data histogram
	buffer->baseline_all = EM_algorithm(buffer->hist_counts_all, buffer->total_all);
	...
	{
		logger->log("Baseline: Last " +
		                std::to_string(stm->baseline_config.hist_window_period) +
		                " s window has baseline value: " + std::to_string(mu0_all) +
		                " ± " + std::to_string(sigma0_all) + " ADCs.",
		            1);
	}
}
