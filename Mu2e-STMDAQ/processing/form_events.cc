...
void FormEvents::get_events(std::shared_ptr<DataStruct>& buffer)
{
	...
	while(count < n)
	{
		...
		while(leftInPacket > fw_eHdr_len)
		{
			...
			size_t event_len =
			    static_cast<uint16_t>(data_ptr[hdr_start_loc + fw_eHdr.EvLen]);
	...
	logger->log("Event number = " + std::to_string(EWT)
		  ...
		  +  "  tem=" + std::to_string(((em_byte4 >> 6) & 0x3)),1);
			...
			{
				logger->log("FormEvents: Warning! Data length to copy = " +
				                std::to_string(data_len) +
				                ", which exceeds the length left in the packet = " +
				                std::to_string(leftInPacket) + ". At EWT = " +
				                std::to_string(current_EWT) + ", skipping packet",
				            2);
			...
			{
				logger->log(
				    "FormEvents: Error: Attemping to copy event portion of length " +
				        std::to_string(data_len) + " resulting in a total of " +
				        std::to_string(current_event_count + data_len) +
				        " ADCs to single event buffer of length " +
				        std::to_string(event_to_copy.size()),
				    0);
				return;
			}
	...
	if(buffer->raw_len != total)
	{
		std::string error = "ERROR in FormEvents::get_events. For last EWT in buffer = " +
		                    ...
		                    ") != accumulated data size in header map (" +
		                    std::to_string(total) + ")";
		logger->log(error, 0);
		return;
	}
...
// Setup new event
void FormEvents::new_event(int16_t* data_ptr,
                           size_t   hdr_start_loc,
                           uint64_t EWT,
                           size_t   event_len)
{
...
int FormEvents::store_event(std::shared_ptr<DataStruct>& buffer, size_t& adc_count)
{
	...
	{
		logger->log("FormEvents: Error: Event " + std::to_string(current_EWT) +
		                " has accumulated length of " +
		                std::to_string(current_event_count) +
		                " ADC values but expected " + std::to_string(current_event_len) +
		                " ADC values!",
		            2);
	}
	...
	eHdr_to_copy[sw_eHdr.RAW_LEN] =
	    current_event_count;      // Length of event's ADC data in header
...
void FormEvents::insert_missing_ewts(std::shared_ptr<DataStruct>& buffer)
{
	...
	{
		logger->log(
		    "FormEvents:insert_missing_ewts: Error! Number of EWTs "
		    "lost for buffer " +
		        std::to_string(lost_EWT_count) + " exceeds events space left of " +
		        std::to_string(event_slots_available),
		    0);
		return;
	}
	...
	for(size_t g = 0; g < buffer->lost_EWTs.size(); ++g)
	{
		...
		for(uint64_t missing = first_missing; missing <= last_missing; ++missing)
		{
			...
			{
				logger->log(
				    "FormEvents:insert_missing_ewts: EWT vector full, "
	...
	std::sort(EWTs.begin(),
	          EWTs.begin() + buffer->EWT_count,
	          [](const EWT_info& a, const EWT_info& b) { return a.EWT < b.EWT; });
