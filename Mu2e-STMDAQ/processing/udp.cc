...
UDP::UDP(const std::shared_ptr<AsyncLogger>&   logger_,
    ...
    ,  // Server or client
    ip((stm->master_config.ch_num) ? stm->udp_config.rcv_ip : stm->udp_config.snd_ip)
    ,  // IP address
    port((stm->master_config.ch_num) ? stm->udp_config.rcv_port
                                     : stm->udp_config.snd_port)
    ,  // Port
    max_packet_num(stm->buffer_config.max_packet_num)
    , wait_time(0.0)
    ,  // Time waiting with no packets received
...

{
	...
	{
		if(logger)
			logger->log("UDP: socket creation failed. Exiting...", 0);
		return;
	}
	...
	if(logger)
		logger->log("UDP: Configuring UDP " + type + " for " +
		                stm->master_config.ch_name + " channel...",
		            1);
...
void UDP::get_mem_max()
{
	...
	if(logger)
		logger->log(
		    "UDP: /proc/sys/net/core/rmem_max = " + std::to_string(RMEM_MAX) + " bytes.",
		    1);
	...
	if(logger)
		logger->log(
		    "UDP: /proc/sys/net/core/wmem_max = " + std::to_string(WMEM_MAX) + " bytes.",
		    1);
}
...
void UDP::configure_server()
{
	...
	if(logger)
		logger->log("UDP: SO_RCVBUF set to: " + std::to_string(actual_rmem_max) + " (" +
		                std::to_string(actual_rmem_max / 2) +
		                ") bytes [RMEM_MAX = " + std::to_string(RMEM_MAX) + " byes].",
		            1);
	...
	{
		if(logger)
			logger->log("UDP: Unable to bind socket to IP: " +
			                std::string(inet_ntoa(address.sin_addr)) + ", Port: " +
			                std::to_string(ntohs(address.sin_port)) + ". Exiting....",
			            0);  // Handle binding failure
		return;
	}
	...
	// Print to user
	if(logger)
		logger->log("UDP: CREATED UDP SERVER. IP = " + ip + ", PORT = " +
		                std::to_string(port) + ", socket = " + std::to_string(socket_fd) +
		                ", timeout = " + std::to_string(read_timeout.tv_sec) + " s, " +
		                std::to_string(read_timeout.tv_usec) + " us.",
		            1);
}
...
void UDP::configure_client()
{
	...
	// Print the actual buffer size
	if(logger)
		logger->log("UDP: SO_SNDBUF set to: " + std::to_string(actual_wmem_max) + " (" +
		                std::to_string(actual_wmem_max / 2) +
		                ") bytes [WMEM_MAX = " + std::to_string(WMEM_MAX) + " byes].",
		            1);

	// Print to user
	if(logger)
		logger->log("UDP: CREATED UDP CLIENT. IP = " + ip +
...
void UDP::receive_data(std::shared_ptr<DataStruct>& buffer)
{
	...
	{
		std::cerr << "ERROR: raw buffer too small: " << available << " bytes available, "
		          << required << " required.\n";
		std::abort();
	}
	...
	while(!stop::should_stop())
	{
		...
		{
			if(logger)
				logger->log("UDP: UDP::receive_data - poll failed!", 0);
			break;
		}
		...
		else if(poll_ret > 0)
		{
			...
			int retval = recvmmsg(socket_fd,
			                      &messages[packets_this_call],
			                      max_packet_num - packets_this_call,
			                      MSG_DONTWAIT,
			                      &timeout);
			...
			if(retval < 0)
			{
				...
				if(logger)
					logger->log(
					    "UDP: UDP::receive_data - recvmmsg failed with error code: " +
					        (std::to_string(errno)) + " " + errorStr,
					    0);
			...
			if(logger && waiting_for_data)
				logger->log("UDP: Receiving data...", 1);
		...
		else
		{
			...
			if(!waiting_for_data)
			{
				...
				{
					if(logger)
						logger->log("UDP: Waiting for data from STM firmware...", 1);
				}
				else
				{
					...
					if(logger)
						logger->log(
						    "UDP: Stopped receiving from STM firmware. Waiting for "
						    "data...",
						    2);
				}
			}
...
void UDP::send_packet(const std::vector<int16_t>& data_vec)
{
	...
	{
		if(logger)
			logger->log(std::string("send_packet error: ") + strerror(errno), 0);
	}
}
