    ...
    , buffer_pool_total(buffer_pool_size * thread_num)
{
	...
	logger->log("BufferPool: Estimating total header creation size of " +
	                std::to_string(max_event_num) + " events per struct * " +
	                std::to_string(sizeof(sw_event_header)) +
	                " byte raw event headers = " + std::to_string(header_size_tot) +
	                " bytes.",
	            1);

	// Calculate total struct size
	double struct_size =
	    (raw_size + zs_size + ph_size + baseline_size + header_size_tot) *
	    (double)(1 + extra / 100);
	logger->log("BufferPool: Estimating total struct size ~ " +
	                std::to_string(struct_size) + " bytes = " + std::to_string(raw_size) +
	                " bytes (raw) + " + std::to_string(zs_size) + " bytes (zs) + " +
	                std::to_string(ph_size) + " bytes (ph) + " +
	                std::to_string(baseline_size) + " bytes (baseline) + " +
	                std::to_string(header_size_tot) + " bytes (headers) + " +
	                std::to_string(extra) + " % (metadata).",
	            1);
	...
	// Log to user
	logger->log(
	    "BufferPool: Starting STM DAQ with " + std::to_string(thread_num) + " threads...",
	    1);
	logger->log("BufferPool: " + std::to_string(thread_num) +
	                " queues (capacity = " + std::to_string(queue_capacity) + " * " +
	                std::to_string(struct_size) + " byte data structs).",
	            1);
	...
	if(buffer_mem > FREE_MEM * SAFETY)
	{
		...
		std::string warning =
		    ...
		    " GB > 80% of available memory = " + std::to_string(FREE_MEM * 1e-9) + " GB";
	...
	const std::vector<std::pair<std::string, op_any>>& selectedOperations =
	    om->getUseOps();
	std::vector<std::string> op_names(thread_num);
	for(int i = 0; i < thread_num; i++)
		op_names[i] = selectedOperations[i].first;
