    ...
    , running_(false)
{
	size_t baseline_bytes =
	    sizeof(dqm_data_baseline) +
	    ((baseline_nbins - 1) * sizeof(uint64_t)) +  // -1 for array initialised with 1
	    (noise_len * sizeof(int16_t)) + 7 +          // Alignment padding
	...
	size_t raw_bytes =
	    ...
	    sizeof(uint64_t);                    // End counter
	size_t peak_bytes =
	    sizeof(dqm_data_peak) +
	    (((2 * peak_nbins) - 1) *
	     sizeof(uint64_t)) +  // Two histograms -1 for array initialised with size 1
	    sizeof(uint64_t);     // End counter

	// Register shared memory blocks
	registerBlock<dqm_data_raw>(DQMPageType::RAW, "/dqm_raw_data", raw_bytes);
	registerBlock<dqm_data_baseline>(
	    DQMPageType::BASELINE, "/dqm_adc_baseline", baseline_bytes);
...
void DQM::histogram_peaks(std::shared_ptr<DataStruct>& buffer)
{
	...
	++window_index;
	if(window_index == window_size_buffers)
		window_index = 0;
}
...
void DQM::update_dqm_baseline(std::shared_ptr<DataStruct>& buffer)
{
	...
	std::memcpy(
	    hist_dest, buffer->hist_counts_all.data(), baseline_nbins * sizeof(uint64_t));

	// Copy noise data
	size_t copy_len = std::min(buffer->noise_data.size(), static_cast<size_t>(noise_len));
	int16_t* noise_dest = reinterpret_cast<int16_t*>(baseline->baseline_data +
	                                                 (baseline_nbins * sizeof(uint64_t)));
	std::memcpy(noise_dest, buffer->noise_data.data(), copy_len * sizeof(int16_t));
	// Fill rest with zeroes
	if(copy_len < noise_len)
		std::memset(noise_dest + copy_len, 0, (noise_len - copy_len) * sizeof(int16_t));

	// Align with 8 byte for gen end write
	uintptr_t noise_end_addr =
	    reinterpret_cast<uintptr_t>(noise_dest) + (noise_len * sizeof(int16_t));
	...
	gen_end_ptr->store(baseline->gen_start.load(std::memory_order_acquire),
...
void DQM::update_dqm_raw(std::shared_ptr<DataStruct>& buffer)
{
	...
	std::memcpy(raw_dest, buffer->raw.data(), copy_len * sizeof(int16_t));
	// Fill rest with zeroes
	if(copy_len < raw_len)
		std::memset(raw_dest + copy_len, 0, (raw_len - copy_len) * sizeof(int16_t));

	// Align with 8 byte for gen end write
	uintptr_t raw_end_addr =
	    reinterpret_cast<uintptr_t>(raw_dest) + (raw_len * sizeof(int16_t));
	uintptr_t aligned_gen_end_addr = (raw_end_addr + 7) & ~7;
	auto* gen_end_ptr = reinterpret_cast<std::atomic<uint64_t>*>(aligned_gen_end_addr);

	// Complete safe write
	gen_end_ptr->store(raw->gen_start.load(std::memory_order_acquire),
	                   std::memory_order_release);
}
...
void DQM::update_dqm_peak(std::shared_ptr<DataStruct>& buffer)
{
	...
	// Copy data
	std::memcpy(peaks->peak_data, peak_hist_all.data(), peak_nbins * sizeof(uint64_t));
	...
	auto* gen_end_ptr =
	    reinterpret_cast<std::atomic<uint64_t>*>(peaks->peak_data + 2 * peak_nbins);

	// Complete safe write
	gen_end_ptr->store(peaks->gen_start.load(std::memory_order_acquire),
	                   std::memory_order_release);
...
void DQM::update_cpu_performance(std::shared_ptr<DataStruct>& buffer)
{
	...
	uintptr_t data_end_addr =
	    reinterpret_cast<uintptr_t>(daq_perf->ops_data) + (op_num * sizeof(op_entry));
	uintptr_t aligned_gen_end_addr = (data_end_addr + 7) & ~7;
	auto* gen_end_ptr = reinterpret_cast<std::atomic<uint64_t>*>(aligned_gen_end_addr);

	gen_end_ptr->store(daq_perf->gen_start.load(std::memory_order_acquire),
	                   std::memory_order_release);
}
