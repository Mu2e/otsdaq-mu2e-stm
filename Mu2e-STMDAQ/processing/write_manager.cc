    ...
    , event(3 * stm->buffer_config.raw_len, 0)
{
	...
	{
		logger->log(
		    "WriteManager:: Error! Mismatch with number of write streams from config! "
		    "num = " +
		        std::to_string(num) + ", stream_num = " + std::to_string(stream_num),
		    0);
	...
	for(auto& tb : total_bytes)
		tb.store(0, std::memory_order_relaxed);
	...
	for(int i = 0; i < stream_num; i++)
	{
		...
		// Queues
		ctx[i].file_queue =
		    std::make_unique<FileQueue>(stm->write_config.file_buffer_depth);
		ctx[i].retired_files =
		    std::make_unique<FileQueue>(stm->write_config.file_buffer_depth);
		logger->log("WriteManager: Initialized file_queue and retired_files for " +
		                streamID[i] + " ctx.",
		            1);
		// Thread
		file_opener[i] =
		    std::thread(&WriteManager::file_manager_thread, this, i, std::ref(ctx[i]));
		logger->log("WriteManager: Launched file_manager_thread for " + streamID[i] + ".",
		            1);
	}
...
size_t WriteManager::write_events(Context& this_ctx, std::shared_ptr<DataStruct> buffer)
{
	...
	for(size_t i = 0; i < n; ++i)
	{
		...
		{
			logger->log(
			    "WriteManager::write_events: Event builder returned event data length of "
			    "zero size...",
			    2);
			continue;
		}
...
size_t WriteManager::write_vector(const std::string&          type,
                                  Context&                    this_ctx,
...
void WriteManager::write_bytes(Context& this_ctx, const void* data_ptr, size_t data_bytes)
{
	...
	file->stream->write(reinterpret_cast<const char*>(data_ptr),
	                    static_cast<std::streamsize>(data_bytes));
	this_ctx.current_file_size += data_bytes;
}

// Check if we need to switch to a new file
void WriteManager::check_file(Context&                     this_ctx,
                              std::shared_ptr<FileHandle>& file,
                              size_t                       data_bytes)
{
	// Check if we need to have enough space in this file
	if(file && this_ctx.current_file_size + data_bytes <= this_ctx.max_file_size)
		return;
	...
	while(true)
	{
		...
		auto new_file =
		    std::atomic_exchange(&this_ctx.next_file, std::shared_ptr<FileHandle>{});
	...
	while(!stop::should_stop())
	{
		...
		while(this_ctx.file_queue_size.load() < stm->write_config.file_buffer_depth)
		{
			std::string filename = generate_filename(
			    this_ctx.prefix, streamID[idx], timestamp, this_ctx.subrun++);
			...
			else
			{
				// File open failed
				logger->log(
				    "WriteManager: ERROR! Failed to open file: " + filepath.string() +
				        ". Check disk space, directory existence, and file permissions.",
				    0);
				break;
			}
		}
		...
		while(this_ctx.retired_files->pop(closing))
		{
			if(closing && closing->stream && closing->stream->is_open())
			{
				...
				std::cout << "WriteManager: Closed completed file " << closing->filepath
				          << std::endl;
			}
		}
...
void WriteManager::logger_thread()
{
	...
	while(!stop::should_stop())
	{
		...
		for(int i = 0; i < stream_num; i++)
			gbps[i] = 8.0 * (cur[i] - last[i]) / (1e9 * dt);

		// Calculate total write speed in bytes per second
		double total_bps = 0;
		for(int i = 0; i < stream_num; i++)
			total_bps += (cur[i] - last[i]);
		total_bps /= dt;
		// and in Gbit/s
		...
		if(elapsed >= stm->write_config.log_interval_sec)
		{
			...
			if(total_gbps > 0)
			{
				...
				for(int i = 0; i < stream_num; i++)
					output += streamID[i] + " = " + format_gbps(gbps[i]) + " | ";
			...
			if(total_bps > 0)
			{
				...
				// Log to user
				logger->log("WriteManager: Disk: free = " + format_gb(space.available) +
				                " (" + format_percent(percent_available) +
				                "%) | time until full: " + time_ss.str(),
				            1);
			}
...
void WriteManager::cleanup_unused_files(Context& this_ctx)
{
	...
	if(current && current->stream && current->stream->is_open())
	{
		if(this_ctx.current_file_size == 0)
		{
			...
			std::cout << "WriteManager: Deleted unused file: " << current->filepath
			          << std::endl;
		}
		else
		{
	...
	while(this_ctx.file_queue->pop(f))
	{
		if(f && f->stream && f->stream->is_open())
		{
			...
			std::cout << "WriteManager: Deleted unused file: " << f->filepath
			          << std::endl;
		}
	}
}
