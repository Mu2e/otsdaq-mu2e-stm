...
// Frontend threads
#include "STMDAQ-TestBeam/frontEnd/fe_threads.hh"
...
// Gen data
#include "STMDAQ-TestBeam/frontEnd/genData.hh"
...
static const int PRESCALE = 4;
static const int DISTRIBUTE_RAW = 5;
static const int WRITE = 6;
static const int COMPARE = 7;
...
static const uint32_t numberhb = 10000; //0xFFFF;
...
int main(){
  ...
  std::vector<int> processID; // Use vector for dynamic sizing
  process_count = 0;
  for (int i = 0; i < process_max; i++) {
    if (process[i]) {
      processID.push_back(i); // Add process identifier to the vector
      process_count++;
    }
  }
  ...
  std::cout << "Making queues \n";
  std::vector<queue_buffer*> process_queue(process_num);
  for (int i = 0; i < process_num; i++) {
    process_queue[i] = new queue_buffer(buffer_len);
  }
  std::cout << "Queues made \n";
  ...
  static const int cores = CPU_COUNT(&cpuset) - 1; // (leave one for main)
  ...
  int thread_count = 0; // No DQM thread
	...
	if (processID[i] == EVENTS){
	  ...
	  if (processID[i] == DISTRIBUTE_RAW){
	    process_thread[i][j] = new std::thread (&FEthreads::distribute_raw, // Check data function
						    ...
						    process_queue[i], // Distribute queue
						    buffer_len
						    );
