...
FEthreads::FEthreads(bool writeData_) : writeData(writeData_) {

  if (writeData) std::cout << "WRITING DATA TO BINARY FILE" << std::endl;
}
...
void FEthreads::distribute_raw(int chan, int socket, 
   ...
   while(!udp.timeout){
     ...
     while(count < retval){
       ...
       if(sendToArtDaq && retval!=0) sw_udp.sendOne(event_buffer, size_to_copy, socket);
       // Increment counters
       count += event_len+fw_tHdr_Len;
       tot_len+=count;
       event_counter++;
     }
     // If writing, push data to write queue
     if (writeData) pushq->push(chan,dist_buffer,retval);    	     
